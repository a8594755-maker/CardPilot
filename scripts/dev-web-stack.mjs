...
async function main() {
  ...
  const backendRunning = backendPortListening ? await isCardPilotBackendHealthy() : false;
  ...
  if (backendPortListening && !backendRunning) {
    ...
    console.error(
      '[dev] Stop the process on :4000 or set VITE_DEV_SERVER_TARGET to the correct backend.',
    );
    process.exit(1);
  }
