...
$Preregistration = Join-Path $Repo "reports\v5_vr002c1_cpu_default_generator_correction_preregistration_${Token}_20260723.json"
$PreregistrationSha256 = 'a0a9ff27017257a27cad92bacf2a69f64a1442b218495a3d6d6a76ea7244948e'
$PreimplementationAudit = Join-Path $Repo "reports\v5_vr002c1_cpu_default_generator_correction_preregistration_audit_${Token}_20260723.json"
$PreimplementationAuditSha256 = 'cfa2a0836deb6345fea46680077278ae7db98a36e6b76d762b7493858b73bf19'
$ImplementationAudit = Join-Path $Repo "reports\v5_vr002c1_implementation_audit_${Token}_20260723.json"
$Core = Join-Path $Repo "scripts\alpha_holdem\v5_vr002c1_qboost_core_${Token}.py"
$Trainer = Join-Path $Repo "scripts\alpha_holdem\v5_vr002c1_train_${Token}.py"
...
$OutputRoot = Join-Path $Repo "models\alpha_holdem_v5_hybrid\v5_vr002c1_${Token}_20260723"
...
$RunId = "v5_vr002c1_qboost_stagea_${Token}"

function Assert-Hash([string]$Path, [string]$Expected, [string]$Label) {
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw "VR002C1 missing ${Label}: $Path"
    }
    $Actual = (Get-FileHash -Algorithm SHA256 -LiteralPath $Path).Hash.ToLowerInvariant()
    if ($Actual -ne $Expected) {
        throw "VR002C1 ${Label} hash mismatch: expected $Expected got $Actual"
    }
}
...
if (-not (Test-Path -LiteralPath $Core -PathType Leaf)) { throw "VR002C1 core absent: $Core" }
if (-not (Test-Path -LiteralPath $Trainer -PathType Leaf)) { throw "VR002C1 trainer absent: $Trainer" }
...
if ($Mode -eq 'Probe') {
    if (Test-Path -LiteralPath $OutputRoot) { throw 'VR002C1 probe output-root collision.' }
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { throw "VR002C1 probe child exit $LASTEXITCODE" }
    } finally {
    ...
    if (Test-Path -LiteralPath $OutputRoot) { throw 'VR002C1 probe wrote output.' }
    exit 0
}

if (Test-Path -LiteralPath $OutputRoot) {
    throw "VR002C1 StageA output-root collision: $OutputRoot"
}
if (-not (Test-Path -LiteralPath $ImplementationAudit -PathType Leaf)) {
    throw 'VR002C1 StageA requires the frozen implementation-audit report.'
}
$Audit = Get-Content -LiteralPath $ImplementationAudit -Raw | ConvertFrom-Json
if ($Audit.status -ne 'VR002C1_IMPLEMENTATION_AUDIT_PASS_STAGEA_AUTHORIZED') {
    throw "VR002C1 implementation audit is not launch-authorizing: $($Audit.status)"
}
if ($Audit.identity_sha256 -ne $Identity -or $Audit.preregistration_sha256 -ne $PreregistrationSha256) {
    throw 'VR002C1 implementation-audit identity mismatch.'
}
...
$WindowAuditor = Join-Path $Repo "scripts\alpha_holdem\v5_vr002c1_window_audit_${Token}.py"
...
if (
    ...
    throw 'VR002C1 implementation source changed after audit.'
}
