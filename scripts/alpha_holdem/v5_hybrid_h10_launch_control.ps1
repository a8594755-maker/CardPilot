...
$runId='v5_hybrid_h10_control_catchmse_same33834_20m_r1_20260715'
...
$preflight=(Resolve-Path 'reports/v5_hybrid_h10_preflight_20260715.json').Path
...
$ready=$pf.overall -eq 'PASS_READY_H10_CONTROL_LAUNCH' -and $pf.design_lock_sha256 -eq $expected -and $path1 -and $path1.PriorityClass -eq 'BelowNormal' -and (-not(Test-Path $runDir))
...
if($manifest.run_id -ne $runId -or $manifest.config.h10_window_arm -ne 'control' -or (-not [bool]$manifest.config.h8_value_head_catchup_after_kl_stop) -or $manifest.config.h10_catchup_loss -ne 'mse' -or $manifest.config.h10_design_lock_sha256 -ne $expected){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw 'manifest H10 control identity'}
...
[pscustomobject]@{status='H10_CONTROL_LAUNCHED_REARM_PASS';trainer_pid=$trainer.Id;run_id=$runId;design_lock_sha256=$expected;path1_pid=37656}|ConvertTo-Json
