#!/usr/bin/env python3
"""Offline fail-closed implementation audit for H11 robust catch-up-only loss."""
...
from scripts.alpha_holdem.test_v5_hybrid_h11_implementation import run_update
...
def main() -> int:
    ...
    try:
        ...
        check(
            ...
            and prereg.get("experiment_id") == "H11"
        ...
        check(
            ...
            prereg["single_variable"] == {
                ...
                "beta_selection": "unchanged canonical SmoothL1 default; no holdout,H9/H10 partial-outcome or CAL score search",
            },
        )
        check(
            "trainer_cli_identity",
            all(token in trainer_source for token in (
                "--h11-window-arm", "--h11-catchup-loss",
                "--h11-catchup-smooth-l1-beta", "--h11-preregistration", "--h11-design-lock",
            )),
        )
        check(
        ...
        active_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h11_active_window.py").read_text(encoding="utf-8")
        ...
        protocol_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h11_protocol_watch.py").read_text(encoding="utf-8")
        check(
        ...
        check(
            ...
            and "H11 terminal transition requires an active control or treatment window" in active_source,
        )
        check(
        ...
        check(
            "canonical_rearm_strict_h11_contract",
            all(token in rearm_source for token in (
                "$script:isHybridH11Arm",
        ...
        result = {
            "schema_version": "v5.hybrid.h11.implementation_audit.v1",
            "checked_at": datetime.now(timezone.utc).isoformat(),
            "overall": "PASS_H11_IMPLEMENTATION" if not errors else "FAIL_CLOSED",
            "checks": checks,
            "details": details,
            ...
            "source_sha256": {
                ...
                "ppo": sha(args.ppo),
                "test": sha(args.test),
                "active_window": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h11_active_window.py"),
                "protocol_watch": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h11_protocol_watch.py"),
