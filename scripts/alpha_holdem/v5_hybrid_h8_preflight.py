#!/usr/bin/env python3
"""Fail-closed H8 control launch preflight."""
...
def main() -> int:
    ...
    check("identity", lock.get("design_id") == "H8" and lock.get("status") == "LOCKED", "lock identity")
    ...
    check(
        "source_identity",
        int(checkpoint.get("iteration", -1)) == 32617
        and int(checkpoint.get("total_hands", -1)) == 536001286
        and checkpoint.get("h7_window_arm") == "treatment"
    ...
    anchor = Path(lock["h7_nostop_anchor"]["path"])
    check("anchor_hash", anchor.is_file() and sha(anchor) == lock["h7_nostop_anchor"]["sha256"], "anchor hash")
    ...
    for process in psutil.process_iter(["pid", "cmdline", "nice"]):
        try:
            ...
            if "v5_hybrid_h8_mirror.py" in command or "slumbot" in command.lower():
                forbidden.append({"pid": process.pid, "token": "h8_mirror_or_slumbot"})
            ...
            pass
    check("no_h8_trainer", not active, "H8 trainer")
    ...
    temporary_root = Path.cwd() / ".test_tmp" / "h8_preflight"
    temporary_root.mkdir(parents=True, exist_ok=True)
    for index, (tool, extra) in enumerate((
        ("v5_hybrid_h8_endpoint_watch.py", ["--arm", "control"]),
        ("v5_hybrid_h8_endpoint_watch.py", ["--arm", "treatment"]),
        ("v5_hybrid_h8_protocol_watch.py", ["--arm", "control"]),
        ("v5_hybrid_h8_protocol_watch.py", ["--arm", "treatment"]),
        ("v5_hybrid_h8_completion_watch.py", ["--repo", str(Path.cwd())]),
    )):
    ...
    result = {
        "schema_version": "v5.hybrid.h8.preflight.v1",
        "checked_at": datetime.now(timezone.utc).isoformat(),
        "overall": "PASS_READY_H8_CONTROL_LAUNCH" if not errors else "FAIL_CLOSED",
        "checks": checks,
        "errors": errors,
