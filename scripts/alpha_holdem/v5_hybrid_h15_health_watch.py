#!/usr/bin/env python3
"""Exact lock-bound H15 endpoint-health producer.
...
SCHEMA = "v5.hybrid.h15.health_watch_status.v1"
...
def prepare_view(run_dir: Path, manifest_raw: str, manifest: dict[str, Any]) -> tuple[Path, dict[str, Any]]:
    ...
    view = run_dir / ".h15_health_compat"
    ...
    provenance = {
        "schema_version": "v5.hybrid.h15.health_log_adapter.v1",
...
def publish(run_dir: Path, view: Path, provenance: dict[str, Any]) -> dict[str, Any]:
    ...
    health["h15_health_producer"] = provenance
...
def main() -> int:
    ...
    try:
        ...
        expected_self = lock.get("tools", {}).get("scripts/alpha_holdem/v5_hybrid_h15_health_watch.py")
        ...
        while True:
            ...
            if manifest.get("run_id") != arm.get("run_id") or config.get("h15_window_arm") != args.arm:
                raise ValueError("H15 manifest identity mismatch")
        ...
        atomic_json(status_path, {
            ...
            "checked_at": datetime.now(timezone.utc).isoformat(),
            "overall": "FAIL_CLOSED",
            "state": "H15_HEALTH_PRODUCER_FAILURE",
            "arm": args.arm,
            "design_lock_sha256": args.expected_lock_sha256.lower(),
