param([switch]$ValidateOnly,[switch]$LaunchAuthorized)
$ErrorActionPreference='Stop'
$Repo=(Resolve-Path '.').Path
$controlId='v5_hybrid_h18_control_catchmse_same35051_20m_r1_20260719'
$runId='v5_hybrid_h18_treatment_catchsmoothl1b1_same35051_20m_r1_20260719'
...
$prereg=(Resolve-Path 'reports/v5_hybrid_h18_preregistration_20260719.json').Path
$lock=(Resolve-Path 'reports/v5_hybrid_h18_design_lock_v1_20260719.json').Path
...
$preregHash=(Get-FileHash $prereg -Algorithm SHA256).Hash; if (-not $preregHash.Equals('8f1f3df1c7fee8c4f8990d5354313330d497f75b1d81de6ceb1f7aa9b4225481',[StringComparison]::OrdinalIgnoreCase)) { throw "prereg hash actual=$preregHash" }
...
$endpoint=Get-Content (Join-Path $controlDir 'h18_control_endpoint_status.json') -Raw|ConvertFrom-Json
$protocol=Get-Content (Join-Path $controlDir 'h18_control_protocol_status.json') -Raw|ConvertFrom-Json
...
$mirrorDir=Resolve-Path 'reports/h18_mirror_001_v3_20260719'
$unexpectedMirror=@(Get-ChildItem $mirrorDir|Where-Object{$_.Name -notin @('manifest.json','measurement_lock.json')})
$controlOrdered=Get-CimInstance Win32_Process | Where-Object { ([string]$_.CommandLine) -like '*v5_hybrid_h18_ordered_rearm.py*' -and ([string]$_.CommandLine) -like '*--arm control*' }
...
if(-not $LaunchAuthorized){throw 'H18 treatment launch requires an explicit post-lock LaunchAuthorized invocation;current heartbeat forbids automatic launch'}
if(-not $ready){throw 'H18 treatment not ready'}
$controlPerf=Join-Path $Repo 'reports/v5_hybrid_h18_control_perf_cal_20260719.json'
$perf=Join-Path $Repo 'reports/v5_hybrid_h18_treatment_perf_cal_20260719.json'
$perfAudit=Join-Path $Repo 'reports/v5_hybrid_h18_treatment_perf_cal_audit_20260719.json'
if(-not(Test-Path $controlPerf)){throw 'H18 treatment requires immutable control PERF-CAL baseline'}
if(-not(Test-Path $perf)){
 python scripts/alpha_holdem/v5_hybrid_h18_perf_cal.py --source $source --source-sha256 '96a007039b0baa29f0c39b0bd7adc67d8ca0733a41a261203f52430e60b5ca13' --arm treatment --seed 2026071983 --rows 4096 --mini-batch-size 1024 --ppo-epochs 4 --target-kl 1e-12 --warmup-updates 4 --timed-updates 16 --repeats 7 --device cuda --path1-pid $path1Pid --path1-workers 6 --out $perf
 if($LASTEXITCODE -ne 0){throw 'H18 treatment PERF-CAL failed closed'}
}
if(-not(Test-Path $perfAudit)){
 python scripts/alpha_holdem/v5_hybrid_h18_perf_cal_audit.py --artifact $perf --tool scripts/alpha_holdem/v5_hybrid_h18_perf_cal.py --path1-pid $path1Pid --path1-workers 6 --out $perfAudit
 if($LASTEXITCODE -ne 0){throw 'H18 treatment PERF-CAL audit failed closed'}
}
...
if($perfValue.overall -ne 'PASS' -or $perfValue.arm -ne 'treatment' -or $perfValue.authority -ne 'IMMEDIATE_PER_ARM_GATE' -or [double]$perfValue.timing.full_update_throughput_ratio -lt 0.85 -or [double]$perfValue.timing.mse_repeat_stability_ratio -lt 0.95 -or (-not [bool]$perfValue.equivalence.pass) -or $perfAuditValue.overall -ne 'PASS' -or $perfAuditValue.artifact_sha256 -ne $perfHash){throw 'H18 treatment immutable representative PERF-CAL evidence invalid'}
python scripts/alpha_holdem/v5_hybrid_h18_active_window.py activate --sentinel $sentinel --design-lock $lock --expected-lock-sha256 $expected --arm treatment --run-id $runId
if($LASTEXITCODE -ne 0){throw 'H18 treatment active-window activation failed'}
New-Item -ItemType Directory -Path $runDir|Out-Null
$prov=Join-Path $runDir 'opponent_assignment_provenance.jsonl'
$out=Join-Path $runDir 'latest.pt'
$args=@('-u','scripts/alpha_holdem/train_v5.py','--resume',$source,'--allow-resume','--no-reset-optimizer','--run-id',$runId,'--run-dir',$runDir,'--out',$out,'--total-hands','596021901','--device','cuda','--workers','22','--hands-per-iter','16384','--rollout-mode','multi','--rollout-envs-per-worker','16','--inference-min-batch-slots','256','--inference-batch-deadline-us','1000','--ppo-epochs','4','--ppo-target-kl','0.03','--mini-batch-size','1024','--lr','0.0003','--gamma','0.999','--entropy-coef','0.05','--entropy-floor','0.3','--seed','20260703','--worker-seed-base','73000','--fixed-training-deal-stream','--opponent-assignment','per-iteration','--opponent-assignment-provenance-file',$prov,'--pool-strategy','loss-kbest','--k-best','5','--opponent-groups','5','--self-play-fraction','0.2','--mirror-self-play-deals','--allin-runout-ev','--allin-runout-ev-max-runouts','200','--preflop-action-prior-coef','0.01','--preflop-action-prior-target','0.24,0.36,0.38,0.02','--postflop-action-prior-coef','0.02','--postflop-action-prior-target','0.15,0.30,0.52,0.03','--critic-contract','critic_v1','--value-coef','0.5','--save-interval','1','--snapshot-every','200','--h18-window-arm','treatment','--h8-value-head-catchup-after-kl-stop','--h18-catchup-loss','smooth_l1','--h18-catchup-smooth-l1-beta','1.0','--h18-preregistration',$prereg,'--h18-preregistration-sha256','8f1f3df1c7fee8c4f8990d5354313330d497f75b1d81de6ceb1f7aa9b4225481','--h18-design-lock',$lock,'--h18-design-lock-sha256',$expected)
...
if($manifest.run_id -ne $runId -or $manifest.config.h18_window_arm -ne 'treatment' -or (-not [bool]$manifest.config.h8_value_head_catchup_after_kl_stop) -or $manifest.config.h18_catchup_loss -ne 'smooth_l1' -or $manifest.config.h18_design_lock_sha256 -ne $expected){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw 'manifest H18 treatment identity'}
$priorCompletion=@(Get-CimInstance Win32_Process -Filter "Name='python.exe'" | Where-Object {
    $command=[string]$_.CommandLine
    $command -like '*v5_hybrid_h18_completion_watch.py*' -and $command -like "*$controlId*"
})
if($priorCompletion.Count -gt 1){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw "multiple prior H18 completion supervisors: $($priorCompletion.ProcessId -join ',')"}
...
if($priorCompletion.Count -eq 1){
    ...
    if(Get-Process -Id $retiredCompletionPid -ErrorAction SilentlyContinue){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw 'prior H18 completion supervisor did not exit'}
}
try{
 ...
 $orderedPath=Join-Path $runDir 'h18_ordered_rearm_status.json';$orderedDeadline=(Get-Date).AddSeconds(190);$ordered=$null
...
[pscustomobject]@{status='H18_TREATMENT_LAUNCHED_REARM_PASS';trainer_pid=$trainer.Id;run_id=$runId;design_lock_sha256=$expected;path1_pid=$path1Pid;ordered_rearm_state=$ordered.state;retired_control_completion_pid=$retiredCompletionPid}|ConvertTo-Json
