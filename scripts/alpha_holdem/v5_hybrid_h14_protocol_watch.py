#!/usr/bin/env python3
"""H14 throughput, entropy and full-provenance resource-isolation watcher."""
...
FORBIDDEN_PROCESS_TOKENS = (
    "v5_hybrid_h14_mirror.py", "v5_h1_calibration.py", "v5_slumbot",
...
def main() -> int:
    ...
    while True:
        ...
        if violations:
            action = stop(pid)
            write(status_path, {
                "schema_version": "v5.hybrid.h14.protocol_status.v1",
                "checked_at": datetime.now(timezone.utc).isoformat(),
                "overall": "FAIL",
                "state": "H14_INCONCLUSIVE_RESOURCE_ISOLATION_VIOLATION",
                "arm": args.arm,
                "pid": pid,
                "stop_action": action,
        ...
        if len(values) >= 20 and all(float(item.get("entropy", 99)) < 0.3 for item in values[-20:]):
            action = stop(pid)
            write(status_path, {"overall": "FAIL", "state": "H14_FAIL_PROTOCOL_ABORT_ENTROPY20", "arm": args.arm, "pid": pid, "stop_action": action, "resource_isolation_violations": []})
            return 3
        ...
        if len(values) >= 61:
            ...
            if args.arm == "control":
                ...
                if first["status"] == "FAIL":
                    action = stop(pid)
                    write(status_path, {"overall": "FAIL", "state": "H14_FAIL_PROTOCOL_ABORT_FIRST60_THROUGHPUT", "arm": args.arm, "pid": pid, "stop_action": action, "first60": first, "resource_isolation_violations": []})
                    return 3
