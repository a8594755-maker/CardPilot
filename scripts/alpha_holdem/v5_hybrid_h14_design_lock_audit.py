#!/usr/bin/env python3
"""Independent fail-closed H14 design-lock audit."""
...
def main() -> int:
    ...
    check("identity", value.get("design_id") == "H14" and value.get("status") == "LOCKED")
    source = value.get("source", {})
    check(
        "source",
        source.get("iteration") == 35051
    ...
    check(
        "anchor",
        anchor.get("iteration") == 35051
        and anchor.get("hands") == 576021901
    ...
    check(
        ...
        value.get("control_plane", {}).get("exact_lifecycle_child_registry")
        == {
    ...
    repair = value.get("control_plane_repair", {})
    check(
        "repair_audit",
        repair.get("audit_checks") == "32/32 PASS"
        and Path(repair.get("artifact", {}).get("path", "")).is_file()
        and sha(Path(repair["artifact"]["path"])) == repair["artifact"]["sha256"]
        and Path(repair.get("audit", {}).get("path", "")).is_file()
        and sha(Path(repair["audit"]["path"])) == repair["audit"]["sha256"],
    )
    check(
        "path1_exact",
        value.get("resource_isolation", {}).get("path1_existing_job")
        == "MAY_CONTINUE_EXISTING_PID23720_SIX_BELOWNORMAL_CPU_WORKERS_NO_RESTART_EXPANSION_OR_NEW_WORKERS",
    )
    check(
    ...
    result = {
        "schema_version": "v5.hybrid.h14.design_lock_audit.v1",
        "checked_at": datetime.now(timezone.utc).isoformat(),
        "overall": "PASS_IMMUTABLE_H14_DESIGN_LOCK" if not errors else "FAIL_CLOSED",
        "checks": checks,
        "errors": errors,
