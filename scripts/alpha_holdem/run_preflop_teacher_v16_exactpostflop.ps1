...
$stdout = "models/preflop_teacher_v16_exactpostflop_launcher.stdout.log"
$stderr = "models/preflop_teacher_v16_exactpostflop_launcher.stderr.log"
...
$arguments = @(
    ...
    "--allow-resume",
    "--out", "models/slumbot_br_preflop_teacher_v16_exactpostflop_262k_20260725/latest.pt",
    "--run-id", "slumbot_br_preflop_teacher_v16_exactpostflop_262k_20260725",
    "--device", "cuda",
    "--workers", "20",
    "--rollout-mode", "multi",
...
if ($null -eq $process.ExitCode -or $process.ExitCode -ne 0) {
    throw "v16 trainer failed: pid=$($process.Id) exit_code=$($process.ExitCode)"
}
