#!/usr/bin/env python3
"""Fail-closed H10 endpoint freezer for fresh contemporaneous arms."""
...
def main() -> int:
    ...
    while True:
        ...
        if config.get("h10_window_arm") != args.arm or abs(float(config.get("ppo_target_kl", -1)) - target) > 1e-12: errors.append("H10 arm identity mismatch")
        if not bool(config.get("h8_value_head_catchup_after_kl_stop")): errors.append("H10 catch-up must be enabled in both arms")
        if config.get("h10_catchup_loss") != expected_loss or float(config.get("h10_catchup_smooth_l1_beta", -1)) != 1.0: errors.append("H10 catch-up loss identity mismatch")
        ...
        if checkpoint.get("h10_window_arm") != args.arm or abs(float(checkpoint.get("ppo_target_kl", -1)) - target) > 1e-12: errors.append("checkpoint H10 arm mismatch")
        if not bool(checkpoint.get("h8_value_head_catchup_after_kl_stop")): errors.append("checkpoint H10 catch-up disabled")
        if checkpoint.get("h10_catchup_loss") != expected_loss or float(checkpoint.get("h10_catchup_smooth_l1_beta", -1)) != 1.0: errors.append("checkpoint H10 loss mismatch")
        ...
        if checkpoint_config.get("h10_preregistration_sha256") != lock["preregistration"]["sha256"]: errors.append("checkpoint prereg mismatch")
        if checkpoint_config.get("h10_design_lock_sha256") != sha(lock_path): errors.append("checkpoint lock mismatch")
        if errors:
        ...
        write(run_dir / f"h10_{args.arm}_assignment_provenance_audit.json", provenance)
        if provenance.get("overall") != "PASS": readiness.append("provenance audit failed")
        resource = load(run_dir / f"h10_{args.arm}_protocol_status.json")
        ...
        frozen = run_dir / f"h10_{args.arm}_endpoint.pt"
