#!/usr/bin/env python3
"""Resource-matched, reporting-only performance calibration for H13.
...
SCHEMA = "v5.hybrid.h13.perf_cal.v1"
LOSS_RATIO_MIN = 0.95
...
def build_model_and_optimizer(checkpoint: dict[str, Any], device: str) -> tuple[AlphaHoldemNet, torch.optim.Adam]:
    if checkpoint.get("critic_contract") != CRITIC_V1:
        raise ValueError("H13 PERF-CAL requires critic_v1 source")
...
def main() -> int:
    ...
    try:
        ...
        payload = {
            ...
            "classification": "H13_PERF_CAL_PASS" if passed else "H13_PERF_CAL_FAIL",
            "arm": args.arm,
            "source": {
                "path": str(source),
        ...
        failure = {
            ...
            "checked_at": datetime.now(timezone.utc).isoformat(),
            "overall": "FAIL_CLOSED",
            "classification": "H13_PERF_CAL_EXECUTION_FAILURE",
            "arm": args.arm,
