"""Fresh RS004 live-native exact-cent paired-MC32 resolver.
...
IDENTITY_SHA256 = "a4f6cdb3461aa6ef4ea29af61938082ccfe2497ad4177aa16317292aad1a6dfb"
...
IMPL_AUDIT = ROOT / "reports" / f"v5_rs004_live_native_paired_mc32_implementation_audit_{TOKEN}_20260722.json"
...
def verify_authority_inputs() -> dict[str, Any]:
    verify_file(PREREG, PREREG_SHA256, 16380)
    verify_file(PREREG_AUDIT, PREREG_AUDIT_SHA256, 6698)
    prereg = json.loads(PREREG.read_text(encoding="utf-8"))
    if prereg["identity"]["sha256"] != IDENTITY_SHA256 or prereg["identity"]["token"] != TOKEN:
        raise RuntimeError("rs004_identity_mismatch")
    checked = 0
...
def validate_device_contract(expected_nonce: str) -> dict[str, Any]:
    if os.environ.get("RS004_DEVICE_MODE") != "CUDA_ONLY_SINGLE_GPU_NO_CPU_RESOLVER_FALLBACK":
        raise RuntimeError("device_mode_mismatch")
    if os.environ.get("RS004_NONCE") != expected_nonce:
        raise RuntimeError("device_nonce_mismatch")
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "0":
        raise RuntimeError("cuda_visibility_mismatch")
    return {"device_mode": os.environ["RS004_DEVICE_MODE"], "nonce": expected_nonce, "cuda_visible_devices": "0"}


def contract_probe(nonce: str) -> int:
    verify_authority_inputs()
    device = validate_device_contract(nonce)
    print(canonical_json({"classification": "RS004_CONTRACT_PROBE_PASS", "identity_sha256": IDENTITY_SHA256, **device, "files_written": 0}))
    return 0
...
def run_self_test(level: str) -> int:
    ...
    if level == "deep":
        ...
        for row in witnessed:
            ...
            live_checks += 1
        actor = H11Actor(os.environ["RS004_NONCE"])
    ...
    print(canonical_json({"classification": "RS004_SELF_TEST_PASS", "level": level, "rows": len(sample), "transitions": transitions, "live_checks": live_checks, "mirror_checks": mirror_checks, "files_written": 0}))
    return 0
...
def run_qualification(root: Path, implementation_audit_sha256: str, nonce: str) -> int:
    ...
    if impl.get("classification") != "PASS / RS004_IMPLEMENTATION_AUDIT_PASS_QUALIFICATION_READY_ONLY":
    ...
    result = {"schema_version": "v5.rs004.qualification.result.v1", "identity_sha256": IDENTITY_SHA256, "classification": "PASS / RS004_LIVE_NATIVE_QUALIFICATION_PASS" if all(gates.values()) else "NONPASS / RS004_LIVE_NATIVE_QUALIFICATION_GATE_NONPASS", "gates": gates, "pass_count": sum(gates.values()), "check_count": len(gates), "metrics": metrics, "checkpoint_sha256": sha256_file(CHECKPOINT), "quick5k_authority": "PENDING_INDEPENDENT_RESULT_AUDIT" if all(gates.values()) else "NONE", "network_or_slumbot_hands": 0}
