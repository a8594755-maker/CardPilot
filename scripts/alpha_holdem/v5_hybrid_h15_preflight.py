#!/usr/bin/env python3
"""Fail-closed H15 control launch preflight."""
...
def main() -> int:
    ...
    check("identity", lock.get("design_id") == "H15" and lock.get("status") == "LOCKED", "lock identity")
    ...
    for process in psutil.process_iter(["pid", "cmdline", "nice"]):
        try:
            ...
            if any(token in lowered for token in (
                "v5_hybrid_h15_mirror.py", "v5_h1_calibration.py", "play_slumbot",
    ...
    temporary_root = Path.cwd() / ".test_tmp" / "h15_preflight"
    temporary_root.mkdir(parents=True, exist_ok=True)
    for index, (tool, extra) in enumerate((
        ("v5_hybrid_h15_endpoint_watch.py", ["--arm", "control"]),
        ("v5_hybrid_h15_endpoint_watch.py", ["--arm", "treatment"]),
        ("v5_hybrid_h15_protocol_watch.py", ["--arm", "control"]),
        ("v5_hybrid_h15_protocol_watch.py", ["--arm", "treatment"]),
        ("v5_hybrid_h15_completion_watch.py", ["--repo", str(Path.cwd())]),
    )):
    ...
    active_tool = Path("scripts/alpha_holdem/v5_hybrid_h15_active_window.py")
    ...
    result = {
        "schema_version": "v5.hybrid.h15.preflight.v1",
        "checked_at": datetime.now(timezone.utc).isoformat(),
        "overall": "PASS_READY_H15_CONTROL_LAUNCH" if not errors else "FAIL_CLOSED",
        "checks": checks,
        "errors": errors,
        "active_h15_trainers": active,
