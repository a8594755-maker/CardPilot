...
$controlId='v5_hybrid_h10_control_catchmse_same33834_20m_r1_20260715'
$runId='v5_hybrid_h10_treatment_catchsmoothl1b1_same33834_20m_r1_20260715'
...
$prereg=(Resolve-Path 'reports/v5_hybrid_h10_post_cal_preregistration_20260715.json').Path
$lock=(Resolve-Path 'reports/v5_hybrid_h10_design_lock_20260715.json').Path
...
$preregHash=(Get-FileHash $prereg -Algorithm SHA256).Hash; if (-not $preregHash.Equals('cf562528360e05e4683bc3bd04edc19ba49ea98c2a2ddeb4d92f45805eab11fc',[StringComparison]::OrdinalIgnoreCase)) { throw "prereg hash actual=$preregHash" }
...
$endpoint=Get-Content (Join-Path $controlDir 'h10_control_endpoint_status.json') -Raw|ConvertFrom-Json
$protocol=Get-Content (Join-Path $controlDir 'h10_control_protocol_status.json') -Raw|ConvertFrom-Json
...
$mirrorDir=Resolve-Path 'reports/h10_mirror_001_20260715'
...
if($manifest.run_id -ne $runId -or $manifest.config.h10_window_arm -ne 'treatment' -or (-not [bool]$manifest.config.h8_value_head_catchup_after_kl_stop) -or $manifest.config.h10_catchup_loss -ne 'smooth_l1' -or $manifest.config.h10_design_lock_sha256 -ne $expected){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw 'manifest H10 treatment identity'}
$priorCompletion=@(Get-CimInstance Win32_Process -Filter "Name='python.exe'" | Where-Object {
    $command=[string]$_.CommandLine
    $command -like '*v5_hybrid_h10_completion_watch.py*' -and $command -like "*$controlId*"
})
if($priorCompletion.Count -gt 1){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw "multiple prior H10 completion supervisors: $($priorCompletion.ProcessId -join ',')"}
...
if($priorCompletion.Count -eq 1){
    ...
    if(Get-Process -Id $retiredCompletionPid -ErrorAction SilentlyContinue){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw 'prior H10 completion supervisor did not exit'}
}
...
[pscustomobject]@{status='H10_TREATMENT_LAUNCHED_REARM_PASS';trainer_pid=$trainer.Id;run_id=$runId;design_lock_sha256=$expected;path1_pid=37656;retired_control_completion_pid=$retiredCompletionPid}|ConvertTo-Json
