#!/usr/bin/env python3
"""Atomic fail-closed active-window sentinel for H15, including either-arm aborts."""
...
CONTROL = "v5_hybrid_h15_control_catchmse_same35051_20m_r1_20260719"
TREATMENT = "v5_hybrid_h15_treatment_catchsmoothl1b1_same35051_20m_r1_20260719"
...
def lock_is_exact(path: Path, expected: str) -> tuple[bool, dict]:
    if not path.is_file() or sha(path) != expected.lower():
        return False, {}
    value = load(path)
    return value.get("design_id") == "H15" and value.get("status") == "LOCKED", value
...
def main() -> int:
    ...
    if not exact:
        raise SystemExit("H15 design-lock identity/hash mismatch")
    ...
    if args.action == "validate":
        ...
        valid_h15 = (
            ...
            and current.get("design_id") == "H15"
            and current.get("design_lock_sha256") == args.expected_lock_sha256.lower()
            and isinstance(current.get("history"), list)
        )
        print(json.dumps({"overall": "PASS" if valid_prior or valid_h15 else "FAIL_CLOSED", "sentinel": current}, indent=2))
        return 0 if valid_prior or valid_h15 else 2
    now = datetime.now(timezone.utc).isoformat()
    if args.action == "activate":
        expected_run = CONTROL if args.arm == "control" else TREATMENT
        if args.run_id != expected_run:
            raise SystemExit("H15 arm/run identity mismatch")
        if args.arm == "control":
            ...
            if not (
                current.get("design_id") == "H15"
                and current.get("active") is True
                and current.get("terminal") is False
                and current.get("arm") == "control"
            ):
                raise SystemExit("H15 treatment requires the same active control window")
        ...
        value = {
            ...
            "active": True,
            "terminal": False,
            "state": f"H15_{args.arm.upper()}_ACTIVE",
            "arm": args.arm,
            "run_id": args.run_id,
        ...
        if judgment.get("overall") != args.verdict or judgment.get("design_id") != "H15":
            raise SystemExit("H15 judgment identity/verdict mismatch")
        if current.get("terminal") is True:
            same = current.get("verdict") == args.verdict and current.get("judgment_sha256") == judgment_hash
            if not same:
                raise SystemExit("conflicting H15 terminal sentinel")
        ...
        if not (
            ...
            raise SystemExit("H15 terminal transition requires an active control or treatment window")
        history = list(current.get("history") or [])
        history.append({
            "at": now,
            "event": "H15_TERMINAL",
        ...
        value.update({
            "active": False,
            "terminal": True,
            "state": f"H15_TERMINAL_{args.verdict}",
            "verdict": args.verdict,
            "judgment_path": str(args.judgment.resolve()),
            "judgment_sha256": judgment_hash,
            "history": history,
