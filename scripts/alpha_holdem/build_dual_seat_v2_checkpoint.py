#!/usr/bin/env python3
"""Package two compatible frozen actors as one position-aware dual checkpoint."""
...
def position_adapter_hidden(state: dict[str, torch.Tensor]) -> int:
    key = "position_policy_adapters.0.0.weight"
...
def build_actor(checkpoint: dict) -> AlphaHoldemNet:
    ...
    model = AlphaHoldemNet(
        ...
        position_adapter_hidden=position_adapter_hidden(state),
    )
