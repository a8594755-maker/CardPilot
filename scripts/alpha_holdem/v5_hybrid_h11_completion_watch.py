#!/usr/bin/env python3
"""Duplicate-safe H11 post-arm evaluation and terminal judgment supervisor."""
...
def status(path: Path, **payload) -> None:
    value = {
        "schema_version": "v5.hybrid.h11.completion_watch_status.v1",
...
def active_h11_trainers() -> list[int]:
    active = []
...
def main() -> int:
    ...
    try:
        ...
        tools = lock["tools"]
        mirror_tool = repo / "scripts/alpha_holdem/v5_hybrid_h11_mirror.py"
        judge_tool = repo / "scripts/alpha_holdem/v5_hybrid_h11_judge.py"
        active_window_tool = repo / "scripts/alpha_holdem/v5_hybrid_h11_active_window.py"
        if (
            sha(mirror_tool) != tools["scripts/alpha_holdem/v5_hybrid_h11_mirror.py"]
            or sha(judge_tool) != tools["scripts/alpha_holdem/v5_hybrid_h11_judge.py"]
            or sha(active_window_tool) != tools["scripts/alpha_holdem/v5_hybrid_h11_active_window.py"]
        ):
        ...
        mirror_tool_sha = tools["scripts/alpha_holdem/v5_hybrid_h11_mirror.py"]
        if (
        ...
        endpoint_status = {
            arm: directories[arm] / f"h11_{arm}_endpoint_status.json"
            for arm in directories
        }
        protocol = {
            arm: directories[arm] / f"h11_{arm}_protocol_status.json"
            for arm in directories
        }
        outputs = {
        ...
        judgment = repo / "reports/v5_hybrid_h11_judgment_20260715.json"
        ...
        while True:
            if judgment.is_file():
                ...
                if (
                    ...
                    status(
                        ...
                        h11_judgment=str(judgment.resolve()),
                    )
                    return 0
            terminal = None
            for arm in ("control", "treatment"):
            ...
            if terminal:
                run(
                    ...
                    status_path,
                    "RUNNING_H11_PROTOCOL_TERMINAL_JUDGMENT",
                )
                continue
            endpoints = {
            ...
            if not all(endpoints.values()):
                ...
                continue
            active = active_h11_trainers()
            if active:
                status(
                    status_path,
                    overall="PENDING",
                    state="WAITING_FOR_ALL_H11_TRAINERS_TO_EXIT",
                    active_h11_trainers=active,
                )
                time.sleep(max(1, args.poll_seconds))
                continue
            endpoint_paths = {
            ...
            if load(audit).get("overall") != "PASS_IMMUTABLE_H11_MIRROR":
