...
$controlId='v5_hybrid_h9_control_catchmse_same33834_20m_r1_20260714'
$runId='v5_hybrid_h9_treatment_catchsmoothl1b1_same33834_20m_r1_20260714'
...
$endpoint=Get-Content (Join-Path $controlDir 'h9_control_endpoint_status.json') -Raw|ConvertFrom-Json
$protocol=Get-Content (Join-Path $controlDir 'h9_control_protocol_status.json') -Raw|ConvertFrom-Json
...
$mirrorDir=Resolve-Path 'reports/h9_mirror_001_20260714'
...
if($manifest.run_id -ne $runId -or $manifest.config.h9_window_arm -ne 'treatment' -or (-not [bool]$manifest.config.h8_value_head_catchup_after_kl_stop) -or $manifest.config.h9_catchup_loss -ne 'smooth_l1' -or $manifest.config.h9_design_lock_sha256 -ne $expected){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw 'manifest H9 treatment identity'}
$priorCompletion=@(Get-CimInstance Win32_Process -Filter "Name='python.exe'" | Where-Object {
    $command=[string]$_.CommandLine
    $command -like '*v5_hybrid_h9_completion_watch.py*' -and $command -like "*$controlId*"
})
if($priorCompletion.Count -gt 1){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw "multiple prior H9 completion supervisors: $($priorCompletion.ProcessId -join ',')"}
...
if($priorCompletion.Count -eq 1){
    ...
    if(Get-Process -Id $retiredCompletionPid -ErrorAction SilentlyContinue){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw 'prior H9 completion supervisor did not exit'}
}
...
[pscustomobject]@{status='H9_TREATMENT_LAUNCHED_REARM_PASS';trainer_pid=$trainer.Id;run_id=$runId;design_lock_sha256=$expected;path1_pid=37656;retired_control_completion_pid=$retiredCompletionPid}|ConvertTo-Json
