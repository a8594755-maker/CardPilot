...
$runId='v5_hybrid_h16_control_catchmse_same35051_20m_r1_20260719'
...
$preflight=(Resolve-Path 'reports/v5_hybrid_h16_preflight_v1_20260719.json').Path
...
$ready=$pf.overall -eq 'PASS_READY_H16_CONTROL_LAUNCH' -and $pf.design_lock_sha256 -eq $expected -and $path1 -and $path1.PriorityClass -eq 'BelowNormal' -and $path1Workers.Count -eq 6 -and (-not(Test-Path $runDir))
...
if(-not $ready){throw 'H16 control not ready'}
$perf=Join-Path $Repo 'reports/v5_hybrid_h16_control_perf_cal_20260719.json'
$perfAudit=Join-Path $Repo 'reports/v5_hybrid_h16_control_perf_cal_audit_20260719.json'
if(-not(Test-Path $perf)){
 python scripts/alpha_holdem/v5_hybrid_h16_perf_cal.py --source $source --source-sha256 '96a007039b0baa29f0c39b0bd7adc67d8ca0733a41a261203f52430e60b5ca13' --arm control --seed 2026071916 --rows 4096 --mini-batch-size 1024 --ppo-epochs 4 --target-kl 1e-12 --warmup-updates 2 --timed-updates 8 --repeats 5 --device cuda --path1-pid $path1Pid --path1-workers 6 --out $perf
 if($LASTEXITCODE -ne 0){throw 'H16 control PERF-CAL failed closed'}
}
if(-not(Test-Path $perfAudit)){
 ...
 if($LASTEXITCODE -ne 0){throw 'H16 control PERF-CAL audit failed closed'}
}
...
if($perfValue.overall -ne 'PASS' -or $perfValue.arm -ne 'control' -or [double]$perfValue.timing.full_update_throughput_ratio -lt 0.85 -or [double]$perfValue.timing.mse_repeat_stability_ratio -lt 0.95 -or (-not [bool]$perfValue.identity.pass) -or $perfAuditValue.overall -ne 'PASS' -or $perfAuditValue.artifact_sha256 -ne $perfHash){throw 'H16 control immutable representative PERF-CAL evidence invalid'}
python scripts/alpha_holdem/v5_hybrid_h16_active_window.py activate --sentinel $sentinel --design-lock $lock --expected-lock-sha256 $expected --arm control --run-id $runId
if($LASTEXITCODE -ne 0){throw 'H16 control active-window activation failed'}
New-Item -ItemType Directory -Path $runDir|Out-Null
$prov=Join-Path $runDir 'opponent_assignment_provenance.jsonl'
$out=Join-Path $runDir 'latest.pt'
$args=@('-u','scripts/alpha_holdem/train_v5.py','--resume',$source,'--allow-resume','--no-reset-optimizer','--run-id',$runId,'--run-dir',$runDir,'--out',$out,'--total-hands','596021901','--device','cuda','--workers','22','--hands-per-iter','16384','--rollout-mode','multi','--rollout-envs-per-worker','16','--inference-min-batch-slots','256','--inference-batch-deadline-us','1000','--ppo-epochs','4','--ppo-target-kl','0.03','--mini-batch-size','1024','--lr','0.0003','--gamma','0.999','--entropy-coef','0.05','--entropy-floor','0.3','--seed','20260703','--worker-seed-base','73000','--fixed-training-deal-stream','--opponent-assignment','per-iteration','--opponent-assignment-provenance-file',$prov,'--pool-strategy','loss-kbest','--k-best','5','--opponent-groups','5','--self-play-fraction','0.2','--mirror-self-play-deals','--allin-runout-ev','--allin-runout-ev-max-runouts','200','--preflop-action-prior-coef','0.01','--preflop-action-prior-target','0.24,0.36,0.38,0.02','--postflop-action-prior-coef','0.02','--postflop-action-prior-target','0.15,0.30,0.52,0.03','--critic-contract','critic_v1','--value-coef','0.5','--save-interval','1','--snapshot-every','200','--h16-window-arm','control','--h8-value-head-catchup-after-kl-stop','--h16-catchup-loss','mse','--h16-catchup-smooth-l1-beta','1.0','--h16-preregistration',$prereg,'--h16-preregistration-sha256','51065761b6b291ef757ea467611203cbe79d45a5e4d54c163edaf79ef8fa1bb0','--h16-design-lock',$lock,'--h16-design-lock-sha256',$expected)
...
if($manifest.run_id -ne $runId -or $manifest.config.h16_window_arm -ne 'control' -or (-not [bool]$manifest.config.h8_value_head_catchup_after_kl_stop) -or $manifest.config.h16_catchup_loss -ne 'mse' -or $manifest.config.h16_design_lock_sha256 -ne $expected){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw 'manifest H16 control identity'}
try{
...
[pscustomobject]@{status='H16_CONTROL_LAUNCHED_REARM_PASS';trainer_pid=$trainer.Id;run_id=$runId;design_lock_sha256=$expected;path1_pid=$path1Pid;ordered_rearm_state=$ordered.state}|ConvertTo-Json
