#!/usr/bin/env python3
"""Lock-bound dependency-ordered watcher supervisor for H18."""
...
SCHEMA = "v5.hybrid.h18.ordered_rearm_status.v1"
STAGES = {
...
def active_h18_trainers() -> list[int]:
    result = []
    for process in psutil.process_iter(["pid", "cmdline"]):
        try:
            command = " ".join(process.info.get("cmdline") or [])
            if "train_v5.py" in command and "v5_hybrid_h18_" in command:
...
def terminate_trainers() -> list[dict[str, Any]]:
    actions = []
    for pid in active_h18_trainers():
        try:
...
def command_map(repo: Path, run_dir: Path, arm: str, lock_path: Path, lock_sha: str, registry_path: Path) -> dict[str, tuple[list[str], Path, Path, Path]]:
    ...
    return {
        "health": (
            [python, "-u", str(repo / "scripts/alpha_holdem/v5_hybrid_h18_health_watch.py"), "--run-dir", str(run_dir), "--arm", arm, *common, "--status-json", str(run_dir / f"h18_{arm}_health_status.json"), "--poll-seconds", "15"],
            run_dir / f"h18_{arm}_health_status.json", run_dir / f"h18_{arm}_health_watch.out.log", run_dir / f"h18_{arm}_health_watch.err.log",
        ),
        "protocol": (
            [python, "-u", str(repo / "scripts/alpha_holdem/v5_hybrid_h18_protocol_watch.py"), "--arm", arm, *common, "--status-json", str(run_dir / f"h18_{arm}_protocol_status.json"), "--poll-seconds", "15", "--allowed-supervisor-pid", str(supervisor["pid"]), "--allowed-supervisor-command-sha256", supervisor["command_line_sha256"], "--allowed-supervisor-creation-time-epoch", str(supervisor["creation_time_epoch"]), "--allowed-supervisor-executable", supervisor["executable"], "--allow-registry", str(registry_path)],
            run_dir / f"h18_{arm}_protocol_status.json", run_dir / f"h18_{arm}_protocol_watch.out.log", run_dir / f"h18_{arm}_protocol_watch.err.log",
        ),
        "endpoint": (
            [python, "-u", str(repo / "scripts/alpha_holdem/v5_hybrid_h18_endpoint_watch.py"), "--arm", arm, *common, "--status-json", str(run_dir / f"h18_{arm}_endpoint_status.json"), "--poll-seconds", "15"],
            run_dir / f"h18_{arm}_endpoint_status.json", run_dir / f"h18_{arm}_endpoint_watch.out.log", run_dir / f"h18_{arm}_endpoint_watch.err.log",
        ),
        "treatment_launch": (
            [python, "-u", str(repo / "scripts/alpha_holdem/v5_hybrid_h18_treatment_launch_watch.py"), "--control-dir", str(control_dir), "--treatment-dir", str(treatment_dir), "--launcher", str(repo / "scripts/alpha_holdem/v5_hybrid_h18_launch_treatment.ps1"), "--design-lock", str(lock_path), "--expected-lock-sha256", lock_sha, "--status-json", str(control_dir / "h18_treatment_launch_watch_status.json"), "--poll-seconds", "15"],
            control_dir / "h18_treatment_launch_watch_status.json", control_dir / "h18_treatment_launch_watch.out.log", control_dir / "h18_treatment_launch_watch.err.log",
        ),
        "completion": (
            [python, "-u", str(repo / "scripts/alpha_holdem/v5_hybrid_h18_completion_watch.py"), "--repo", str(repo), *common, "--status-json", str(run_dir / "h18_completion_watch_status.json"), "--poll-seconds", "15"],
            run_dir / "h18_completion_watch_status.json", run_dir / "h18_completion_watch.out.log", run_dir / "h18_completion_watch.err.log",
        ),
    }
...
def main() -> int:
    ...
    registry_path = (args.allow_registry or (run_dir / "h18_lifecycle_allow_registry.json")).resolve()
    ...
    try:
        ...
        tool_keys = {
            "health": "scripts/alpha_holdem/v5_hybrid_h18_health_watch.py",
            "protocol": "scripts/alpha_holdem/v5_hybrid_h18_protocol_watch.py",
            "endpoint": "scripts/alpha_holdem/v5_hybrid_h18_endpoint_watch.py",
            "treatment_launch": "scripts/alpha_holdem/v5_hybrid_h18_treatment_launch_watch.py",
            "completion": "scripts/alpha_holdem/v5_hybrid_h18_completion_watch.py",
        }
        ...
        if sha256(Path(__file__).resolve()) != lock.get("tools", {}).get("scripts/alpha_holdem/v5_hybrid_h18_ordered_rearm.py"):
        ...
        preserve_dir = run_dir / f"h18_ordered_rearm_preserved_{stamp}"
        preserved = []
        started_at = datetime.now(timezone.utc).isoformat()
        registry: dict[str, Any] = {
            "schema_version": "v5.hybrid.h18.lifecycle_allow_registry.v1",
            "design_id": "H18",
        ...
        def update_registry(label: str, command: list[str], pid: int | None) -> None:
            rel = f"scripts/alpha_holdem/v5_hybrid_h18_{label}_watch.py"
            if label == "treatment_launch":
                rel = "scripts/alpha_holdem/v5_hybrid_h18_treatment_launch_watch.py"
        ...
        while True:
            ...
            if sentinel.get("terminal") is True and str(sentinel.get("state", "")).startswith("H18_TERMINAL"):
                return 0
            manifest = load(run_dir / "run_manifest.json")
            active = active_h18_trainers()
            for label, process in children.items():
                ...
                if active and not expected_terminal:
                    raise RuntimeError(f"{label} exited while H18 trainer active: {child_status}")
        ...
        for label, process in list(children.items()):
            ...
            if entry.get("pid"):
                cleanup_results.append({
                    "role": label,
                    **terminate_identity_tree(entry, "v5_hybrid_h18_", 15.0),
                })
        lifecycle_tokens = (
            "v5_hybrid_h18_health_watch.py",
            "v5_hybrid_h18_protocol_watch.py",
            "v5_hybrid_h18_endpoint_watch.py",
            "v5_hybrid_h18_treatment_launch_watch.py",
            "v5_hybrid_h18_completion_watch.py",
        )
        final_survivors = []
        for process in psutil.process_iter(["pid", "cmdline"]):
            try:
                ...
                continue
        cleanup_artifact = run_dir / f"h18_{args.arm}_lifecycle_cleanup.json" if "run_dir" in locals() else None
        if cleanup_artifact is not None:
            atomic_json(cleanup_artifact, {
                "schema_version": "v5.hybrid.h18.lifecycle_cleanup.v1",
                "checked_at": datetime.now(timezone.utc).isoformat(),
