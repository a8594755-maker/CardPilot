#!/usr/bin/env python3
"""Lock-bound dependency-ordered watcher supervisor for H13."""
...
SCHEMA = "v5.hybrid.h13.ordered_rearm_status.v1"
STAGES = {
...
def terminate_trainers() -> list[dict[str, Any]]:
    actions = []
    for pid in active_h13_trainers():
        try:
...
def command_map(repo: Path, run_dir: Path, arm: str, lock_path: Path, lock_sha: str) -> dict[str, tuple[list[str], Path, Path, Path]]:
    ...
    return {
        "health": (
            [python, "-u", str(repo / "scripts/alpha_holdem/v5_hybrid_h13_health_watch.py"), "--run-dir", str(run_dir), "--arm", arm, *common, "--status-json", str(run_dir / f"h13_{arm}_health_status.json"), "--poll-seconds", "15"],
            run_dir / f"h13_{arm}_health_status.json", run_dir / f"h13_{arm}_health_watch.out.log", run_dir / f"h13_{arm}_health_watch.err.log",
        ),
        "protocol": (
            [python, "-u", str(repo / "scripts/alpha_holdem/v5_hybrid_h13_protocol_watch.py"), "--arm", arm, *common, "--status-json", str(run_dir / f"h13_{arm}_protocol_status.json"), "--poll-seconds", "15", "--allowed-supervisor-pid", str(supervisor_pid), "--allowed-supervisor-command-sha256", supervisor_command_sha],
            run_dir / f"h13_{arm}_protocol_status.json", run_dir / f"h13_{arm}_protocol_watch.out.log", run_dir / f"h13_{arm}_protocol_watch.err.log",
        ),
        "endpoint": (
            [python, "-u", str(repo / "scripts/alpha_holdem/v5_hybrid_h13_endpoint_watch.py"), "--arm", arm, *common, "--status-json", str(run_dir / f"h13_{arm}_endpoint_status.json"), "--poll-seconds", "15"],
            run_dir / f"h13_{arm}_endpoint_status.json", run_dir / f"h13_{arm}_endpoint_watch.out.log", run_dir / f"h13_{arm}_endpoint_watch.err.log",
        ),
        "treatment_launch": (
            [python, "-u", str(repo / "scripts/alpha_holdem/v5_hybrid_h13_treatment_launch_watch.py"), "--control-dir", str(control_dir), "--treatment-dir", str(treatment_dir), "--launcher", str(repo / "scripts/alpha_holdem/v5_hybrid_h13_launch_treatment.ps1"), "--design-lock", str(lock_path), "--expected-lock-sha256", lock_sha, "--status-json", str(control_dir / "h13_treatment_launch_watch_status.json"), "--poll-seconds", "15"],
            control_dir / "h13_treatment_launch_watch_status.json", control_dir / "h13_treatment_launch_watch.out.log", control_dir / "h13_treatment_launch_watch.err.log",
        ),
        "completion": (
            [python, "-u", str(repo / "scripts/alpha_holdem/v5_hybrid_h13_completion_watch.py"), "--repo", str(repo), *common, "--status-json", str(run_dir / "h13_completion_watch_status.json"), "--poll-seconds", "15"],
            run_dir / "h13_completion_watch_status.json", run_dir / "h13_completion_watch.out.log", run_dir / "h13_completion_watch.err.log",
        ),
    }
...
def main() -> int:
    ...
    try:
        ...
        tool_keys = {
            "health": "scripts/alpha_holdem/v5_hybrid_h13_health_watch.py",
            "protocol": "scripts/alpha_holdem/v5_hybrid_h13_protocol_watch.py",
            "endpoint": "scripts/alpha_holdem/v5_hybrid_h13_endpoint_watch.py",
            "treatment_launch": "scripts/alpha_holdem/v5_hybrid_h13_treatment_launch_watch.py",
            "completion": "scripts/alpha_holdem/v5_hybrid_h13_completion_watch.py",
        }
        ...
        if sha256(Path(__file__).resolve()) != lock.get("tools", {}).get("scripts/alpha_holdem/v5_hybrid_h13_ordered_rearm.py"):
        ...
        preserve_dir = run_dir / f"h13_ordered_rearm_preserved_{stamp}"
        preserved = []
        ...
        while True:
            ...
            if sentinel.get("terminal") is True and str(sentinel.get("state", "")).startswith("H13_TERMINAL"):
                return 0
            ...
            for label, process in children.items():
                ...
                if active and not expected_terminal:
                    raise RuntimeError(f"{label} exited while H13 trainer active: {child_status}")
