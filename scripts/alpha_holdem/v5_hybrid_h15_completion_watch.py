#!/usr/bin/env python3
"""Duplicate-safe H15 post-arm evaluation and terminal judgment supervisor."""
...
def status(path: Path, **payload) -> None:
    value = {
        "schema_version": "v5.hybrid.h15.completion_watch_status.v1",
...
def main() -> int:
    ...
    try:
        ...
        tools = lock["tools"]
        mirror_tool = repo / "scripts/alpha_holdem/v5_hybrid_h15_mirror.py"
        judge_tool = repo / "scripts/alpha_holdem/v5_hybrid_h15_judge.py"
        active_window_tool = repo / "scripts/alpha_holdem/v5_hybrid_h15_active_window.py"
        if (
            sha(mirror_tool) != tools["scripts/alpha_holdem/v5_hybrid_h15_mirror.py"]
            or sha(judge_tool) != tools["scripts/alpha_holdem/v5_hybrid_h15_judge.py"]
            or sha(active_window_tool) != tools["scripts/alpha_holdem/v5_hybrid_h15_active_window.py"]
        ):
        ...
        mirror_tool_sha = tools["scripts/alpha_holdem/v5_hybrid_h15_mirror.py"]
        if (
        ...
        endpoint_status = {
            arm: directories[arm] / f"h15_{arm}_endpoint_status.json"
            for arm in directories
        }
        protocol = {
            arm: directories[arm] / f"h15_{arm}_protocol_status.json"
            for arm in directories
        }
        outputs = {
        ...
        judgment = repo / "reports/v5_hybrid_h15_judgment_20260719.json"
        ...
        while True:
            if judgment.is_file():
                ...
                if (
                    ...
                    status(
                        ...
                        h15_judgment=str(judgment.resolve()),
                    )
                    return 0
            terminal = None
            for arm in ("control", "treatment"):
            ...
            if terminal:
                run(
                    ...
                    status_path,
                    "RUNNING_H15_PROTOCOL_TERMINAL_JUDGMENT",
                )
                continue
            endpoints = {
            ...
            if active:
                status(
                    status_path,
                    overall="PENDING",
                    state="WAITING_FOR_ALL_H15_TRAINERS_TO_EXIT",
            ...
            if load(audit).get("overall") != "PASS_IMMUTABLE_H15_MIRROR":
