#!/usr/bin/env python3
"""Fail-closed H9 endpoint freezer for fresh contemporaneous arms."""
...
def main() -> int:
    ...
    if lock.get("design_id") != "H9" or lock.get("status") != "LOCKED": errors.append("lock identity/status")
    ...
    while True:
        ...
        if config.get("h9_window_arm") != args.arm or abs(float(config.get("ppo_target_kl", -1)) - target) > 1e-12: errors.append("H9 arm identity mismatch")
        if not bool(config.get("h8_value_head_catchup_after_kl_stop")): errors.append("H9 catch-up must be enabled in both arms")
        if config.get("h9_catchup_loss") != expected_loss or float(config.get("h9_catchup_smooth_l1_beta", -1)) != 1.0: errors.append("H9 catch-up loss identity mismatch")
        ...
        if checkpoint.get("h9_window_arm") != args.arm or abs(float(checkpoint.get("ppo_target_kl", -1)) - target) > 1e-12: errors.append("checkpoint H9 arm mismatch")
        if not bool(checkpoint.get("h8_value_head_catchup_after_kl_stop")): errors.append("checkpoint H9 catch-up disabled")
        if checkpoint.get("h9_catchup_loss") != expected_loss or float(checkpoint.get("h9_catchup_smooth_l1_beta", -1)) != 1.0: errors.append("checkpoint H9 loss mismatch")
        ...
        if checkpoint_config.get("h9_preregistration_sha256") != lock["preregistration"]["sha256"]: errors.append("checkpoint prereg mismatch")
        if checkpoint_config.get("h9_design_lock_sha256") != sha(lock_path): errors.append("checkpoint lock mismatch")
        if errors:
        ...
        write(run_dir / f"h9_{args.arm}_assignment_provenance_audit.json", provenance)
        if provenance.get("overall") != "PASS": readiness.append("provenance audit failed")
        resource = load(run_dir / f"h9_{args.arm}_protocol_status.json")
        ...
        frozen = run_dir / f"h9_{args.arm}_endpoint.pt"
