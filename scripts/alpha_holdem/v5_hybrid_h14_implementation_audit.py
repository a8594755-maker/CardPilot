#!/usr/bin/env python3
"""Offline fail-closed implementation audit for H14 robust catch-up-only loss."""
...
from scripts.alpha_holdem.test_v5_hybrid_h14_implementation import run_update
...
def main() -> int:
    ...
    try:
        ...
        check(
            ...
            and prereg.get("experiment_id") == "H14"
        ...
        check(
            ...
            prereg["single_variable"] == {
                ...
                "beta_selection": "unchanged canonical SmoothL1 default;no CAL-EXT-002,official-score,holdout or terminal-partial search",
            },
        )
        check(
            "trainer_cli_identity",
            all(token in trainer_source for token in (
                "--h14-window-arm", "--h14-catchup-loss",
                "--h14-catchup-smooth-l1-beta", "--h14-preregistration", "--h14-design-lock",
            )),
        )
        check(
            "trainer_fail_closed_contract",
            all(token in trainer_source for token in (
                "H14 arm run_id or fixed endpoint mismatch",
                "H14 exact canonical source checkpoint identity/hash mismatch",
                "H14 forbidden H9/H10/H11/H12/H13/CAL source path",
                "H14 arms require target-KL0.03 and value-head catch-up enabled",
        ...
        active_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h14_active_window.py").read_text(encoding="utf-8")
        ...
        protocol_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h14_protocol_watch.py").read_text(encoding="utf-8")
        check(
        ...
        check(
            ...
            and "H14 terminal transition requires an active control or treatment window" in active_source,
        )
        check(
        ...
        check(
            "canonical_rearm_strict_h14_contract",
            all(token in rearm_source for token in (
                "$script:isHybridH14Arm",
                "Launch-H14OrderedRearm",
                "v5_hybrid_h14_ordered_rearm.py",
                "H14 exact ordered lifecycle terminally blocks every generic project path",
            )),
        )
        health_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h14_health_watch.py").read_text(encoding="utf-8")
        ordered_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h14_ordered_rearm.py").read_text(encoding="utf-8")
        perf_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h14_perf_cal.py").read_text(encoding="utf-8")
        check(
        ...
        check(
            ...
            all(token in perf_source for token in (
                "LOSS_RATIO_MIN = 0.95", "COMMON_BASELINE_RATIO_MIN = 0.95",
                "alternating_mse_smoothl1", "value_head_only", "control-baseline",
            )),
        )
        ...
        result = {
            "schema_version": "v5.hybrid.h14.implementation_audit.v2",
            "checked_at": datetime.now(timezone.utc).isoformat(),
            "overall": "PASS_H14_IMPLEMENTATION" if not errors else "FAIL_CLOSED",
            "checks": checks,
            "details": details,
            ...
            "source_sha256": {
                ...
                "ppo": sha(args.ppo),
                "test": sha(args.test),
                "active_window": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h14_active_window.py"),
                "protocol_watch": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h14_protocol_watch.py"),
                "canonical_rearm": sha(rearm_path),
                "health_watch": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h14_health_watch.py"),
                "ordered_rearm": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h14_ordered_rearm.py"),
                "perf_cal": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h14_perf_cal.py"),
            },
