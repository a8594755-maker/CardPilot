...
PREREG = ROOT / "reports" / "v5_hybrid_h16_preregistration_20260719.json"
LOCK = sorted((ROOT / "reports").glob("v5_hybrid_h16_design_lock_v*_20260719.json"))[-1]
JUDGE = ROOT / "scripts" / "alpha_holdem" / "v5_hybrid_h16_judge.py"
...
def test_h16_judge_is_the_exact_locked_tool() -> None:
    lock = json.loads(LOCK.read_text(encoding="utf-8-sig"))
    assert sha(JUDGE) == lock["tools"]["scripts/alpha_holdem/v5_hybrid_h16_judge.py"]
    assert sha(PREREG) == lock["preregistration"]["sha256"]


def test_h16_lock_preserves_every_registered_gate_semantically() -> None:
...
def test_h16_terminal_checks_cover_all_registered_gates() -> None:
    ...
    assert 'classification = "H16_FAIL_REGISTERED_GATE"' in source
    assert 'classification = "H16_PASS_ALL_REGISTERED_GATES"' in source
    assert 'classification = "H16_INCONCLUSIVE_FIXED_SAMPLE"' in source
