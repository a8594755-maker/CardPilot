#!/usr/bin/env python3
"""PCV012 block-local device-envelope CUDA timing (reporting only)."""
...
PREREG_SHA = "614887a88dce34e7236401738238b2b59e6c0b23e35979aa6f14368cee872aa0"
SOURCE_SHA = "96a007039b0baa29f0c39b0bd7adc67d8ca0733a41a261203f52430e60b5ca13"
SOURCE_ITERATION = 35051
SOURCE_HANDS = 576021901
TRANSITION_SEED = 2026071988
UPDATE_SEED = 2026971988
CONDITIONING_SEED_BASE = 2027971988
ORDERS = (
    ...
    while True:
        for mode in ("mse", "smooth_l1"):
            ...
            if not forced_shape(stats):
                raise RuntimeError("PCV012 block-local conditioning update violated forced shape")
...
def main() -> int:
    ...
    if args.out.exists():
        raise SystemExit("refusing to overwrite immutable PCV012 result")
    if sha256(args.preregistration) != PREREG_SHA or sha256(args.source) != SOURCE_SHA:
        raise SystemExit("PCV012 preregistration/source identity mismatch")
    if not torch.cuda.is_available():
        raise SystemExit("PCV012 CUDA unavailable")
    forbidden = active_forbidden_processes()
    if forbidden:
        raise SystemExit(f"PCV012 forbidden process(es): {forbidden}")
    ...
    if int(checkpoint.get("iteration", -1)) != SOURCE_ITERATION or int(checkpoint.get("total_hands", -1)) != SOURCE_HANDS:
        raise SystemExit("PCV012 source iteration/hands mismatch")
    ...
    if not resource_pass(conditioning_before):
        raise RuntimeError("PCV012 phase-aware resource gate failed before conditioning")
    ...
    while True:
        for mode in ("mse", "smooth_l1"):
            ...
            if not forced_shape(stats):
                raise RuntimeError("PCV012 conditioning update violated forced shape")
    ...
    base = {
        "schema_version": "v5.pcv012.result.v1",
        ...
        "pcv010_or_pcv011_measurement_data_read_or_reused": False,
        "trainer_started": False,
        "checkpoint_written": False,
        "official_hands": 0,
        "causal_method_behavior_or_strength_inference": "FORBIDDEN",
        "next_authority": "ROUTE_REVIEW024_ONLY",
        "dependency_sha256": {
    ...
    if not conditioning_pass or not conditioning_resource_pass:
        result = {
            **base,
            "overall": "FAIL_CLOSED",
            "classification": "PCV012_FAIL_CLOSED_INITIAL_ENVELOPE_NO_MEASUREMENT",
    ...
    for block_index, order_name in enumerate(ORDERS):
        ...
        gate_before = resource_snapshot(args.path1_pid, 2 + block_index * 3)
        if not resource_pass(gate_before):
            raise RuntimeError(f"PCV012 phase-aware resource gate failed before block-local gate {block_index}")
        ...
        gate_after = resource_snapshot(args.path1_pid, 3 + block_index * 3)
        ...
        if not block_gate["envelope_pass"] or not resource_pass(gate_after):
            result = {
                **base,
                "overall": "FAIL_CLOSED",
                "classification": "PCV012_FAIL_CLOSED_BLOCK_LOCAL_ENVELOPE_NO_LATER_BLOCKS",
        ...
        for mode in modes:
            for update_index in range(12):
                ...
                if not forced_shape(stats):
                    raise RuntimeError("PCV012 timed update violated forced shape")
                values[mode].append(elapsed_ms)
                all_stats.append(stats)
        device_after = telemetry()
        phase_after = resource_snapshot(args.path1_pid, 4 + block_index * 3)
        if not resource_pass(phase_after):
            raise RuntimeError(f"PCV012 phase-aware resource gate failed after block {block_index}")
    ...
    if device_excursion:
        attribution = "IN_BLOCK_DEVICE_EXCURSION_PERSISTS"
    elif stability >= 0.95:
        attribution = "BLOCK_LOCAL_CONTROL_ASSOCIATION_SUPPORTED"
    else:
        attribution = "BLOCK_LOCAL_CONTROL_DID_NOT_RECOVER_STABILITY"
    ...
    result = {
        **base,
        ...
        "classification": "PCV012_PASS_MEASUREMENT_COMPLETE" if passed else "PCV012_FAIL_CLOSED",
