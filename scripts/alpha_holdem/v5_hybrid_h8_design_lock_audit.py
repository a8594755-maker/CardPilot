#!/usr/bin/env python3
"""Independent fail-closed H8 design-lock audit."""
...
def main() -> int:
    ...
    check("identity", value.get("design_id") == "H8" and value.get("status") == "LOCKED")
    source = value.get("source", {})
    check(
        "source",
        source.get("iteration") == 32617
        and source.get("hands") == 536001286
        and Path(source["path"]).is_file()
        and sha(Path(source["path"])) == source["sha256"],
    )
    anchor = value.get("h7_nostop_anchor", {})
    check(
        "anchor",
        anchor.get("iteration") == 32617
        and anchor.get("hands") == 536005488
    ...
    check(
        ...
        == {
            "name": "value_head_only_catchup_after_kl_stop",
            "control": False,
            "treatment": True,
    ...
    result = {
        "schema_version": "v5.hybrid.h8.design_lock_audit.v1",
        "checked_at": datetime.now(timezone.utc).isoformat(),
        "overall": "PASS_IMMUTABLE_H8_DESIGN_LOCK" if not errors else "FAIL_CLOSED",
        "checks": checks,
        "errors": errors,
