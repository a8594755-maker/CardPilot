#!/usr/bin/env python3
"""Run the immutable CPV003 trainerless lifecycle stress gate."""
...
def forbidden_processes() -> list[dict]:
    ...
    for process in psutil.process_iter(["pid", "cmdline"]):
        ...
        if any(token in lowered for token in ("train_v5.py", "play_slumbot", "v5_slumbot_benchmark", "v5_mirror_eval")):
            found.append({"pid": process.pid, "command": command})
    return found


def start_supervisor(workspace: Path, cycle: int, design_sha: str, token: str, suppress: bool = False) -> subprocess.Popen[bytes]:
    ...
    return subprocess.Popen(command, cwd=ROOT, stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, creationflags=creationflags)
...
def main() -> int:
    ...
    if args.out.exists() or args.out_dir.exists():
        raise SystemExit("refusing to overwrite CPV003 artifacts")
    prereg = json.loads(args.preregistration.read_text(encoding="utf-8"))
    if sha(args.preregistration) != args.expected_preregistration_sha256.lower():
        raise SystemExit("preregistration hash mismatch")
    if prereg.get("design_id") != "CPV003_TRAINERLESS_LIFECYCLE_STRESS_GATE":
    ...
    for cycle in range(int(prereg["fixture"]["stress_cycles"])):
        ...
        workspace.mkdir()
        token = f"cpv003-{prereg['fixture']['seed']}-{cycle}"
        started = time.monotonic()
        process = start_supervisor(workspace, cycle, args.expected_preregistration_sha256.lower(), token)
        supervisor_expected = None
        try:
            ...
            for role in ROLES:
                ...
                role_results[role] = validate_registered_child(
                    entry,
                    capture_process(entry["pid"]),
                    allowed_roles=ROLES,
                    expected_design_lock_sha256=args.expected_preregistration_sha256.lower(),
            ...
            for expected_token, mutation in mutations.items():
                ...
                adversarial[expected_token] = {
                    "pass": checked["overall"] == "FAIL_CLOSED" and expected_token in checked["tokens"] and checked["trigger_provenance_complete"] is True,
                    "result": checked,
                }
            ...
            cycle_checks = {
                ...
                "all_roles": len(role_results) == 5 and all(result["overall"] == "PASS" for result in role_results.values()),
                "adversarial": all(value["pass"] for value in adversarial.values()),
                ...
                "cleanup": shutdown is not None and shutdown.get("cleanup_elapsed_seconds", cleanup_deadline + 1) <= cleanup_deadline and not survivors and all(not item.get("survivors") for item in shutdown.get("cleanup", [])),
            }
            if not all(cycle_checks.values()):
                errors.append(f"cycle_{cycle:02d}")
            cycles.append({
                "cycle": cycle,
                "token": token,
                ...
                "survivors": survivors,
                "checks": cycle_checks,
                "overall": "PASS" if all(cycle_checks.values()) else "FAIL",
            })
        except Exception as exc:
            errors.append(f"cycle_{cycle:02d}:{type(exc).__name__}:{exc}")
        finally:
    ...
    failure_token = f"cpv003-{prereg['fixture']['seed']}-readiness-failure"
    failure_process = start_supervisor(failure_workspace, 999, args.expected_preregistration_sha256.lower(), failure_token, suppress=True)
    ...
    aggregate = {
        ...
        "all_cleanup_pass": len(cycles) == 20 and all(cycle["checks"]["cleanup"] for cycle in cycles),
    ...
    result = {
        "schema_version": "v5.cpv003.result.v1",
        "checked_at": datetime.now(timezone.utc).isoformat(),
        "design_id": prereg["design_id"],
        "overall": "PASS" if not errors and all(value for key, value in aggregate.items() if isinstance(value, bool)) and aggregate["cycles_passed"] == 20 else "FAIL_CLOSED",
        "classification": "CPV003_PASS_TRAINERLESS_LIFECYCLE_STRESS" if not errors else "CPV003_FAIL_REGISTERED_GATE",
        ...
        "gpu_used": False,
        "official_hands": 0,
        "terminal_effect": prereg["terminal_effect"]["PASS"] if not errors else prereg["terminal_effect"]["FAIL_OR_INCONCLUSIVE"],
        "strength_claim": "FORBIDDEN",
    }
    atomic_json(args.out, result)
    print(json.dumps({key: result[key] for key in ("overall", "classification", "aggregate", "errors", "terminal_effect")}, indent=2, sort_keys=True))
