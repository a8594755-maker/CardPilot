#!/usr/bin/env python3
"""Launch and audit the locked EXP-W1 100k common-deal primary after both arms."""
...
def locked_classification(summary: dict[str, Any], audit: dict[str, Any],
                          max_halfwidth_bb100: float) -> tuple[str, str]:
    if audit.get('status') != 'PASS':
        return 'FAIL', 'BUNDLE_AUDIT_FAILED'
    effect = (summary.get('primary_effects') or {}).get('post_vs_pre_direct')
    if not isinstance(effect, dict):
        return 'FAIL', 'TREATMENT_VS_CONTROL_PRIMARY_EFFECT_MISSING'
    halfwidth = float(effect['ci95_halfwidth_bb100'])
    lower = float(effect['ci95_lower_bb100'])
    upper = float(effect['ci95_upper_bb100'])
    if halfwidth > max_halfwidth_bb100:
        return 'INCONCLUSIVE', 'LOCKED_PRECISION_GATE_FAILED'
    if lower > 0.0:
        return 'PASS', 'TREATMENT_VS_CONTROL_LOWER_BOUND_POSITIVE'
    if upper <= 0.0:
        return 'FAIL', 'TREATMENT_VS_CONTROL_UPPER_BOUND_NONPOSITIVE'
    return 'INCONCLUSIVE', 'TREATMENT_VS_CONTROL_OVERLAPS_ZERO'
...
def main() -> int:
    ...
    if (
        ...
        static_errors.append('EXP-W1 design lock identity mismatch')
    if int(measurement.get('pairs') or -1) != 100000 or int(measurement.get('seed') or -1) != 2026071102:
    ...
    while True:
        ...
        out_dir = repo / 'reports/exp_w1_primary_100k'
        outputs = {
        ...
        write_json(status_path, {'checked_at': now_iso(), 'overall': 'PENDING', 'state': 'PRIMARY_100K_RUNNING',
                                 'pairs': 100000, 'seed': int(measurement['seed']), 'priority': 'below-normal'})
        command = [
            sys.executable, str(evaluator), '--pre', str(control['checkpoint_path']), '--post', str(treatment['checkpoint_path']),
            '--native', str(native), '--pre-label', 'EXP-W1 control endpoint', '--post-label', 'EXP-W1 treatment endpoint',
            ...
            '--expected-native-hands', str(measurement['native_anchor']['hands']), '--pairs', '100000', '--seed', str(measurement['seed']),
        ...
        decision, reason = locked_classification(summary, audit, float(lock['numerical_gates']['primary_100k_paired']['max_ci_halfwidth_bb100']))
        ...
        row = (f"\n| {local} | EXP-W1 exactly100k common-deal primary terminal {decision} | "
               ...
               "[event_id=v5-exp_w1-primary-100k-terminal] |\n")
