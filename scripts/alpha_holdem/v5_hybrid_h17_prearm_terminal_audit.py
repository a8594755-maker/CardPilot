#!/usr/bin/env python3
"""Independent terminal audit for H17 pre-arm calibration failure."""
...
OUT = REPORTS / "v5_hybrid_h17_prearm_terminal_audit_20260719.json"
...
def main() -> int:
    if OUT.exists():
        raise SystemExit("refusing to overwrite immutable H17 terminal audit")
    paths = {
        "prereg": REPORTS / "v5_hybrid_h17_preregistration_20260719.json",
        "lock": REPORTS / "v5_hybrid_h17_design_lock_v1_20260719.json",
        "lock_audit": REPORTS / "v5_hybrid_h17_design_lock_audit_v1_20260719.json",
        "preflight": REPORTS / "v5_hybrid_h17_preflight_v1_20260719.json",
        "perf": REPORTS / "v5_hybrid_h17_control_perf_cal_20260719.json",
        "perf_audit": REPORTS / "v5_hybrid_h17_control_perf_cal_audit_20260719.json",
        "judgment": REPORTS / "v5_hybrid_h17_judgment_20260719.json",
    }
    expected = {
        "prereg": "df256560d69928c9f70e6df5457c1575cc81124ba80f34f9b15261293cefe7fc",
    ...
    checks["lock_and_audit_pass"] = lock.get("status") == "LOCKED" and lock_audit.get("overall") == "PASS_IMMUTABLE_H17_DESIGN_LOCK"
    checks["preflight_pass"] = preflight.get("overall") == "PASS_READY_H17_CONTROL_LAUNCH"
    checks["perf_registered_gate_fail"] = (
        perf.get("overall") == "FAIL_CLOSED"
        and perf.get("classification") == "H17_REPRESENTATIVE_PERF_CAL_FAIL"
    ...
    checks["judgment_identity"] = judgment.get("verdict") == "FAIL" and judgment.get("classification") == "H17_FAIL_PREARM_REPRESENTATIVE_PERF_CAL_GATE_NO_LAUNCH"
    control = ROOT / "models/alpha_holdem_v5_hybrid/v5_hybrid_h17_control_catchmse_same35051_20m_r1_20260719"
    treatment = ROOT / "models/alpha_holdem_v5_hybrid/v5_hybrid_h17_treatment_catchsmoothl1b1_same35051_20m_r1_20260719"
    ...
    checks["sentinel_not_h17_active"] = not (sentinel.get("design_id") == "H17" and sentinel.get("terminal") is not True)
    trainers = []
    forbidden = []
    for process in psutil.process_iter(["pid", "cmdline"]):
        try:
            ...
            if any(token in command for token in ("v5_hybrid_h17_mirror.py", "v5_slumbot", "play_slumbot")):
    ...
    artifact = {
        "schema_version": "v5.hybrid.h17.prearm_terminal_audit.v1",
