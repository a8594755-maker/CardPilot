...
$controlId='v5_hybrid_h16_control_catchmse_same35051_20m_r1_20260719'
$runId='v5_hybrid_h16_treatment_catchsmoothl1b1_same35051_20m_r1_20260719'
...
$prereg=(Resolve-Path 'reports/v5_hybrid_h16_preregistration_20260719.json').Path
$lock=(Resolve-Path 'reports/v5_hybrid_h16_design_lock_v1_20260719.json').Path
...
$preregHash=(Get-FileHash $prereg -Algorithm SHA256).Hash; if (-not $preregHash.Equals('51065761b6b291ef757ea467611203cbe79d45a5e4d54c163edaf79ef8fa1bb0',[StringComparison]::OrdinalIgnoreCase)) { throw "prereg hash actual=$preregHash" }
...
$endpoint=Get-Content (Join-Path $controlDir 'h16_control_endpoint_status.json') -Raw|ConvertFrom-Json
$protocol=Get-Content (Join-Path $controlDir 'h16_control_protocol_status.json') -Raw|ConvertFrom-Json
...
$mirrorDir=Resolve-Path 'reports/h16_mirror_001_v3_20260719'
$unexpectedMirror=@(Get-ChildItem $mirrorDir|Where-Object{$_.Name -notin @('manifest.json','measurement_lock.json')})
$controlOrdered=Get-CimInstance Win32_Process | Where-Object { ([string]$_.CommandLine) -like '*v5_hybrid_h16_ordered_rearm.py*' -and ([string]$_.CommandLine) -like '*--arm control*' }
...
if(-not $ready){throw 'H16 treatment not ready'}
$controlPerf=Join-Path $Repo 'reports/v5_hybrid_h16_control_perf_cal_20260719.json'
$perf=Join-Path $Repo 'reports/v5_hybrid_h16_treatment_perf_cal_20260719.json'
$perfAudit=Join-Path $Repo 'reports/v5_hybrid_h16_treatment_perf_cal_audit_20260719.json'
if(-not(Test-Path $controlPerf)){throw 'H16 treatment requires immutable control PERF-CAL baseline'}
if(-not(Test-Path $perf)){
 python scripts/alpha_holdem/v5_hybrid_h16_perf_cal.py --source $source --source-sha256 '96a007039b0baa29f0c39b0bd7adc67d8ca0733a41a261203f52430e60b5ca13' --arm treatment --seed 2026071917 --rows 4096 --mini-batch-size 1024 --ppo-epochs 4 --target-kl 1e-12 --warmup-updates 2 --timed-updates 8 --repeats 5 --device cuda --path1-pid $path1Pid --path1-workers 6 --out $perf
 if($LASTEXITCODE -ne 0){throw 'H16 treatment PERF-CAL failed closed'}
}
if(-not(Test-Path $perfAudit)){
 python scripts/alpha_holdem/v5_hybrid_h16_perf_cal_audit.py --artifact $perf --tool scripts/alpha_holdem/v5_hybrid_h16_perf_cal.py --out $perfAudit
 if($LASTEXITCODE -ne 0){throw 'H16 treatment PERF-CAL audit failed closed'}
}
...
if($manifest.run_id -ne $runId -or $manifest.config.h16_window_arm -ne 'treatment' -or (-not [bool]$manifest.config.h8_value_head_catchup_after_kl_stop) -or $manifest.config.h16_catchup_loss -ne 'smooth_l1' -or $manifest.config.h16_design_lock_sha256 -ne $expected){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw 'manifest H16 treatment identity'}
$priorCompletion=@(Get-CimInstance Win32_Process -Filter "Name='python.exe'" | Where-Object {
    $command=[string]$_.CommandLine
    $command -like '*v5_hybrid_h16_completion_watch.py*' -and $command -like "*$controlId*"
})
if($priorCompletion.Count -gt 1){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw "multiple prior H16 completion supervisors: $($priorCompletion.ProcessId -join ',')"}
...
if($priorCompletion.Count -eq 1){
    ...
    if(Get-Process -Id $retiredCompletionPid -ErrorAction SilentlyContinue){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw 'prior H16 completion supervisor did not exit'}
}
try{
 ...
 $orderedPath=Join-Path $runDir 'h16_ordered_rearm_status.json';$orderedDeadline=(Get-Date).AddSeconds(190);$ordered=$null
...
[pscustomobject]@{status='H16_TREATMENT_LAUNCHED_REARM_PASS';trainer_pid=$trainer.Id;run_id=$runId;design_lock_sha256=$expected;path1_pid=$path1Pid;ordered_rearm_state=$ordered.state;retired_control_completion_pid=$retiredCompletionPid}|ConvertTo-Json
