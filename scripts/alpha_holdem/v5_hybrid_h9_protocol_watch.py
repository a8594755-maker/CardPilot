#!/usr/bin/env python3
"""H9 throughput, entropy and resource-isolation protocol watcher."""
...
FORBIDDEN_PROCESS_TOKENS = ("v5_hybrid_h9_mirror.py", "v5_h1_calibration.py compare", "v5_slumbot", "slumbot_match")
...
def forbidden_processes(trainer_pid: int) -> list[dict]:
    ...
    for process in psutil.process_iter(["pid", "cmdline"]):
        try:
            ...
            if any(token in command for token in FORBIDDEN_PROCESS_TOKENS): found.append({"pid": process.pid, "token": next(token for token in FORBIDDEN_PROCESS_TOKENS if token in command)})
...
def main() -> int:
    ...
    while True:
        ...
        if violations:
            action = stop(pid); write(status_path, {"overall": "FAIL", "state": "H9_INCONCLUSIVE_RESOURCE_ISOLATION_VIOLATION", "arm": args.arm, "pid": pid, "stop_action": action, "resource_isolation_violations": violations}); return 3
        if len(values) >= 20 and all(float(item.get("entropy", 99)) < 0.3 for item in values[-20:]):
            action = stop(pid); write(status_path, {"overall": "FAIL", "state": "H9_FAIL_PROTOCOL_ABORT_ENTROPY20", "arm": args.arm, "pid": pid, "stop_action": action, "resource_isolation_violations": []}); return 3
