...
PREREG = ROOT / "reports" / "v5_hybrid_h15_preregistration_20260719.json"
LOCK = sorted((ROOT / "reports").glob("v5_hybrid_h15_design_lock_v*_20260719.json"))[-1]
JUDGE = ROOT / "scripts" / "alpha_holdem" / "v5_hybrid_h15_judge.py"
...
def test_h15_judge_is_the_exact_locked_tool() -> None:
    lock = json.loads(LOCK.read_text(encoding="utf-8-sig"))
    assert sha(JUDGE) == lock["tools"]["scripts/alpha_holdem/v5_hybrid_h15_judge.py"]
    assert sha(PREREG) == lock["preregistration"]["sha256"]


def test_h15_lock_preserves_every_registered_gate_semantically() -> None:
...
def test_h15_terminal_checks_cover_all_registered_gates() -> None:
    ...
    assert 'classification = "H15_FAIL_REGISTERED_GATE"' in source
    assert 'classification = "H15_PASS_ALL_REGISTERED_GATES"' in source
    assert 'classification = "H15_INCONCLUSIVE_FIXED_SAMPLE"' in source
