...
$CorrectionToken = '8bf8cedf78b6e8c8fe153802908ed893'
$Trainer = "C:\Users\a8594\CardPilot\scripts\alpha_holdem\v5_lg003c1_train_${CorrectionToken}.py"
$Preregistration = "C:\Users\a8594\CardPilot\reports\v5_lg003_cleanroom_diversity_league_preregistration_${ParentToken}_20260723.json"
...
$OutputRoot = "C:\Users\a8594\CardPilot\models\alpha_holdem_v5_hybrid\v5_lg003c1_${CorrectionToken}_20260723"

$Arm = if ($Mode -in @('ProbeControl', 'StageAControl')) { 'control_uniform' } else { 'treatment_diversity' }
...
$RunId = "v5_lg003c1_${Arm}_${Stage}_${CorrectionToken}"
...
if ($Mode -like 'Probe*') {
    if (Test-Path -LiteralPath $OutputRoot) { throw 'LG003C1 probe root collision.' }
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { throw "LG003C1 probe child exit $LASTEXITCODE" }
    } finally {
    ...
    if (Test-Path -LiteralPath $OutputRoot) { throw 'LG003C1 probe wrote output.' }
    exit 0
}

if (Test-Path -LiteralPath $RunDir) { throw "LG003C1 run collision: $RunDir" }
& $Python @TrainerArgs
exit $LASTEXITCODE
