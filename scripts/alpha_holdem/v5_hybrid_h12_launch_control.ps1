...
$runId='v5_hybrid_h12_control_catchmse_same35051_20m_r1_20260716'
...
$preflight=(Resolve-Path 'reports/v5_hybrid_h12_preflight_20260716.json').Path
...
$ready=$pf.overall -eq 'PASS_READY_H12_CONTROL_LAUNCH' -and $pf.design_lock_sha256 -eq $expected -and $path1 -and $path1.PriorityClass -eq 'BelowNormal' -and (-not(Test-Path $runDir))
...
if(-not $ready){throw 'H12 control not ready'}
$perf=Join-Path $Repo 'reports/v5_hybrid_h12_control_perf_cal_20260716.json'
$perfAudit=Join-Path $Repo 'reports/v5_hybrid_h12_control_perf_cal_audit_20260716.json'
if(-not(Test-Path $perf)){
 python scripts/alpha_holdem/v5_hybrid_h12_perf_cal.py --source $source --source-sha256 '96a007039b0baa29f0c39b0bd7adc67d8ca0733a41a261203f52430e60b5ca13' --arm control --seed 2026071601 --batch-size 1024 --warmup 10 --steps 40 --repeats 3 --device cuda --path1-pid 37656 --path1-workers 6 --out $perf
 if($LASTEXITCODE -ne 0){throw 'H12 control PERF-CAL failed closed'}
}
if(-not(Test-Path $perfAudit)){
 python scripts/alpha_holdem/v5_hybrid_h12_perf_cal_audit.py --artifact $perf --tool scripts/alpha_holdem/v5_hybrid_h12_perf_cal.py --out $perfAudit
 if($LASTEXITCODE -ne 0){throw 'H12 control PERF-CAL audit failed closed'}
}
...
if($manifest.run_id -ne $runId -or $manifest.config.h12_window_arm -ne 'control' -or (-not [bool]$manifest.config.h8_value_head_catchup_after_kl_stop) -or $manifest.config.h12_catchup_loss -ne 'mse' -or $manifest.config.h12_design_lock_sha256 -ne $expected){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw 'manifest H12 control identity'}
try{
...
[pscustomobject]@{status='H12_CONTROL_LAUNCHED_REARM_PASS';trainer_pid=$trainer.Id;run_id=$runId;design_lock_sha256=$expected;path1_pid=37656}|ConvertTo-Json
