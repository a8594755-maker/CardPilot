#!/usr/bin/env python3
"""Fail-closed H7 endpoint freezer for fresh contemporaneous arms."""
...
def main() -> int:
    ...
    if lock.get("design_id") != "H7" or lock.get("status") != "LOCKED": errors.append("lock identity/status")
    if not arm or lock.get("arm_budget", {}).get("minimum_endpoint_hands") != 535989661: errors.append("arm/budget incomplete")
    if errors:
    ...
    while True:
        ...
        target = 0.03 if args.arm == "treatment" else 0.0
        if manifest.get("run_id") != arm["run_id"]: errors.append("run_id mismatch")
        if config.get("h7_window_arm") != args.arm or abs(float(config.get("ppo_target_kl", -1)) - target) > 1e-12: errors.append("H7 arm identity mismatch")
        ...
        if checkpoint.get("h7_window_arm") != args.arm or abs(float(checkpoint.get("ppo_target_kl", -1)) - target) > 1e-12: errors.append("checkpoint H7 arm mismatch")
        checkpoint_config = checkpoint.get("config") or {}
        if checkpoint_config.get("h7_preregistration_sha256") != lock["preregistration"]["sha256"]: errors.append("checkpoint prereg mismatch")
        if checkpoint_config.get("h7_design_lock_sha256") != sha(lock_path): errors.append("checkpoint lock mismatch")
        if errors:
        ...
        try:
            provenance = audit_provenance(Path(arm["provenance_path"]), expected_run_id=arm["run_id"], expected_mode="per-iteration", expected_workers=22, expected_groups=5, expected_worker_seed_base=73000, expected_first_iteration=31401, expected_last_iteration=iteration)
        except Exception as exc: provenance = {"overall": "FAIL", "errors": [str(exc)]}
        write(run_dir / f"h7_{args.arm}_assignment_provenance_audit.json", provenance)
        if provenance.get("overall") != "PASS": readiness.append("provenance audit failed")
        resource = load(run_dir / f"h7_{args.arm}_protocol_status.json")
        ...
        frozen = run_dir / f"h7_{args.arm}_endpoint.pt"
