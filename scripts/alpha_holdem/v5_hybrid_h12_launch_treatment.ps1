...
$controlId='v5_hybrid_h12_control_catchmse_same35051_20m_r1_20260716'
$runId='v5_hybrid_h12_treatment_catchsmoothl1b1_same35051_20m_r1_20260716'
...
$prereg=(Resolve-Path 'reports/v5_hybrid_h12_preregistration_v3_20260716.json').Path
$lock=(Resolve-Path 'reports/v5_hybrid_h12_design_lock_v2_20260716.json').Path
...
$preregHash=(Get-FileHash $prereg -Algorithm SHA256).Hash; if (-not $preregHash.Equals('7ecd7a4342f75a92f4d4f12493bcd5fda9e3e92e7f43f023f8779becbdb48e57',[StringComparison]::OrdinalIgnoreCase)) { throw "prereg hash actual=$preregHash" }
...
$endpoint=Get-Content (Join-Path $controlDir 'h12_control_endpoint_status.json') -Raw|ConvertFrom-Json
$protocol=Get-Content (Join-Path $controlDir 'h12_control_protocol_status.json') -Raw|ConvertFrom-Json
...
$mirrorDir=Resolve-Path 'reports/h12_mirror_001_v3_20260716'
...
if(-not $ready){throw 'H12 treatment not ready'}
$controlPerf=Join-Path $Repo 'reports/v5_hybrid_h12_control_perf_cal_20260716.json'
$perf=Join-Path $Repo 'reports/v5_hybrid_h12_treatment_perf_cal_20260716.json'
$perfAudit=Join-Path $Repo 'reports/v5_hybrid_h12_treatment_perf_cal_audit_20260716.json'
if(-not(Test-Path $controlPerf)){throw 'H12 treatment requires immutable control PERF-CAL baseline'}
if(-not(Test-Path $perf)){
 python scripts/alpha_holdem/v5_hybrid_h12_perf_cal.py --source $source --source-sha256 '96a007039b0baa29f0c39b0bd7adc67d8ca0733a41a261203f52430e60b5ca13' --arm treatment --control-baseline $controlPerf --seed 2026071601 --batch-size 1024 --warmup 10 --steps 40 --repeats 3 --device cuda --path1-pid 37656 --path1-workers 6 --out $perf
 if($LASTEXITCODE -ne 0){throw 'H12 treatment PERF-CAL failed closed'}
}
if(-not(Test-Path $perfAudit)){
 python scripts/alpha_holdem/v5_hybrid_h12_perf_cal_audit.py --artifact $perf --tool scripts/alpha_holdem/v5_hybrid_h12_perf_cal.py --control-baseline $controlPerf --out $perfAudit
 if($LASTEXITCODE -ne 0){throw 'H12 treatment PERF-CAL audit failed closed'}
}
...
if($manifest.run_id -ne $runId -or $manifest.config.h12_window_arm -ne 'treatment' -or (-not [bool]$manifest.config.h8_value_head_catchup_after_kl_stop) -or $manifest.config.h12_catchup_loss -ne 'smooth_l1' -or $manifest.config.h12_design_lock_sha256 -ne $expected){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw 'manifest H12 treatment identity'}
$priorCompletion=@(Get-CimInstance Win32_Process -Filter "Name='python.exe'" | Where-Object {
    $command=[string]$_.CommandLine
    $command -like '*v5_hybrid_h12_completion_watch.py*' -and $command -like "*$controlId*"
})
if($priorCompletion.Count -gt 1){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw "multiple prior H12 completion supervisors: $($priorCompletion.ProcessId -join ',')"}
...
if($priorCompletion.Count -eq 1){
    ...
    if(Get-Process -Id $retiredCompletionPid -ErrorAction SilentlyContinue){Stop-Process -Id $trainer.Id -Force -ErrorAction SilentlyContinue;throw 'prior H12 completion supervisor did not exit'}
}
try{
...
[pscustomobject]@{status='H12_TREATMENT_LAUNCHED_REARM_PASS';trainer_pid=$trainer.Id;run_id=$runId;design_lock_sha256=$expected;path1_pid=37656;retired_control_completion_pid=$retiredCompletionPid}|ConvertTo-Json
