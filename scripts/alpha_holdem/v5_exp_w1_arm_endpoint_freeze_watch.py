#!/usr/bin/env python3
"""Freeze and fail-closed audit the first completed EXP-W1 arm endpoint."""
...
def main() -> int:
    ...
    if (
        ...
        or int(lock.get('lock_revision') or -1) not in {2, 3}
    ):
        static_errors.append('EXP-W1 design lock status/id/revision mismatch')
    ...
    while True:
        ...
        warmup = checkpoint.get('exp_w1_value_warmup')
        warmup = warmup if isinstance(warmup, dict) else {}
        if args.arm == 'control':
            if warmup.get('status') != 'DISABLED' or int(warmup.get('epochs') or 0) != 0:
                errors.append('control checkpoint EXP-W1 warmup state is not exact DISABLED/0')
        else:
            if warmup.get('status') != 'PASS' or int(warmup.get('epochs') or 0) != 8:
                errors.append('treatment checkpoint EXP-W1 warmup state is not exact PASS/8')
            report_path = Path(str(warmup.get('report_path') or ''))
            if (
                not report_path.exists()
                or sha256_path(report_path) != warmup.get('report_sha256')
            ):
                errors.append('treatment immutable warmup report identity/hash failure')
        ...
        provenance_out = run_dir / f'exp_w1_{args.arm}_assignment_provenance_audit.json'
        ...
        frozen = run_dir / f'v5_exp_w1_{args.arm}_endpoint.pt'
        if frozen.exists():
            write_status(status_path, {'checked_at': now_iso(), 'overall': 'FAIL',
        ...
        row = (
            f"\n| {local_time} | EXP-W1 {args.arm} endpoint frozen and audited | "
            ...
            f"[event_id=v5-exp_w1-{args.arm}-endpoint-frozen] |\n"
        )
