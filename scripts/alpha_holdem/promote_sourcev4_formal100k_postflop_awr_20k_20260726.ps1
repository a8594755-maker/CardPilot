...
$quickDir = 'models\bench_sourcev4_slumbot_formal100k_postflop_awr_selected_pure_fresh5k_20260726'
$sourcePolicy = 'models\sourcev4_slumbot_formal100k_postflop_awr_adapter256_mappingfix_20260726\selected.pt'
    ...
    $active = @(
        Get-CimInstance Win32_Process |
            Where-Object {
                $_.CommandLine -and
                $_.CommandLine -match
                    'bench_sourcev4_slumbot_formal100k_postflop_awr_selected_pure_fresh5k_20260726'
            }
    )
    if ($active.Count -eq 0) {
        throw 'Formal100k postflop AWR fresh5k exited without one CI summary'
    }
...
if ($summaries.Count -ne 1) {
    throw 'Timed out waiting for the formal100k postflop AWR fresh5k summary'
}
...
# A quick5k is noisy.  Promote a candidate that materially improves on the
# -31.3374 bb/100 best completed pure reference and still has positive support.
$promote = (
...
$decision = [ordered]@{
    ...
    candidate = 'sourcev4_formal100k_postflop_awr_epoch1'
...
$freezeManifest = [ordered]@{
    ...
    source_offline_decision_samples = 71895
    ...
    training_method = 'advantage-weighted regression on Slumbot formal100k postflop decisions'
...
$externalDir = 'models\bench_sourcev4_formal100k_postflop_awr_epoch1_pure_fresh20k_20260726'
if (Test-Path -LiteralPath $externalDir) {
    throw "Fresh20k output already exists: $externalDir"
}
New-Item -ItemType Directory -Path $externalDir | Out-Null
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
    'scripts/alpha_holdem/bench_v55_slumbot.ps1' `
    -ModelPath (Resolve-Path -LiteralPath $frozenPolicy).Path `
    -Tag 'sourcev4_formal100k_postflop_awr_epoch1_pure_fresh20k_20260726' `
...
if ($LASTEXITCODE -ne 0) {
    throw 'Fresh20k benchmark failed'
}

$twentySummaries = @(
    Get-ChildItem -LiteralPath $externalDir -Filter '*_ci_summary.json' -File
)
...
$formalDecision = [ordered]@{
    ...
    launch_formal100k = $formalPlausible
    decided_at = (Get-Date).ToUniversalTime().ToString('o')
}
$formalDecision |
    ConvertTo-Json -Depth 6 |
    Set-Content -LiteralPath (Join-Path $externalDir 'formal100k_decision.json') `
        -Encoding UTF8
if (-not $formalPlausible) {
    exit 0
}

$formalDir = 'models\bench_sourcev4_formal100k_postflop_awr_epoch1_pure_formal100k_20260726'
...
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
    'scripts/alpha_holdem/bench_v55_slumbot.ps1' `
    -ModelPath (Resolve-Path -LiteralPath $frozenPolicy).Path `
    -Tag 'sourcev4_formal100k_postflop_awr_epoch1_pure_formal100k_20260726' `
