#!/usr/bin/env python3
"""Frozen H11 40k three-endpoint common-deal mirror evaluator."""
...
PREREG_SHA = "d493b1f9e936d89f0c2e51a0b6c5dbc5a8dd20b312d5f9cd5e415f43f44528d0"
...
ANCHOR_SHA = "7c388ec68114d497f775157def3c0b3abc82db7ee25baf7e7cb7e9e916f66438"
...
def load_lock(path: Path, expected: str, manifest: Path) -> dict:
    ...
    if value.get("design_id") != "H11-MIRROR-001" or value.get("status") != "LOCKED":
...
def prepare(out: Path) -> dict:
    ...
    for index in range(PAIRS):
        ...
        deals.append(
            {
                "index": index,
                "deal_id": f"h11mirror-{SEED}-{index:05d}-{deck_hash[:16]}",
                "deck_sha256": deck_hash,
                "opponent_pool_id": ACTIVE_POOL_IDS[index % 5],
            }
        )
    value = {
        "schema_version": "v5.hybrid.h11.mirror_manifest.v1",
        "design_id": "H11",
...
def audit_manifest(value: dict) -> list[str]:
    errors = []
    if value.get("schema_version") != "v5.hybrid.h11.mirror_manifest.v1" or value.get("preregistration_sha256") != PREREG_SHA:
...
def checkpoint_identity(path: Path, arm: str) -> dict:
    ...
    if arm == "anchor":
        ...
        if checkpoint.get("h8_window_arm") != "treatment" or float(checkpoint.get("ppo_target_kl", -1)) != 0.03:
            raise ValueError("anchor H8 identity")
        if int(checkpoint.get("iteration", -1)) != 33834 or int(checkpoint.get("total_hands", -1)) != 556011085:
        ...
        return {
            ...
            "hands": 556011085,
            "arm": arm,
            "source_identity": "H8_TREATMENT_SOURCE_ANCHOR",
        }
    expected_loss = "mse" if arm == "control" else "smooth_l1"
    if checkpoint.get("h11_window_arm") != arm or abs(float(checkpoint.get("ppo_target_kl", -1)) - 0.03) > 1e-12:
        raise ValueError("endpoint H11 arm identity")
    if not bool(checkpoint.get("h8_value_head_catchup_after_kl_stop")):
        raise ValueError("endpoint H11 catch-up disabled")
    if checkpoint.get("h11_catchup_loss") != expected_loss or float(checkpoint.get("h11_catchup_smooth_l1_beta", -1)) != 1.0:
        raise ValueError("endpoint H11 catch-up loss identity")
    if config.get("h11_preregistration_sha256") != PREREG_SHA:
        raise ValueError("endpoint prereg identity")
    if not 576011085 <= int(checkpoint.get("total_hands", -1)) <= 576061085:
...
def run_arm(manifest_path, endpoint, arm, out, device, lock_path, lock_sha, runtime):
    ...
    with out.open("x", encoding="utf-8", newline="\n") as stream:
        for expected in manifest["deals"]:
            ...
            stream.write(
                json.dumps(
                    {
                        "schema_version": "v5.hybrid.h11.mirror_pair.v1",
                        "arm": arm,
    ...
    summary = {
        "schema_version": "v5.hybrid.h11.mirror_arm_summary.v1",
        "arm": arm,
        "endpoint": identity,
...
def read_rows(path: Path, arm: str, manifest: dict):
    ...
    for index, row in enumerate(values):
        ...
        if (
            row.get("schema_version") != "v5.hybrid.h11.mirror_pair.v1"
            or row.get("arm") != arm
...
def audit_bundle(manifest_path, control, treatment, anchor, out, lock_path, lock_sha):
    ...
    value = {
        "schema_version": "v5.hybrid.h11.mirror_audit.v1",
        "overall": "PASS_IMMUTABLE_H11_MIRROR" if not errors else "FAIL_CLOSED",
        "errors": errors,
        "manifest_sha256": sha256_file(manifest_path),
        "measurement_lock_sha256": sha256_file(lock_path),
        "tool_sha256": sha256_file(TOOL),
...
def judge(manifest_path, control, treatment, anchor, audit, out, lock_path, lock_sha):
    ...
    if audit_value.get("overall") != "PASS_IMMUTABLE_H11_MIRROR":
    ...
    value = {
        "schema_version": "v5.hybrid.h11.mirror_judgment.v1",
        "status": status,
        "pairs": PAIRS,
        "comparisons": {
