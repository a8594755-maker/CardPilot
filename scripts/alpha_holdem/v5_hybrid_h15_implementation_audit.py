#!/usr/bin/env python3
"""Offline fail-closed implementation audit for H15 robust catch-up-only loss."""
...
from scripts.alpha_holdem.test_v5_hybrid_h15_implementation import run_update
...
def main() -> int:
    ...
    try:
        ...
        check(
            ...
            and prereg.get("experiment_id") == "H15"
        ...
        check(
            ...
            all(token in trainer_source for token in (
                "--h15-window-arm", "--h15-catchup-loss",
                "--h15-catchup-smooth-l1-beta", "--h15-preregistration", "--h15-design-lock",
            )),
        )
        check(
            "trainer_fail_closed_contract",
            all(token in trainer_source for token in (
                "H15 arm run_id or fixed endpoint mismatch",
                "H15 exact canonical source checkpoint identity/hash mismatch",
                "H15 forbidden H9/H10/H11/H12/H13/CAL source path",
                "H15 arms require target-KL0.03 and value-head catch-up enabled",
        ...
        active_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h15_active_window.py").read_text(encoding="utf-8")
        ...
        protocol_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h15_protocol_watch.py").read_text(encoding="utf-8")
        check(
        ...
        check(
            ...
            and "H15 terminal transition requires an active control or treatment window" in active_source,
        )
        check(
        ...
        check(
            "canonical_rearm_strict_h15_contract",
            all(token in rearm_source for token in (
                "$script:isHybridH15Arm",
        ...
        health_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h15_health_watch.py").read_text(encoding="utf-8")
        ordered_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h15_ordered_rearm.py").read_text(encoding="utf-8")
        perf_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h15_perf_cal.py").read_text(encoding="utf-8")
        check(
        ...
        result = {
            "schema_version": "v5.hybrid.h15.implementation_audit.v2",
            "checked_at": datetime.now(timezone.utc).isoformat(),
            "overall": "PASS_H15_IMPLEMENTATION" if not errors else "FAIL_CLOSED",
            "checks": checks,
            "details": details,
            ...
            "source_sha256": {
                ...
                "ppo": sha(args.ppo),
                "test": sha(args.test),
                "active_window": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h15_active_window.py"),
                "protocol_watch": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h15_protocol_watch.py"),
                "canonical_rearm": sha(rearm_path),
                "health_watch": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h15_health_watch.py"),
                "ordered_rearm": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h15_ordered_rearm.py"),
                "perf_cal": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h15_perf_cal.py"),
            },
