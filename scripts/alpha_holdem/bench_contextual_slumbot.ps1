...
if (Test-Path -LiteralPath $OutDir) { throw "Output directory already exists: $OutDir" }
if ($HandsPerSession -le 0 -or $Sessions -lt 2) { throw 'Positive hands and at least two sessions are required' }

$BenchMutex = [System.Threading.Mutex]::new($false, 'Local\CardPilotSlumbotBenchV1')
try {
    try { $null = $BenchMutex.WaitOne() } catch [System.Threading.AbandonedMutexException] {}
    $active = @(Get-CimInstance Win32_Process | Where-Object {
        $_.Name -match '^python(?:\.exe)?$' -and $_.CommandLine -and
        $_.CommandLine -match 'play_slumbot(?:_v6_journaled)?\.py'
    })
    if ($active.Count -gt 0) { throw 'Another Slumbot evaluation process is already active' }
    ...
    for ($index = 1; $index -le $Sessions; $index++) {
        ...
        $arguments = @(
            ...
            '--observation-bridge', 'posterior-context',
            '--hands', "$HandsPerSession", '--seed', "$($SeedBase + $index)",
            '--session-id', $label, '--out-dir', $sessionDir,
        ...
        $jobs += [pscustomobject]@{Index=$index; Process=$process; SessionDir=$sessionDir; Stdout=$stdout; Stderr=$stderr}
        Write-Host "Started $label PID=$($process.Id)"
        if ($index -lt $Sessions -and $LaunchStaggerSeconds -gt 0) { Start-Sleep -Seconds $LaunchStaggerSeconds }
    }
    while ($true) {
    ...
    foreach ($job in $jobs) {
        ...
        $job.Process.Refresh()
        if ($job.Process.ExitCode -ne 0) { $failures += "s$($job.Index) exit=$($job.Process.ExitCode) stderr=$($job.Stderr)" }
    }
    if ($failures.Count -gt 0) { throw "One or more fixed sessions failed; no retry/replacement is permitted: $($failures -join '; ')" }
    ...
    $auditArgs += @('--model', $ModelPath, '--base-model', $BaseModel, '--observation-bridge', 'posterior-context')
    $auditText = & $PythonExe @auditArgs
    if ($LASTEXITCODE -ne 0) { throw 'Journal/session stateful replay audit failed' }
    ...
    if ([int]$ci.hands -ne ($HandsPerSession * $Sessions)) { throw "Raw CI hand count mismatch: $($ci.hands)" }
