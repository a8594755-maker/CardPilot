#!/usr/bin/env python3
"""Fail-closed H18 control launch preflight."""
...
def main() -> int:
    ...
    check("identity", lock.get("design_id") == "H18" and lock.get("status") == "LOCKED", "lock identity")
    ...
    normalized_source = str(source.resolve()).replace("\\", "/").lower()
    check(
        "source_not_forbidden",
        not any(token in normalized_source for token in (
            "v5_hybrid_h9_", "v5_hybrid_h10_", "v5_hybrid_h11_treatment_",
            "v5_hybrid_h12_", "v5_hybrid_h13_", "v5_hybrid_h14_",
            "v5_hybrid_h15_", "v5_hybrid_h16_", "v5_hybrid_h17_",
            "cal-ext-", "cal_ext_", "benchmark",
        )),
        "forbidden H9/H10/H11/H12/H13/H14/H15/H16/H17/CAL source path",
    )
    launch_policy = lock.get("launch_policy", {})
    check(
        "current_no_launch_policy",
        launch_policy.get("current_authority") == "NONE_CURRENT_HEARTBEAT"
        and launch_policy.get("launcher_requires_explicit_switch") == "LaunchAuthorized"
        and launch_policy.get("validate_only_allowed") is True,
    ...
    for process in psutil.process_iter(["pid", "cmdline", "nice"]):
        try:
            ...
            if any(token in lowered for token in (
                "v5_hybrid_h18_mirror.py", "v5_h1_calibration.py", "play_slumbot",
            ...
            if "multiprocessing.spawn_main" in command:
                parent = process.ppid()
                if not psutil.pid_exists(parent):
                    dead_parent_spawn_children.append(process.pid)
        except Exception:
            pass
    ...
    check(
        ...
        len(path1) == 1
        and path1[0]["children"] == 6
        and int(path1[0]["nice"]) == int(psutil.BELOW_NORMAL_PRIORITY_CLASS),
        "Path-1 existing six-worker job",
    )
    check("no_dead_parent_spawn_children", not dead_parent_spawn_children, "dead-parent Python spawn children")
    ...
    temporary_root = Path.cwd() / ".test_tmp" / "h18_preflight"
    temporary_root.mkdir(parents=True, exist_ok=True)
    for index, (tool, extra) in enumerate((
        ("v5_hybrid_h18_endpoint_watch.py", ["--arm", "control"]),
        ("v5_hybrid_h18_endpoint_watch.py", ["--arm", "treatment"]),
        ("v5_hybrid_h18_protocol_watch.py", ["--arm", "control"]),
        ("v5_hybrid_h18_protocol_watch.py", ["--arm", "treatment"]),
        ("v5_hybrid_h18_completion_watch.py", ["--repo", str(Path.cwd())]),
    )):
        ...
        if "protocol_watch" in tool:
            command.extend(["--allow-registry", str(temporary_root / f"registry_{index}.json")])
    ...
    active_tool = Path("scripts/alpha_holdem/v5_hybrid_h18_active_window.py")
    ...
    result = {
        "schema_version": "v5.hybrid.h18.preflight.v1",
        "checked_at": datetime.now(timezone.utc).isoformat(),
        "overall": "PASS_READY_H18_CONTROL_LAUNCH" if not errors else "FAIL_CLOSED",
        "checks": checks,
        "errors": errors,
        "active_h18_trainers": active,
        "forbidden_evaluators": forbidden,
        "path1_existing_job": path1,
        "design_lock_sha256": sha(args.design_lock),
        "launch_authority": "NONE_CURRENT_HEARTBEAT",
