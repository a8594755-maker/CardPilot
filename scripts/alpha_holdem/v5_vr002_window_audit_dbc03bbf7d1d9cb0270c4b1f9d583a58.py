#!/usr/bin/env python3
"""Independent auditor for the sole registered VR002 first-crossing Stage-A endpoint."""
...
def main() -> int:
    ...
    if not all(checks.values()):
        ...
        raise RuntimeError(f"incomplete VR002 endpoint bundle: {missing}")
    ...
    status = (
        "VR002_VALID_ENDPOINT_MECHANISM_PASS_QUICK5K_MANDATORY"
        if validity_pass and mechanism_pass
        else "VR002_VALID_ENDPOINT_MECHANISM_FAIL_QUICK5K_MANDATORY"
        if validity_pass
        else "VR002_INVALID_ENDPOINT_ABORT_PRESERVE_NO_QUICK5K"
    )
    result = {
        "schema_version": "v5.vr002.stage_a.window_audit.v1",
        "status": status,
