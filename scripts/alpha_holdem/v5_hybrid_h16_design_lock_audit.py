#!/usr/bin/env python3
"""Independent fail-closed audit of the immutable H16 design lock."""
...
def main() -> int:
    ...
    check(
        "identity",
        value.get("schema_version") == "v5.hybrid.h16.design_lock.v1"
        and value.get("design_id") == "H16"
    ...
    check("preregistration", prereg.get("sha256") == "51065761b6b291ef757ea467611203cbe79d45a5e4d54c163edaf79ef8fa1bb0")
    check("preregistration_audit", prereg_audit.get("overall") == "PASS" and prereg_audit.get("checks") == "22/22 PASS")
    check("implementation_audit", implementation.get("overall") == "PASS_H16_IMPLEMENTATION" and implementation.get("checks") == "23/23 PASS")
    check("integration_audit", integration.get("overall") == "PASS" and integration.get("checks") == "31/31 PASS")
    check("route_review", route.get("route_exhausted") is False and route.get("selected_next") == "H16_REPRESENTATIVE_FULL_PPO_PERF_CAL_SAME_SCIENCE")
    ...
    result = {
        "schema_version": "v5.hybrid.h16.design_lock_audit.v1",
        "checked_at": datetime.now(timezone.utc).isoformat(),
        "overall": "PASS_IMMUTABLE_H16_DESIGN_LOCK" if not errors else "FAIL_CLOSED",
        "checks": checks,
