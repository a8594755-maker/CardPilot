...
LG004_TOKEN = '8ef9c64242a75f99bfe04d44de5b643b'
LG004_PREREG_SHA256 = '156b54be70472e9f139672ba5f537a6db39cbea240c2cc21055f679c9f46ae05'
LG004_ASSIGNMENT_TOKEN = 'fbd630ab6a689913afc1cee8a63066dd'
...
def lg004_assignment_u64(absolute_iteration: int) -> int:
    payload = (
...
def lg004_select_opponent(arm: str, absolute_iteration: int, pool_snapshots):
    if arm not in LG004_WEIGHTS:
        raise ValueError(f'unknown LG004 arm: {arm}')
    ids = [int(snapshot.get('id', -1)) for snapshot in pool_snapshots]
    if tuple(ids) != LG004_CHECKPOINT_ORDER or len(set(ids)) != len(ids):
        raise ValueError(f'LG004 frozen pool mismatch: {ids}')
    u64 = lg004_assignment_u64(absolute_iteration)
    unit = u64 / float(1 << 64)
    ...
    if unit >= 0.2:
        ...
        cumulative = 0.0
        for member_id in sorted(LG004_WEIGHTS[arm]):
            cumulative += LG004_WEIGHTS[arm][member_id]
            if conditional_unit < cumulative:
                selected_member_id = member_id
                break
        if selected_member_id is None:
            selected_member_id = max(LG004_WEIGHTS[arm])
    ...
    assignment = {
        ...
        'conditional_weights_by_member_id': {
            str(k): v for k, v in sorted(LG004_WEIGHTS[arm].items())
        },
...
def lg004_enrich_provenance_record(record: dict, assignment: dict) -> dict:
    enriched = dict(record)
    enriched.pop('record_sha256', None)
    enriched['schema_version'] = 'v5.lg004.opponent_assignment_provenance.v1'
    enriched['lg004'] = {
        'registration_token': LG004_TOKEN,
        'registration_sha256': LG004_PREREG_SHA256,
        **assignment,
    }
...
def main():
    ...
    parser.add_argument('--lg004-preregistration', default='')
    parser.add_argument('--lg004-preregistration-sha256', default='')
    parser.add_argument('--lg004-contract-probe', action='store_true')
    ...
    lg004_active = args.lg004_arm != 'none'
    try:
    ...
    if args.lg004_contract_probe and not lg004_active:
        parser.error('LG004 contract probe requires an active LG004 arm')
    if lg004_active:
        workspace = Path(__file__).resolve().parents[2]
        expected_prereg = (
            workspace / 'reports'
            / 'v5_lg004_membership_preregistration_8ef9c64242a75f99bfe04d44de5b643b_20260723.json'
        ).resolve()
        supplied_prereg = Path(args.lg004_preregistration)
        if (
            not supplied_prereg.is_absolute()
            or supplied_prereg.resolve() != expected_prereg
            or args.lg004_preregistration_sha256.lower() != LG004_PREREG_SHA256
            or not expected_prereg.is_file()
            or sha256_path(expected_prereg) != LG004_PREREG_SHA256
        ):
            parser.error('LG004 preregistration identity mismatch')
        legacy_arms = (
            args.h2_window_arm, args.h6_window_arm, args.h7_window_arm,
            args.h8_window_arm, args.h9_window_arm, args.h10_window_arm,
            args.h11_window_arm,
        ...
        if any(arm != 'none' for arm in legacy_arms) or args.showdown_ev_value_targets:
            parser.error('LG004 forbids every legacy behavior arm')
        exact = {
        ...
        if (
            ...
            parser.error('LG004 resume/output/provenance contract mismatch')
        if args.total_hands not in (581021901, 596021901):
            parser.error('LG004 target hand endpoint is not registered')
        if args.max_runtime_seconds not in (10800.0, 21600.0):
            parser.error('LG004 wall-clock bound is not registered')
        ...
        if (
            ...
            or sha256_path(source_path) != LG004_SOURCE_SHA256
        ):
            parser.error('LG004 exact source checkpoint mismatch')
        ...
        for label, raw in (
            ...
            if not path.is_absolute():
                parser.error(f'LG004 {label} must be absolute')
            try:
                path.resolve().relative_to(output_root)
            except ValueError:
                parser.error(f'LG004 {label} escapes registered root')
        lg004_contract = {
            'registration_token': LG004_TOKEN,
            'registration_sha256': LG004_PREREG_SHA256,
            'source_checkpoint_sha256': LG004_SOURCE_SHA256,
            'source_iteration': 35051,
            'source_total_hands': 576021901,
            'source_pool_order': list(LG004_SOURCE_ORDER),
            ...
            'assignment_seed': LG004_ASSIGNMENT_SEED,
            'conditional_weights': {
                str(k): v for k, v in sorted(LG004_WEIGHTS[args.lg004_arm].items())
            },
            'pool_mutation_disabled': True,
        }
        if args.lg004_contract_probe:
            run_dir = Path(args.run_dir)
            if run_dir.exists():
                parser.error('LG004 zero-output probe requires absent run directory')
            probe = {
                'schema_version': 'v5.lg004.contract_probe.v1',
                'status': 'PASS',
                'arm': args.lg004_arm,
                'contract': lg004_contract,
                'selector_samples': [
                    {
                        'absolute_iteration': absolute_iteration,
                        **lg004_select_opponent(
                            args.lg004_arm, absolute_iteration, snapshots,
                        )[1],
                    }
    ...
    def assign_opponents():
        ...
        if pool.size() == 0:
            ...
            if lg004_active:
                selected_index, lg004_assignment = lg004_select_opponent(
                    args.lg004_arm, int(iteration) + 1, pool.snapshots,
                )
        ...
        if assignment_provenance_fh is not None:
            ...
            if lg004_active:
                record = lg004_enrich_provenance_record(record, lg004_assignment)
    ...
    try:
        ...
        while total_hands < args.total_hands:
            ...
            if iter_hands >= args.hands_per_iter and len(iter_transitions) > 0:
                ...
                # Snapshot
                if iteration % args.snapshot_every == 0 and not lg004_active:
                    snap = pool.add(
