...
$checkpoint = "models/slumbot_br_actoronly_top_pair_size_v1_20260725/latest.pt"
$runDir = "models/slumbot_br_actoronly_top_pair_size_v1_ext3k_20260725"
New-Item -ItemType Directory -Force -Path $runDir | Out-Null

$processes = @()
for ($part = 1; $part -le 8; $part++) {
    $stem = "bench_v55_actoronly_top_pair_v1_ext_part$part"
    $arguments = @(
        "-X", "utf8", "-u",
...
if ($successfulHands -lt 2850) {
    throw "actor-only top-pair extension produced only $successfulHands hands"
}
