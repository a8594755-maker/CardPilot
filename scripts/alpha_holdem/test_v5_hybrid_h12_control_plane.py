...
class H12ControlPlaneTests(unittest.TestCase):
    ...
    def run_active(self, root: Path, *extra: str) -> subprocess.CompletedProcess[str]:
        ...
        return subprocess.run([
            sys.executable,
            str(ROOT / "scripts/alpha_holdem/v5_hybrid_h12_active_window.py"),
            *extra,
    ...
    def test_control_protocol_abort_can_terminalize_without_treatment(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            activated = self.run_active(
                ...
                "v5_hybrid_h12_control_catchmse_same35051_20m_r1_20260716",
            )
            self.assertEqual(activated.returncode, 0, activated.stderr)
            judgment = root / "judgment.json"
            judgment.write_text(json.dumps({"design_id": "H12", "overall": "INCONCLUSIVE"}), encoding="utf-8")
    ...
    def test_treatment_protocol_abort_can_terminalize(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            self.assertEqual(self.run_active(root, "activate", "--arm", "control", "--run-id", "v5_hybrid_h12_control_catchmse_same35051_20m_r1_20260716").returncode, 0)
            self.assertEqual(self.run_active(root, "activate", "--arm", "treatment", "--run-id", "v5_hybrid_h12_treatment_catchsmoothl1b1_same35051_20m_r1_20260716").returncode, 0)
            judgment = root / "judgment.json"
            judgment.write_text(json.dumps({"design_id": "H12", "overall": "FAIL"}), encoding="utf-8")
    ...
    def test_conflicting_terminal_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            first.write_text(json.dumps({"design_id": "H12", "overall": "INCONCLUSIVE"}), encoding="utf-8")
            self.assertEqual(self.run_active(root, "terminal", "--verdict", "INCONCLUSIVE", "--judgment", str(first)).returncode, 0)
            second = root / "second.json"
            second.write_text(json.dumps({"design_id": "H12", "overall": "FAIL"}), encoding="utf-8")
    ...
    def test_locked_lifecycle_and_path1_are_allowed(self) -> None:
        processes = [
            FakeProcess(1, ["python", "C:/x/CardPilot/scripts/alpha_holdem/v5_hybrid_h12_protocol_watch.py"]),
    ...
    def test_exact_ordered_supervisor_identity_is_allowed(self) -> None:
        command = ["python", "C:/x/CardPilot/scripts/alpha_holdem/v5_hybrid_h12_ordered_rearm.py", "--arm", "control"]
