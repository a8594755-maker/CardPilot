...
while ((Get-Date) -lt $deadline) {
    if (Test-Path -LiteralPath $slotMarker -PathType Leaf) {
        $slotRecord = Get-Content -LiteralPath $slotMarker -Raw |
            ConvertFrom-Json
        if (
            [string]$slotRecord.status -in @(
...
if (-not (Test-Path -LiteralPath $slotMarker -PathType Leaf)) {
    throw 'Timed out waiting for the fast position-adapter GPU slot'
}
...
$opponents = @(
    $scaledOpponent,
    (Join-Path $oldOpponentRoot 'best.pt'),
    (Join-Path $oldOpponentRoot 'epoch_7.pt')
)
$opponentRecords = @()
foreach ($opponent in $opponents) {
    ...
    $opponentRecords += [ordered]@{
        path = (Resolve-Path -LiteralPath $opponent).Path
        sha256 = (
...
$stem = 'sourcev4_standard10_sb_only_scaled_position_adapter10m'
$runId = "${stem}_20260727"
$runDir = Join-Path 'models' $runId
$candidatePath = Join-Path $runDir 'latest.pt'
if (Test-Path -LiteralPath $runDir) {
    throw "SB-only scaled position-adapter output already exists: $runDir"
}
...
$record = [ordered]@{
    ...
    status = 'RUNNING'
    hypothesis = (
        'Standard32 improved the SB seat from +1.235 to +20.1544 bb/100 ' +
        'while degrading BB. A learned SB-only residual against the stronger ' +
        'scaled opponent can capture that seat gain while preserving the ' +
        'exact Standard10 BB actor.'
    )
    material_change = (
        'From frozen Standard10, train only the zero-initialized SB position ' +
        'residual plus critic for 10M hands against 25% self-play and three ' +
        'learned Slumbot opponents. Shared actor and BB residual are frozen.'
    )
    comparison_baseline = [ordered]@{
        standard10_fresh20k_bb_per_100 = -11.4275
        standard10_bb_seat_bb_per_100 = -24.09
        standard10_sb_seat_bb_per_100 = 1.235
        standard32_sb_seat_bb_per_100 = 20.1544
    }
    ...
    scaled_opponent_offline_decision_samples = 1250000
    trainable_actor_parameters = 'position_policy_adapters.1.*'
    frozen_actor_parameters = (
        'all actor parameters except position_policy_adapters.1.*'
    )
    optimizer = [ordered]@{
        algorithm = 'Adam'
...
& python -X utf8 -u scripts/alpha_holdem/train_v5.py `
    ...
    --position-adapter-training-seat sb `
...
if ($LASTEXITCODE -ne 0) {
    ...
    $record | ConvertTo-Json -Depth 10 |
        Set-Content -LiteralPath $recordPath -Encoding UTF8
    throw 'SB-only scaled position-adapter 10M training failed'
}
...
if ($LASTEXITCODE -ne 0) {
    throw 'Could not inspect SB-only scaled checkpoint'
}
$totalHands = [int64]$checkpointSummary[-4]
$newTrainingHands = $totalHands - 10283876
if ($newTrainingHands -lt 10000000) {
    throw "SB-only scaled adapter stopped early at $newTrainingHands new hands"
}
if (
    [int]$checkpointSummary[-3] -ne 256 -or
    [int]$checkpointSummary[-2] -ne 1 -or
    [string]$checkpointSummary[-1] -ne 'sb'
) {
    throw 'SB-only scaled checkpoint metadata mismatch'
}
...
& python -X utf8 scripts/alpha_holdem/play_slumbot.py `
    ...
    --policy-mode greedy
if ($LASTEXITCODE -ne 0) {
    throw 'SB-only scaled deployment dry-run failed'
}
...
if (Test-Path -LiteralPath $quickDir) {
    throw "SB-only scaled fresh5k output already exists: $quickDir"
}
...
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
    ...
    -Strategy model
if ($LASTEXITCODE -ne 0) {
    throw 'SB-only scaled fresh5k failed'
}
...
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
    ...
    -OutputStem $stem `
    -TrainingMethod (
        'pure SB-only position-residual PPO against 25pct self-play plus ' +
        'three learned Slumbot opponents; inherited actor and BB frozen'
    ) `
...
if ($LASTEXITCODE -ne 0) {
    throw 'SB-only scaled promotion workflow failed'
}
...
if ($summaryPath.Count -ne 1) {
    throw 'SB-only scaled fresh5k did not produce one CI summary'
}
