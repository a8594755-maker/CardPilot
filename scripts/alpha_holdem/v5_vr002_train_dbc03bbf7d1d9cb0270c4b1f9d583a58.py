#!/usr/bin/env python3
"""VR002: frozen-H11 faithful Q-boost-core training window.
...
from alpha_holdem import v5_vr002_qboost_core_dbc03bbf7d1d9cb0270c4b1f9d583a58 as qcore


VR002_TOKEN = "dbc03bbf7d1d9cb0270c4b1f9d583a58"
VR002_IDENTITY = "dbc03bbf7d1d9cb0270c4b1f9d583a586b82c23a655de48a4fb2139ac00a3fb1"
VR002_PREREG_SHA256 = "029411e18760455197471a12f0c00c07d08e6d3123e3d8d62e4b51bc6b7b6fcd"
...
def actor_epoch(model, optimizer, rows, advantages, device, args):
    actor_rows = [r for r in rows if r["request_model"] == HERO_MODEL_ID]
    ...
    model.train()
    totals = {"policy_loss": 0.0, "entropy": 0.0, "approx_kl": 0.0, "minibatches": 0}
...
def run_update(model, optimizer, q_model, q_optimizer, q_generator, hands, device, args):
    ...
    for _ in range(args.ppo_epochs):
        _, _, _, _, advantages, _ = calculate_rollout(model, q_model, hands, device)
        actor_stats = actor_epoch(model, optimizer, rows, advantages, device, args)
        executed += 1
...
def parse_args():
    ...
    p.add_argument("--vr002-preregistration", required=True)
    p.add_argument("--vr002-preregistration-sha256", required=True)
    ...
    p.add_argument("--run-id", default=f"v5_vr002_{VR002_TOKEN}")
...
def main():
    ...
    if int(checkpoint["total_hands"]) != VR002_SOURCE_HANDS or int(checkpoint["iteration"]) != 35051:
    ...
    total_hands, iteration = VR002_SOURCE_HANDS, 35051
    generation = args.vr002_actor_generation_initial
    ...
    def checkpoint_payload():
        return {
            ...
            "version": "v5.vr002.qboost", "run_id": args.run_id,
            "config": vars(args), "critic_contract": CRITIC_V1,
            "vr002": {
                "identity": VR002_IDENTITY, "identity_sha256": VR002_IDENTITY,
                "token": VR002_TOKEN,
                "preregistration_sha256": VR002_PREREG_SHA256,
                "source_checkpoint_sha256": VR002_SOURCE_SHA256,
                "source_total_hands": VR002_SOURCE_HANDS,
    ...
    try:
        while True:
            ...
            actor_lr = inherited_absolute_progress_lr(total_hands, args.total_hands, args.lr)
            ...
            record = {
                ...
                "identity": VR002_IDENTITY,
                "recorded_at": datetime.now(timezone.utc).isoformat(),
                "iteration": iteration, "total_hands": total_hands,
                "new_hands": total_hands - VR002_SOURCE_HANDS,
            ...
            if total_hands >= args.total_hands:
                delta = total_hands - VR002_SOURCE_HANDS
                if not 5_000_000 <= delta <= 5_050_000:
                ...
                manifest = {
                    ...
                    "first_crossing": True, "identity": VR002_IDENTITY,
                    "trainer_sha256": trainer_sha, "core_sha256": core_sha,
                    "source_hands": VR002_SOURCE_HANDS, "total_hands": total_hands,
