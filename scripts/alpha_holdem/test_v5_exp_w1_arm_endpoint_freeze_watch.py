...
from v5_exp_w1_arm_endpoint_freeze_watch import config_errors, endpoint_errors


class ExpW1EndpointFreezeWatchTests(unittest.TestCase):
    ...
    def test_runtime_contract_requires_exact_health_and_empty_stderr(self):
        source = Path(__file__).with_name('v5_exp_w1_arm_endpoint_freeze_watch.py').read_text(encoding='utf-8')
        ...
        self.assertIn("warmup.get('status') != 'PASS'", source)
        self.assertIn("lock.get('design_id') != 'EXP-W1'", source)
        self.assertIn("not in {2, 3}", source)
