#!/usr/bin/env python3
"""H15 throughput, entropy and full-provenance resource-isolation watcher."""
...
FORBIDDEN_PROCESS_TOKENS = (
    "v5_hybrid_h15_mirror.py", "v5_h1_calibration.py", "v5_slumbot",
...
def main() -> int:
    ...
    write(status_path, {
        "schema_version": "v5.hybrid.h15.protocol_status.v1",
        "overall": "PENDING",
        "state": "INITIAL_READY",
        "arm": args.arm,
    ...
    while True:
        ...
        if violations:
            ...
            write(status_path, {
                ...
                "overall": "FAIL",
                "state": "H15_INCONCLUSIVE_RESOURCE_ISOLATION_VIOLATION",
                "arm": args.arm,
                "pid": pid,
                "stop_action": action,
        ...
        if len(values) >= 20 and all(float(item.get("entropy", 99)) < 0.3 for item in values[-20:]):
            action = stop(pid)
            write(status_path, {"overall": "FAIL", "state": "H15_FAIL_PROTOCOL_ABORT_ENTROPY20", "arm": args.arm, "pid": pid, "stop_action": action, "resource_isolation_violations": []})
            return 3
        ...
        if len(values) >= 61:
            ...
            if args.arm == "control":
                ...
                if first["status"] == "FAIL":
                    action = stop(pid)
                    write(status_path, {"overall": "FAIL", "state": "H15_FAIL_PROTOCOL_ABORT_FIRST60_THROUGHPUT", "arm": args.arm, "pid": pid, "stop_action": action, "first60": first, "resource_isolation_violations": []})
                    return 3
