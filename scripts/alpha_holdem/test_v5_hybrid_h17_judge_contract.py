...
PREREG = ROOT / "reports" / "v5_hybrid_h17_preregistration_20260719.json"
LOCK = sorted((ROOT / "reports").glob("v5_hybrid_h17_design_lock_v*_20260719.json"))[-1]
JUDGE = ROOT / "scripts" / "alpha_holdem" / "v5_hybrid_h17_judge.py"
...
def test_h17_judge_is_the_exact_locked_tool() -> None:
    lock = json.loads(LOCK.read_text(encoding="utf-8-sig"))
    assert sha(JUDGE) == lock["tools"]["scripts/alpha_holdem/v5_hybrid_h17_judge.py"]
    assert sha(PREREG) == lock["preregistration"]["sha256"]


def test_h17_lock_preserves_every_registered_gate_semantically() -> None:
...
def test_h17_terminal_checks_cover_all_registered_gates() -> None:
    ...
    assert 'classification = "H17_FAIL_REGISTERED_GATE"' in source
    assert 'classification = "H17_PASS_ALL_REGISTERED_GATES"' in source
    assert 'classification = "H17_INCONCLUSIVE_FIXED_SAMPLE"' in source
