...
$priorSelection = 'models\sourcev4_slumbot_history500k_postflop_awr_adapter256_mappingfix_20260726\proxy_curve\selection.json'
$deadline = (Get-Date).AddHours(3)
while ((Get-Date) -lt $deadline) {
    if (Test-Path -LiteralPath $priorSelection) {
        break
    }
    $prior = @(
        Get-CimInstance Win32_Process |
            Where-Object {
                $_.CommandLine -and
                $_.CommandLine -match 'run_sourcev4_history500k_postflop_awr_pipeline_20260726'
            }
    )
    if ($prior.Count -eq 0) {
        throw 'Historical opponent-AWR pipeline exited before proxy selection'
    }
    Start-Sleep -Seconds 20
}
if (-not (Test-Path -LiteralPath $priorSelection)) {
    throw 'Timed out waiting for historical opponent-AWR proxy selection'
}

$outputDir = 'models\sourcev4_history500k_hero_awr_adapter256_mappingfix_20260726'
if (Test-Path -LiteralPath $outputDir) {
    throw "Historical hero-AWR output already exists: $outputDir"
}
...
if ($LASTEXITCODE -ne 0) {
    throw 'Historical hero AWR training failed'
}
...
foreach ($epoch in 1..3) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Historical hero-AWR proxy probe failed for epoch $epoch"
    }
}
...
$externalDir = 'models\bench_sourcev4_history500k_hero_awr_selected_pure_fresh5k_20260726'
...
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
    ...
    -Tag 'sourcev4_history500k_hero_awr_selected_pure_fresh5k_20260726' `
