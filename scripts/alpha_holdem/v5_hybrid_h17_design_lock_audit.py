#!/usr/bin/env python3
"""Independent fail-closed audit of the immutable H17 design lock."""
...
def main() -> int:
    ...
    check(
        "identity",
        value.get("schema_version") == "v5.hybrid.h17.design_lock.v1"
        and value.get("design_id") == "H17"
    ...
    check("preregistration", prereg.get("sha256") == "df256560d69928c9f70e6df5457c1575cc81124ba80f34f9b15261293cefe7fc")
    check("preregistration_audit", prereg_audit.get("overall") == "PASS" and prereg_audit.get("checks") == "25/25 PASS")
    check("implementation_audit", implementation.get("overall") == "PASS_H17_IMPLEMENTATION" and implementation.get("checks") == "23/23 PASS")
    check("integration_audit", integration.get("overall") == "PASS" and integration.get("checks") == "31/31 PASS")
    check("route_review", route.get("route_exhausted") is False and route.get("selected_next") == "H17_CORRECTED_DETERMINISTIC_TRIGGER_SAME_SCIENCE_BASELINE_SMOOTHL1_KERNEL")
    ...
    check(
        ...
        and calibration.get("warmup_updates") == 2
        and calibration.get("timed_updates") == 8
        and calibration.get("repeats") == 5
    ...
    result = {
        "schema_version": "v5.hybrid.h17.design_lock_audit.v1",
        "checked_at": datetime.now(timezone.utc).isoformat(),
        "overall": "PASS_IMMUTABLE_H17_DESIGN_LOCK" if not errors else "FAIL_CLOSED",
        "checks": checks,
