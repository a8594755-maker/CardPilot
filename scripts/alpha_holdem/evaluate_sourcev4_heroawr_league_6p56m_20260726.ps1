...
while ((Get-Date) -lt $deadline) {
    ...
    if ($trainer.Count -eq 0) {
        throw 'League trainer exited before the iteration-200 checkpoint'
    }
    Start-Sleep -Seconds 20
}
$matches = @(Get-ChildItem -Path $checkpoint -File -ErrorAction SilentlyContinue)
if ($matches.Count -ne 1) {
    throw "Expected one iteration-200 checkpoint, found $($matches.Count)"
}

$outputDir = Join-Path $runDir 'internal_curve_6p56m'
New-Item -ItemType Directory -Path $outputDir -Force | Out-Null
$candidateJson = Join-Path $outputDir 'league_6p56m.json'
$probeOutput = & python -X utf8 -u `
    ...
    --out-md (Join-Path $outputDir 'league_6p56m.md')
$probeOutput | Out-Host
if ($LASTEXITCODE -ne 0) {
    throw 'League 6.56M internal probe failed'
}
