...
# Use the GPU slot released by the SLUMBOT_FREE 10M experiment.  The
# external Standard10/Standard32 evidence localizes the largest loss to BB, so
# this window freezes the full inherited actor and the SB residual exactly.
$slotMarker = (
...
if (-not $slotReady) {
    throw 'Timed out waiting for the SLUMBOT_FREE GPU slot'
}
...
$stem = 'sourcev4_standard10_bb_only_position_adapter10m'
$runDir = Join-Path 'models' "${stem}_20260727"
$candidatePath = Join-Path $runDir 'latest.pt'
if (Test-Path -LiteralPath $runDir) {
    throw "BB-only position-adapter output already exists: $runDir"
}
...
if ($LASTEXITCODE -ne 0) {
    throw 'BB-only position-adapter 10M training failed'
}
...
if ($LASTEXITCODE -ne 0) {
    throw 'Could not inspect BB-only position-adapter checkpoint'
}
$totalHands = [int64]$checkpointSummary[-4]
$newTrainingHands = $totalHands - 10283876
if ($newTrainingHands -lt 10000000) {
    throw "BB-only position-adapter stopped early at $newTrainingHands new hands"
}
if (
    [int]$checkpointSummary[-3] -ne 256 -or
    [int]$checkpointSummary[-2] -ne 1 -or
    [string]$checkpointSummary[-1] -ne 'bb'
) {
    throw 'BB-only position-adapter checkpoint metadata mismatch'
}
...
& python -X utf8 scripts/alpha_holdem/play_slumbot.py `
    ...
    --policy-mode greedy
if ($LASTEXITCODE -ne 0) {
    throw 'BB-only position-adapter deployment dry-run failed'
}
...
$record = [ordered]@{
    ...
    hypothesis = (
        'Standard10 loses primarily from BB. Updating only a BB residual can ' +
        'repair the weak seat without the SB regression caused by shared or ' +
        'two-seat policy drift.'
    )
    material_change = (
        'From frozen Standard10, train only the zero-initialized BB residual ' +
        'expert plus critic for 10M hands. The inherited actor and complete ' +
        'SB residual remain frozen.'
    )
...
if (Test-Path -LiteralPath $quickDir) {
    throw "BB-only position-adapter fresh5k output already exists: $quickDir"
}
...
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
    ...
    -Strategy model
if ($LASTEXITCODE -ne 0) {
    throw 'BB-only position-adapter fresh5k failed'
}
...
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
    ...
    -OutputStem $stem `
    -TrainingMethod (
        'pure BB-only position-residual PPO on 40pct self-play plus three ' +
        'learned Slumbot opponents; inherited actor and SB residual frozen'
    ) `
...
if ($LASTEXITCODE -ne 0) {
    throw 'BB-only position-adapter promotion workflow failed'
}
...
if ($summaryPath.Count -ne 1) {
    throw 'BB-only position-adapter fresh5k did not produce one CI summary'
}
