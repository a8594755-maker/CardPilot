"""PCV017 exact-V5.5 teacher-interface bounded CPU smoke.
...
DEAL_SEED = 2026072091
ROLLOUT_SEED = 2026972091
DEALS = 16
ROWS = 64
...
def make_teacher_row(row_id: int, deal_id: int, state: HUNLGameState) -> tuple[dict[str, Any], int]:
    ...
    row = {
        "schema_version": "v5.pcv017.teacher_row.smoke.v1",
...
def terminal_probe(deal_id: int, terminal_class: str) -> dict[str, Any]:
    ...
    payload = {
        "schema_version": "v5.pcv017.terminal_probe.v1",
        "classification": "FORBIDDEN_TRAINING_SMOKE_ONLY",
        "deal_id": deal_id,
...
def run(args: argparse.Namespace) -> int:
    ...
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "":
        raise RuntimeError("gpu_visibility_not_empty")
    ...
    raw_metrics = {
        "schema_version": "v5.pcv017.raw_metrics.v1",
    ...
    gates = {
        "input_hashes_8_of_8": len(verified_inputs) == 8,
        ...
        "gpu_visible_or_used_false": os.environ.get("CUDA_VISIBLE_DEVICES") == "",
    }
    passed = all(gates.values())
    result = {
        "schema_version": "v5.pcv017.result.v1",
        "design_id": "PCV017",
        "judged_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "verdict": "PASS" if passed else "FAIL",
        "classification": "PCV017_PASS_EXACT_V55_INTERFACE_AND_BOUNDED_CPU_SMOKE" if passed else "PCV017_FAIL_CLOSED_EXACT_V55_INTERFACE_OR_BOUNDED_CPU_SMOKE_GATE",
        ...
        "authority": {
            ...
            "nonpass_next": "SEPARATELY_REGISTERED_ROUTE_REVIEW029",
        },
        "path1_action": False,
...
def parse_args() -> argparse.Namespace:
    ...
    if not args.self_test and not all((args.preregistration, args.preregistration_audit, args.implementation_audit, args.implementation_audit_sha256, args.output)):
