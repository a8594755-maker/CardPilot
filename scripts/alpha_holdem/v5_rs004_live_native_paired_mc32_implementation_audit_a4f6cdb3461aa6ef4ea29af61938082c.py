"""Independent RS004 implementation audit and exactly-two probe owner."""
...
TOKEN = "a4f6cdb3461aa6ef4ea29af61938082c"
IDENTITY = "a4f6cdb3461aa6ef4ea29af61938082ccfe2497ad4177aa16317292aad1a6dfb"
PYTHON = Path(r"C:\Users\a8594\AppData\Local\Programs\Python\Python312\python.exe")
PREREG = ROOT / "reports" / f"v5_rs004_live_native_paired_mc32_census_key_correction_preregistration_{TOKEN}_20260722.json"
PREREG_AUDIT = ROOT / "reports" / f"v5_rs004_live_native_paired_mc32_census_key_correction_preregistration_audit_{TOKEN}_20260722.json"
RUNNER = ROOT / "scripts" / "alpha_holdem" / f"v5_rs004_live_native_paired_mc32_{TOKEN}.py"
LAUNCHER = ROOT / "scripts" / "alpha_holdem" / f"v5_rs004_live_native_paired_mc32_launcher_{TOKEN}.ps1"
RESULT_AUDITOR = ROOT / "scripts" / "alpha_holdem" / f"v5_rs004_live_native_paired_mc32_audit_{TOKEN}.py"
IMPLEMENTATION_AUDITOR = Path(__file__).resolve()
RESULT = ROOT / "reports" / f"v5_rs004_live_native_paired_mc32_implementation_audit_{TOKEN}_20260722.json"
QUAL_ROOT = ROOT / "reports" / f"v5_rs004_live_native_paired_mc32_qualification_{TOKEN}_20260722"
QUICK5K_ROOT = ROOT / "models" / f"bench_v55_rs004_{TOKEN}_greedy_quick5k_20260722"
...
PREREG_SHA256 = "d6fd7ec547c7fcaee1f42f3a4e8074525ca86f1bc25ddf0aef197bc0dc374b2b"
PREREG_AUDIT_SHA256 = "b00417328f267021ab27e84f43c1210732c0d3a5b4c20fc13e22f8fccf18f258"
RUNNER_SHA256 = "3041e15ae0398579681d61995dd100d84297228200f8815d749b8e12407d315a"
LAUNCHER_SHA256 = "065c248b3631b6c1ffdf3fe11b0322a00109816fe8fbf0193883c081e17d93ea"
RESULT_AUDITOR_SHA256 = "2b1962679b5946d4fb2e68d55309bb1a54ea7cef70f0b4eeeca015fd535d955a"
...
PROBE_NONCES = ["RS004_PROBE_A_2034972296", "RS004_PROBE_B_2035972296"]
SELFTEST_NONCE = "RS004_SELFTEST_2033972296"
    ...
    for path in dump_paths:
        with path.open("r", encoding="utf-8") as handle:
            for line in handle:
                raw = json.loads(line)
                rows += 1
                hand_key = (str(path.resolve()), int(raw["hand_idx"]))
                by_hand[hand_key].append(int(raw["move_idx"]))
...
def static_checks() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    check(
        "preregistration_audit_pass58",
        prereg_audit.get("pass_count") == 58
        and prereg_audit.get("check_count") == 58
    ...
    check(
        "all26_frozen_inputs_exact",
        len(prereg["frozen_authority_inputs"]) == 26 and not input_failures,
        input_failures,
    )
    ...
    check(
        ...
        "import v5_rs004" not in auditor_text and "from v5_rs004" not in auditor_text,
    )
    launcher_markers = [
        str(PYTHON),
        str(RUNNER),
...
def execute() -> tuple[dict[str, Any], bool]:
    ...
    if not all(item["pass"] for item in checks):
        value = {
            "schema_version": "v5.rs004.implementation_audit.v1",
            "classification": "FAIL_CLOSED / RS004_IMPLEMENTATION_AUDIT_STATIC_PREPROBE_FAILURE_NO_QUALIFICATION",
            "overall": "FAIL_CLOSED",
            "checks": checks,
    ...
    check(
        ...
        and selftest_json.get("classification") == "RS004_SELF_TEST_PASS"
    ...
    if not all(item["pass"] for item in checks):
        value = {
            "schema_version": "v5.rs004.implementation_audit.v1",
            "classification": "FAIL_CLOSED / RS004_IMPLEMENTATION_AUDIT_SELFTEST_PREPROBE_FAILURE_NO_QUALIFICATION",
            "overall": "FAIL_CLOSED",
            "checks": checks,
    ...
    check(
        ...
        all(
            ...
            and item["parsed"].get("classification") == "RS004_CONTRACT_PROBE_PASS"
    ...
    value = {
        "schema_version": "v5.rs004.implementation_audit.v1",
        "classification": (
            "PASS / RS004_IMPLEMENTATION_AUDIT_PASS_QUALIFICATION_READY_ONLY"
            if passed
            else "FAIL_CLOSED / RS004_IMPLEMENTATION_AUDIT_FAILURE_NO_QUALIFICATION"
        ),
        "overall": "PASS" if passed else "FAIL_CLOSED",
        "program_id": "RS004_LIVE_NATIVE_PAIRED_MC32_LCB95_ROOT_RESOLVER",
    ...
    if args.mode == "Preflight":
        ...
        print(
            canonical_json(
                {
                    "classification": (
                        "RS004_PREPROBE_PREFLIGHT_PASS"
                        if passed
                        else "RS004_PREPROBE_PREFLIGHT_FAIL"
                    ),
                    "checks": checks,
                    "files_written": 0,
                    "probe_children_launched": 0,
