#!/usr/bin/env python3
"""After EXP-W1 PASS, lock and run exact-endpoint promotion/formal routing."""
...
def main() -> int:
    ...
    main_lock = Path(args.design_lock).resolve()
    lock_sha = sha256_path(main_lock)
    lock = read_json(main_lock)
    ...
    endpoint_tool = repo / 'scripts/alpha_holdem/v5_exp_w1_arm_endpoint_freeze_watch.py'
    self_path = Path(__file__).resolve()
    static_errors: list[str] = []
    if (
        lock_sha != args.expected_lock_sha256.lower()
        or lock.get('status') != 'LOCKED'
        or lock.get('design_id') != 'EXP-W1'
        or int(lock.get('lock_revision') or -1) != 1
    ):
        static_errors.append('EXP-W1 main design lock identity mismatch')
    locked_tools = {
        str(item.get('path', '')).replace(chr(92), '/'): item.get('sha256')
        for item in lock.get('tool_sha256', [])
    }
    ...
    for path in (pipeline, relative_tool, endpoint_tool, self_path):
        relative = str(path.relative_to(repo)).replace(chr(92), '/')
        if not path.exists() or sha256_path(path) != locked_tools.get(relative):
            static_errors.append(f'locked tool identity mismatch: {relative}')
    if static_errors:
    ...
    while True:
        ...
        promotion_lock_path = repo / 'reports/v5_exp_w1_promotion_design_lock.json'
        ledger = repo / 'reports/v5_experiment_ledger.md'
        ledger_bytes = ledger.read_bytes()
        promotion_tag = 'v5_exp_w1_treatment_endpoint_promotion20k'
        formal_tag = 'v5_exp_w1_treatment_endpoint_formal100k'
        promotion_lock = {
            'schema_version': 'v5.exp_w1.promotion_lock.v1', 'status': 'LOCKED', 'locked_at': now_iso(),
            ...
            'promotion': {'policy': 'greedy-direct', 'sessions': 12, 'hands_per_session': 1700, 'hands': 20400,
                          ...
                          'full_bundle_required': True, 'no_quality_gate_rationale': 'EXP-W1 primary PASS is the locked method quality gate'},
        ...
        append_ledger(repo, f"EXP-W1 immutable promotion sub-lock published | Primary100k PASS and exact treatment endpoint SHA{treatment_sha} bound before any Slumbot launch. Lock {promotion_lock_path} SHA{promotion_lock_sha} pins greedy-direct12x1700 direct BelowNormal full bundle, fresh V4 model/report/12 hand-file hashes, Welch relative CI lower>0, point-only forbidden, and same-checkpoint formal rule. [event_id=v5-exp_w1-promotion-sublock-published]")

        treatment_run = Path(str(lock['arms']['treatment']['run_dir'])).resolve()
        promo_status = repo / 'reports/v5_exp_w1_promotion20k_status.json'
        promo_plan_json = repo / 'reports/v5_exp_w1_promotion20k_plan.json'
        promo_plan_md = repo / 'reports/v5_exp_w1_promotion20k_plan.md'
        promo_log = repo / 'reports/v5_exp_w1_promotion20k_watch.log'
        ...
        relative_path = repo / 'reports/v5_exp_w1_promotion_relative_v4_ci.json'
        ...
        program_gate_path = repo / 'reports/v5_exp_w1_promotion_program_gate.json'
        ...
        if not strong:
            ...
            append_ledger(repo, f"EXP-W1 promotion20k non-strong; Tier2 frozen | Full promotion pipeline and fresh-V4 Welch gate completed; blockers {blockers}. No formal launch; route review required. [event_id=v5-exp_w1-promotion-nonstrong-tier2-frozen]")
            return 0

        formal_status = repo / 'reports/v5_exp_w1_formal100k_status.json'
        formal_cmd = [sys.executable, str(pipeline), '--run-dir', str(treatment_run), '--checkpoint', str(treatment_checkpoint),
                      ...
                      '--plan-json', str(repo / 'reports/v5_exp_w1_formal100k_plan.json'),
                      '--plan-md', str(repo / 'reports/v5_exp_w1_formal100k_plan.md'),
                      '--status-json', str(formal_status), '--log', str(repo / 'reports/v5_exp_w1_formal100k_watch.log')]
        ...
        append_ledger(repo, "EXP-W1 same-checkpoint formal100k completed | Combined promotion strong gate allowed exact treatment endpoint greedy-direct formal100k; full pipeline status PASS. Strength classification is only the audited formal gate result. [event_id=v5-exp_w1-formal100k-complete]")
        return 0
