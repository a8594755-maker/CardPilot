#!/usr/bin/env python3
"""Offline fail-closed implementation audit for H9 robust catch-up-only loss."""
...
from scripts.alpha_holdem.test_v5_hybrid_h9_implementation import run_update
...
def main() -> int:
    ...
    try:
        ...
        check(
            ...
            and prereg.get("experiment_id") == "H9"
        ...
        check(
            ...
            prereg["single_variable"] == {
                ...
                "beta_selection": "canonical SmoothL1 default and one critic-v1 unit equals one big blind; no holdout or H8 outcome search",
            },
        )
        check(
            "trainer_cli_identity",
            all(token in trainer_source for token in (
                "--h9-window-arm", "--h9-catchup-loss",
                "--h9-catchup-smooth-l1-beta", "--h9-preregistration", "--h9-design-lock",
            )),
        )
        check(
            "trainer_fail_closed_contract",
            all(token in trainer_source for token in (
                "H9 arm run_id or fixed endpoint mismatch",
                "H9 exact source checkpoint identity/hash mismatch",
                "H9 arms require target-KL0.03 and value-head catch-up enabled",
        ...
        result = {
            "schema_version": "v5.hybrid.h9.implementation_audit.v1",
            "checked_at": datetime.now(timezone.utc).isoformat(),
            "overall": "PASS_H9_IMPLEMENTATION" if not errors else "FAIL_CLOSED",
            "checks": checks,
            "details": details,
