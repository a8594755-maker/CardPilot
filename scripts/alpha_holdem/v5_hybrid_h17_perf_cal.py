#!/usr/bin/env python3
"""Registered representative full-PPO pre-arm calibration for H17.
...
SCHEMA = "v5.hybrid.h17.representative_perf_cal.v1"
...
def build_model_and_optimizer(
    ...
    if checkpoint.get("critic_contract") != CRITIC_V1:
        raise ValueError("H17 PERF-CAL requires critic_v1 source")
...
def main() -> int:
    ...
    try:
        ...
        payload = {
            ...
            "classification": (
                "H17_REPRESENTATIVE_PERF_CAL_PASS"
                if passed else "H17_REPRESENTATIVE_PERF_CAL_FAIL"
            ),
            "arm": args.arm,
            "source": {
        ...
        failure = {
            ...
            "checked_at": datetime.now(timezone.utc).isoformat(),
            "overall": "FAIL_CLOSED",
            "classification": "H17_REPRESENTATIVE_PERF_CAL_EXECUTION_FAILURE",
            "arm": args.arm,
