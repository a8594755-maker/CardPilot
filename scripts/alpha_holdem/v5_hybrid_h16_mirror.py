#!/usr/bin/env python3
"""Frozen H16 40k three-endpoint common-deal mirror evaluator."""
...
def load_lock(path: Path, expected: str, manifest: Path) -> dict:
    ...
    if value.get("design_id") != "H16-MIRROR-001" or value.get("status") != "LOCKED":
...
def prepare(out: Path) -> dict:
    ...
    for index in range(PAIRS):
        ...
        deals.append(
            {
                "index": index,
                "deal_id": f"h16mirror-{SEED}-{index:05d}-{deck_hash[:16]}",
                "deck_sha256": deck_hash,
                "opponent_pool_id": ACTIVE_POOL_IDS[index % 5],
            }
        )
    value = {
        "schema_version": "v5.hybrid.h16.mirror_manifest.v1",
        "design_id": "H16",
...
def audit_manifest(value: dict) -> list[str]:
    errors = []
    if value.get("schema_version") != "v5.hybrid.h16.mirror_manifest.v1" or value.get("preregistration_sha256") != PREREG_SHA:
...
def checkpoint_identity(path: Path, arm: str) -> dict:
    ...
    if checkpoint.get("h16_window_arm") != arm or abs(float(checkpoint.get("ppo_target_kl", -1)) - 0.03) > 1e-12:
        raise ValueError("endpoint H16 arm identity")
    if not bool(checkpoint.get("h8_value_head_catchup_after_kl_stop")):
        raise ValueError("endpoint H16 catch-up disabled")
    if checkpoint.get("h16_catchup_loss") != expected_loss or float(checkpoint.get("h16_catchup_smooth_l1_beta", -1)) != 1.0:
        raise ValueError("endpoint H16 catch-up loss identity")
    if config.get("h16_preregistration_sha256") != PREREG_SHA:
...
def run_arm(manifest_path, endpoint, arm, out, device, lock_path, lock_sha, runtime):
    ...
    with out.open("x", encoding="utf-8", newline="\n") as stream:
        for expected in manifest["deals"]:
            ...
            stream.write(
                json.dumps(
                    {
                        "schema_version": "v5.hybrid.h16.mirror_pair.v1",
                        "arm": arm,
    ...
    summary = {
        "schema_version": "v5.hybrid.h16.mirror_arm_summary.v1",
        "arm": arm,
        "endpoint": identity,
...
def read_rows(path: Path, arm: str, manifest: dict):
    ...
    for index, row in enumerate(values):
        ...
        if (
            row.get("schema_version") != "v5.hybrid.h16.mirror_pair.v1"
            or row.get("arm") != arm
...
def audit_bundle(manifest_path, control, treatment, anchor, out, lock_path, lock_sha):
    ...
    value = {
        "schema_version": "v5.hybrid.h16.mirror_audit.v1",
        "overall": "PASS_IMMUTABLE_H16_MIRROR" if not errors else "FAIL_CLOSED",
        "errors": errors,
        "manifest_sha256": sha256_file(manifest_path),
        "measurement_lock_sha256": sha256_file(lock_path),
        "tool_sha256": sha256_file(TOOL),
...
def judge(manifest_path, control, treatment, anchor, audit, out, lock_path, lock_sha):
    ...
    if audit_value.get("overall") != "PASS_IMMUTABLE_H16_MIRROR":
    ...
    value = {
        "schema_version": "v5.hybrid.h16.mirror_judgment.v1",
        "status": status,
        "pairs": PAIRS,
        "comparisons": {
