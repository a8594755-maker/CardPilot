#!/usr/bin/env python3
"""Independent fail-closed audit for immutable PCV012 result."""
...
def main() -> int:
    ...
    if args.out.exists():
        raise SystemExit("refusing to overwrite immutable PCV012 audit")
    ...
    checks: dict[str, bool] = {
        ...
        "pcv010_pcv011_measurement_not_used": result["pcv010_or_pcv011_measurement_data_read_or_reused"] is False,
        ...
        "inference_and_authority_frozen": result["causal_method_behavior_or_strength_inference"] == "FORBIDDEN" and result["next_authority"] == "ROUTE_REVIEW024_ONLY",
    }

    recomputed: dict[str, Any] = {"conditioning_envelope_pass": envelope_recomputed, "conditioning_terminal_ranges": terminal_ranges}
    if result["classification"] == "PCV012_FAIL_CLOSED_INITIAL_ENVELOPE_NO_MEASUREMENT":
        checks.update({
    ...
    elif result["classification"] == "PCV012_FAIL_CLOSED_BLOCK_LOCAL_ENVELOPE_NO_LATER_BLOCKS":
        ...
        resource_rows = [conditioning["resource_before"], conditioning["resource_after"]] + [row for index in range(8) for row in (block_gates[index]["resource_before"], block_gates[index]["resource_after"], blocks[index]["resource_after"])]
        ...
        expected_attribution = "IN_BLOCK_DEVICE_EXCURSION_PERSISTS" if device_excursion else ("BLOCK_LOCAL_CONTROL_ASSOCIATION_SUPPORTED" if stability >= 0.95 else "BLOCK_LOCAL_CONTROL_DID_NOT_RECOVER_STABILITY")
        checks.update({
            "pass_classification_exact": result["overall"] == "PASS" and result["classification"] == "PCV012_PASS_MEASUREMENT_COMPLETE" and envelope_recomputed,
            ...
            "gate_after_is_block_before": all(blocks[index]["resource_before"] == block_gates[index]["resource_after"] for index in range(8)),
    ...
    audit = {
        "schema_version": "v5.pcv012.result_audit.v1",
        ...
        "classification": result["classification"] if not failed else "PCV012_AUDIT_FAIL_CLOSED",
        "recomputed": recomputed,
        "official_hands": 0,
        "behavior_launch_authority": "NONE_ROUTE_REVIEW024_ONLY",
    }
