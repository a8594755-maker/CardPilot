...
SCRIPT = ROOT / "scripts/alpha_holdem/v5_hybrid_h14_ordered_rearm.py"
REARM = ROOT / "scripts/alpha_holdem/v5_rearm_watchers.ps1"
SPEC = importlib.util.spec_from_file_location("h14_rearm", SCRIPT)
...
def test_canonical_rearm_classifies_h14_and_blocks_generic_paths() -> None:
    with tempfile.TemporaryDirectory(dir=ROOT / ".test_tmp") as directory:
        run_dir = Path(directory)
        (run_dir / "run_manifest.json").write_text(json.dumps({
            "run_id": "v5_hybrid_h14_control_catchmse_same35051_20m_r1_20260717",
            "iteration": 35051,
        ...
        assert classification["is_hybrid_h14_arm"] is True
        assert classification["block_generic_eval_and_slumbot"] is True


def test_canonical_h14_branch_launches_only_ordered_supervisor() -> None:
    source = REARM.read_text(encoding="utf-8-sig")
    branch = source.split("if ($script:isHybridH14Arm)", 1)[1].split("} elseif ($script:isHybridH11Arm)", 1)[0]
    assert "Launch-H14OrderedRearm" in branch
    assert "Launch-Health" not in branch
    assert "Launch-H11" not in branch
    assert "H14 exact ordered lifecycle terminally blocks every generic project path" in branch
