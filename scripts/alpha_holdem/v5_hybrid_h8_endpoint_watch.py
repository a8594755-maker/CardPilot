#!/usr/bin/env python3
"""Fail-closed H8 endpoint freezer for fresh contemporaneous arms."""
...
def main() -> int:
    ...
    if lock.get("design_id") != "H8" or lock.get("status") != "LOCKED": errors.append("lock identity/status")
    if not arm or lock.get("arm_budget", {}).get("minimum_endpoint_hands") != 556001286: errors.append("arm/budget incomplete")
    if errors:
    ...
    while True:
        ...
        target = 0.03
        expected_catchup = args.arm == "treatment"
        if manifest.get("run_id") != arm["run_id"]: errors.append("run_id mismatch")
        if config.get("h8_window_arm") != args.arm or abs(float(config.get("ppo_target_kl", -1)) - target) > 1e-12: errors.append("H8 arm identity mismatch")
        if bool(config.get("h8_value_head_catchup_after_kl_stop")) != expected_catchup: errors.append("H8 catch-up arm identity mismatch")
        ...
        if checkpoint.get("h8_window_arm") != args.arm or abs(float(checkpoint.get("ppo_target_kl", -1)) - target) > 1e-12: errors.append("checkpoint H8 arm mismatch")
        if bool(checkpoint.get("h8_value_head_catchup_after_kl_stop")) != expected_catchup: errors.append("checkpoint H8 catch-up mismatch")
        ...
        if checkpoint_config.get("h8_preregistration_sha256") != lock["preregistration"]["sha256"]: errors.append("checkpoint prereg mismatch")
        if checkpoint_config.get("h8_design_lock_sha256") != sha(lock_path): errors.append("checkpoint lock mismatch")
        if errors:
        ...
        try:
            provenance = audit_provenance(Path(arm["provenance_path"]), expected_run_id=arm["run_id"], expected_mode="per-iteration", expected_workers=22, expected_groups=5, expected_worker_seed_base=73000, expected_first_iteration=32618, expected_last_iteration=iteration)
        except Exception as exc: provenance = {"overall": "FAIL", "errors": [str(exc)]}
        write(run_dir / f"h8_{args.arm}_assignment_provenance_audit.json", provenance)
        if provenance.get("overall") != "PASS": readiness.append("provenance audit failed")
        resource = load(run_dir / f"h8_{args.arm}_protocol_status.json")
        ...
        frozen = run_dir / f"h8_{args.arm}_endpoint.pt"
