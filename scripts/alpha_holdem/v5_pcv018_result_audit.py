"""Independent fail-closed auditor for the immutable PCV018 bounded smoke."""
...
def audit(args: argparse.Namespace) -> int:
    ...
    checks["result_schema_and_design"] = result.get("schema_version") == "v5.pcv018.result.v1" and result.get("design_id") == "PCV018"
    ...
    checks["implementation_audit_pass_with_two_exact_contract_probes"] = (
        ...
        and all(
            ...
            and probe.get("stdout_json", {}).get("PCV018_DEVICE_MODE") == DEVICE_MODE
            and probe.get("stdout_json", {}).get("PCV018_CONTRACT_NONCE") == CONTRACT_NONCE
    ...
    checks["runner_hash"] = (
        sha256_file(Path(args.runner)) == result.get("runner_sha256") == implementation_audit.get("runner_sha256")
    )
    checks["launcher_hash_matches_implementation_audit"] = sha256_file(Path(args.launcher)) == implementation_audit.get("launcher_sha256")
    checks["device_contract_exact"] = (
        result.get("device_contract", {}).get("CUDA_VISIBLE_DEVICES") == "-1"
        and result.get("device_contract", {}).get("PCV018_DEVICE_MODE") == DEVICE_MODE
        and result.get("device_contract", {}).get("PCV018_CONTRACT_NONCE") == CONTRACT_NONCE
    ...
    checks["input_hashes_15_of_15"] = (
        len(result.get("verified_inputs", [])) == 15
        and len(prereg["frozen_existing_inputs"]) == 15
        and all(sha256_file(Path(x["path"])) == x["sha256"] for x in result["verified_inputs"])
        and {(x["path"], x["sha256"]) for x in result["verified_inputs"]}
        == {(x["path"], x["sha256"]) for x in prereg["frozen_existing_inputs"]}
    )
    ...
    checks["row_schema_exact"] = all(row.get("schema_version") == "v5.pcv018.teacher_row.smoke.v1" for row in rows)
    ...
    checks["probe_schema_exact"] = all(row.get("schema_version") == "v5.pcv018.terminal_probe.v1" for row in probes)
    ...
    checks["scope_seeds_and_counts"] = (
        result["scope"]["deal_seed"] == 2026072092
        and result["scope"]["rollout_seed"] == 2026972092
        and result["scope"]["contract_nonce_seed"] == 2027972092
    ...
    checks["result_pass_classification_exact"] = result.get("verdict") == "PASS" and result.get("classification") == "PCV018_PASS_EXACT_V55_INTERFACE_AND_BOUNDED_CPU_SMOKE"
    checks["training_full_asset_behavior_authority_none"] = (
        ...
        and result["authority"]["nonpass_next"] == "SEPARATELY_REGISTERED_ROUTE_REVIEW030"
    )
    ...
    checks["preregistered_outputs_exact"] = all(
        str(root / name) == prereg["immutable_outputs"][key]
        for name, key in (
    ...
    audit_result = {
        "schema_version": "v5.pcv018.result_audit.v1",
        ...
        "terminal_classification": result.get("classification") if not failed else "PCV018_FAIL_CLOSED_RESULT_AUDIT",
