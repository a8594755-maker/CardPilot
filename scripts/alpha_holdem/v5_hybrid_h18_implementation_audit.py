#!/usr/bin/env python3
"""Offline fail-closed implementation audit for H18 robust catch-up-only loss."""
...
from scripts.alpha_holdem.test_v5_hybrid_h18_implementation import run_update
...
def main() -> int:
    ...
    try:
        ...
        check(
            ...
            and prereg.get("experiment_id") == "H18"
        ...
        check(
            ...
            all(token in trainer_source for token in (
                "--h18-window-arm", "--h18-catchup-loss",
                "--h18-catchup-smooth-l1-beta", "--h18-preregistration", "--h18-design-lock",
            )),
        )
        check(
            "trainer_fail_closed_contract",
            all(token in trainer_source for token in (
                "H18 arm run_id or fixed endpoint mismatch",
                "H18 exact canonical source checkpoint identity/hash mismatch",
                "H18 forbidden H9/H10/H11/H12/H13/H14/H15/H16/H17/CAL source path",
                "H18 arms require target-KL0.03 and value-head catch-up enabled",
        ...
        active_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h18_active_window.py").read_text(encoding="utf-8")
        ...
        protocol_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h18_protocol_watch.py").read_text(encoding="utf-8")
        check(
        ...
        check(
            ...
            and "H18 terminal transition requires an active control or treatment window" in active_source,
        )
        check(
        ...
        check(
            "canonical_rearm_strict_h18_contract",
            all(token in rearm_source for token in (
                "$script:isHybridH18Arm",
                "Launch-H18OrderedRearm",
                "v5_hybrid_h18_ordered_rearm.py",
                "H18 exact ordered lifecycle terminally blocks every generic project path",
            )),
        )
        health_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h18_health_watch.py").read_text(encoding="utf-8")
        ordered_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h18_ordered_rearm.py").read_text(encoding="utf-8")
        perf_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h18_perf_cal.py").read_text(encoding="utf-8")
        check(
        ...
        check(
            ...
            all(token in perf_source for token in (
                ...
                "full_trinal_clip_ppo_update", "timed_updates", "tolerance_pair",
                "value_head_catchup_epochs", "ALTERNATING_MSE_SMOOTHL1",
                "FORCED_OLD_LOG_PROB_OFFSET = 10.0", "torch.cuda.Event",
                "MODEL_TOLERANCE = 1e-6", "OPTIMIZER_TOLERANCE = 1e-8",
        ...
        result = {
            "schema_version": "v5.hybrid.h18.implementation_audit.v2",
            "checked_at": datetime.now(timezone.utc).isoformat(),
            "overall": "PASS_H18_IMPLEMENTATION" if not errors else "FAIL_CLOSED",
            "checks": checks,
            "details": details,
            ...
            "source_sha256": {
                ...
                "ppo": sha(args.ppo),
                "test": sha(args.test),
                "active_window": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h18_active_window.py"),
                "protocol_watch": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h18_protocol_watch.py"),
                "canonical_rearm": sha(rearm_path),
                "health_watch": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h18_health_watch.py"),
                "ordered_rearm": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h18_ordered_rearm.py"),
                "perf_cal": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h18_perf_cal.py"),
            },
