#!/usr/bin/env python3
"""Fail-closed terminal H8 judgment using fixed contemporaneous arms and H7 anchor."""
...
def mse(control: Path, treatment: Path, anchor: Path, holdout: Path, device: str) -> dict:
    ...
    return {
        ...
        "h7_nostop_anchor_normalized_mse": float(anchor_array.mean()),
        "primary_reduction": {
            "point": primary_point,
...
def verify_lock(path: Path, expected: str) -> dict:
    ...
    lock = load(path)
    if lock.get("design_id") != "H8" or lock.get("status") != "LOCKED" or lock.get("preregistration", {}).get("sha256") != PREREG_SHA:
        raise ValueError("design lock identity")
    if lock.get("tools", {}).get("scripts/alpha_holdem/v5_hybrid_h8_judge.py") != sha(TOOL):
...
def catchup_accounting(control_rows: list[dict], treatment_rows: list[dict]) -> dict:
    control_ok = all(
        not bool(row.get("value_head_catchup_enabled"))
        and int(row.get("value_head_catchup_epochs", 0)) == 0
        and int(row.get("value_head_catchup_minibatches", 0)) == 0
        for row in control_rows
    )
    treatment_enabled = all(bool(row.get("value_head_catchup_enabled")) for row in treatment_rows)
    ...
    for row in treatment_rows:
        ...
        if catchup_epochs != expected or (expected > 0 and catchup_minibatches <= 0) or (expected == 0 and catchup_minibatches != 0):
            exact = False
    return {
        "control_disabled_exact": control_ok,
    ...
    try:
        ...
        if terminal:
            ...
            if "RESOURCE_ISOLATION" in state:
                ...
                classification = "H8_INCONCLUSIVE_RESOURCE_ISOLATION_VIOLATION"
            elif state.startswith("H8_FAIL_PROTOCOL_ABORT_"):
                verdict = "FAIL"
                classification = state
            else:
                raise ValueError("unrecognized protocol terminal")
            result = {
                "schema_version": "v5.hybrid.h8.judgment.v1",
            ...
            anchor = Path(lock["measurement"]["h7_nostop_anchor_path"])
            if not anchor.is_file() or sha(anchor) != ANCHOR_SHA:
                raise ValueError("H7 no-stop anchor identity")
            mirror = load(args.mirror_judgment)
            if mirror.get("schema_version") != "v5.hybrid.h8.mirror_judgment.v1":
            ...
            mirror_anchor = mirror.get("comparisons", {}).get("treatment_vs_h7_nostop_anchor", {})
            checks = {
                "catchup_control_disabled": catchup["control_disabled_exact"],
            ...
            if deterministic_failure or clearly_adverse:
                verdict = "FAIL"
                classification = "H8_FAIL_REGISTERED_GATE"
            elif all(checks.values()):
                verdict = "PASS"
                classification = "H8_PASS_ALL_REGISTERED_GATES"
            else:
                verdict = "INCONCLUSIVE"
                classification = "H8_INCONCLUSIVE_FIXED_SAMPLE"
            result = {
                "schema_version": "v5.hybrid.h8.judgment.v1",
                "checked_at": datetime.now(timezone.utc).isoformat(),
                "overall": verdict,
