...
PREREG = ROOT / "reports" / "v5_hybrid_h10_post_cal_preregistration_20260715.json"
LOCK = ROOT / "reports" / "v5_hybrid_h10_design_lock_20260715.json"
JUDGE = ROOT / "scripts" / "alpha_holdem" / "v5_hybrid_h10_judge.py"
...
def test_h10_judge_is_the_exact_locked_tool() -> None:
    lock = json.loads(LOCK.read_text(encoding="utf-8-sig"))
    assert sha(JUDGE) == lock["tools"]["scripts/alpha_holdem/v5_hybrid_h10_judge.py"]
    assert sha(PREREG) == lock["preregistration"]["sha256"]


def test_h10_lock_preserves_every_registered_gate_semantically() -> None:
...
def test_h10_terminal_checks_cover_all_registered_gates() -> None:
    ...
    assert 'classification = "H10_FAIL_REGISTERED_GATE"' in source
    assert 'classification = "H10_PASS_ALL_REGISTERED_GATES"' in source
    assert 'classification = "H10_INCONCLUSIVE_FIXED_SAMPLE"' in source
