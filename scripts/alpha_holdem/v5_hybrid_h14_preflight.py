#!/usr/bin/env python3
"""Fail-closed H14 control launch preflight."""
...
def main() -> int:
    ...
    check("identity", lock.get("design_id") == "H14" and lock.get("status") == "LOCKED", "lock identity")
    ...
    check(
        ...
        "forbidden H9/H10/H11/H12/H13/CAL source path",
    )
    ...
    for process in psutil.process_iter(["pid", "cmdline", "nice"]):
        try:
            ...
            if any(token in lowered for token in (
                "v5_hybrid_h14_mirror.py", "v5_h1_calibration.py", "play_slumbot",
    ...
    temporary_root = Path.cwd() / ".test_tmp" / "h14_preflight"
    temporary_root.mkdir(parents=True, exist_ok=True)
    for index, (tool, extra) in enumerate((
        ("v5_hybrid_h14_endpoint_watch.py", ["--arm", "control"]),
        ("v5_hybrid_h14_endpoint_watch.py", ["--arm", "treatment"]),
        ("v5_hybrid_h14_protocol_watch.py", ["--arm", "control"]),
        ("v5_hybrid_h14_protocol_watch.py", ["--arm", "treatment"]),
        ("v5_hybrid_h14_completion_watch.py", ["--repo", str(Path.cwd())]),
    )):
    ...
    active_tool = Path("scripts/alpha_holdem/v5_hybrid_h14_active_window.py")
    ...
    result = {
        "schema_version": "v5.hybrid.h14.preflight.v1",
        "checked_at": datetime.now(timezone.utc).isoformat(),
        "overall": "PASS_READY_H14_CONTROL_LAUNCH" if not errors else "FAIL_CLOSED",
        "checks": checks,
        "errors": errors,
        "active_h14_trainers": active,
