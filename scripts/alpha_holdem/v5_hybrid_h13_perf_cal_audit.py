#!/usr/bin/env python3
"""Independent fail-closed audit for H13 PERF-CAL artifacts."""
...
from v5_hybrid_h13_perf_cal import (
...
def main() -> int:
    ...
    check("schema", artifact.get("schema_version") == "v5.hybrid.h13.perf_cal.v1")
    check("overall_pass", artifact.get("overall") == "PASS" and artifact.get("classification") == "H13_PERF_CAL_PASS")
    ...
    check("path1_unchanged", artifact.get("path1", {}).get("coordinator_pid") == 37656 and artifact.get("path1", {}).get("worker_count") == 6 and artifact.get("path1", {}).get("priority") == "BelowNormal" and artifact.get("path1", {}).get("changed") is False)
    ...
    payload = {
        "schema_version": "v5.hybrid.h13.perf_cal_audit.v1",
