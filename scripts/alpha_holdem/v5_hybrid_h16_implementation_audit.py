#!/usr/bin/env python3
"""Offline fail-closed implementation audit for H16 robust catch-up-only loss."""
...
from scripts.alpha_holdem.test_v5_hybrid_h16_implementation import run_update
...
def main() -> int:
    ...
    try:
        ...
        check(
            ...
            and prereg.get("experiment_id") == "H16"
        ...
        check(
            ...
            all(token in trainer_source for token in (
                "--h16-window-arm", "--h16-catchup-loss",
                "--h16-catchup-smooth-l1-beta", "--h16-preregistration", "--h16-design-lock",
            )),
        )
        check(
            "trainer_fail_closed_contract",
            all(token in trainer_source for token in (
                "H16 arm run_id or fixed endpoint mismatch",
                "H16 exact canonical source checkpoint identity/hash mismatch",
                "H16 forbidden H9/H10/H11/H12/H13/H14/H15/CAL source path",
                "H16 arms require target-KL0.03 and value-head catch-up enabled",
        ...
        active_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h16_active_window.py").read_text(encoding="utf-8")
        ...
        protocol_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h16_protocol_watch.py").read_text(encoding="utf-8")
        check(
        ...
        check(
            ...
            and "H16 terminal transition requires an active control or treatment window" in active_source,
        )
        check(
        ...
        check(
            "canonical_rearm_strict_h16_contract",
            all(token in rearm_source for token in (
                "$script:isHybridH16Arm",
                "Launch-H16OrderedRearm",
                "v5_hybrid_h16_ordered_rearm.py",
                "H16 exact ordered lifecycle terminally blocks every generic project path",
            )),
        )
        health_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h16_health_watch.py").read_text(encoding="utf-8")
        ordered_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h16_ordered_rearm.py").read_text(encoding="utf-8")
        perf_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h16_perf_cal.py").read_text(encoding="utf-8")
        check(
        ...
        result = {
            "schema_version": "v5.hybrid.h16.implementation_audit.v2",
            "checked_at": datetime.now(timezone.utc).isoformat(),
            "overall": "PASS_H16_IMPLEMENTATION" if not errors else "FAIL_CLOSED",
            "checks": checks,
            "details": details,
            ...
            "source_sha256": {
                ...
                "ppo": sha(args.ppo),
                "test": sha(args.test),
                "active_window": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h16_active_window.py"),
                "protocol_watch": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h16_protocol_watch.py"),
                "canonical_rearm": sha(rearm_path),
                "health_watch": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h16_health_watch.py"),
                "ordered_rearm": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h16_ordered_rearm.py"),
                "perf_cal": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h16_perf_cal.py"),
            },
