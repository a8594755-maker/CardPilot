...
SCRIPT = ROOT / "scripts/alpha_holdem/v5_hybrid_h15_ordered_rearm.py"
REARM = ROOT / "scripts/alpha_holdem/v5_rearm_watchers.ps1"
SPEC = importlib.util.spec_from_file_location("h15_rearm", SCRIPT)
...
def test_canonical_rearm_classifies_h15_and_blocks_generic_paths() -> None:
...
def test_canonical_h15_branch_launches_only_ordered_supervisor() -> None:
    source = REARM.read_text(encoding="utf-8-sig")
    branch = source.split("if ($script:isHybridH15Arm)", 1)[1].split("} elseif ($script:isHybridH11Arm)", 1)[0]
    assert "Launch-H15OrderedRearm" in branch
    assert "Launch-Health" not in branch
    assert "Launch-H11" not in branch
    assert "H15 exact ordered lifecycle terminally blocks every generic project path" in branch
