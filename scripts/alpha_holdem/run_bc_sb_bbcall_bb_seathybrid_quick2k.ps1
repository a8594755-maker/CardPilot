...
$sbCheckpoint = "models/bc/v3_anchor_5M_d1/best.pt"
$bbCheckpoint = "models/slumbot_br_bbcall_stage3_500k_20260725/latest.pt"
$runDir = "models/slumbot_bc_sb_bbcall_bb_seathybrid_quick2k_20260725"
New-Item -ItemType Directory -Force -Path $runDir | Out-Null

$processes = @()
for ($part = 1; $part -le 8; $part++) {
    $stem = "bench_v55_bc_sb_bbcall_bb_part$part"
    $arguments = @(
        "-X", "utf8", "-u",
        ...
        "--sb-model", $sbCheckpoint,
        "--bb-model", $bbCheckpoint
    )
...
if ($successfulHands -lt 1900) {
    throw "seat hybrid quick2k produced only $successfulHands hands"
}
