...
PREREG = ROOT / "reports" / "v5_hybrid_h9_preregistration_20260714.json"
LOCK = ROOT / "reports" / "v5_hybrid_h9_design_lock_20260714.json"
JUDGE = ROOT / "scripts" / "alpha_holdem" / "v5_hybrid_h9_judge.py"
...
def test_h9_judge_is_the_exact_locked_tool() -> None:
    lock = json.loads(LOCK.read_text(encoding="utf-8-sig"))
    assert sha(JUDGE) == lock["tools"]["scripts/alpha_holdem/v5_hybrid_h9_judge.py"]
    assert sha(PREREG) == lock["preregistration"]["sha256"]


def test_h9_lock_preserves_every_registered_gate_semantically() -> None:
...
def test_h9_terminal_checks_cover_all_registered_gates() -> None:
    ...
    assert 'classification = "H9_FAIL_REGISTERED_GATE"' in source
    assert 'classification = "H9_PASS_ALL_REGISTERED_GATES"' in source
    assert 'classification = "H9_INCONCLUSIVE_FIXED_SAMPLE"' in source
