...
def test_h16_rearm_validate_only_classifies_and_blocks_generic_paths() -> None:
...
def test_h16_strict_rearm_contains_only_locked_lifecycle_launches() -> None:
    source = REARM.read_text(encoding="utf-8-sig")
    required = (
        "if ($script:isHybridH16Arm)",
        ...
        "reports/v5_hybrid_h16_design_lock_v",
        "_20260719.json",
    )
...
def test_launcher_requires_rearm_status_survival_true() -> None:
    for name in ("v5_hybrid_h16_launch_control.ps1", "v5_hybrid_h16_launch_treatment.ps1"):
