...
$args = @(
    ...
    '--run-id', 'sourcev4_heroawr_league_criticv2_scale10m_20260726',
    '--run-dir', $runDir,
...
if ($LASTEXITCODE -ne 0) {
    throw 'Critic-v2 scale10M training failed'
}

$candidate = Join-Path $runDir 'latest.pt'
$probeDir = Join-Path $runDir 'internal_curve_final'
...
if ($LASTEXITCODE -ne 0) {
    throw 'Critic-v2 scale10M internal probe failed'
}

$externalDir = (
    'models\bench_sourcev4_heroawr_league_criticv2_scale10m_' +
...
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
    ...
    -Tag 'sourcev4_heroawr_league_criticv2_scale10m_pure_fresh5k_20260726' `
    ...
    -Strategy model
if ($LASTEXITCODE -ne 0) {
    throw 'Critic-v2 scale10M fresh5k failed'
}

$finalHandsText = & python -X utf8 -c (
    "import torch; c=torch.load(r'$candidate'," +
    "map_location='cpu',weights_only=False); print(int(c['total_hands']))"
)
if ($LASTEXITCODE -ne 0) {
    throw 'Could not read critic-v2 scale10M hand count'
}
$newHands = [int64]$finalHandsText - 3315295
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
    'scripts/alpha_holdem/promote_fresh5k_candidate.ps1' `
    -Candidate 'sourcev4_heroawr_league_criticv2_scale10m' `
    -QuickDir $externalDir `
    -SourcePolicy $candidate `
    -OutputStem 'sourcev4_heroawr_league_criticv2_scale10m' `
    -TrainingMethod (
        'continued normalized deeper critic-v2 fixed pure-policy league PPO'
    ) `
