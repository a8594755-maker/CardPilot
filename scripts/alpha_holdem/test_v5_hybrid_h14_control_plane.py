...
import v5_hybrid_h14_protocol_watch as protocol
...
class H14ControlPlaneTests(unittest.TestCase):
    def scan(self, processes, entries=None, supervisor_pid=None, supervisor_sha=None):
        rel = "scripts/alpha_holdem/v5_hybrid_h14_health_watch.py"
        script = ROOT / rel
    ...
    def run_active(self, root: Path, *extra: str) -> subprocess.CompletedProcess[str]:
        ...
        return subprocess.run([
            sys.executable,
            str(ROOT / "scripts/alpha_holdem/v5_hybrid_h14_active_window.py"),
            *extra,
    ...
    def test_control_protocol_abort_can_terminalize_without_treatment(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            activated = self.run_active(
                ...
                "v5_hybrid_h14_control_catchmse_same35051_20m_r1_20260717",
            )
            self.assertEqual(activated.returncode, 0, activated.stderr)
            judgment = root / "judgment.json"
            judgment.write_text(json.dumps({"design_id": "H14", "overall": "INCONCLUSIVE"}), encoding="utf-8")
    ...
    def test_treatment_protocol_abort_can_terminalize(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            self.assertEqual(self.run_active(root, "activate", "--arm", "control", "--run-id", "v5_hybrid_h14_control_catchmse_same35051_20m_r1_20260717").returncode, 0)
            self.assertEqual(self.run_active(root, "activate", "--arm", "treatment", "--run-id", "v5_hybrid_h14_treatment_catchsmoothl1b1_same35051_20m_r1_20260717").returncode, 0)
            judgment = root / "judgment.json"
            judgment.write_text(json.dumps({"design_id": "H14", "overall": "FAIL"}), encoding="utf-8")
    ...
    def test_conflicting_terminal_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            first.write_text(json.dumps({"design_id": "H14", "overall": "INCONCLUSIVE"}), encoding="utf-8")
            self.assertEqual(self.run_active(root, "terminal", "--verdict", "INCONCLUSIVE", "--judgment", str(first)).returncode, 0)
            second = root / "second.json"
            second.write_text(json.dumps({"design_id": "H14", "overall": "FAIL"}), encoding="utf-8")
    ...
    def test_exact_ordered_supervisor_identity_is_allowed(self) -> None:
        command = ["python", "C:/x/CardPilot/scripts/alpha_holdem/v5_hybrid_h14_ordered_rearm.py", "--arm", "control"]
        fake = FakeProcess(41212, command)
        joined = " ".join(command)
        self.assertEqual(self.scan([fake], supervisor_pid=41212, supervisor_sha=protocol.command_sha(joined)), [])
    ...
    def test_ordered_supervisor_pid_with_wrong_command_hash_fails_closed(self) -> None:
        ...
        found = self.scan([fake], supervisor_pid=41212, supervisor_sha="0" * 64)
