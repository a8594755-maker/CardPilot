#!/usr/bin/env python3
"""Frozen H8 40k three-endpoint common-deal mirror evaluator."""
...
PREREG_SHA = "ecc7f9bc30918c8a2c0b07dbe6ca8dd5d06cdfc6bd3e71b45b543a80e33d5713"
...
ANCHOR_SHA = "468f7a854e59387f2dda3bef7287a934a31d0ef75a5ec402db18bce02290d71b"
...
def load_lock(path: Path, expected: str, manifest: Path) -> dict:
    ...
    if value.get("design_id") != "H8-MIRROR-001" or value.get("status") != "LOCKED":
...
def prepare(out: Path) -> dict:
    ...
    for index in range(PAIRS):
        ...
        deals.append(
            {
                "index": index,
                "deal_id": f"h8mirror-{SEED}-{index:05d}-{deck_hash[:16]}",
                "deck_sha256": deck_hash,
                "opponent_pool_id": ACTIVE_POOL_IDS[index % 5],
            }
        )
    value = {
        "schema_version": "v5.hybrid.h8.mirror_manifest.v1",
        "design_id": "H8",
...
def audit_manifest(value: dict) -> list[str]:
    errors = []
    if value.get("schema_version") != "v5.hybrid.h8.mirror_manifest.v1" or value.get("preregistration_sha256") != PREREG_SHA:
...
def checkpoint_identity(path: Path, arm: str) -> dict:
    ...
    if arm == "anchor":
        ...
        if checkpoint.get("h7_window_arm") != "control" or float(checkpoint.get("ppo_target_kl", -1)) != 0.0:
            raise ValueError("anchor H7 identity")
        if int(checkpoint.get("iteration", -1)) != 32617 or int(checkpoint.get("total_hands", -1)) != 536005488:
        ...
        return {
            ...
            "hands": 536005488,
            "arm": arm,
            "source_identity": "H7_NO_STOP_CONTROL_ANCHOR",
        }
    expected_catchup = arm == "treatment"
    if checkpoint.get("h8_window_arm") != arm or abs(float(checkpoint.get("ppo_target_kl", -1)) - 0.03) > 1e-12:
        raise ValueError("endpoint H8 arm identity")
    if bool(checkpoint.get("h8_value_head_catchup_after_kl_stop")) != expected_catchup:
        raise ValueError("endpoint H8 catch-up identity")
    if config.get("h8_preregistration_sha256") != PREREG_SHA:
        raise ValueError("endpoint prereg identity")
    if not 556001286 <= int(checkpoint.get("total_hands", -1)) <= 556051286:
    ...
    return {
        ...
        "arm": arm,
        "ppo_target_kl": 0.03,
        "value_head_catchup": expected_catchup,
    }
...
def run_arm(manifest_path, endpoint, arm, out, device, lock_path, lock_sha, runtime):
    ...
    with out.open("x", encoding="utf-8", newline="\n") as stream:
        for expected in manifest["deals"]:
            ...
            stream.write(
                json.dumps(
                    {
                        "schema_version": "v5.hybrid.h8.mirror_pair.v1",
                        "arm": arm,
    ...
    summary = {
        "schema_version": "v5.hybrid.h8.mirror_arm_summary.v1",
        "arm": arm,
        "endpoint": identity,
...
def read_rows(path: Path, arm: str, manifest: dict):
    ...
    for index, row in enumerate(values):
        ...
        if (
            row.get("schema_version") != "v5.hybrid.h8.mirror_pair.v1"
            or row.get("arm") != arm
...
def audit_bundle(manifest_path, control, treatment, anchor, out, lock_path, lock_sha):
    ...
    value = {
        "schema_version": "v5.hybrid.h8.mirror_audit.v1",
        "overall": "PASS_IMMUTABLE_H8_MIRROR" if not errors else "FAIL_CLOSED",
        "errors": errors,
        "manifest_sha256": sha256_file(manifest_path),
        "measurement_lock_sha256": sha256_file(lock_path),
        "tool_sha256": sha256_file(TOOL),
...
def judge(manifest_path, control, treatment, anchor, audit, out, lock_path, lock_sha):
    ...
    if audit_value.get("overall") != "PASS_IMMUTABLE_H8_MIRROR":
    ...
    treatment_anchor = comparison(
        ...
        "treatment_minus_h7_nostop_anchor",
    )
    ...
    value = {
        "schema_version": "v5.hybrid.h8.mirror_judgment.v1",
        "status": status,
        "pairs": PAIRS,
        "comparisons": {
            "treatment_vs_control": treatment_control,
            "treatment_vs_h7_nostop_anchor": treatment_anchor,
        },
