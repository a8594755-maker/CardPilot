"""PCV019 exact-V5.5 bounded CPU smoke with invocation-robust control plane.

The parent launcher sets CUDA_VISIBLE_DEVICES=-1 (present-nonempty, PowerShell 5.1-safe)
plus PCV019_DEVICE_MODE and PCV019_CONTRACT_NONCE, and this child validates the exact
contract at its own boundary before any output. Science, interface, scope shape, and
gates are preserved from immutable PCV018 with fresh PCV019 seeds/paths/outputs. The
registered behavior-neutral correction is in the audit invocation control plane.
...
DEAL_SEED = 2026072093
ROLLOUT_SEED = 2026972093
DEALS = 16
ROWS = 64
...
def validate_device_contract() -> dict[str, Any]:
    ...
    observed_mode = os.environ.get("PCV019_DEVICE_MODE")
    observed_nonce = os.environ.get("PCV019_CONTRACT_NONCE")
    if observed_cvd != "-1":
    ...
    contract_string = (
        ...
        f"|PCV019_DEVICE_MODE={observed_mode}"
        f"|PCV019_CONTRACT_NONCE={observed_nonce}"
    ...
    return {
        ...
        "PCV019_DEVICE_MODE": observed_mode,
        "PCV019_CONTRACT_NONCE": observed_nonce,
...
def contract_probe() -> int:
    ...
    payload = {
        "schema_version": "v5.pcv019.contract_probe.v1",
        "design_id": "PCV019",
        **contract,
...
def make_teacher_row(row_id: int, deal_id: int, state: HUNLGameState) -> tuple[dict[str, Any], int]:
    ...
    row = {
        "schema_version": "v5.pcv019.teacher_row.smoke.v1",
...
def terminal_probe(deal_id: int, terminal_class: str) -> dict[str, Any]:
    ...
    payload = {
        "schema_version": "v5.pcv019.terminal_probe.v1",
        "classification": "FORBIDDEN_TRAINING_SMOKE_ONLY",
        "deal_id": deal_id,
...
def run(args: argparse.Namespace) -> int:
    ...
    if len(verified_inputs) != 31:
        raise RuntimeError("frozen_input_count_not_31")
    device_contract = validate_device_contract()
    output = Path(args.output).resolve(strict=False)
    registered_output = Path(prereg["immutable_outputs"]["root"]).resolve(strict=False)
    if output != registered_output:
        raise RuntimeError("registered_output_root_mismatch")
    if output.exists():
    ...
    raw_metrics = {
        "schema_version": "v5.pcv019.raw_metrics.v1",
    ...
    gates = {
        "input_hashes_31_of_31": len(verified_inputs) == 31,
    ...
    result = {
        "schema_version": "v5.pcv019.result.v1",
        "design_id": "PCV019",
        "judged_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "verdict": "PASS" if passed else "FAIL",
        "classification": "PCV019_PASS_INVOCATION_ROBUST_EXACT_V55_INTERFACE_AND_BOUNDED_CPU_SMOKE" if passed else "PCV019_FAIL_CLOSED_INVOCATION_DEVICE_INTERFACE_OR_BOUNDED_CPU_SMOKE_GATE",
        ...
        "authority": {
            ...
            "h19_or_later": "NONE",
            "official_hands": 0,
            "pass_next": "SEPARATELY_REGISTERED_FULL_TEACHER_ASSET_DESIGN_REVIEW_NO_AUTOMATIC_GENERATION",
            "nonpass_next": "SEPARATELY_REGISTERED_ROUTE_REVIEW031",
        },
        "path1_action": False,
