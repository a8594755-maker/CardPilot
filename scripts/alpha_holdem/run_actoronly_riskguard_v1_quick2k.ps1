...
$checkpoint = "models/actoronly_riskguard_v1_20260725/latest.pt"
$runDir = "models/actoronly_riskguard_v1_quick2k_20260725"
New-Item -ItemType Directory -Force -Path $runDir | Out-Null

$processes = @()
for ($part = 1; $part -le 8; $part++) {
    $stem = "bench_v55_actoronly_riskguard_v1_part$part"
    $arguments = @(
        "-X", "utf8", "-u",
...
if ($successfulHands -lt 1900) {
    throw "actor-only riskguard quick2k produced only $successfulHands hands"
}
