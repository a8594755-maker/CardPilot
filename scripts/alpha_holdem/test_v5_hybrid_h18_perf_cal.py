...
SCRIPT = ROOT / "scripts/alpha_holdem/v5_hybrid_h18_perf_cal.py"
SPEC = importlib.util.spec_from_file_location("h18_perf", SCRIPT)
...
def test_registered_gate_constants_are_frozen() -> None:
    ...
    assert MODULE.FORCED_OLD_LOG_PROB_OFFSET == 10.0
...
def test_cuda_event_timing_and_tolerance_pair_are_the_registered_instruments() -> None:
    source = SCRIPT.read_text(encoding="utf-8")
    assert "torch.cuda.Event(enable_timing=True)" in source
    assert "tolerance_pair(" in source
    assert "bitwise_identity_used_as_gate" in source
    assert "4096, 1024, 4, 1e-12, 4, 16, 7" in source
