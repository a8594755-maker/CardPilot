...
$sbPreflopCheckpoint = "models/slumbot_br_preflop_teacher_v14b_262k_20260725/latest.pt"
$runDir = "models/slumbot_br_v14b_sbpreflop_stage4_quick2k_20260725"
New-Item -ItemType Directory -Force -Path $runDir | Out-Null

$processes = @()
for ($part = 1; $part -le 8; $part++) {
    $stem = "bench_v55_v14b_sbpreflop_stage4_q2k_part$part"
    $arguments = @(
        "-X", "utf8", "-u",
        "scripts/alpha_holdem/play_slumbot.py",
        "--strategy", "sb_preflop_hybrid",
        "--hands", "250",
        "--device", "cpu",
        "--policy-mode", "greedy",
        ...
        "--fallback-model", $sbPreflopCheckpoint
    )
...
if ($successfulHands -lt 1900) {
    throw "SB-preflop hybrid quick2k produced only $successfulHands hands"
}
