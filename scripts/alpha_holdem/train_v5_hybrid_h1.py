...
LG001_REGISTRATION_TOKEN = '5ee42cb09c534cb3a294be701e94047f'
LG001_PREREGISTRATION_PATH = Path(
    r'C:\Users\a8594\CardPilot\reports\v5_lg001_unified_behavior_window_preregistration_5ee42cb09c534cb3a294be701e94047f_20260722.json'
)
LG001_PREREGISTRATION_SHA256 = '2d0a306ae005028a0745012dba5711316defee7f57bc1e2663e6726135be4125'
LG001_SOURCE_CHECKPOINT = Path(
    r'C:\Users\a8594\CardPilot\models\alpha_holdem_v5_hybrid\v5_hybrid_h11_control_catchmse_same33834_20m_r1_20260715\h11_control_endpoint.pt'
)
LG001_SOURCE_SHA256 = '96a007039b0baa29f0c39b0bd7adc67d8ca0733a41a261203f52430e60b5ca13'
LG001_SOURCE_ITERATION = 35051
LG001_SOURCE_HANDS = 576_021_901
LG001_ASSIGNMENT_SEED = 2026072201
LG001_SELF_WEIGHT = 0.20
LG001_MEMBER_ORDER = (103, 109, 115, 120, 129)
LG001_MEMBER_SPECS = {
    103: {'iteration': 26200, 'hands': 430445532, 'state_sha256': 'cdec36f3deb27470a61c586b6491cd5de44aa3a99194b026a6e491a3121335a1'},
    109: {'iteration': 27400, 'hands': 450186098, 'state_sha256': 'aee38c625bf0faada6b163f23aeb4cc539d67f7cbe46dc234aa4f46b18960953'},
    115: {'iteration': 28600, 'hands': 469929538, 'state_sha256': 'ed92c7724486e446c13e5d4c623327d4288c68652964b7b4f35c0d2a7630d0c1'},
    120: {'iteration': 29600, 'hands': 486379183, 'state_sha256': '86c3d7bacce72dd5749c21deaad3865c7313c9f118860cbc7e9b8b378070494e'},
    129: {'iteration': 31400, 'hands': 515989661, 'state_sha256': '9d008780ac3cd259579131532df9775b53b4e3c95c7b0f12f2aac70bc915b255'},
}
LG001_CONDITIONAL_WEIGHTS = {
    ...
    'treatment_diversity': {
        ...
        129: 0.188367537756600,
    },
}
LG001_OUTPUT_ROOT = Path(
    r'C:\Users\a8594\CardPilot\models\alpha_holdem_v5_hybrid\v5_lg001_5ee42cb09c534cb3a294be701e94047f_20260722'
)
LG001_STAGE_PATHS = {
    ('control_uniform', 'stage_a'): LG001_OUTPUT_ROOT / 'control_uniform_5m',
    ('treatment_diversity', 'stage_a'): LG001_OUTPUT_ROOT / 'treatment_diversity_5m',
    ('treatment_diversity', 'stage_b'): LG001_OUTPUT_ROOT / 'treatment_diversity_20m',
}
LG001_STAGE_TARGETS = {'stage_a': 581_021_901, 'stage_b': 596_021_901}


def lg001_file_sha256(path: Path) -> str:
...
def lg001_state_dict_sha256(state_dict) -> str:
    """Hash a state dict with the exact H4 tensor identity algorithm."""
    digest = hashlib.sha256()
...
def lg001_validate_frozen_pool(pool_snapshots) -> list[dict]:
    """Fail closed unless the exact five registered snapshot states are present."""
    if len(pool_snapshots) != len(LG001_MEMBER_ORDER):
        raise RuntimeError(f'LG001 requires exactly five pool snapshots, got {len(pool_snapshots)}')
    by_id = {}
    for local_index, snapshot in enumerate(pool_snapshots):
        member_id = int(snapshot.get('id', -1))
        if member_id in by_id or member_id not in LG001_MEMBER_SPECS:
            raise RuntimeError(f'LG001 unknown or duplicate pool member id {member_id}')
        spec = LG001_MEMBER_SPECS[member_id]
        actual_hash = lg001_state_dict_sha256(snapshot.get('state_dict') or {})
        if int(snapshot.get('iteration', -1)) != spec['iteration']:
            raise RuntimeError(f'LG001 member {member_id} iteration mismatch')
        if int(snapshot.get('hands', -1)) != spec['hands']:
            raise RuntimeError(f'LG001 member {member_id} hands mismatch')
        if actual_hash != spec['state_sha256']:
            raise RuntimeError(f'LG001 member {member_id} state hash mismatch')
        by_id[member_id] = {
            'member_id': member_id,
            'local_index': int(local_index),
            **spec,
        }
    if set(by_id) != set(LG001_MEMBER_ORDER):
        raise RuntimeError('LG001 frozen member set mismatch')
    return [by_id[member_id] for member_id in LG001_MEMBER_ORDER]


def lg001_assignment_u64(applies_to_iteration: int) -> int:
    material = (
        f'LG001|{LG001_REGISTRATION_TOKEN}|{LG001_ASSIGNMENT_SEED}|'
        f'{int(applies_to_iteration)}'
    ).encode('ascii')
    return int.from_bytes(hashlib.sha256(material).digest()[:8], 'big')


def lg001_select_opponent(arm: str, applies_to_iteration: int, pool_snapshots,
                          validated_members=None) -> dict:
    """Return the registered deterministic all-worker assignment for one iteration."""
    if arm not in LG001_CONDITIONAL_WEIGHTS:
        raise ValueError(f'unknown LG001 arm {arm!r}')
    members = (
        list(validated_members)
        if validated_members is not None
        else lg001_validate_frozen_pool(pool_snapshots)
    )
    by_id = {row['member_id']: row for row in members}
    weights = LG001_CONDITIONAL_WEIGHTS[arm]
    if set(weights) != set(LG001_MEMBER_ORDER) or not math.isclose(sum(weights.values()), 1.0, abs_tol=1e-15):
        raise RuntimeError('LG001 conditional weight vector invalid')
    u64 = lg001_assignment_u64(applies_to_iteration)
    u = u64 / float(1 << 64)
    base = {
        'schema_version': 'v5.lg001.assignment.v1',
        'registration_token': LG001_REGISTRATION_TOKEN,
        'preregistration_sha256': LG001_PREREGISTRATION_SHA256,
        'assignment_seed': LG001_ASSIGNMENT_SEED,
        'applies_to_iteration': int(applies_to_iteration),
        'draw_material': 'LG001|registration_token|assignment_seed|absolute_applies_to_iteration',
        'draw_u64': str(u64),
        'draw_u64_hex': f'{u64:016x}',
        'self_weight': LG001_SELF_WEIGHT,
        'conditional_pool_weights': {str(k): weights[k] for k in LG001_MEMBER_ORDER},
        'arm': arm,
    }
    if u < LG001_SELF_WEIGHT:
        return {**base, 'kind': 'self_play', 'local_index': HERO_MODEL_ID,
                'member_id': None, 'member_state_sha256': None, 'selected_weight': LG001_SELF_WEIGHT}
    conditional_u = (u - LG001_SELF_WEIGHT) / (1.0 - LG001_SELF_WEIGHT)
    cumulative = 0.0
    selected_id = None
    for member_id in LG001_MEMBER_ORDER:
        cumulative += weights[member_id]
        if conditional_u < cumulative:
            selected_id = member_id
            break
    if selected_id is None:
        selected_id = LG001_MEMBER_ORDER[-1]
    ...
    return {
        **base,
        ...
        'member_state_sha256': selected['state_sha256'],
        'selected_weight': (1.0 - LG001_SELF_WEIGHT) * weights[selected_id],
    }


def lg001_stage_for_args(arm: str, total_hands: int) -> str:
    if int(total_hands) == LG001_STAGE_TARGETS['stage_a']:
        return 'stage_a'
    if arm == 'treatment_diversity' and int(total_hands) == LG001_STAGE_TARGETS['stage_b']:
        return 'stage_b'
    raise ValueError(f'LG001 arm={arm} has unregistered total_hands={total_hands}')
...
def build_assignment_provenance_record(*, run_id: str, applies_to_iteration: int,
                                       ...
                                       lg001_assignment: dict | None = None):
    ...
    if lg001_assignment is not None:
        record['lg001'] = dict(lg001_assignment)
...
def main():
    ...
    parser.add_argument('--lg001-contract', default='',
                        help='Opt-in only: exact registered LG001 preregistration path.')
    parser.add_argument('--lg001-arm', choices=('none', 'control_uniform', 'treatment_diversity'),
                        default='none', help='Opt-in only: registered LG001 arm identity.')
    ...
    lg001_enabled = bool(args.lg001_contract or args.lg001_arm != 'none')
    lg001_stage = None
    lg001_contract_payload = None
    if lg001_enabled:
        if not args.lg001_contract or args.lg001_arm == 'none':
            parser.error('LG001 requires both --lg001-contract and --lg001-arm')
        contract_path = Path(args.lg001_contract)
        if not contract_path.is_absolute() or contract_path.resolve() != LG001_PREREGISTRATION_PATH.resolve():
            parser.error('LG001 contract path must be the exact registered absolute path')
        if not contract_path.is_file() or lg001_file_sha256(contract_path) != LG001_PREREGISTRATION_SHA256:
            parser.error('LG001 preregistration identity/hash mismatch')
        try:
            lg001_contract_payload = json.loads(contract_path.read_text(encoding='utf-8'))
        except (OSError, json.JSONDecodeError) as exc:
            parser.error(f'LG001 preregistration cannot be read: {exc}')
        if lg001_contract_payload.get('registration_token') != LG001_REGISTRATION_TOKEN:
            parser.error('LG001 registration token mismatch')
        try:
            lg001_stage = lg001_stage_for_args(args.lg001_arm, args.total_hands)
        except ValueError as exc:
            parser.error(str(exc))
        expected_dir = LG001_STAGE_PATHS.get((args.lg001_arm, lg001_stage))
        if expected_dir is None:
            parser.error('LG001 arm/stage combination is not registered')
        expected_out = expected_dir / 'latest.pt'
        expected_provenance = expected_dir / 'opponent_assignment_provenance.jsonl'
        if not args.run_dir or not Path(args.run_dir).is_absolute() or Path(args.run_dir).resolve() != expected_dir.resolve():
            parser.error('LG001 requires the exact registered absolute --run-dir')
        if not args.out or not Path(args.out).is_absolute() or Path(args.out).resolve() != expected_out.resolve():
            parser.error('LG001 requires the exact registered absolute --out path')
        if (
            not args.opponent_assignment_provenance_file
            or not Path(args.opponent_assignment_provenance_file).is_absolute()
            or Path(args.opponent_assignment_provenance_file).resolve() != expected_provenance.resolve()
        ):
            parser.error('LG001 requires the exact registered assignment provenance path')
        if expected_dir.exists() or expected_out.exists() or expected_provenance.exists():
            parser.error('LG001 output collision: registered single-attempt path already exists')
        if args.overwrite:
            parser.error('LG001 forbids --overwrite')
        if not args.resume or not Path(args.resume).is_absolute():
            parser.error('LG001 requires an absolute resume checkpoint')
        expected_resume = (
            LG001_SOURCE_CHECKPOINT
            if lg001_stage == 'stage_a'
            else LG001_STAGE_PATHS[('treatment_diversity', 'stage_a')] / 'latest.pt'
        )
        if Path(args.resume).resolve() != expected_resume.resolve():
            parser.error('LG001 resume path is not the exact registered stage source')
        if not args.allow_resume or args.reset_optimizer or args.reset_hand_counter:
            parser.error('LG001 requires resume with optimizer and hand counter preserved')
        if args.device != 'cuda':
            parser.error('LG001 requires CUDA and forbids CPU fallback')
        if os.environ.get('CUDA_VISIBLE_DEVICES') != '0':
            parser.error('LG001 requires child-observed CUDA_VISIBLE_DEVICES=0')
        exact_values = {
            'workers': (args.workers, 22),
            'hands_per_iter': (args.hands_per_iter, 16384),
            'starting_stack': (args.starting_stack, 200.0),
            'env_version': (args.env_version, 'v55'),
            'lr': (args.lr, 0.0003),
            'ppo_epochs': (args.ppo_epochs, 4),
            'mini_batch_size': (args.mini_batch_size, 1024),
            'epsilon': (args.epsilon, 0.0),
            'gamma': (args.gamma, 0.999),
            'delta1': (args.delta1, 3.0),
            'entropy_coef': (args.entropy_coef, 0.05),
            'postflop_action_prior_coef': (args.postflop_action_prior_coef, 0.02),
            'preflop_action_prior_coef': (args.preflop_action_prior_coef, 0.01),
            'k_best': (args.k_best, 5),
            'pool_strategy': (args.pool_strategy, 'loss-kbest'),
            'pool_history_limit': (args.pool_history_limit, 200),
            'self_play_fraction': (args.self_play_fraction, 0.2),
            'opponent_assignment': (args.opponent_assignment, 'per-iteration'),
            'rollout_mode': (args.rollout_mode, 'multi'),
            'rollout_envs_per_worker': (args.rollout_envs_per_worker, 16),
            'inference_min_batch_slots': (args.inference_min_batch_slots, 256),
            'inference_batch_deadline_us': (args.inference_batch_deadline_us, 1000.0),
            'worker_seed_base': (args.worker_seed_base, 73000),
            'seed': (args.seed, 20260703),
            'critic_contract': (args.critic_contract, CRITIC_V1),
            'value_coef': (args.value_coef, 0.5),
            'save_interval': (args.save_interval, 1),
        }
        mismatches = [name for name, (actual, expected) in exact_values.items() if actual != expected]
        if mismatches:
            parser.error(f'LG001 frozen config mismatch: {mismatches}')
        if not args.fixed_training_deal_stream or not args.mirror_self_play_deals or not args.allin_runout_ev:
            parser.error('LG001 requires the exact fixed/mirrored/all-in-EV variance contract')
        if args.allin_runout_ev_max_runouts != 200:
            parser.error('LG001 requires exactly 200 all-in EV runouts')
        if args.postflop_action_prior_target_values != [0.15, 0.30, 0.52, 0.03]:
            parser.error('LG001 postflop action prior target mismatch')
        if args.preflop_action_prior_target_values != [0.24, 0.36, 0.38, 0.02]:
            parser.error('LG001 preflop action prior target mismatch')
        expected_wall = 10800.0 if lg001_stage == 'stage_a' else 21600.0
        if args.max_runtime_seconds != expected_wall:
            parser.error('LG001 wall ceiling mismatch')
        args.lg001_stage = lg001_stage
        args.lg001_preregistration_sha256 = LG001_PREREGISTRATION_SHA256
    ...
    device = args.device
    W = args.workers
    if lg001_enabled and (device != 'cuda' or not torch.cuda.is_available()):
        raise RuntimeError('LG001 requires an available CUDA device; CPU fallback is forbidden')
    ...
    lg001_runtime_state = {
        'members': [],
        'provenance_tail_sha256': None,
        'provenance_tail_iteration': None,
    }
    ...
    def checkpoint_payload() -> dict:
        ...
        if lg001_enabled:
            payload['lg001'] = {
                'schema_version': 'v5.lg001.checkpoint_contract.v1',
                'registration_token': LG001_REGISTRATION_TOKEN,
                'preregistration_sha256': LG001_PREREGISTRATION_SHA256,
                'source_checkpoint_sha256': LG001_SOURCE_SHA256,
                'arm': args.lg001_arm,
                'stage': lg001_stage,
                'self_play_weight': LG001_SELF_WEIGHT,
                'conditional_pool_weights': {
                    str(member_id): LG001_CONDITIONAL_WEIGHTS[args.lg001_arm][member_id]
                    for member_id in LG001_MEMBER_ORDER
                },
                'assignment_seed': LG001_ASSIGNMENT_SEED,
                'assignment_draw': 'SHA256_FIRST_U64_BIG_ENDIAN_OF_LG001_TOKEN_SEED_ABSOLUTE_ITERATION',
                'pool_members': list(lg001_runtime_state['members']),
                'pool_membership_mutation': 'FORBIDDEN',
                'provenance_tail_sha256': lg001_runtime_state['provenance_tail_sha256'],
                'provenance_tail_iteration': lg001_runtime_state['provenance_tail_iteration'],
            }
        return payload
    ...
    if args.resume and os.path.exists(args.resume):
        resume_sha256 = None
        if lg001_enabled:
            resume_sha256 = lg001_file_sha256(Path(args.resume))
            if lg001_stage == 'stage_a' and resume_sha256 != LG001_SOURCE_SHA256:
                raise RuntimeError('LG001 Stage A source checkpoint hash mismatch')
        ckpt = torch.load(args.resume, map_location=device, weights_only=False)
        if lg001_enabled and lg001_stage == 'stage_a':
            if int(ckpt.get('iteration', -1)) != LG001_SOURCE_ITERATION:
                raise RuntimeError('LG001 Stage A source iteration mismatch')
            if int(ckpt.get('total_hands', -1)) != LG001_SOURCE_HANDS:
                raise RuntimeError('LG001 Stage A source hand count mismatch')
            if ckpt.get('env_version') != 'v55' or ckpt.get('obs_version') != 'v55':
                raise RuntimeError('LG001 Stage A source environment/observation mismatch')
            if ckpt.get('action_space_version') != '9slot_v5':
                raise RuntimeError('LG001 Stage A source action-space mismatch')
        if lg001_enabled and lg001_stage == 'stage_b':
            source_lg001 = ckpt.get('lg001') or {}
            if source_lg001.get('registration_token') != LG001_REGISTRATION_TOKEN:
                raise RuntimeError('LG001 Stage B source token mismatch')
            if source_lg001.get('preregistration_sha256') != LG001_PREREGISTRATION_SHA256:
                raise RuntimeError('LG001 Stage B preregistration mismatch')
            if source_lg001.get('source_checkpoint_sha256') != LG001_SOURCE_SHA256:
                raise RuntimeError('LG001 Stage B lineage source mismatch')
            if source_lg001.get('arm') != 'treatment_diversity' or source_lg001.get('stage') != 'stage_a':
                raise RuntimeError('LG001 Stage B must resume the exact Stage A treatment')
            if not source_lg001.get('provenance_tail_sha256'):
                raise RuntimeError('LG001 Stage B source lacks provenance continuity')
            source_hands = int(ckpt.get('total_hands', -1))
            if not LG001_STAGE_TARGETS['stage_a'] <= source_hands <= LG001_STAGE_TARGETS['stage_a'] + 50_000:
                raise RuntimeError('LG001 Stage B source hand count outside registered Stage A endpoint')
            lg001_runtime_state['provenance_tail_sha256'] = source_lg001['provenance_tail_sha256']
            lg001_runtime_state['provenance_tail_iteration'] = int(
                source_lg001.get('provenance_tail_iteration', ckpt.get('iteration', -1))
            )
        ...
        if lg001_enabled:
            lg001_runtime_state['members'] = lg001_validate_frozen_pool(pool.snapshots)
            expected_weights = {
                str(member_id): LG001_CONDITIONAL_WEIGHTS[args.lg001_arm][member_id]
                for member_id in LG001_MEMBER_ORDER
            }
            if lg001_stage == 'stage_b':
                source_lg001 = ckpt.get('lg001') or {}
                if source_lg001.get('conditional_pool_weights') != expected_weights:
                    raise RuntimeError('LG001 Stage B treatment weights changed')
        print(
    ...
    assignment_provenance_last_sha = lg001_runtime_state['provenance_tail_sha256']
    assignment_provenance_last_iteration = lg001_runtime_state['provenance_tail_iteration']
    ...
    def assign_opponents():
        ...
        group_metadata = None
        lg001_assignment = None
        if lg001_enabled:
            applies_to_iteration = int(iteration) + 1
            lg001_assignment = lg001_select_opponent(
                args.lg001_arm, applies_to_iteration, pool.snapshots,
                validated_members=lg001_runtime_state['members'],
            )
            assigned_np[:] = int(lg001_assignment['local_index'])
            group_metadata = [{
                'group_id': 0,
                'workers': list(range(W)),
                'opponent_id': int(assigned_np[0]),
                'lg001_member_id': lg001_assignment['member_id'],
            }]
        elif pool.size() == 0:
            assigned_np[:] = -1
            group_metadata = [{
        ...
        if assignment_provenance_fh is not None:
            ...
            record = build_assignment_provenance_record(
                ...
                lg001_assignment=lg001_assignment,
            )
            ...
            if lg001_enabled:
                lg001_runtime_state['provenance_tail_sha256'] = record['record_sha256']
                lg001_runtime_state['provenance_tail_iteration'] = applies_to_iteration
    ...
    try:
        ...
        while total_hands < args.total_hands:
            ...
            if iter_hands >= args.hands_per_iter and len(iter_transitions) > 0:
                ...
                if lg001_enabled and total_hands > args.total_hands + 50_000:
                    raise RuntimeError('LG001 endpoint overshoot exceeds 50,000 hands')
                ...
                # Snapshot
                if not lg001_enabled and iteration % args.snapshot_every == 0:
                    snap = pool.add(
