...
foreach ($path in @($source) + $fixedOpponents) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Missing aggressive mimic-v3 input: $path"
    }
}

$runDir = (
    'models\sourcev4_fullnet_allstreet_mimic3_' +
    'aggressive10m_20260726'
)
if (Test-Path -LiteralPath $runDir) {
    throw "Aggressive mimic-v3 output already exists: $runDir"
}
...
$args = @(
    ...
    '--run-id', 'sourcev4_fullnet_allstreet_mimic3_aggressive10m_20260726',
    '--run-dir', $runDir,
...
if ($LASTEXITCODE -ne 0) {
    throw 'Aggressive all-street mimic-v3 10M training failed'
}
...
[ordered]@{
    ...
    hypothesis = (
        'The held-out-qualified all-street league is useful, but the ' +
        'conservative lr=3e-6/KL=2 continuation underfits the correction.'
    )
    material_change = (
        'From the same source and league, reset optimizer and train all actor ' +
        'weights for 10M hands with lr=1e-5, preflop lr=3e-6, and source KL=0.5.'
    )
...
$externalDir = (
    ...
    'aggressive10m_pure_fresh5k_20260726'
)
New-Item -ItemType Directory -Path $externalDir | Out-Null
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
    'scripts/alpha_holdem/bench_v55_slumbot.ps1' `
    -ModelPath $candidate `
    -Tag 'sourcev4_fullnet_allstreet_mimic3_aggressive10m_pure_fresh5k_20260726' `
    ...
    -Strategy model
if ($LASTEXITCODE -ne 0) {
    throw 'Aggressive all-street mimic-v3 fresh5k failed'
}

& powershell.exe -NoProfile -ExecutionPolicy Bypass -File `
    'scripts/alpha_holdem/promote_fresh5k_candidate.ps1' `
    -Candidate 'sourcev4_fullnet_allstreet_mimic3_aggressive10m' `
    -QuickDir $externalDir `
    -SourcePolicy $candidate `
    -OutputStem 'sourcev4_fullnet_allstreet_mimic3_aggressive10m' `
    -TrainingMethod `
    'aggressive-full-network-PPO-against-held-out-qualified-mimic-v3' `
