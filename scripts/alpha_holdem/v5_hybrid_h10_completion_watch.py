#!/usr/bin/env python3
"""Duplicate-safe H10 post-arm evaluation and terminal judgment supervisor."""
...
def status(path: Path, **payload) -> None:
    value = {
        "schema_version": "v5.hybrid.h10.completion_watch_status.v1",
...
def active_h10_trainers() -> list[int]:
    active = []
    for process in psutil.process_iter(["pid", "cmdline"]):
        try:
            command = " ".join(process.info.get("cmdline") or [])
            if "train_v5.py" in command and "v5_hybrid_h10_" in command:
...
def main() -> int:
    ...
    try:
        ...
        lock = load(lock_path)
        if lock.get("design_id") != "H10" or lock.get("status") != "LOCKED":
            raise ValueError("design lock identity")
        tools = lock["tools"]
        mirror_tool = repo / "scripts/alpha_holdem/v5_hybrid_h10_mirror.py"
        judge_tool = repo / "scripts/alpha_holdem/v5_hybrid_h10_judge.py"
        active_window_tool = repo / "scripts/alpha_holdem/v5_hybrid_h10_active_window.py"
        if (
            sha(mirror_tool) != tools["scripts/alpha_holdem/v5_hybrid_h10_mirror.py"]
            or sha(judge_tool) != tools["scripts/alpha_holdem/v5_hybrid_h10_judge.py"]
            or sha(active_window_tool) != tools["scripts/alpha_holdem/v5_hybrid_h10_active_window.py"]
        ):
        ...
        mirror_tool_sha = tools["scripts/alpha_holdem/v5_hybrid_h10_mirror.py"]
        if (
        ...
        endpoint_status = {
            arm: directories[arm] / f"h10_{arm}_endpoint_status.json"
            for arm in directories
        }
        protocol = {
            arm: directories[arm] / f"h10_{arm}_protocol_status.json"
            for arm in directories
        }
        outputs = {
        ...
        judgment = repo / "reports/v5_hybrid_h10_judgment_20260715.json"
        ...
        while True:
            if judgment.is_file():
                ...
                if (
                    ...
                    status(
                        ...
                        h10_judgment=str(judgment.resolve()),
                    )
                    return 0
            terminal = None
            for arm in ("control", "treatment"):
            ...
            if terminal:
                run(
                    ...
                    status_path,
                    "RUNNING_H10_PROTOCOL_TERMINAL_JUDGMENT",
                )
                continue
            endpoints = {
            ...
            if not all(endpoints.values()):
                ...
                continue
            active = active_h10_trainers()
            if active:
                status(
                    status_path,
                    overall="PENDING",
                    state="WAITING_FOR_ALL_H10_TRAINERS_TO_EXIT",
                    active_h10_trainers=active,
                )
                time.sleep(max(1, args.poll_seconds))
                continue
            endpoint_paths = {
            ...
            if load(audit).get("overall") != "PASS_IMMUTABLE_H10_MIRROR":
