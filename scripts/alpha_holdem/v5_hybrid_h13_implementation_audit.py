#!/usr/bin/env python3
"""Offline fail-closed implementation audit for H13 robust catch-up-only loss."""
...
from scripts.alpha_holdem.test_v5_hybrid_h13_implementation import run_update
...
def main() -> int:
    ...
    try:
        ...
        check(
            ...
            and prereg.get("experiment_id") == "H13"
        ...
        check(
            ...
            all(token in trainer_source for token in (
                "--h13-window-arm", "--h13-catchup-loss",
                "--h13-catchup-smooth-l1-beta", "--h13-preregistration", "--h13-design-lock",
            )),
        )
        check(
            "trainer_fail_closed_contract",
            all(token in trainer_source for token in (
                "H13 arm run_id or fixed endpoint mismatch",
                "H13 exact canonical source checkpoint identity/hash mismatch",
                "H13 forbidden H9/H10/H11/CAL source path",
                "H13 arms require target-KL0.03 and value-head catch-up enabled",
        ...
        active_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h13_active_window.py").read_text(encoding="utf-8")
        ...
        protocol_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h13_protocol_watch.py").read_text(encoding="utf-8")
        check(
        ...
        check(
            ...
            and "H13 terminal transition requires an active control or treatment window" in active_source,
        )
        check(
        ...
        check(
            "canonical_rearm_strict_h13_contract",
            all(token in rearm_source for token in (
                "$script:isHybridH13Arm",
                "Launch-H13OrderedRearm",
                "v5_hybrid_h13_ordered_rearm.py",
                "H13 exact ordered lifecycle terminally blocks every generic project path",
            )),
        )
        health_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h13_health_watch.py").read_text(encoding="utf-8")
        ordered_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h13_ordered_rearm.py").read_text(encoding="utf-8")
        perf_source = (REPO / "scripts/alpha_holdem/v5_hybrid_h13_perf_cal.py").read_text(encoding="utf-8")
        check(
        ...
        result = {
            "schema_version": "v5.hybrid.h13.implementation_audit.v2",
            "checked_at": datetime.now(timezone.utc).isoformat(),
            "overall": "PASS_H13_IMPLEMENTATION" if not errors else "FAIL_CLOSED",
            "checks": checks,
            "details": details,
            ...
            "source_sha256": {
                ...
                "ppo": sha(args.ppo),
                "test": sha(args.test),
                "active_window": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h13_active_window.py"),
                "protocol_watch": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h13_protocol_watch.py"),
                "canonical_rearm": sha(rearm_path),
                "health_watch": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h13_health_watch.py"),
                "ordered_rearm": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h13_ordered_rearm.py"),
                "perf_cal": sha(REPO / "scripts/alpha_holdem/v5_hybrid_h13_perf_cal.py"),
            },
