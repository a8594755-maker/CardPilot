#!/usr/bin/env python3
"""Duplicate-safe H9 post-arm evaluation and terminal judgment supervisor."""
...
def status(path: Path, **payload) -> None:
    value = {
        "schema_version": "v5.hybrid.h9.completion_watch_status.v1",
...
def active_h9_trainers() -> list[int]:
    active = []
...
def main() -> int:
    ...
    try:
        ...
        lock = load(lock_path)
        if lock.get("design_id") != "H9" or lock.get("status") != "LOCKED":
            raise ValueError("design lock identity")
        tools = lock["tools"]
        mirror_tool = repo / "scripts/alpha_holdem/v5_hybrid_h9_mirror.py"
        judge_tool = repo / "scripts/alpha_holdem/v5_hybrid_h9_judge.py"
        if (
            sha(mirror_tool) != tools["scripts/alpha_holdem/v5_hybrid_h9_mirror.py"]
            or sha(judge_tool) != tools["scripts/alpha_holdem/v5_hybrid_h9_judge.py"]
        ):
        ...
        mirror_tool_sha = tools["scripts/alpha_holdem/v5_hybrid_h9_mirror.py"]
        if (
        ...
        endpoint_status = {
            arm: directories[arm] / f"h9_{arm}_endpoint_status.json"
            for arm in directories
        }
        protocol = {
            arm: directories[arm] / f"h9_{arm}_protocol_status.json"
            for arm in directories
        }
        outputs = {
        ...
        judgment = repo / "reports/v5_hybrid_h9_judgment_20260714.json"
        while True:
            if judgment.is_file():
                ...
                if (
                    ...
                    status(
                        ...
                        h9_judgment=str(judgment.resolve()),
                    )
                    return 0
            terminal = None
            for arm in ("control", "treatment"):
            ...
            if terminal:
                run(
                    ...
                    status_path,
                    "RUNNING_H9_PROTOCOL_TERMINAL_JUDGMENT",
                )
                continue
            endpoints = {
            ...
            if not all(endpoints.values()):
                ...
                continue
            active = active_h9_trainers()
            if active:
                status(
                    status_path,
                    overall="PENDING",
                    state="WAITING_FOR_ALL_H9_TRAINERS_TO_EXIT",
            ...
            if load(audit).get("overall") != "PASS_IMMUTABLE_H9_MIRROR":
