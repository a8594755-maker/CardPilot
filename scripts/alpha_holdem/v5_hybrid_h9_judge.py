#!/usr/bin/env python3
"""Fail-closed terminal H9 judgment using fixed contemporaneous arms and source anchor."""
...
def verify_lock(path: Path, expected: str) -> dict:
    ...
    lock = load(path)
    if lock.get("design_id") != "H9" or lock.get("status") != "LOCKED" or lock.get("preregistration", {}).get("sha256") != PREREG_SHA:
        raise ValueError("design lock identity")
    if lock.get("tools", {}).get("scripts/alpha_holdem/v5_hybrid_h9_judge.py") != sha(TOOL):
    ...
    try:
        ...
        if terminal:
            ...
            if "RESOURCE_ISOLATION" in state:
                ...
                classification = "H9_INCONCLUSIVE_RESOURCE_ISOLATION_VIOLATION"
            elif state.startswith("H9_FAIL_PROTOCOL_ABORT_"):
                verdict = "FAIL"
                classification = state
            else:
                raise ValueError("unrecognized protocol terminal")
            result = {
                "schema_version": "v5.hybrid.h9.judgment.v1",
            ...
            if not anchor.is_file() or sha(anchor) != ANCHOR_SHA:
                raise ValueError("H9 source anchor identity")
            mirror = load(args.mirror_judgment)
            if mirror.get("schema_version") != "v5.hybrid.h9.mirror_judgment.v1":
            ...
            if deterministic_failure or clearly_adverse:
                verdict = "FAIL"
                classification = "H9_FAIL_REGISTERED_GATE"
            elif all(checks.values()):
                verdict = "PASS"
                classification = "H9_PASS_ALL_REGISTERED_GATES"
            else:
                verdict = "INCONCLUSIVE"
                classification = "H9_INCONCLUSIVE_FIXED_SAMPLE"
            result = {
                "schema_version": "v5.hybrid.h9.judgment.v1",
                "checked_at": datetime.now(timezone.utc).isoformat(),
                "overall": verdict,
