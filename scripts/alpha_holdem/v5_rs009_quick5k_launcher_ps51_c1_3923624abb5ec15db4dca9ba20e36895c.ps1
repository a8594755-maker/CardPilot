...
$CorrectionAudit = Join-Path $Root "reports\v5_rs009_quick5k_ps51_launcher_correction_audit_${Token}_20260723.json"
$SessionAudit = Join-Path $Root "reports\v5_rs009_quick5k_implementation_audit_${Token}_20260723.json"
$SessionAuditSha256 = '374e16348a27b251ae0fbe0ab8a7306d10e06a5563dc417a6bb8f41f9f8806ca'
$QuickRoot = Join-Path $Root 'models\bench_v55_rs009_72e9bb6b8a4f4618aa6657710b66c5c9_greedy_quick5k_20260723'

if ($CorrectionAuditSha256 -notmatch '^[0-9a-f]{64}$') {
    throw 'correction_audit_sha256_invalid'
}
if (-not (Test-Path -LiteralPath $CorrectionAudit -PathType Leaf)) {
    throw 'correction_audit_missing'
}
$ObservedCorrectionSha = (Get-FileHash -LiteralPath $CorrectionAudit -Algorithm SHA256).Hash.ToLowerInvariant()
if ($ObservedCorrectionSha -ne $CorrectionAuditSha256) {
    throw 'correction_audit_sha256_mismatch'
}
$CorrectionJson = Get-Content -LiteralPath $CorrectionAudit -Raw | ConvertFrom-Json
if ($CorrectionJson.classification -ne 'PASS / RS009_QUICK5K_PS51_LAUNCHER_CORRECTION_AUDIT_PASS_NETWORK_READY') {
    throw 'correction_audit_nonpass'
}
if ((Get-FileHash -LiteralPath $SessionAudit -Algorithm SHA256).Hash.ToLowerInvariant() -ne $SessionAuditSha256) {
    throw 'session_implementation_audit_sha256_mismatch'
}
...
$Invocation = [ordered]@{
    ...
    correction_audit_sha256 = $CorrectionAuditSha256
    session_implementation_audit_sha256 = $SessionAuditSha256
...
$Invocation | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $InvocationPath -Encoding UTF8 -NoNewline
...
for ($Part = 1; $Part -le 4; $Part++) {
    ...
    $Args = @(
        ...
        '--max-attempts', '1500',
        '--nonce', $Nonce,
        '--implementation-audit-sha256', $SessionAuditSha256
    )
    $Environment = @{
        CUDA_VISIBLE_DEVICES = '0'
...
$LaunchResult | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $LaunchResultPath -Encoding UTF8 -NoNewline
if ($Failed) {
    exit 1
}
exit 0
