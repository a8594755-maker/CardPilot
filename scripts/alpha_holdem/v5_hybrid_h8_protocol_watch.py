#!/usr/bin/env python3
"""H8 throughput, entropy and resource-isolation protocol watcher."""
...
FORBIDDEN_PROCESS_TOKENS = ("v5_hybrid_h8_mirror.py", "v5_h1_calibration.py compare", "v5_slumbot", "slumbot_match")
...
def main() -> int:
    ...
    while True:
        ...
        if violations:
            action = stop(pid); write(status_path, {"overall": "FAIL", "state": "H8_INCONCLUSIVE_RESOURCE_ISOLATION_VIOLATION", "arm": args.arm, "pid": pid, "stop_action": action, "resource_isolation_violations": violations}); return 3
        if len(values) >= 20 and all(float(item.get("entropy", 99)) < 0.3 for item in values[-20:]):
            action = stop(pid); write(status_path, {"overall": "FAIL", "state": "H8_FAIL_PROTOCOL_ABORT_ENTROPY20", "arm": args.arm, "pid": pid, "stop_action": action, "resource_isolation_violations": []}); return 3
