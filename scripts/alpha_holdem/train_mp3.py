    ...
    for _ in range(epochs):
        ...
        for start in range(0, n, mini_batch_size):
            ...
            with torch.no_grad():
                ...
                approx_kl = (old_lp_t[idx] - new_lp).mean()
            ...
            if use_postflop_action_prior:
                loss = loss + float(action_prior_coef) * postflop_action_prior_loss
            if use_preflop_action_prior:
                loss = loss + float(preflop_action_prior_coef) * preflop_action_prior_loss
            if use_preflop_sb_open_action_prior:
                loss = loss + float(preflop_sb_open_action_prior_coef) * preflop_sb_open_action_prior_loss
            if use_preflop_bb_vs_open_action_prior:
                loss = loss + float(preflop_bb_vs_open_action_prior_coef) * preflop_bb_vs_open_action_prior_loss
