...
function pushLog(message) {
  ...
  const compact = raw.length > MAX_LOG_LINE_CHARS
    ? `${raw.slice(0, MAX_LOG_LINE_CHARS)} ...[truncated]`
    : raw;
...
async function fetchDashboardSummary(port) {
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/stats`, { signal: AbortSignal.timeout(2500) });
    if (!response.ok) {
    ...
    return {
      ...
      freeRAM: Number((stats.freeRAM ?? 0).toFixed ? stats.freeRAM.toFixed(1) : stats.freeRAM ?? 0),
