...
const REQUIRED_SERVER_KEYS = ['SUPABASE_URL', 'SUPABASE_ANON_KEY', 'SUPABASE_SERVICE_ROLE_KEY'];

const REQUIRED_WEB_KEYS = ['VITE_SERVER_URL', 'VITE_SUPABASE_URL', 'VITE_SUPABASE_ANON_KEY'];
...
function checkServerEnv(env) {
  ...
  if (setCount !== 0 && setCount !== REQUIRED_SERVER_KEYS.length) {
    ...
    failed = true;
    console.log(
      '[FAIL] Server Supabase keys are all missing. Production club persistence will not work.',
    );
  } else {
...
function checkWebEnv(env) {
  ...
  if (!hasValue(env, 'VITE_SERVER_URL')) {
    ...
    if (!httpsOk) {
      console.log(
        '[WARN] VITE_SERVER_URL is not HTTPS. Browser security/CORS may fail in production.',
      );
    }
    const pointsToLocal =
...
async function checkHealthz(env, serverUrlOverride) {
  ...
  try {
    ...
    console.log(
      `[${mark(enabled)}] GET ${healthzUrl} -> supabaseEnabled=${String(data?.supabaseEnabled)}`,
    );
    if (!enabled) {
