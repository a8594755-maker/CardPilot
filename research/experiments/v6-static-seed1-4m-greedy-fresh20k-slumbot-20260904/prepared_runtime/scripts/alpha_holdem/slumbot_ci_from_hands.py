...
BIG_BLIND = 100.0
# Active frozen Standard10 reference, measured on 20,000 fresh hands in the
# current V4-observation/V5.5-action harness.  Keep this synchronized with
# research/experiments/baseline-standard10-20260829/experiment.json.
DEFAULT_BASELINE_BB100 = -11.4275
DEFAULT_BASELINE_HANDS_MIN = 20_000


def load_rewards(paths: list[Path]) -> list[float]:
    if len(set(path.resolve() for path in paths)) != len(paths):
        raise ValueError("duplicate resolved hand-results file")
        ...
        with path.open("r", encoding="utf-8-sig") as fp:
            for line in fp:
                ...
                if "winnings_bb" in row:
                    reward = float(row["winnings_bb"])
                elif "winnings_chips" in row:
                    reward = float(row["winnings_chips"]) / BIG_BLIND
                else:
                    raise ValueError(f"{path} row missing winnings_bb/winnings_chips")
                if not math.isfinite(reward) or abs(reward) > 200.0:
                    raise ValueError(f"{path} nonfinite or out-of-stack reward")
                if "winnings_chips" in row:
                    chips = float(row["winnings_chips"])
                    if (not math.isfinite(chips) or not chips.is_integer()
                            or not math.isclose(chips/BIG_BLIND, reward, abs_tol=1e-9, rel_tol=0)):
                        raise ValueError(f"{path} inconsistent chip/bb reward")
                rewards_bb.append(reward)
    return rewards_bb
...
def expand_inputs(patterns: list[str]) -> list[Path]:
    ...
    if len(set(resolved)) != len(resolved):
        raise ValueError("overlapping inputs would double-count a hand-results file")
    return resolved
...
def classify_level(
    ...
    if l6:
        ...
        level = "L2"
        meaning = "moderately negative point estimate"
    elif bb_per_100 >= -50.0:
        level = "L1"
        meaning = "substantially negative point estimate"
    else:
        level = "L0"
        meaning = "severely negative point estimate"
...
def summarize(
    ...
    if any(not math.isfinite(value) or abs(value) > 200.0 for value in rewards_bb):
        raise ValueError("nonfinite or out-of-stack reward")
...
def main() -> int:
    ...
    parser.add_argument("--baseline-bb100", type=float, default=DEFAULT_BASELINE_BB100,
                        help="Active frozen-policy Slumbot baseline point estimate.")
