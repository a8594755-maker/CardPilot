...
def multi_objective_gradient_diagnostics(model, objectives, aggregate_loss):
    """Measure normalized objective geometry without changing the update."""
    named = [(name, p) for name, p in model.named_parameters() if p.requires_grad]
    if not named:
        raise ValueError('multi-objective diagnostics require trainable parameters')
    params = [parameter for _, parameter in named]

    def gradient_vector(loss):
    ...
    raw_vectors = [gradient_vector(objectives[name]) for name in names]
    raw_norms = [torch.linalg.vector_norm(vector) for vector in raw_vectors]
    zero_gradient_objectives = [
        name for name, norm in zip(names, raw_norms)
        if float(norm.item()) <= 1e-20
    ]
    retained = [
        (name, vector, norm)
        for name, vector, norm in zip(names, raw_vectors, raw_norms)
        if float(norm.item()) > 1e-20
    ]
    if not retained:
        raise ValueError('multi-objective diagnostic encountered only zero gradients')
    names = [name for name, _, _ in retained]
    vectors = torch.stack([vector for _, vector, _ in retained])
    norms = torch.stack([norm for _, _, norm in retained])
    units = vectors / norms.unsqueeze(1)
    cosine_matrix = units @ units.T
    aggregate = gradient_vector(aggregate_loss)
    aggregate_norm = torch.linalg.vector_norm(aggregate)
    alignments = (
        units @ (aggregate / aggregate_norm)
        if float(aggregate_norm.item()) > 1e-20
        else torch.zeros_like(norms)
    )
    off_diagonal = cosine_matrix[
        torch.triu(torch.ones_like(cosine_matrix, dtype=torch.bool), diagonal=1)
    ]
    return {
        'schema': 'cardpilot.ppo_opponent_seat_gradient_geometry.v1',
        'objective_names': names,
        'zero_gradient_objectives': zero_gradient_objectives,
        'objective_gradient_l2': [float(value) for value in norms],
        'pairwise_cosine_matrix': cosine_matrix.detach().cpu().tolist(),
        'pairwise_negative_count': int((off_diagonal < 0.0).sum().item()),
        'pairwise_below_negative_0_10': int(
            (off_diagonal < -0.10).sum().item()
        ),
        'aggregate_gradient_l2': float(aggregate_norm.item()),
        'aggregate_alignments': [float(value) for value in alignments],
        'aggregate_harmful_objective_count': int(
            (alignments <= 0.0).sum().item()
        ),
        'trainable_parameter_count': sum(parameter.numel() for parameter in params),
    }


def minimum_norm_actor_gradient(model, objectives, aggregate_loss):
    """Return a norm-matched MGDA gradient for the trainable policy heads."""
    named = [
        (name, parameter)
        for name, parameter in model.named_parameters()
        if parameter.requires_grad
        and name.startswith(('policy_head.', 'preflop_policy_head.'))
    ]
    if not named:
        raise ValueError('online MGDA requires trainable policy-head parameters')
    parameters = [parameter for _, parameter in named]

    def gradient_parts(loss):
        gradients = torch.autograd.grad(
    ...
    for name, loss in objectives.items():
        parts = gradient_parts(loss)
        flat = torch.cat([part.reshape(-1) for part in parts])
        norm = torch.linalg.vector_norm(flat)
        if float(norm.item()) <= 1e-20:
            zero_gradient_objectives.append(name)
        else:
            retained.append((name, flat / norm))
    if len(retained) < 2:
        raise ValueError('online MGDA requires at least two nonzero objectives')

    names = [name for name, _ in retained]
    units = torch.stack([unit for _, unit in retained])
    gram = (units @ units.T).detach().cpu().numpy().astype(np.float64)
    count = len(names)
    best = None
    for active_count in range(1, count + 1):
        for active in itertools.combinations(range(count), active_count):
            ...
            if np.linalg.norm(kkt @ solution - target) > 1e-7:
                continue
            if float(active_weights.min()) < -1e-10:
                continue
            ...
            inactive = set(range(count)) - set(active)
            if any(products[index] < objective - 1e-7 for index in inactive):
                continue
            candidate = (objective, tuple(active), weights, products)
            if best is None or candidate[0] < best[0]:
                best = candidate
    if best is None:
        raise RuntimeError('online MGDA minimum-norm simplex solve failed')
    objective, active, weights, products = best
    direction = torch.as_tensor(weights, device=units.device, dtype=units.dtype) @ units
    direction_norm = torch.linalg.vector_norm(direction)
    if float(direction_norm.item()) <= 1e-12:
        raise RuntimeError('online MGDA has no strict common-descent direction')

    aggregate_parts = gradient_parts(aggregate_loss)
    aggregate = torch.cat([part.reshape(-1) for part in aggregate_parts])
    aggregate_norm = torch.linalg.vector_norm(aggregate)
    if float(aggregate_norm.item()) <= 1e-20:
        raise RuntimeError('online MGDA cannot norm-match a zero actor gradient')
    scaled = direction * (aggregate_norm / direction_norm)
    alignments = units @ (scaled / torch.linalg.vector_norm(scaled))
    split = []
    offset = 0
    for parameter in parameters:
        count_values = parameter.numel()
        split.append(scaled[offset:offset + count_values].reshape_as(parameter))
        offset += count_values
    report = {
        'schema': 'cardpilot.ppo_opponent_seat_mgda_update.v1',
        'objective_names': names,
        'zero_gradient_objectives': zero_gradient_objectives,
        'simplex_weights': [float(value) for value in weights],
        'active_objectives': [names[index] for index in active],
        'minimum_norm_objective': float(objective),
        'kkt_min_product_minus_objective': float(products.min() - objective),
        'intervention_alignments': [float(value) for value in alignments],
        'worst_intervention_alignment': float(alignments.min().item()),
        'ordinary_actor_gradient_l2': float(aggregate_norm.item()),
        'intervention_actor_gradient_l2': float(
            torch.linalg.vector_norm(scaled).item()
        ),
    }
    return parameters, split, report


def greedy_advantage_margin_loss(
    logits,
    actions,
    advantages,
    policy_rows,
    ...
    """Promote sampled positive-advantage actions over current greedy actions."""
    if logits.ndim != 2:
        raise ValueError('greedy margin logits must be rank two')
    if actions.shape != advantages.shape or actions.shape != policy_rows.shape:
        raise ValueError('greedy margin row tensors must have matching shapes')
    if logits.shape[0] != actions.shape[0]:
        raise ValueError('greedy margin logits and rows must align')
    if float(margin) <= 0.0:
        raise ValueError('greedy advantage margin must be positive')
    greedy_actions = logits.detach().argmax(dim=-1)
    positive_rows = policy_rows & (advantages > 0.0)
    eligible_rows = positive_rows & (actions != greedy_actions)
    if not bool(eligible_rows.any().item()):
        zero = logits.sum() * 0.0
        return zero, eligible_rows.sum(), positive_rows.sum()
    selected_logits = logits.gather(1, actions.unsqueeze(-1)).squeeze(-1)
    greedy_logits = logits.gather(
        1,
        greedy_actions.unsqueeze(-1),
    ).squeeze(-1)
    weights = advantages[eligible_rows].detach()
    weights = weights / weights.mean().clamp_min(1e-8)
    gaps = selected_logits[eligible_rows] - greedy_logits[eligible_rows]
    loss = (weights * F.relu(float(margin) - gaps)).mean()
    return loss, eligible_rows.sum(), positive_rows.sum()


def source_greedy_margin_preservation_loss(
    logits,
    reference_logits,
    actions,
    advantages,
    ...
    release_advantage,
    row_weights=None,
    scale_release_by_row_weight=False,
):
    """Preserve a source argmax boundary unless reward evidence releases it."""
    if logits.ndim != 2 or reference_logits.shape != logits.shape:
        raise ValueError('source greedy margin logits must be aligned rank-two tensors')
    if legal_masks.shape != logits.shape:
        raise ValueError('source greedy margin legal masks must align with logits')
    if actions.shape != advantages.shape or actions.shape != policy_rows.shape:
        raise ValueError('source greedy margin row tensors must have matching shapes')
    if logits.shape[0] != actions.shape[0]:
        raise ValueError('source greedy margin logits and rows must align')
    if row_weights is not None:
        if row_weights.shape != actions.shape:
            raise ValueError('source greedy row weights must align with rows')
        if not bool(torch.isfinite(row_weights).all().item()):
            raise ValueError('source greedy row weights must be finite')
        if not bool((row_weights > 0.0).all().item()):
            raise ValueError('source greedy row weights must be positive')
    if scale_release_by_row_weight and row_weights is None:
        raise ValueError('risk-scaled release requires source greedy row weights')
    if float(max_margin) <= 0.0:
        raise ValueError('source greedy max margin must be positive')
    if float(release_advantage) < 0.0:
        raise ValueError('source greedy release advantage must be nonnegative')
    legal = legal_masks > 0.0
    has_alternative = legal.sum(dim=-1) > 1
    masked_reference = reference_logits.detach().masked_fill(~legal, -torch.inf)
    source_actions = masked_reference.argmax(dim=-1)
    reference_source = masked_reference.gather(
        1, source_actions.unsqueeze(-1)
    ).squeeze(-1)
    reference_other = masked_reference.scatter(
        1, source_actions.unsqueeze(-1), -torch.inf
    ).max(dim=-1).values
    target_margin = (reference_source - reference_other).clamp(
        min=0.0, max=float(max_margin)
    )
    masked_logits = logits.masked_fill(~legal, -torch.inf)
    current_source = masked_logits.gather(
        1, source_actions.unsqueeze(-1)
    ).squeeze(-1)
    current_other = masked_logits.scatter(
        1, source_actions.unsqueeze(-1), -torch.inf
    ).max(dim=-1).values
    current_margin = current_source - current_other
    release_threshold = float(release_advantage)
    if scale_release_by_row_weight:
        release_threshold = release_threshold * row_weights.detach().to(
            advantages.dtype
        )
    released = (
        policy_rows
        & (actions != source_actions)
        & (advantages > release_threshold)
    )
    eligible = policy_rows & has_alternative & ~released
    violations = eligible & (current_margin < target_margin)
    if not bool(eligible.any().item()):
        zero = logits.sum() * 0.0
        return zero, eligible.sum(), released.sum(), violations.sum()
    hinge = F.relu(target_margin[eligible] - current_margin[eligible])
    if row_weights is None:
        loss = hinge.mean()
    else:
        weights = row_weights[eligible].detach().to(hinge.dtype)
        # Normalize by the weight sum so risk weighting changes relative credit,
        # not the global auxiliary-loss scale.
        loss = (weights * hinge).sum() / weights.sum().clamp_min(1e-8)
    return loss, eligible.sum(), released.sum(), violations.sum()


def source_greedy_public_risk_weights(
    extras,
    *,
    max_weight=1.0,
    full_weight_pot_fraction=0.15,
):
    """Outcome-blind source-preservation weights from public remaining stacks.

    The first two extra features are hero and villain stack fractions relative
    to the starting effective stack.  In a zero-rake heads-up hand,
    ``2 - hero - villain`` is therefore the current pot divided by that stack.
    A max weight of one is the exact backward-compatible, unweighted path.
    """
    if extras.ndim != 2 or extras.shape[1] < 2:
        raise ValueError('source greedy risk weights require two stack features')
    if float(max_weight) < 1.0:
        raise ValueError('source greedy risk max weight must be at least one')
    if float(full_weight_pot_fraction) <= 0.0:
        raise ValueError('source greedy risk full pot fraction must be positive')
    pot_fraction = (2.0 - extras[:, 0] - extras[:, 1]).clamp(
        min=0.0,
        max=float(full_weight_pot_fraction),
    )
    ramp = pot_fraction / float(full_weight_pot_fraction)
    return 1.0 + (float(max_weight) - 1.0) * ramp


def temperature_policy_distribution(logits, temperature=1.0):
    """Return policy probabilities/distribution under a positive temperature."""
    if logits.ndim != 2:
        raise ValueError('policy logits must be rank two')
...
def preflop_context_masks(card_arr, action_arr):
    """Return public-state masks for contextual preflop action priors.

    ``sb_open`` is the first decision of a preflop hand. ``bb_vs_open`` is the
    next decision when the sole recorded action came from the opponent and was
    aggressive.  Keeping this classification as a small pure helper makes the
    training contract independently testable.
    """
    board_count_arr = card_arr[:, 4].sum(axis=(1, 2))
    postflop_arr = board_count_arr >= 2.5
    ...
    sb_open_arr = (~postflop_arr) & (preflop_slot_count_arr <= 0.5)
    bb_vs_open_arr = (
        (~postflop_arr)
    ...
    return postflop_arr, sb_open_arr, bb_vs_open_arr
...
def trinal_clip_ppo_update(
    ...
    reference_policy_kl_direction='reference_to_current',
    gradient_diagnostic_minibatches=0,
    opponent_seat_gradient_diagnostic_minibatches=0,
    opponent_seat_mgda=False,
    ...
    greedy_advantage_margin_coef=0.0,
    greedy_advantage_margin=0.1,
    source_greedy_margin_coef=0.0,
    source_greedy_margin_max=0.1,
    source_greedy_margin_release_advantage=1.0,
    source_greedy_risk_max_weight=1.0,
    source_greedy_risk_full_pot_fraction=0.15,
    source_greedy_risk_scale_release=False,
    policy_temperature=1.0,
    reference_policy_temperature=None,
    ...
    if int(opponent_seat_gradient_diagnostic_minibatches) < 0:
        raise ValueError(
            'opponent-seat gradient diagnostic minibatches must be non-negative'
        )
    ...
    if reference_policy_kl_direction not in {
        'reference_to_current', 'current_to_reference'
    }:
        raise ValueError('unsupported source-policy KL direction')
    use_source_greedy_margin = (
        reference_policy is not None
        and float(source_greedy_margin_coef or 0.0) > 0.0
    )
    ...
    if use_reference_policy_kl or use_source_greedy_margin:
    ...
    if float(greedy_advantage_margin_coef or 0.0) < 0.0:
        raise ValueError('greedy advantage margin coefficient must be nonnegative')
    if float(greedy_advantage_margin or 0.0) <= 0.0:
        raise ValueError('greedy advantage margin must be positive')
    if float(source_greedy_margin_coef or 0.0) < 0.0:
        raise ValueError('source greedy margin coefficient must be nonnegative')
    if float(source_greedy_margin_max or 0.0) <= 0.0:
        raise ValueError('source greedy max margin must be positive')
    if float(source_greedy_margin_release_advantage or 0.0) < 0.0:
        raise ValueError('source greedy release advantage must be nonnegative')
    if float(source_greedy_risk_max_weight or 0.0) < 1.0:
        raise ValueError('source greedy risk max weight must be at least one')
    if float(source_greedy_risk_full_pot_fraction or 0.0) <= 0.0:
        raise ValueError('source greedy risk full pot fraction must be positive')
    if (
        float(source_greedy_risk_max_weight or 1.0) > 1.0
        and not use_source_greedy_margin
    ):
        raise ValueError('source greedy risk weighting requires source greedy margin')
    if source_greedy_risk_scale_release and not use_source_greedy_margin:
        raise ValueError('risk-scaled release requires source greedy margin')
    if (
        source_greedy_risk_scale_release
        and float(source_greedy_risk_max_weight or 1.0) <= 1.0
    ):
        raise ValueError('risk-scaled release requires risk max weight above one')
    if float(source_greedy_margin_coef or 0.0) > 0.0 and reference_policy is None:
        raise ValueError('source greedy margin requires a reference policy')
    if float(policy_temperature or 0.0) <= 0.0:
        raise ValueError('policy temperature must be positive')
    if reference_policy_temperature is None:
        reference_policy_temperature = policy_temperature
    if float(reference_policy_temperature or 0.0) <= 0.0:
        raise ValueError('reference policy temperature must be positive')
    ...
    opponent_arr = np.array([
        int(t[16]) if len(t) > 16 else -999 for t in transitions
    ], dtype=np.int16)
    if int(opponent_seat_gradient_diagnostic_minibatches) > 0:
        known_opponent_rows = opponent_arr >= -1
        if not np.any(known_opponent_rows):
            raise ValueError(
                'opponent-seat gradient diagnostics require at least one '
                'transition with field 16'
            )
        if np.any((opponent_arr != -999) & ~known_opponent_rows):
            raise ValueError('invalid opponent identity transition metadata')
        if np.any(known_opponent_rows & ((player_arr < 0) | (player_arr > 1))):
            raise ValueError(
                'known opponent identities require valid seat metadata'
            )
    if opponent_seat_mgda and np.any(opponent_arr < -1):
        raise ValueError(
            'online opponent-seat MGDA requires identity-complete replay'
        )
    ...
    source_greedy_risk_weights_t = source_greedy_public_risk_weights(
        extras_t,
        max_weight=source_greedy_risk_max_weight,
        full_weight_pot_fraction=source_greedy_risk_full_pot_fraction,
    )
    ...
    opponent_t = torch.tensor(opponent_arr, dtype=torch.int16, device=device)
    ...
    if use_action_prior or policy_postflop_only or use_preflop_teacher:
        ...
        _, preflop_sb_open_arr, preflop_bb_vs_open_arr = preflop_context_masks(
            card_arr,
            action_arr,
        )
    ...
    if action_q_advantage:
        ...
        with torch.no_grad():
            for start in range(0, selected_indices.numel(), mini_batch_size):
                ...
                q_probs = F.softmax(
                    q_logits.float() / float(policy_temperature), dim=-1
                )
    ...
    tgreedy_margin_t = torch.zeros((), device=device)
    tgreedy_margin_eligible_t = torch.zeros((), device=device)
    tgreedy_margin_positive_t = torch.zeros((), device=device)
    tsource_greedy_margin_t = torch.zeros((), device=device)
    tsource_greedy_eligible_t = torch.zeros((), device=device)
    tsource_greedy_released_t = torch.zeros((), device=device)
    tsource_greedy_violations_t = torch.zeros((), device=device)
    ...
    for epoch_index in range(epochs):
        ...
        for start in range(0, n, mini_batch_size):
            ...
            if use_amp:
                logits = logits.float()
                values = values.float()
            probs, dist = temperature_policy_distribution(
                logits, policy_temperature
            )
            ...
            if use_reference_policy_kl:
                ...
                reference_policy_kl_rows = categorical_policy_kl(
                    logits,
                    reference_logits,
                    masks_t[idx],
                    temperature=float(reference_policy_temperature),
                    direction=reference_policy_kl_direction,
                )
            ...
            greedy_margin_loss = logits.sum() * 0.0
            greedy_margin_eligible = torch.zeros((), device=device)
            greedy_margin_positive = torch.zeros((), device=device)
            if float(greedy_advantage_margin_coef or 0.0) > 0.0:
                (
                    greedy_margin_loss,
                    greedy_margin_eligible,
                    greedy_margin_positive,
                ) = greedy_advantage_margin_loss(
                    logits,
                    acts_t[idx],
                    adv_t[idx],
                    policy_rows,
                    greedy_advantage_margin,
                )
            source_greedy_margin_loss = logits.sum() * 0.0
            source_greedy_eligible = torch.zeros((), device=device)
            source_greedy_released = torch.zeros((), device=device)
            source_greedy_violations = torch.zeros((), device=device)
            if use_source_greedy_margin:
                (
                    source_greedy_margin_loss,
                    source_greedy_eligible,
                    source_greedy_released,
                    source_greedy_violations,
                    ...
                    row_weights=source_greedy_risk_weights_t[idx],
                    scale_release_by_row_weight=(
                        source_greedy_risk_scale_release
                    ),
                )
            ...
            actor_loss = (
                actor_loss
                + float(greedy_advantage_margin_coef or 0.0)
                * greedy_margin_loss
            )
            actor_loss = (
                actor_loss
                + float(source_greedy_margin_coef or 0.0)
                * source_greedy_margin_loss
            )
            actor_loss = (
                actor_loss
            ...
            objective_probe_enabled = (
                opponent_seat_mgda
                or epoch_updates
                < int(opponent_seat_gradient_diagnostic_minibatches)
            )
            objectives = {}
            objective_support = {}
            known_policy_rows = None
            if objective_probe_enabled:
                opponent_policy = opponent_t[idx][policy_rows]
                seat_policy = player_t[idx][policy_rows]
                surrogate_loss_rows = -torch.min(surr1, surr2)
                known_policy_rows = opponent_policy >= -1
                categories = (
                    ('selfplay', opponent_policy == -1),
                    ('pool0', opponent_policy == 0),
                    ('pool1', opponent_policy == 1),
                    ('pool2', opponent_policy == 2),
                    ('dynamic_snapshot', opponent_policy >= 3),
                )
                for category, category_rows in categories:
                    for seat in (0, 1):
                        selected_rows = category_rows & (seat_policy == seat)
                        if bool(selected_rows.any().item()):
                            objective_name = f'{category}_seat{seat}'
                            objectives[objective_name] = surrogate_loss_rows[
                                selected_rows
                            ].mean()
                            objective_support[objective_name] = int(
                                selected_rows.sum().item()
                            )
                if use_reference_policy_kl:
                    known_minibatch_rows = opponent_t[idx] >= -1
                    known_reference_rows = reference_rows & known_minibatch_rows
                    if bool(known_reference_rows.any().item()):
                        objectives['source_kl'] = reference_policy_kl_rows[
                            known_reference_rows
                        ].mean()
                        objective_support['source_kl'] = int(
                            known_reference_rows.sum().item()
                        )
                if (
                    epoch_updates
                    < int(opponent_seat_gradient_diagnostic_minibatches)
                    ...
                    diagnostic = multi_objective_gradient_diagnostics(
                        model, objectives, actor_loss,
                    )
                    diagnostic.update(
                        epoch=epoch_index + 1,
                        minibatch=epoch_updates + 1,
                        objective_support=objective_support,
                        known_identity_policy_rows=int(
                            known_policy_rows.sum().item()
                        ),
                        unknown_identity_policy_rows=int(
                            (~known_policy_rows).sum().item()
                        ),
                        known_identity_fraction=float(
                            known_policy_rows.to(torch.float32).mean().item()
                        ),
                    )
                    opponent_seat_gradient_diagnostics.append(diagnostic)

            mgda_parameters = None
            mgda_gradients = None
            if opponent_seat_mgda:
                if len(objectives) < 2:
                    raise RuntimeError(
                        'online opponent-seat MGDA has insufficient objectives'
                    )
                mgda_parameters, mgda_gradients, mgda_report = (
                    minimum_norm_actor_gradient(model, objectives, actor_loss)
                )
                mgda_report.update(
                    epoch=epoch_index + 1,
                    minibatch=epoch_updates + 1,
                ...
                opponent_seat_mgda_updates.append(mgda_report)
            ...
            if action_q_advantage:
                ...
                if opponent_seat_mgda:
                    for parameter, gradient in zip(
                        mgda_parameters, mgda_gradients
                    ):
            ...
            tgreedy_margin_t += greedy_margin_loss.detach()
            tgreedy_margin_eligible_t += greedy_margin_eligible.detach()
            tgreedy_margin_positive_t += greedy_margin_positive.detach()
            tsource_greedy_margin_t += source_greedy_margin_loss.detach()
            tsource_greedy_eligible_t += source_greedy_eligible.detach()
            tsource_greedy_released_t += source_greedy_released.detach()
            tsource_greedy_violations_t += source_greedy_violations.detach()
            te_t += entropy.detach()
            tkl_t += approx_kl
    ...
    return {
        ...
        'greedy_advantage_margin_loss': (
            float(tgreedy_margin_t.item()) * inv_nu
        ),
        'greedy_advantage_margin_coef': float(
            greedy_advantage_margin_coef or 0.0
        ),
        'greedy_advantage_margin': float(greedy_advantage_margin),
        'greedy_advantage_margin_eligible_rows': int(
            tgreedy_margin_eligible_t.item()
        ),
        'greedy_advantage_margin_positive_rows': int(
            tgreedy_margin_positive_t.item()
        ),
        'greedy_advantage_margin_eligible_fraction': (
            float(tgreedy_margin_eligible_t.item())
            / max(float(tgreedy_margin_positive_t.item()), 1.0)
        ),
        'source_greedy_margin_loss': (
            float(tsource_greedy_margin_t.item()) * inv_nu
        ),
        'source_greedy_margin_coef': float(source_greedy_margin_coef or 0.0),
        'source_greedy_margin_max': float(source_greedy_margin_max),
        ...
        'source_greedy_risk_weight_mean': float(
            source_greedy_risk_weights_t.mean().item()
        ),
        'source_greedy_risk_weight_saturated_fraction': float(
            (
                (
                    source_greedy_risk_weights_t
                    >= float(source_greedy_risk_max_weight) - 1e-7
                ).float().mean().item()
                if float(source_greedy_risk_max_weight) > 1.0
                else 0.0
            )
        ),
        'source_greedy_margin_eligible_rows': int(
            tsource_greedy_eligible_t.item()
        ),
        'source_greedy_margin_released_rows': int(
            tsource_greedy_released_t.item()
        ),
        'source_greedy_margin_violation_rows': int(
            tsource_greedy_violations_t.item()
        ),
        'source_greedy_margin_violation_fraction': (
            float(tsource_greedy_violations_t.item())
            / max(float(tsource_greedy_eligible_t.item()), 1.0)
        ),
        ...
        'reference_policy_kl_direction': reference_policy_kl_direction,
        'reference_policy_temperature': float(reference_policy_temperature),
        'gradient_diagnostics': gradient_diagnostics,
        'opponent_seat_gradient_diagnostics': (
            opponent_seat_gradient_diagnostics
        ),
        'opponent_seat_mgda_updates': opponent_seat_mgda_updates,
