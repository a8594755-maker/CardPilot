...
def compare(control, treatment):
    ...
    rows = []
    if len(control['anchors']) != 3 or len(treatment['anchors']) != 3:
        raise ValueError('Exactly three registered anchors required')
...
def admission(primary, versus_source):
    return (len(primary) == len(versus_source) == 3
...
def validate_coefficient(config, rows, coefficient):
    expected = dict(source_policy_kl_coef=coefficient, seed=20260907, worker_seed_base=2026090700,
...
def main():
    ...
    for name, candidate in selection.items():
        ...
        documents[name] = doc
    for label, first, second in [('weak_vs_control', 'control', 'weak'),
    ...
    admitted = admission(analysis['comparisons']['weak_vs_control'], analysis['comparisons']['weak_vs_source'])
    ...
    result = dict(status='PASS', arms={}, comparisons=analysis['comparisons'], cell_reviews=cell_reviews,
                  ...
                  decision='WEAK_SOURCE_KL_ADMITS_CONFIRMATION' if admitted else 'WEAK_SOURCE_KL_NOT_ADMITTED')
    for arm, batch, initial_lr in [('control', 1024, .00003), ('weak', 1024, .00003)]:
        run = BASE / arm
        ...
        if (audit['status'] != 'PASS' or physical < 262144 or physical != candidate['physical_hands']
        ...
        for name, value in ckpt['model'].items():
            ...
            if name.startswith(('policy_head.', 'preflop_policy_head.', 'value_head.')):
                ...
                unchanged += 1
        if len(changes) != 10 or unchanged != 76 or any(value <= 0 for value in changes.values()):
        ...
        coefficient = 1. if arm == 'control' else .01
        validate_coefficient(config, rows, coefficient)
        ...
        result['arms'][arm] = dict(physical_hands=physical, overshoot=physical-262144,
    ...
    lines = ['# Matched source-KL weakening pilot', '', f"Decision: `{result['decision']}`.", '',
    ...
    lines += ['', 'Primary contrast: weak-KL minus matched control, bb/100; confidence intervals use mirrored pairs.', '',
        '| Anchor | Delta | 95% CI | Bonferroni 98.333% CI |', '|---|---:|---:|---:|']
    for row in result['comparisons']['weak_vs_control']:
