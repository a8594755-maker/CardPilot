...
ev.require(ev.BASE == ctl.BASE == BASE, 'wrong experiment module binding')
...
def accounting_from_runs(runs):
    ...
    ev.require(all(r['unknown_crash_suffix_hands'] == 0 for r in runs),
               'normal-completion summary cannot reinterpret an interrupted attempt')
    ev.require(all(r['new_physical_hands'] == r['new_transition_hands'] +
                   r['new_no_decision_hands'] + r['residual_worker_tail_hands'] and
                   r['subprocess_wall_seconds'] > 0 for r in runs), 'invalid completed-run accounting')
...
def verify_all_jobs(base, stage_count):
    expected = {f'{arm}_stage{stage}' for arm in ('static', 'moving256')
                for stage in range(1, stage_count + 1)}
    expected |= {f'{prefix}_{arm}_stage{stage}' for prefix in ('job_eval', 'drift')
                 for arm in ('static', 'moving256') for stage in range(1, stage_count + 1)}
    paths = list(base.glob('*/process.json'))
    ev.require({p.parent.name for p in paths} == expected, 'missing or unexpected tracked jobs')
    inputs = {}
    for path in paths:
        ev.require(not ctl.owner_live(ev.read_json(path)), 'tracked job still live')
        terminal_path = path.parent / 'termination.json'
        receipt = ev.read_json(terminal_path)
        ev.require(receipt['exit_code'] == 0 and not receipt['observer_errors']
                   and not receipt['remaining_observed_child_pids'], 'incomplete tracked job receipt')
        ...
        inputs.update({str(p): ev.sha(p) for p in (path, terminal_path, path.parent / 'command.json')})
    return inputs
...
def build_report(base):
    ...
    inputs.update(verify_all_jobs(base, stage_count))
    ...
    return {'schema': 'cardpilot.phase_reference.post_terminal_report.v1', 'passed': True,
            ...
            'provenance_limitations': 'Seed3 legacy parent needs initial main-RNG bootstrap; statistical not bitwise worker continuation; earlier-boundary freshness remains limited by parent preflight.',
            'scope_label_clarification': 'Frozen aggregate_stage ci_scope text says Seed1 due to a copied descriptive label. This report is Seed3; verified parent SHA and evaluation seeds determine identity. Frozen source and raw summaries remain unchanged.',
            'next_decision': 'COMBINE_SEED1_SEED3_SAME_DOSE_EVIDENCE_BEFORE_RESOURCE_DECISION' if stage_count == 2 else
