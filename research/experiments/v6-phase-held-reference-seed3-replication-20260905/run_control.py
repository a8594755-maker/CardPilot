...
ROOT = BASE.parents[2]
PARENT = ROOT / 'research/experiments/v6-static-current-kl-4m-scale-20260904/seed3/latest.pt'
PARENT_SHA = 'ba4ed0a5761f3b804053713f5246c20faced8a6949c969e9e6557033e1ae2810'
...
INITIAL_PHYSICAL = 4194908
INITIAL_ITERATION = 880
TRAINING_SEED = 20263003
WORKER_SEED_BASE = 2026300300
FIXED_DEAL_START = 39300000
LINEAGE_RUN_ID = 'v6_nashpg_static_seed3_20260903'
...
def training_command(arm, stage, parent_path, parent_physical):
    ...
    set_option(argv, '--seed', TRAINING_SEED)
    set_option(argv, '--worker-seed-base', WORKER_SEED_BASE)
    set_option(argv, '--fixed-training-deal-start-index', FIXED_DEAL_START)
    set_option(argv, '--run-id', LINEAGE_RUN_ID)
...
def preflight():
    ...
    for experiment in ('v6-static-current-kl-4m-scale-20260904',
                       ...
                       'v6-phase-held-reference-control-20260904',
                       'v6-phase-reference-greedy-paired-fresh80k-20260905'):
    ...
    names = {'train_v5.py', 'train_v5_managed_candidate.py', 'play_slumbot_v6_journaled.py',
             'audit_bridge_cli.py', 'gpu_qualification_v2_handoff.py', 'run_pair.py', 'resume_control.py', 'run_control.py', 'v6_public_opponent_matched_eval.py'}
    ...
    parent_review = BASE / 'seed3_parent_preflight.json'
    checks['seed3_parent_preflight_passed'] = False
    if parent_review.is_file():
        parent_evidence = evidence.read_json(parent_review)
        check_hashes(parent_evidence['input_sha256'])
        checks['seed3_parent_preflight_passed'] = (
            parent_evidence['passed'] and parent_evidence['parent_sha256'] == PARENT_SHA
            and parent_evidence['iteration'] == INITIAL_ITERATION
            and parent_evidence['physical_hands'] == INITIAL_PHYSICAL
            and parent_evidence['legacy_main_rng_bootstrap_required'])
...
class Controller:
    def __init__(self):
        ...
        self.inputs[str(BASE / 'seed3_parent_preflight.json')] = sha(BASE / 'seed3_parent_preflight.json')
        self.inputs.update(evidence.read_json(BASE / 'seed3_parent_preflight.json')['input_sha256'])
    ...
    def train(self, arm, stage):
        ...
        def capture(line):
            ...
            boundary = arm == 'moving256' and '[Save]' in line and not initial and (last_iteration - INITIAL_ITERATION) % 256 == 0
    ...
    def evaluate(self, stage):
        ...
        for arm in ('static', 'moving256'):
            ...
            command += ['--pairs-per-anchor', '2048', '--seed', str(20263510 + stage), '--device', 'cuda', '--out-dir', str(run)]
            ...
            command = [sys.executable, '-u', str(ROOT / 'scripts/alpha_holdem/v6_legacy_bridge_drift_audit.py'),
                       ...
                       '--seed', str(20263520 + stage), '--device', 'cuda', '--batch-size', '1024',
