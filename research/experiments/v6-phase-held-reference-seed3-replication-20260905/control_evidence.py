...
def reference_state(checkpoint, activation=880):
...
def initial_reference(arm, parent, initial, equal):
    ...
    if prior is None:
        require(initial['iteration'] == 880, 'only original4M may activate moving reference')
...
def aggregate_stage(stage, base, parent_sha, endpoint_hashes, anchors, prior_corpus):
    ...
    for arm in ('static', 'moving256'):
        ...
        require(summary['command'][summary['command'].index('--seed') + 1] == str(20263510 + stage), 'wrong seed')
        ...
        require(d['status'] == 'PASS' and d['design']['states'] == 20000 and
                d['design']['seed'] == 20263520 + stage and d['overall']['states'] == 20000,
