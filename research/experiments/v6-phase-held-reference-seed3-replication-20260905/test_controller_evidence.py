...
def checkpoint(iteration, reference_value=1, model_value=1):
    ...
    return {'iteration': iteration, 'model': {'w': model_value},
            ...
            'moving_source_policy_reference': {
                ...
                'reference_round': count // 256, 'activation_iteration': 880,
...
@pytest.mark.parametrize('field,value', [('activation_iteration', 881), ('completed_updates', 257),
...
def test_initial_reference_is_actual_parent_and_resume_does_not_rebase():
    original = checkpoint(880)
    del original['moving_source_policy_reference']
    ev.initial_reference('moving256', original, checkpoint(880), lambda a, b: a == b)
    with pytest.raises(ValueError, match='initial reference'):
        ev.initial_reference('moving256', original, checkpoint(880, reference_value=8), lambda a, b: a == b)
...
def test_phase_boundary_and_constant_reference_verified():
    initial = checkpoint(880)
    parent = copy.deepcopy(initial)
    del parent['moving_source_policy_reference']
    windows = [checkpoint(896), checkpoint(1136, 2, 2), checkpoint(1138, 2, 3)]
    rows = [metric(i) for i in range(881, 1139)]
    ...
    assert result['phase_boundaries'][0]['iteration'] == 1136
...
def test_complete_stage_aggregate_contract(tmp_path):
    ...
    for anchor_index, anchor in enumerate(ev.ANCHORS):
        for index in range(2048):
            rows.append(ev.derived_row({'anchor': anchor, 'anchor_seed': 20263511 + 1000003 * anchor_index,
    ...
    for arm in ('static', 'moving256'):
        ...
        run.write_new(directory / 'summary.json', {'status': 'COMPLETED', 'evaluation_hands': 32768,
            ...
            'command': ['python', 'eval.py', '--seed', '20263511'],
        ...
        run.write_new(directory / 'drift_analysis.json', {'status': 'PASS', 'design': {'states': 20000, 'seed': 20263521},
