"""Run the unchanged trainer with one explicitly hash-bound checkpoint I/O override."""
import hashlib
import json
...
CANDIDATE = HERE / 'checkpoint_io_candidate.py'
...
def install_io_override():
    ...
    original.atomic_torch_save = candidate.atomic_torch_save
    return {'schema': 'cardpilot.checkpoint_io_override.v1',
            'input_sha256': {str(p): digest for p, digest in EXPECTED.items()},
            'override': 'alpha_holdem.managed_checkpoint_io.atomic_torch_save',
            'learned_update_algorithm_modified': False}


def main():
    print(json.dumps(install_io_override()), flush=True)
    sys.argv = [str(PRODUCTION), *sys.argv[1:]]
    # Match normal script execution and package fallback imports before runpy.
