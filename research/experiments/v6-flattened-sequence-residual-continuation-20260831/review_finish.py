"""Independent terminal review of the fixed flat-sequence continuation."""
import json
import math
import subprocess
import sys
...
def main():
    ...
    assert checkpoint["source_weights_sha256"] == run.parent.SOURCE_SHA
    assert checkpoint["parent_weights_sha256"] == run.PARENT_SHA and run.sha(run.PARENT / "latest.pt") == run.PARENT_SHA
    ...
    assert all(float(state["step"]) == 4096 for state in checkpoint["optimizer"]["state"].values())
    rows = [json.loads(line) for line in (run.BASE / "training_metrics.jsonl").read_text().splitlines()]
    assert len(rows) == 2048 and [row["step"] for row in rows] == list(range(2049, 4097))
    assert all(row["epoch"] in range(9, 17) and row["rows"] == 1024 and math.isfinite(row["loss"]) and math.isfinite(row["gradient_norm"]) for row in rows)
    cpu = np.load(run.BASE / "cpu_probabilities.npy")
    gpu = np.load(run.BASE / "gpu_probabilities.npy")
    assert cpu.shape == gpu.shape == (69516, 9) and np.isfinite(cpu).all() and np.isfinite(gpu).all()
    parity = run.parent.parity_metrics(cpu, gpu)
    targets = np.load(run.parent.REACH / "validation_relabel/validation_targets.npy")
    metadata = np.load(run.parent.REACH / "validation_relabel/validation_metadata.npz")
    source_probabilities = np.load(run.parent.SOURCE / "development_probabilities.npy")
    treatment_hand, _ = run.parent.hero_tv(gpu, targets, metadata["hands"], metadata["actors"])
    source_hand, _ = run.parent.hero_tv(source_probabilities, targets, metadata["hands"], metadata["actors"])
    gate = run.parent.development_gate(treatment_hand, source_hand)
    ...
    assert decision == analysis["decision"] and gate["passed"] == analysis["development_gate"]["passed"]
    assert parity["passed"] == analysis["runtime_parity"]["passed"]
    ...
    report = {
        ...
        "source_weights_sha256": run.parent.SOURCE_SHA, "parent_weights_sha256": run.PARENT_SHA,
        "adapter_parameter_tensors": 14, "optimizer_steps": 4096,
        "continuation_optimizer_rows_processed": 2097152,
        ...
        "interpretation": "Fixed continuation uses a known development cohort and is not confirmatory poker-strength evidence.",
    }
    run.write("reviewed_analysis.json", report)
    (run.BASE / "result_summary.md").write_text(
        "# Flattened-sequence fixed continuation\n\n"
        ...
        "0 new hands, strength hands, or Slumbot hands. No further supervised residual extension.\n"
    )
    run.log(
    ...
    next_step = (
        ...
        "Stop supervised residual fitting and begin a full sequence-policy reward-training pilot."
    )
    subprocess.run(
        [sys.executable, str(run.ROOT / "research/experiment_log.py"), "finish", run.BASE.name,
         "--status", "COMPLETED", "--summary", f"Flat-sequence fixed continuation completed: {decision}.",
