...
def main():
    ...
    require(owner['pid'] == 91700 and abs(owner['create_time']-1788838495.9526286)<.001, 'owner identity')
    ...
    for row in result['training']:
        ...
        expected_parent = BASE/f'seed{seed}_{arm}_stage1/latest.pt' if stage==2 else BASE.parent/f'v6-family-allocation-two-seed-geometric-20260907/seed{seed}_control_stage2/latest.pt'
        ...
        previous,current=p['environment_hand_accounting'],final['environment_hand_accounting']
        physical=current['completed_hands']-previous['completed_hands']
        transition=final['total_hands']-p['total_hands']
        no_decision=current['no_trainable_decision_hands']-previous['no_trainable_decision_hands']
        replay=final['ppo_replay_cumulative_rows']-p['ppo_replay_cumulative_rows']
        require(physical>=transition>0 and no_decision>=0 and physical-transition-no_decision>=0,'accounting')
        require(replay>0 if arm=='control' else replay==0 and prior.state_equal(p['ppo_replay_rng_state'],final['ppo_replay_rng_state']),'replay semantics')
        delta=dict(new_physical_hands=physical,new_transition_hands=transition,new_no_decision_hands=no_decision,residual_worker_tail_hands=physical-transition-no_decision,new_replay_rows=replay)
        require(all(delta[k]==row[k] for k in ('new_physical_hands','new_transition_hands','new_replay_rows')), 'counts')
        require(final['environment_hand_accounting']['completed_hands'] >= {1:15744527,3:15747327}[seed]+{1:262144,2:1048576}[stage], 'dose')
        ...
        require(not (run/'family_runtime.json').exists(),'family contamination')
        review=read(run/'replay_contract_review.json')
        ms=[m for m in metrics if m['iteration']>p['iteration']]
        ratio=0 if arm=='expanded' else .5
        require(review['passed'] and review['ratio']==ratio and review['buffer_iterations']==2,'replay contract')
        require(all(m['ppo_replay_ratio']==ratio and m['ppo_replay_buffer_iterations']==2 for m in ms),'actual replay config')
        require(sum(m['ppo_replay_rows'] for m in ms)==replay==review['new_replay_rows'],'actual replay accounting')
        require(len(final['ppo_replay_entries'])==2,'rolling replay retention')
        if arm=='expanded':
            fr=read(run/'fresh_only_runtime.json')
            require(fr['parent_sha256']==binding['sha256'] and fr['ratio']==0 and fr['buffer_iterations']==2,'fresh runtime')
        ...
        for name in ('latest.pt','initial_resumed_state.pt','verification.json','parent_contract.json','prefixes.json','h1_training_metrics.jsonl','opponent_assignments.jsonl','run_manifest.json','latest_train.log','initial_resume_gate.json','mixture_runtime.json','mixture_observation.json','fresh_only_runtime.json','replay_contract_review.json'):
    ...
    for stage in (1,2):
        ...
        for seed in (1,3):
            ...
            for arm in ('control','expanded'):
                ...
                expected = {'control':sha(BASE.parent/f'v6-family-allocation-two-seed-geometric-20260907/seed{seed}_control_stage2/latest.pt'),'treatment':sha(BASE/f'seed{seed}_{arm}_stage{stage}/latest.pt'),**{f'anchor:{k}':sha(v) for k,v in anchors.items()}}
                require(summary['input_sha256']==expected and all(sha(p)==expected[n] for n,p in summary['input_paths'].items()), 'eval checkpoints')
                args = summary['command']; require(args[args.index('--seed')+1]==str(20267400+10*seed+stage), 'eval seed')
                arms[arm] = old.raw_map(raw)
                require({k[0] for k in arms[arm]}==set(anchors), 'anchor coverage')
                for j,anchor in enumerate(anchors):
                    rng = random.Random(20267400+10*seed+stage+j*1000003)
                    for i in range(1024):
                        deck = list(range(52)); rng.shuffle(deck)
                        key = (anchor,20267400+10*seed+stage+j*1000003,i)
            ...
            for group,names in groups.items():
                ...
                pairs = [(contrast['connected_minus_detached'],expected['fresh_minus_replay'])]+[(contrast['endpoint_minus_parent'][a],expected['endpoint_minus_root'][b]) for a,b in (('detached','control'),('connected','expanded'))]
                for recomputed, prior_summary in pairs:
                    old.check_reported(recomputed,prior_summary)
                require(contrast['broad_collapse']==expected['broad_collapse'],'collapse recomputation')
    ...
    for p,h in read(BASE/'qualification.json')['prior_corpus'].items():
        ...
        hashes[p] = h
    require(counts['new_physical_hands']==4206708 and counts['new_transition_hands']==3787308, 'total accounting')
