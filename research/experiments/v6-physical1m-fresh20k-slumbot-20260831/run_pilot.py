...
CONFIRM=ROOT/'research/experiments/v6-physical1m-independent-confirmation-20260831'
PARITY=ROOT/'research/experiments/v6-physical1m-deployment-parity-20260831'
REFERENCE=ROOT/'research/experiments/v6-source-fresh20k-slumbot-20260831'
CONFIRM_SHA='23aad3108d3b8c3753f81028a8220ecfa1fd48f89ed746559c5bb7afa52abc0e'
PARITY_SHA='e4c5b27baa56d4a8953a9e4bf61a308f9e845cffef4df210cf68fca6ab523f19'
...
def session_command(i):
    ...
    return [str(READY/'execution_code/source_files/scripts/alpha_holdem/play_slumbot_v6_journaled.py'),
        '--model',str(BASE/'frozen/final.pt'),'--hands','2500','--seed',str(2026100400+i),
        '--session-id',f'v6_physical1m_fresh20k_20260831_s{i:02d}','--out-dir',str(BASE/'sessions'/f's{i:02d}'),'--device','cpu']
...
def verify():
    ...
    assert confirmation['status']=='PASS' and confirmation['confirmation_gate_pass']
    assert confirmation['frozen_final_sha256']==MODEL_SHA
    assert read(CONFIRM/'experiment.json')['status']=='COMPLETED'
    assert sha(PARITY/'analysis.json')==PARITY_SHA
    ...
    assert sha(CONFIRM/'frozen/final.pt')==MODEL_SHA
    ...
    for directory in [READY,REFERENCE]:
        record=read(directory/'experiment.json')
        audit_path=directory/'combined_audit.json'
        item=next(v for p,v in record['artifact_integrity'].items() if Path(p).resolve()==audit_path.resolve())
        assert sha(audit_path)==item['sha256']
...
def excluded_tokens():
    return {h for directory in [READY,REFERENCE] for r in read(directory/'combined_audit.json')['results'] for h in r['token_sha256']}
    ...
    for i in range(1,9):
        ...
        with (BASE/'sessions'/f's{i:02d}'/'hands.jsonl').open() as handle:
            for index,line in enumerate(handle,1):
                ...
                assert row['session_id']==f'v6_physical1m_fresh20k_20260831_s{i:02d}' and row['policy_seed']==2026100400+i
...
def main():
    ...
    for p in psutil.process_iter(['name','cmdline']):
        if p.pid!=psutil.Process().pid and (p.info['name'] or '').lower().startswith('python') and any(
            Path(a).name in ['train_v5.py','play_slumbot.py','play_slumbot_v6.py','play_slumbot_v6_journaled.py','run_probe.py','run_pilot.py','run_baseline.py','v6_mirror_eval.py','run_confirmation.py']
    ...
    (BASE/'frozen').mkdir()
    shutil.copy2(CONFIRM/'frozen/final.pt',BASE/'frozen/final.pt')
    ...
    try:
        ...
        for i,(chips,audit) in enumerate(zip(sessions,combined['results']),1):
            assert audit['session_id']==f'v6_physical1m_fresh20k_20260831_s{i:02d}' and audit['policy_seed']==2026100400+i
