"""Exclusive offline finisher tests; defer all experiment logger writes."""
...
def main():
    output, xml = HERE / 'finish_qualification_v2.json', HERE / 'finish_tests_v2.xml'
    for path in (output, xml):
        review.r.protocol.require(not path.exists(), 'preserve earlier qualification')
    sources = (Path(__file__), HERE / 'finish_review.py', HERE / 'test_finish_review.py',
    ...
    argv = [sys.executable, '-B', '-m', 'pytest', '-q', '-p', 'no:cacheprovider',
            str(HERE / 'test_finish_review.py'), f'--junitxml={xml}']
