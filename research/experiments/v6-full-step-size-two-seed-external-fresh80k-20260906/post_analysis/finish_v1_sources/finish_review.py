...
def finish():
    ...
    finish_qualification = r.read(HERE / 'finish_qualification.json')
    ...
    r.protocol.require(r.sha(HERE / 'finish_tests.xml') == finish_qualification['xml_sha256'],
    ...
    subprocess.run(logger + ['audit', '--id', base.name, '--out', str(audit)], cwd=root, check=True)
