...
def test_optimizer_arms_have_only_registered_differences():
    control, large = values(run.make_command('control')), values(run.make_command('large'))
    allowed = {'--mini-batch-size', '--lr', '--run-id', '--run-dir', '--out', '--opponent-assignment-provenance-file'}
    assert {key for key in control if control[key] != large[key]} == allowed
    assert control['--mini-batch-size'] == '1024' and large['--mini-batch-size'] == '16384'
    assert control['--lr'] == '0.00003' and large['--lr'] == '0.0003'
    for config in [control, large]:
        assert config['--hands-per-iter'] == '8192'
        assert config['--total-environment-hands'] == '131072'
        assert config['--seed'] == '20260898'
    assert '--reset-optimizer' in run.make_command('control')
    assert '--reset-optimizer' in run.make_command('large')
...
def test_admission_requires_primary_gain_and_no_negative_source_points():
    rows = [dict(delta_bb100=1, ci95_lower=.1, ood_valid=True) for _ in range(3)]
    ...
    for row in flat: row['ci95_lower'] = -.1
