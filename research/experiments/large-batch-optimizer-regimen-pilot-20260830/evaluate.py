...
def admission(primary, versus_source):
    ...
    return (valid and all(r['delta_bb100'] > 0 for r in primary)
            and any(r['ci95_lower'] > 0 for r in primary)
...
def main():
    ...
    for name in ['source', 'control', 'large']:
        ...
        command = [str(snapshot / 'v5_mirror_eval.py'), '--candidate', candidate['path'],
                   '--candidate-label', f'large_batch_pilot_{name}', '--policy-mode', MODE,
    ...
    comparisons = {}
    for name, control, treatment in [('large_vs_control', 'control', 'large'),
    ...
    admitted = admission(comparisons['large_vs_control'], comparisons['large_vs_source'])
    assert hands == 73728
    ...
    result = dict(status='PASS', evaluation_hands=hands, seed=SEED, comparisons=comparisons,
                  ...
                  adjusted_secondary_gate=(admitted and any(r['bonferroni_lower'] > 0 for r in comparisons['large_vs_control'])),
                  cell_hashes={name: sha(path) for name, path in paths.items()},
                  claim_scope='exploratory_internal_joint_batch_lr_regimen_not_slumbot')
    ...
    log('--artifact', str(output), '--metric', 'pilot_evaluation_complete=1',
        ...
        '--note', 'Both frozen endpoints evaluated against source and matched optimizer control on independent seed. Complete raw evidence and independent CI recomputation passed. Inspect numerical/Adam health and result before finish; no claim of Slumbot success.')
