"""Matched physical-budget optimizer regimens, followed by fixed final evaluation."""
import hashlib
import json
...
def make_command(arm):
    if arm not in ('control', 'large'):
        raise ValueError('Unknown fixed arm')
    batch, lr = ('1024', '0.00003') if arm == 'control' else ('16384', '0.0003')
    run = BASE / arm
    return ['-u', 'scripts/alpha_holdem/train_v5.py', '--device', 'cuda', '--workers', '12',
        '--hands-per-iter', '8192', '--total-hands', '99999999', '--total-environment-hands', '131072',
        ...
        str(run / 'opponent_assignments.jsonl'), '--rollout-mode', 'single', '--worker-seed-base', '2026089800',
        ...
        '--run-id', f'large_batch_{arm}_20260830', '--run-dir', str(run), '--out', str(run / 'latest.pt'),
        '--seed', '20260898', '--max-runtime-seconds', '7200', '--resume', str(SOURCE),
...
def main():
    ...
    if any((BASE / name).exists() for name in ['control', 'large', 'frozen', 'execution_code']):
    ...
    log(*[item for name in ['copy_manifest.json', 'source_manifest.json', 'code.patch']
        ...
        '--note', 'Full execution snapshot frozen before both arms. Same source/deal seeds/collection/physical budgets; only optimizer batch/LR and output identity differ. No live source edits allowed during this wrapper.')
    total, selected = 0, {'source': dict(path=str(SOURCE), sha256=DIGESTS[0], physical_hands=0)}
    (BASE / 'frozen').mkdir()
    for arm in ['control', 'large']:
        verify_sources(copied)
        run = BASE / arm
        ...
        if count < 131072 or not manifest['environment_hand_accounting']['prefix_complete']:
        ...
        audit = ['scripts/alpha_holdem/audit_train_v5_fixed_pool_session.py', '--run-dir', str(run),
                 '--expected-target-hands', '99999999', '--expected-target-environment-hands', '131072',
