...
from alpha_holdem.validate_compact_v55_bridge import replay
...
def _atomic_torch_save(value: Any, path: Path) -> None:
    """Replace checkpoints atomically; Windows may briefly lock an old file."""
...
def _action_tensor(row: dict[str, Any], obs_version: str) -> np.ndarray:
    ...
    history_shape = str(preflop.get("historyShape", "BC"))
    # These are the actual frozen public roots used by the deployment-matched
    # solver. Action.amount is total committed for raises/bets, while calls
    # carry the engine's default zero amount.
    if history_shape == "BC":
        chronological: list[tuple[int, int, str, float | None]] = [
            (0, 1, "RAISE", 2.5),
            (0, 0, "CALL", None),
        ]
    elif history_shape == "BBC":
        chronological = [
            (0, 1, "RAISE", 2.5),
            (0, 0, "RAISE", 8.75),
            (0, 1, "CALL", None),
        ...
        raise ValueError(f"unsupported compact preflop history shape: {history_shape}")
...
def load_tensors(
    ...
    teacher_masks = np.empty((sample_count, NUM_ACTIONS), dtype=np.float32)
    deployment_masks = np.empty((sample_count, NUM_ACTIONS), dtype=np.float32)
    ...
    for row in iter_rows(paths, sample_count):
        ...
        extras[loaded] = extra
        teacher_masks[loaded] = mask
        deployment_mask, _ = build_action_table(
            replay(row), "preflop_pot_fraction_v2"
        )
        deployment_masks[loaded] = deployment_mask
    ...
    return (
        ...
        torch.from_numpy(teacher_masks),
        torch.from_numpy(deployment_masks),
...
def metrics_for_indices(
    ...
    with torch.no_grad():
        for start in range(0, len(indices), batch_size):
            ...
            hidden, base, teacher_mask, deployment_mask, target, streets = [
                tensor[batch_idx].to(device, non_blocking=True)
                for tensor in features[:6]
            ]
            residual = model.postflop_policy_adapter(hidden)
            teacher_student = base + residual + (1.0 - teacher_mask) * -1e9
            deployment_base = base + (1.0 - deployment_mask) * -1e9
            deployment_student = (
                base + residual + (1.0 - deployment_mask) * -1e9
            )
            source_log = F.log_softmax(deployment_base, dim=-1)
            student_log = F.log_softmax(deployment_student, dim=-1)
            source_prob = source_log.exp()
            teacher_student_log = F.log_softmax(teacher_student, dim=-1)
            target_ce = -(target * teacher_student_log).sum(dim=-1)
            ...
            changed = (
                deployment_base.argmax(dim=-1)
                != deployment_student.argmax(dim=-1)
            ).float()
            correct = (
                teacher_student.argmax(dim=-1) == target.argmax(dim=-1)
            ).float()
            mask_mismatch = (
                teacher_mask != deployment_mask
            ).any(dim=-1).float()
            ...
            totals["mask_mismatch"] += float(mask_mismatch.sum().item())
    ...
    result: dict[str, Any] = {
        ...
        "deployment_mask_mismatch_rate": totals["mask_mismatch"] / count,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="One or more compact JSONL files or directories.",
    )
    ...
    parser.add_argument(
        "--val-board-ids",
        default="",
        help="Optional comma-separated board IDs to hold out deterministically.",
    )
    parser.add_argument(
        "--test-board-ids",
        default="",
        help=(
            "Optional comma-separated final-test board IDs. Requires an explicit "
            "--val-board-ids; test metrics are computed only after the fixed final epoch."
        ),
    )
    parser.add_argument(
        "--street-weights",
    ...
    for data_path in args.data:
        paths.extend(
            sorted(data_path.glob("flop_*.jsonl"))
            if data_path.is_dir()
            else [data_path]
        )
    ...
    if missing_paths:
        raise SystemExit(f"missing compact CFR files: {missing_paths}")
    ...
    unique_boards = torch.unique(tensors[7], sorted=True)
    val_board_ids: list[int] = []
    requested_val_boards = {
        int(value.strip())
        for value in args.val_board_ids.split(",")
        if value.strip()
    }
    requested_test_boards = {
        int(value.strip())
        for value in args.test_board_ids.split(",")
        if value.strip()
    }
    if requested_test_boards and not requested_val_boards:
        raise SystemExit("--test-board-ids requires explicit --val-board-ids")
    if requested_val_boards & requested_test_boards:
        raise SystemExit("validation and test board IDs must be disjoint")
    available_boards = {int(value) for value in unique_boards.tolist()}
    unknown_holdout_boards = (
        requested_val_boards | requested_test_boards
    ) - available_boards
    if unknown_holdout_boards:
        raise SystemExit(
            "holdout board IDs not present in data: "
            f"{sorted(unknown_holdout_boards)}"
        )
    test_board_ids = sorted(requested_test_boards)
    test_mask = torch.zeros(sample_count, dtype=torch.bool)
    for board_id in test_board_ids:
        test_mask |= tensors[7] == board_id
    test_indices = torch.where(test_mask)[0]
    if requested_val_boards:
        val_board_ids = sorted(requested_val_boards)
        val_mask = torch.zeros(sample_count, dtype=torch.bool)
        for board_id in val_board_ids:
            val_mask |= tensors[7] == board_id
        val_indices = torch.where(val_mask)[0]
        train_indices = torch.where(~(val_mask | test_mask))[0]
        if not len(train_indices):
            raise SystemExit("explicit board holdouts leave no training rows")
        effective_val_split = "board_explicit"
    elif args.val_split == "board" and len(unique_boards) > 1:
    ...
    print(
        ...
        f"{len(test_indices):,} final-test; split={effective_val_split}; "
        f"val_boards={val_board_ids}; test_boards={test_board_ids})",
        flush=True,
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for start in range(0, len(epoch_order), args.batch_size):
            ...
            hidden, base, teacher_mask, deployment_mask, target, _ = [
                tensor[batch_idx].to(args.device, non_blocking=True)
                for tensor in features[:6]
            ]
            residual = model.postflop_policy_adapter(hidden)
            teacher_student_logits = (
                base + residual + (1.0 - teacher_mask) * -1e9
            )
            source_logits = base + (1.0 - deployment_mask) * -1e9
            student_logits = (
                base + residual + (1.0 - deployment_mask) * -1e9
            )
            source_log = F.log_softmax(source_logits, dim=-1)
            student_log = F.log_softmax(student_logits, dim=-1)
            source_prob = source_log.exp()
            teacher_student_log = F.log_softmax(
                teacher_student_logits, dim=-1
            )
            target_loss = -(target * teacher_student_log).sum(dim=-1).mean()
            source_kl = (
        ...
        output["offline_distillation"] = {
            ...
            "source_kl_mask": "deployment_cap999_preflop_pot_fraction_v2",
            "target_loss_mask": "solver_teacher_cap1",
            "anchor_features": (
        ...
        _atomic_torch_save(output, epoch_path)
        print(f"saved {epoch_path}", flush=True)

    if len(test_indices):
        final_test = metrics_for_indices(
            model, features, test_indices, args.batch_size, args.device
        )
        output["offline_distillation"]["final_test"] = final_test
        output["offline_distillation"]["runtime_seconds"] = float(
            time.time() - started
        )
        print(f"final_test={json.dumps(final_test, sort_keys=True)}", flush=True)

    _atomic_torch_save(output, args.out)
    print(f"saved final checkpoint {args.out}", flush=True)
