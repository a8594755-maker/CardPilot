...
SAMPLED_POLICY_MODE = "sampled_both_sides"
...
def init_actor(
    ...
    position_street_policy_adapter_hidden = hidden_width(
        state_dict, "position_street_policy_adapters.0.0.weight"
    )
    position_street_raw_policy_adapter_hidden = hidden_width(
        state_dict, "position_street_raw_policy_adapters.0.0.weight"
    )
    flat_sequence_policy_adapter_hidden = hidden_width(
        state_dict, "flat_sequence_policy_adapters.0.0.weight"
    )
    causal_sequence_policy_adapter_hidden = hidden_width(
        state_dict, "causal_sequence_policy_adapters.0.0.weight"
    )
    centralized_critic_hidden = hidden_width(
        state_dict, "centralized_value_head.0.weight"
    )
    position_value_adapter_hidden = hidden_width(
        state_dict, "position_value_adapters.0.0.weight"
    )
    action_q_hidden = hidden_width(state_dict, "action_q_head.0.weight")
    common_kwargs = {
    ...
    if (
        position_value_adapter_hidden > 0
        or action_q_hidden > 0
        or position_street_policy_adapter_hidden > 0
        or position_street_raw_policy_adapter_hidden > 0
        or flat_sequence_policy_adapter_hidden > 0
        or causal_sequence_policy_adapter_hidden > 0
        or centralized_critic_hidden > 0
    ):
        ...
        model = PositionValueAlphaHoldemNet(
            **common_kwargs,
            ...
            action_q_dueling=bool(
                checkpoint_value(checkpoint, "action_q_dueling", False)
            ),
            ...
            centralized_critic_hidden=centralized_critic_hidden,
        ).to(device)
    else:
    ...
    with torch.no_grad():
        model(
            ...
            torch.zeros(
                2,
                3 if bool(getattr(model, "requires_position_feature", False)) else 2,
                device=device,
            ),
        )
...
def init_model(checkpoint: dict[str, Any], device: str) -> torch.nn.Module:
    ...
    def load_seat_actor(prefix: str) -> AlphaHoldemNet:
        ...
        seat_name = "sb" if prefix.startswith("sb_model.") else "bb"
        seat_config = checkpoint.get(f"{seat_name}_actor_config")
        seat_checkpoint = {
            **checkpoint,
            **(seat_config if isinstance(seat_config, dict) else {}),
        }
        return init_actor(seat_checkpoint, seat_state, device)
...
def action_uniform(seed: int, pair_index: int, player: int, decision_index: int) -> float:
    """Counter-based CRN, independent of deck generation and policy identity."""
    payload = f"{ACTION_RNG_SCHEMA}:{seed}:{pair_index}:{player}:{decision_index}".encode()
    bits = int.from_bytes(hashlib.sha256(payload).digest()[:8], "big") >> 11
    return bits / float(1 << 53)


def sample_legal_slot(logits: np.ndarray, legal_slots: list[int], uniform: float) -> int:
    if not legal_slots or not 0.0 <= uniform < 1.0:
        raise ValueError("sampling requires legal slots and a uniform in [0, 1)")
    legal_logits = np.asarray(logits, dtype=np.float64)[legal_slots]
    if not np.all(np.isfinite(legal_logits)):
        raise ValueError("non-finite legal logits")
    weights = np.exp(legal_logits - np.max(legal_logits))
    cdf = np.cumsum(weights / weights.sum())
    index = min(int(np.searchsorted(cdf, uniform, side="right")), len(legal_slots) - 1)
    return int(legal_slots[index])


@torch.no_grad()
def choose_action(
    policy: Policy, state: HUNLGameState, player: int, *, uniform: float | None = None
) -> tuple[int, Any, bool]:
    include_position = bool(
    ...
    if uniform is None:
        ...
        slot = sample_legal_slot(logits[0].detach().cpu().numpy(), legal_slots, uniform)
...
def play_hand(
    *,
    ...
    if policy_mode not in (POLICY_MODE, SAMPLED_POLICY_MODE):
    ...
    while not state.is_terminal():
        ...
        uniform = (
            action_uniform(action_seed, pair_index, player, seat_decisions[player])
            if policy_mode == SAMPLED_POLICY_MODE
            else None
        )
        slot, action, ood_node = choose_action(actor_policy, state, player, uniform=uniform)
...
def summarize_anchor(
    *,
    ...
    result = {
        ...
        "policy_mode": policy_mode,
        "action_rng": (
            {"schema": ACTION_RNG_SCHEMA, "seed": seed}
            if policy_mode == SAMPLED_POLICY_MODE else None
        ),
...
def checkpoint_summary(checkpoint: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "position_street_policy_adapter_hidden": checkpoint_value(
            checkpoint, "position_street_policy_adapter_hidden", 0
        ),
        "position_street_raw_policy_adapter_hidden": checkpoint_value(
            checkpoint, "position_street_raw_policy_adapter_hidden", 0
        ),
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for index, (label, anchor_path) in enumerate(anchor_specs):
        ...
        row = summarize_anchor(
            ...
            policy_mode=getattr(args, "policy_mode", POLICY_MODE),
        )
    ...
    return {
        ...
        "action_rng_schema": (
            ACTION_RNG_SCHEMA
            if getattr(args, "policy_mode", POLICY_MODE) == SAMPLED_POLICY_MODE else None
        ),
        "candidate": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--policy-mode", choices=[POLICY_MODE, SAMPLED_POLICY_MODE], default=POLICY_MODE,
        help="Greedy default is unchanged; sampled mode uses seat-indexed CRN action streams.",
    )
