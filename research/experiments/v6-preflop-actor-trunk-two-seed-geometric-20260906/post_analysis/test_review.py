...
def test_contrast_direction_and_paired_decks():
    ...
    assert len(decks) == 8
    assert out['connected_minus_detached']['pooled']['ci95'] == [0., 0.]
    assert out['connected_minus_detached']['by_seat']['0']['bb100'] == 100.
    assert out['connected_minus_detached']['by_seat']['1']['bb100'] == -100.
...
@pytest.mark.parametrize('affected', ['connected', 'both'])
def test_severe_endpoint_regression_is_not_hidden_by_relative_tie(affected):
    ...
    if affected == 'both':
        assert out['connected_minus_detached']['pooled']['bb100'] == 0


def route_parent(connected):
    origin = {'schema':'cardpilot.preflop_actor_trunk_gradient.v1',
              'source_preflop_trunk_gradient':False,
              'target_preflop_trunk_gradient':True} if connected else None
    return {'config':{'preflop_trunk_gradient':connected},
            'preflop_trunk_gradient':connected,
            'preflop_trunk_gradient_migration':origin,'critic_contract':'critic_v2'}


@pytest.mark.parametrize('connected',[False,True])
def test_gradient_contract_and_origin(connected):
    parent=route_parent(connected)
    assert r.gradient_route(parent,copy.deepcopy(parent),copy.deepcopy(parent),
                            'connected' if connected else 'detached') == {
        'preflop_actor_to_trunk':connected,'critic_to_trunk':False,'origin_preserved':True}


@pytest.mark.parametrize('corruption',['config','metadata','type','origin','critic','missing_origin'])
def test_gradient_contract_corruption_rejected(corruption):
    parent=route_parent(True)
    initial,final=copy.deepcopy(parent),copy.deepcopy(parent)
    if corruption=='config':
        final['config']['preflop_trunk_gradient']=False
    elif corruption=='metadata':
        initial['preflop_trunk_gradient']=False
    elif corruption=='type':
        final['config']['preflop_trunk_gradient']=1
    elif corruption=='origin':
        final['preflop_trunk_gradient_migration']=None
    elif corruption=='critic':
        final['critic_contract']='critic_v1'
    else:
        for checkpoint in (parent,initial,final):
            checkpoint['preflop_trunk_gradient_migration']=None
    with pytest.raises(ValueError):
        r.gradient_route(parent,initial,final,'connected')
