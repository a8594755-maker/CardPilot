"""Terminal-only actor-route learning curves; reuse the qualified independent-deck math.
...
def summarize(report):
    ...
    for seed in ('1', '3'):
        ...
        for stage in (earlier, later):
            require(set(stage['endpoint_minus_parent']) == set(stage['absolute_vs_anchors']) == {'detached', 'connected'},
                    'wrong full-scope gradient-route arms')
        ...
        for arm in ('detached', 'connected'):
            ...
            route = {'preflop_actor_to_trunk': arm == 'connected',
                     'critic_to_trunk': False, 'origin_preserved': True}
            require(first.get('gradient_route') == second.get('gradient_route') == route,
                    'gradient route changed or missing')
            ...
            doses[arm] = physical
        results[seed] = {
            'connected_minus_detached_change': bucket_changes(earlier['connected_minus_detached'], later['connected_minus_detached']),
