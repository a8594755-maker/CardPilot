...
class Controller:
    ...
    def __init__(self):
        ...
        suites = list(ET.parse(BASE / 'controller_tests_final.xml').getroot().iter('testsuite'))
        ...
        for p in (BASE / 'protocol.md', BASE / 'controller_tests_final.xml', PREVIOUS / 'memory_16384.json'):
        ...
        logger('--artifact', str(BASE / 'input_contract.json'), '--artifact', str(BASE / 'ownership.json'),
               '--command', subprocess.list2cmdline(self.owner['command']))
