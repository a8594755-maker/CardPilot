...
CONFIRM=ROOT/'research/experiments/v6-fictitious-average-phase2-deployment-parity-20260831'
PARITY=CONFIRM
REFERENCE=ROOT/'research/experiments/v6-source-fresh20k-slumbot-20260831'
CONFIRM_SHA='adb139b3ba6541ac22e32d52ed6e6451b85a45713635b39cf387098602e51945'
PARITY_SHA='6252c7f7f3d4e5e67d576adfc92c178263ebf75bb068b70ef855098a6c689fa7'
...
def session_command(i):
    ...
    return [str(READY/'execution_code/source_files/scripts/alpha_holdem/play_slumbot_v6_journaled.py'),
        '--model',str(BASE/'frozen/final.pt'),'--hands','2500','--seed',str(2026102200+i),
        '--session-id',f'v6_fictitious_average_phase2_fresh20k_20260831_s{i:02d}','--out-dir',str(BASE/'sessions'/f's{i:02d}'),'--device','cpu']
...
def verify():
    assert isinstance(MODEL_SHA,str) and len(MODEL_SHA)==64 and isinstance(CONFIRM_SHA,str) and len(CONFIRM_SHA)==64 and isinstance(PARITY_SHA,str) and len(PARITY_SHA)==64, 'Assessment digests not frozen; no live launch'
...
def prior_audits():
    ...
    historical=ROOT/'research/experiments/v6-historical-average-fresh20k-slumbot-20260831'
    return [(d,d/'combined_audit.json') for d in [READY,REFERENCE,physical,historical]]+[
    ...
    for i in range(1,9):
        ...
        with (BASE/'sessions'/f's{i:02d}'/'hands.jsonl').open() as handle:
            for index,line in enumerate(handle,1):
                ...
                assert row['session_id']==f'v6_fictitious_average_phase2_fresh20k_20260831_s{i:02d}' and row['policy_seed']==2026102200+i
...
def main():
    ...
    try:
        ...
        for i,(chips,audit) in enumerate(zip(sessions,combined['results']),1):
            assert audit['session_id']==f'v6_fictitious_average_phase2_fresh20k_20260831_s{i:02d}' and audit['policy_seed']==2026102200+i
