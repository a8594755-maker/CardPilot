...
def test_exact_fresh_commands():
    ...
    for i,c in enumerate(commands,1):
        assert c[c.index('--seed')+1]==str(2026102200+i)
        assert c[c.index('--session-id')+1]==f'v6_fictitious_average_phase2_fresh20k_20260831_s{i:02d}'
...
def test_readiness_prerequisites_without_network():
    if run.MODEL_SHA is None or run.CONFIRM_SHA is None or run.PARITY_SHA is None:
        with pytest.raises(AssertionError, match='not frozen'):
            run.verify()
    else:
        run.verify()
    assert run.excluded_tokens()
    assert run.MODEL_SHA is None or isinstance(run.MODEL_SHA,str) and len(run.MODEL_SHA)==64
...
def test_all_prior_corrected_v6_cohorts_excluded():
    ...
    assert len(paths) == 6 and len({str(p) for _, p in paths}) == 6
...
def test_historical_average_live_cohort_is_excluded():
    assert any('v6-historical-average-fresh20k-slumbot' in str(p) for _,p in run.prior_audits())


def test_missing_model_hash_fails_before_any_live_call(monkeypatch):
