...
def main():
    ...
    for i,child in enumerate(execution['children'][:8],1):
        assert child['role']==f'session{i}' and child['command']==[sys.executable,*run.session_command(i)]
    run.verify()
    ...
    recorded_hash(BASE/'combined_audit.json')
    recorded_hash(BASE/'completed_analysis.json')
    ...
    for i,item in enumerate(audit['results'],1):
        ...
        assert item['session_id']==f'v6_fictitious_average_phase2_fresh20k_20260831_s{i:02d}' and item['policy_seed']==2026102200+i
        ...
        for name in ['journal.jsonl','hands.jsonl','summary.json']:
    ...
    report=dict(status='PASS',reviewed_at=datetime.now(timezone.utc).isoformat(),decision=decision,
        ...
        probe_hands_pooled=0,old_baseline_hands_pooled=0,excluded_all_prior_v6_token_chains_disjoint=True,server_rng_independence_proven=False,
    ...
    text='# Frozen phase2 fictitious-average epoch08 fresh20k Slumbot pilot\n\n'
    ...
    text+='All clients exited0. Complete journal/terminal/server-counter/frozen-model replay audit and independent raw arithmetic passed. Token chains are disjoint across all8sessions and all excluded corrected-v6 cohorts (source16probe,source20k,physical-final20k,both selfplay-share20k arms,and historical-average20k). No proof of server RNG independence is claimed. No probe,internal,or old-contract hands are pooled.\n\n'
    ...
    subprocess.run([sys.executable,str(ROOT/'research/experiment_log.py'),'finish',BASE.name,'--status','COMPLETED',
        '--summary',f'Exactly20k fresh frozen phase2 fictitious-average epoch08 hands: {result["bb_per_100"]:+.4f}bb/100; raw95%CI{result["raw_hand_ci95"]}; complete evidence PASS.',
