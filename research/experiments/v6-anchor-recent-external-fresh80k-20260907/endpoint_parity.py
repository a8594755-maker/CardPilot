...
def main():
    ...
    out, failure = BASE / 'endpoint_parity.json', BASE / 'endpoint_parity_failure.json'
    r.protocol.require(not out.exists() and not failure.exists(), 'preserve prior endpoint qualification')
    torch.set_num_threads(1)
    torch.manual_seed(2026470701)
    ...
    try:
        for arm, (path, expected) in r.EXPECTED_MODELS.items():
            ...
            recent = True  # All four current endpoints retain the connected preflop route.
            ...
            r.protocol.require(type(flag) is bool and flag is recent and
                checkpoint.get('preflop_trunk_gradient', False) is recent, 'wrong training route')
            training = candidate.AlphaHoldemNet(norm_layer='gn', critic_contract='critic_v2',
                separate_preflop_head=True, preflop_trunk_gradient=recent).eval()
            with torch.no_grad():
            ...
            models[arm] = {'path': str(path), 'sha256': expected,
                'physical_lineage_hands': r.EXPECTED_HANDS[arm], 'preflop_trunk_gradient': recent,
