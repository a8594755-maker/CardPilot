...
def audited_fixture(tmp_path_factory):
    ...
    for arm in m.r.protocol.ARMS:
        ...
        for item in [row for row in spec['schedule'] if row['arm'] == arm]:
            ...
            chips = 10 if arm.endswith('recent') else -5
...
def test_control_raw_review_hash_accounting_fixture(audited_fixture):
    inputs = {}
    ...
    for arm in m.r.protocol.ARMS:
        ...
        assert moments[arm]['bb_per_100'] == (10 if arm.endswith('recent') else -5)
...
def test_review_rejects_substituted_or_incomplete_audit(audited_fixture, monkeypatch, fault):
    path = audited_fixture / 'seed1_control_combined_audit.json'
