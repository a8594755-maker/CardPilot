...
def parent_gate(record, review, model_sha):
    primary = review.get('primary_contrasts') or {}
    return (record.get('status') == 'COMPLETED' and review.get('status') == 'PASS'
        and review.get('decision') == 'ADMIT_NEXT_RESPONSE_PHASE'
        and review.get('candidate_sha256') == model_sha
        and review.get('evaluation_hands') == 172032 and review.get('new_training_hands') == 0
        and review.get('slumbot_hands') == 0 and review.get('qualification_hands') == 0
        and review.get('goal_achieved') is False and review.get('external_admission') is False
        and set(primary) == {'hedge_against_response','retain_anchor1_2_vs_response'}
        and all(len(r.get('family2_ci95',[])) == 2 and all(math.isfinite(v) for v in r['family2_ci95'])
                and r['family2_ci95'][0] > 0 for r in primary.values()))
