...
PARENT = ROOT/'research/experiments/v6-fictitious-average-phase1-assessment-20260831'
ASSESS = PARENT
TARGET, PAIRS, SEED, EVAL_SEED = 2097152, 4096, 20261018, 20261019
LABELS = ('initial', 'quarter', 'half', 'final')
OPPONENTS = ('average', 'prior', 'response1', 'anchor0', 'anchor1', 'anchor2', 'anchor3', 'anchor4')
EVAL_HANDS = len(LABELS)*len(OPPONENTS)*PAIRS*2
MODEL_SHA = 'a73dd73af04e990bdff9892ba2b3eeebf96ded26ba6d4f099597feb262329d38'
PARENT_SHA = '644231eb6cefb426ab7ed366dce92d743e4bb18ec9e2816f35ffd847d89930bd'
...
def parent_check():
    if PARENT_SHA is None:
        raise ValueError('Strategic assessment not yet admitted; no training')
    ...
    assert stats.parent_gate(read(PARENT/'experiment.json'), prior, MODEL_SHA)
    assert sha(PARENT/'frozen/student.pt') == MODEL_SHA
...
def training_command():
    ...
    for flag, value in {
        ...
        '--seed':SEED, '--worker-seed-base':2026101800,
        '--run-id':'v6_fictitious_response_phase2_20260831', '--run-dir':directory,
...
def initialization_check(path=None):
    ...
    assert digest == MODEL_SHA and checkpoint['training_algorithm'] == 'fictitious_average_phase1_distillation_v1'
    assert checkpoint['resume_contract'].startswith('Not a train_v5 PPO continuation')
    assert len(checkpoint['optimizer']['state']) == 80 and checkpoint['epoch'] == 4
    assert checkpoint['optimizer_steps'] == 1024
    assert all(float(s['step']) == 1024 for s in checkpoint['optimizer']['state'].values())
    ...
    counter = initial_environment_hand_accounting(checkpoint, reset_hand_counter=True, run_id='v6_fictitious_response_phase2_20260831')
    ...
    return dict(status='PASS', operation='NEW_PPO_WEIGHTS_INITIALIZATION_NOT_SL_RESUME', source_sha256=digest,
        identical_initial_tensors=86, source_optimizer_states_preserved=80, source_optimizer_steps_preserved=1024, new_optimizer_states=0,
...
def training_health():
    ...
    for key, expected in dict(self_play_fraction=0, source_policy_kl_coef=0, rollout_mode='multi',
        rollout_envs_per_worker=8, workers=12, seed=SEED, worker_seed_base=2026101800, validate_stream=True,
...
def freeze_curve(checkpoint):
    ...
    for path in sorted((BASE/'production/checkpoints').glob('checkpoint_iter*_hands*.pt')):
        ...
        assert account['prefix_complete'] and account['origin_run_id'] == 'v6_fictitious_response_phase2_20260831'
...
def analyze(candidates, inputs):
    ...
    for label in LABELS:
        for opponent in OPPONENTS:
            ...
            assert summary['command'] == [sys.executable, *eval_command(label, opponent)]
            ...
            absolute[f'{label}_{opponent}'] = {k:v for k,v in stats.estimate(cell).items() if k != 'family25_ci95'}
    ...
    budget_gain = stats.estimate([x-y for x,y in zip(values['final','average'],values['half','average'])])
    return dict(absolute=absolute, paired_response_minus_initial=effects, paired_final_minus_half_average=budget_gain,
...
def main():
    ...
    try:
        ...
        (BASE/'frozen').mkdir()
        inputs = []
        for label, source, expected in [('average', PARENT/'frozen/student.pt', MODEL_SHA),
                ('prior', PARENT/'frozen/prior.pt', PRIOR_SHA),
                ('response1', PARENT/'frozen/response.pt', RESPONSE1_SHA)]+[
        ...
        log('--count', f'new_training_hands={actual}', '--note', 'Training/session audit complete. All4actual-counter-selected checkpoints frozen before32fixed internal cells. No external allocation.')
