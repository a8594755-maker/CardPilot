"""Independent final review of the matched causal-TCN reward evidence."""
import json
import math
import subprocess
import sys
...
def main():
    ...
    points = [item["mean"] for item in per_anchor]; passed = run.gate(tc, ts, points)
    ...
    next_step = "Run a separately preregistered independent causal-TCN training-seed confirmation before external evaluation." if passed else "Close sequence-residual variants and test centralized-critic/EMA actor-critic training."
    subprocess.run([sys.executable, str(run.ROOT / "research/experiment_log.py"), "finish", run.BASE.name, "--status", "COMPLETED", "--summary", f"Matched causal-TCN reward pilot completed: {decision}.", "--conclusion", report["interpretation"], "--decision", decision, "--next-step", next_step, "--count", f"new_training_hands={execution['new_training_hands']}", "--count", f"evaluation_hands={execution['evaluation_hands']}", "--count", "slumbot_hands=0"], cwd=run.ROOT, check=True)
