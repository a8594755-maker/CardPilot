"""Matched production pilot for causal-TCN residual PPO versus head-only PPO."""
...
def training_command(arm, frozen):
    ...
    command = [
        ...
        "--inference-min-batch-slots", "0", "--inference-batch-deadline-us", "700", "--worker-seed-base", "2026103700", "--fixed-training-deal-stream",
        ...
        "--run-id", f"v6_causal_tcn_matched_{arm}_20260831", "--run-dir", str(run), "--out", str(run / "latest.pt"),
        "--seed", "20261037", "--max-runtime-seconds", "1800", "--resume", str(frozen / "source_legacy.pt"), "--allow-resume",
        "--reset-hand-counter", "--reset-optimizer", "--validate-stream",
    ]
    if arm == "control": command.append("--all-policy-heads-only-training")
    else: command.extend(("--causal-sequence-policy-adapter-hidden", "128", "--causal-sequence-adapter-only-training"))
    return command
...
def health(arm, source_state):
    ...
    assert account["origin_run_id"] == f"v6_causal_tcn_matched_{arm}_20260831"
    ...
    if arm == "control":
        ...
        assert len(checkpoint["model"]) == 104 and len(checkpoint["optimizer"]["state"]) == 24
        assert all(torch.equal(checkpoint["model"][key], value) for key, value in source_state.items() if not key.startswith("value_head."))
        new = [key for key in checkpoint["model"] if key.startswith(PREFIXES)]
        assert len(new) == 18 and all(torch.isfinite(checkpoint["model"][key]).all() for key in new)
...
def main():
    ...
    if sys.argv[1:] or any((BASE / name).exists() for name in forbidden): raise ValueError("No restart")
    assert torch.cuda.is_available() and sha(SOURCE) == SOURCE_SHA; process_guard()
    started = time.monotonic(); current = psutil.Process()
    execution = {"status": "RUNNING", "pid": current.pid, "create_time": current.create_time(), "started_at": datetime.now(timezone.utc).isoformat(), "children": [], "arm_training_hands": {}, "new_training_hands": 0, "evaluation_hands": 0}
    write(BASE / "execution.json", execution); success = False
    try:
        ...
        for row in read(ANCHORS / "anchor_manifest.json")[:5]:
            source = Path(row["path"]); assert sha(source) == row["sha256"]; target = frozen / f"anchor{row['index']}.pt"; shutil.copy2(source, target)
        ...
        for arm in ARMS:
            ...
            with (BASE / f"{arm}_stdout.log").open("x") as output:
                ...
                while child.poll() is None:
                    ...
                    if hands != execution["arm_training_hands"].get(arm, 0):
                        execution["arm_training_hands"][arm] = hands; execution["new_training_hands"] = sum(execution["arm_training_hands"].values())
                        log("--count", f"new_training_hands={execution['new_training_hands']}", "--metric", f"active_arm={arm}"); write(BASE / "execution.json", execution)
                    time.sleep(5)
                info["exit_code"] = child.wait()
            if info["exit_code"]: raise RuntimeError(f"{arm} trainer failed; preserve without retry")
            checkpoint, hands = health(arm, source_state); execution["arm_training_hands"][arm] = hands; execution["new_training_hands"] = sum(execution["arm_training_hands"].values())
            shutil.copy2(directory / "latest.pt", frozen / f"{arm}.pt"); log("--count", f"new_training_hands={execution['new_training_hands']}", "--artifact", frozen / f"{arm}.pt", "--note", f"{arm} final frozen after terminal health/session audit PASS")
        candidates = {"source": frozen / "anchor0.pt", "control": frozen / "control.pt", "treatment": frozen / "treatment.pt"}
        write(BASE / "candidate_manifest.json", {name: {"path": str(path), "sha256": sha(path)} for name, path in candidates.items()}); log("--artifact", BASE / "candidate_manifest.json")
        snapshot = BASE / "execution_code/source_files/scripts/alpha_holdem/v6_mirror_eval.py"; jobs = []
        for name, candidate in candidates.items():
            for index in range(5):
                output = BASE / "matrix" / f"{name}_anchor{index}"; command = [str(snapshot), "--candidate", str(candidate), "--anchor", str(frozen / f"anchor{index}.pt"), "--pairs", str(PAIRS), "--seed", "20261038", "--device", "cpu", "--out-dir", str(output)]; record(command); jobs.append((name, index, output, command))
        def evaluate(job):
        ...
        for name, index, output, _ in jobs:
            rows = [json.loads(line) for line in (output / "pairs.jsonl").read_text().splitlines()]; assert len(rows) == PAIRS and [row["pair_index"] for row in rows] == list(range(PAIRS))
        ...
        points = [item["mean"] for item in per_anchor]; passed = gate(tc, ts, points)
        decision = "ADMIT_CAUSAL_TCN_TRAINING_SEED_CONFIRMATION" if passed else "CAUSAL_TCN_MATCHED_REWARD_GATE_NOT_PASSED"
