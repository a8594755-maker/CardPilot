    ...
    for stage in (1, 2):
        ...
        for seed in ('1', '3'):
            report['stages'][str(stage)][seed] = {
                'half_minus_full': bucket(-10. if stage == 1 else 5.),
                'endpoint_minus_parent': {arm: bucket(stage * (3 if arm == 'full' else 4)) for arm in ('full', 'half')},
                'absolute_vs_anchors': {arm: bucket(-100. + stage) for arm in ('full', 'half')},
            }
            for arm in ('full', 'half'):
                report['training_health_and_realized_update_dose'][f'seed{seed}_{arm}_stage{stage}'] = {
                    ...
                    'actual_lr': 9.999999999999996e-05 * (.5 if arm == 'half' else 1.),
                }
    return report
...
def test_both_seeds_arms_seats_and_anchors_preserved_without_input_mutation():
    ...
    for seed in result.values():
        assert seed['half_minus_full_change']['pooled']['bb100'] == 15.
        assert seed['half_minus_full_change']['pooled']['se_bb100'] == pytest.approx(8 ** .5)
        assert set(seed['half_minus_full_change']['by_seat']) == {'0', '1'}
        assert len(seed['half_minus_full_change']['by_anchor']) == 4
        assert seed['endpoint_minus_original_parent_changes']['full']['pooled']['bb100'] == 3.
        assert seed['endpoint_minus_original_parent_changes']['half']['pooled']['bb100'] == 4.
        assert seed['absolute_vs_anchors_changes']['half']['pooled']['bb100'] == 1.
        assert seed['physical_hands_between_endpoints'] == {'full': 1000, 'half': 1000}
    assert report == saved
...
def test_old_heads_arm_cannot_be_mislabeled_as_half_rate():
    report = fixture()
    arms = report['stages']['2']['3']['endpoint_minus_parent']
    arms['heads'] = arms.pop('half')
    with pytest.raises(ValueError, match='LR arms'):
        curve.summarize(report)
...
def test_wrong_realized_training_contract_rejected(key, value):
    report = fixture()
    report['training_health_and_realized_update_dose']['seed1_half_stage2'][key] = value
    with pytest.raises(ValueError):
        curve.summarize(report)
