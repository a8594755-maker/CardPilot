...
def compare_arms(full, half, earlier_decks):
    ...
    endpoints = {arm: buckets([(key[0], [r['treatment_rewards_bb'][s] - r['control_rewards_bb'][s]
                    for s in (0, 1)]) for key, r in rows.items()]) for arm, rows in (('full', full), ('half', half))}
    absolute = {arm: buckets([(key[0], row['treatment_rewards_bb']) for key, row in rows.items()])
                for arm, rows in (('full', full), ('half', half))}
    return {'half_minus_full': contrast, 'endpoint_minus_parent': endpoints, 'absolute_vs_anchors': absolute,
...
def review_stage(base, stage, hashes, previous_decks):
    ...
    for seed in (1, 3):
        ...
        for arm in ('full', 'half'):
            ...
            require(summary['command'][summary['command'].index('--seed') + 1] == str(20263900 + seed * 10 + stage), 'wrong eval seed')
        ...
        result = compare_arms(arms['full'], arms['half'], previous_decks)
        expected = reported['seeds'][str(seed)]
        check_reported(result['half_minus_full'], expected['half_minus_full'])
...
def main(base=BASE):
    ...
    for row in result['training']:
        ...
        lr = 9.999999999999996e-05 * (.5 if arm == 'half' else 1.0)
        ...
        item.update(actual_lr=lr, per_parameter_optimizer_step_deltas=steps,
