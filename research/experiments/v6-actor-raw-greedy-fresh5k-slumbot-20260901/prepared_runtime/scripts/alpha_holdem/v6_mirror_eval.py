...
def play_pair(candidate, anchor, deck, seed, pair_index, device='cpu'):
    outcomes, decisions = [], []
    for candidate_seat in (0, 1):
        ...
        while not state.terminal:
            ...
            incr, _ = decide(candidate if p == candidate_seat else anchor, state, uniform=uniform, device=device)
            seat_counts[p] += 1
...
def main():
    ...
    with (out/'pairs.jsonl').open('x') as handle:
        for index in range(args.pairs):
            deck = list(range(52))
            rng.shuffle(deck)
            result = play_pair(candidate, anchor, deck, args.seed, index, args.device)
