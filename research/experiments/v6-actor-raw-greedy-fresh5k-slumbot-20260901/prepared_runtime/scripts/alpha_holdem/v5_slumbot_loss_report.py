...
def build_summary(label: str, rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    by_position_terminal_street = aggregate(
        hands,
        lambda hand: (
            f"{'SB' if int(hand.get('client_pos') or 0) == 1 else 'BB'}"
            f"@{hand['terminal_type']}@{hand['terminal_street_name']}"
        ),
    )
    by_position_street = aggregate(
        hands,
        lambda hand: (
            f"{'SB' if int(hand.get('client_pos') or 0) == 1 else 'BB'}"
            f"@{hand['terminal_street_name']}"
        ),
    )
    by_position_first_preflop = aggregate(
        hands,
        lambda hand: (
            f"{'SB' if int(hand.get('client_pos') or 0) == 1 else 'BB'}"
            f"@{hand['first_preflop_decision']}"
        ),
    )
    ...
    return {
        ...
        "position_terminal_street": sorted_loss(
            by_position_terminal_street, 40
        ),
        "position_street": sorted_loss(by_position_street, 16),
        "position_first_preflop_decision": sorted_loss(
            by_position_first_preflop, 50
        ),
...
def write_markdown(summary: dict[str, Any], path: Path) -> None:
    ...
    lines.extend(
        table(
            summary["position_terminal_street"],
            "## Position × Terminal Street",
        )
    )
    lines.extend(table(summary["position_street"], "## Position × Street Net"))
    lines.extend(
        table(
            summary["position_first_preflop_decision"],
