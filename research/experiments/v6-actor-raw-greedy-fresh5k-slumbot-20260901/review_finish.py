"""Independent raw/evidence review for same-checkpoint greedy fresh5k."""
import json
...
def main():
    ...
    for index in range(1, 9):
        ...
        assert all(row["policy_mode"] == "greedy" and row["policy_temperature"] == 0 and row["policy_seed"] == 2026104500 + index for row in rows)
        for row in rows:
    ...
    (BASE / "result_summary.md").write_text(
        "# Same-checkpoint generic-greedy fresh5k result\n\n"
        ...
        f"{statistics['positive_sessions']}/8, point delta versus the separate sampled pilot "
        f"{statistics['point_delta_vs_sampled']:+.6f} bb/100. All actions were audited legal "
        "learned-logit argmax decisions. Pilot hands are not qualification hands; Goal not achieved.\n",
        encoding="utf-8")
