...
def main():
    ...
    t.require(owner['command'][-1].replace('\\\\','/').endswith(t.BASE.name+'/run_eval.py') and not live(owner['pid'],owner['create_time']),'owner not terminal')
    ...
    for seed in (1,3):
        ...
        for arm in ('control','expanded'):
            ...
            for ai,name in enumerate(q['anchors']):
                rr=[r for r in rows if r['anchor']==name]; rng=random.Random(20266401+10*seed+1000003*ai)
    ...
    report={'passed':True,'command':sys.orig_argv,'input_sha256':hashes,'evaluation_hands':81920,'unique_decks':10240,
        ...
        'scope':'Fixed-anchor stratified paired normal95CI; conditional on policies, not training-seed population. Standard10 excluded from adaptation group. All four sibling anchors are training opponents of expanded arms: NOT unseen transfer. Shared ancestry limits strategic breadth.'}
