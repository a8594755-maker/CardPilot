...
def validate_decision(row, response, rng, model, policy_mode, decision_fn=external_decision,
    ...
    if model is not None:
        if stateful_decision_fn is not None:
            _, info = stateful_decision_fn(
                model, response, session_policy_state,
                uniform=uniform, device='cpu', policy_mode=policy_mode,
            )
        else:
...
def audit_session(directory, *, model=None, expected_model_sha256=None,
                  ...
                  expected_frozen_artifacts=None, session_state_factory=None,
                  stateful_decision_fn=None):
    ...
    recorded_frozen_artifacts = start.get('frozen_artifact_sha256', {})
    need(isinstance(recorded_frozen_artifacts, dict) and all(
        isinstance(path, str) and isinstance(value, str) and re.fullmatch('[0-9a-f]{64}', value)
        for path, value in recorded_frozen_artifacts.items()), 'Invalid additional frozen artifact identity')
    if expected_frozen_artifacts is not None:
        expected_frozen_artifacts = {
            str(Path(path).resolve()): value for path, value in expected_frozen_artifacts.items()
        }
        need(recorded_frozen_artifacts == expected_frozen_artifacts,
             'Wrong additional frozen artifact identity')
    ...
    session_policy_state = session_state_factory() if session_state_factory is not None else None
    terminal_policy_state = None
    for event in events[1:]:
        ...
        if kind == 'hand_start':
            ...
            if session_policy_state is not None:
                need(event.get('session_policy_state') == session_policy_state.start_hand(event['hand']),
                     'Session policy start state mismatch')
            ...
            try:
                ...
                if terminal is not None:
                    ...
                    if session_policy_state is not None:
                        terminal_policy_state = session_policy_state.finish_hand(attempts, last_response)
            ...
            if session_policy_state is not None:
                record['session_policy_state'] = terminal_policy_state
    ...
    if complete:
        ...
        need(all(sha256_file(Path(p)) == h for p, h in recorded_frozen_artifacts.items()),
             'Current additional artifact differs from frozen evidence')
    return dict(status='PASS' if complete else 'INCOMPLETE', directory=str(directory), session_id=start['session_id'],
                ...
                frozen_artifact_sha256=recorded_frozen_artifacts,
...
def audit_sessions(directories, *, model=None, expected_model_sha256=None,
    ...
    results = [audit_session(p, model=model, expected_model_sha256=expected_model_sha256,
               decision_fn=decision_fn, expected_observation_bridge=expected_observation_bridge,
               expected_frozen_artifacts=expected_frozen_artifacts,
               session_state_factory=session_state_factory,
               stateful_decision_fn=stateful_decision_fn) for p in directories]
    ...
    need(len({hash_object(r['frozen_artifact_sha256']) for r in results}) == 1,
         'Mixed additional frozen artifacts')
    seen = set()
...
def main():
    ...
    parser.add_argument('--observation-bridge', choices=['none', 'legacy-v4', 'dual-contract', 'posterior-context'], default='none')
    parser.add_argument('--base-model')
    args = parser.parse_args()
    model, expected = None, None
    decision_fn, expected_bridge = external_decision, None
    expected_frozen_artifacts = None
    session_state_factory, stateful_decision_fn = None, None
    if args.model:
        import torch
        ...
        if args.observation_bridge == 'legacy-v4':
            ...
            expected, decision_fn, expected_bridge = model.sha256, dual_external_decision, DUAL_CONTRACT
            expected_frozen_artifacts = {str(model.base_path): model.base_sha256}
        elif args.observation_bridge == 'posterior-context':
            if not args.base_model:
                parser.error('--observation-bridge posterior-context requires --base-model')
            from alpha_holdem.contextual_execution_v6 import (
                CONTEXT_CONTRACT, SessionContextTracker,
                external_decision as contextual_external_decision,
                load_policy as load_contextual_policy,
            )
            model = load_contextual_policy(args.model, args.base_model, 'cpu')
            expected, expected_bridge = model.sha256, CONTEXT_CONTRACT
            expected_frozen_artifacts = {str(model.base_path): model.base_sha256}
            session_state_factory = lambda: SessionContextTracker(model.cold_start_hands)
            def stateful_decision_fn(policy, response, tracker, *, uniform, device, policy_mode):
                return contextual_external_decision(
    ...
    result = (audit_sessions(args.session_dir, model=model, expected_model_sha256=expected,
              ...
              expected_frozen_artifacts=expected_frozen_artifacts,
              session_state_factory=session_state_factory,
              stateful_decision_fn=stateful_decision_fn) if len(args.session_dir) > 1 else
              audit_session(args.session_dir[0], model=model, expected_model_sha256=expected,
              decision_fn=decision_fn, expected_observation_bridge=expected_bridge,
              expected_frozen_artifacts=expected_frozen_artifacts,
              session_state_factory=session_state_factory,
              stateful_decision_fn=stateful_decision_fn))
