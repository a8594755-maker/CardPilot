...
def main():
    output = BASE / 'qualification_v2.json'
    failure = BASE / 'qualification_failure_v2.json'
    ...
    try:
        ...
        for before in sorted((BASE/'original/scripts').rglob('*.py')):
            ...
            if sha(before)!=sha(after):
                ...
                diffs.extend(difflib.unified_diff(before.read_text(encoding='utf-8').splitlines(True),after.read_text(encoding='utf-8').splitlines(True),fromfile='original/'+relative.as_posix(),tofile='candidate/'+relative.as_posix()))
