"""Recompute stage1 paired strength and preregistered collapse from raw hands."""
import gzip
...
def main():
    execution = read(BASE/'stage1_evaluation_execution.json')
    assert read(BASE/'stage1_evaluation_terminal.json')['completed_hands'] == 131072
    ...
    for seed in (1,3):
        ...
        for arm in ('control','independent'):
            ...
            endpoint = BASE/f'seed{seed}_{arm}_stage1/latest.pt'
            ...
            prescribed_decks(rows,anchors,contract['eval_seeds'][f'{seed}_1'])
    ...
    output = {'passed_evidence_checks':True,'stage':1,'evaluation_hands':131072,
        'unique_decks':len(unique),'raw_hashes':hashes,'by_seed':results,
        'replicated_broad_collapse_by_arm':collapse,
        'decision':'STOP_FOR_COLLAPSE_REVIEW' if any(collapse.values()) else 'CONTINUE_PREREGISTERED_1M_DOSE',
    ...
    with (BASE/'stage1_raw_review.json').open('x',encoding='utf-8') as handle:
