...
def main():
    ...
    conflicts = [p.pid for p in psutil.process_iter(['pid','name','cmdline'])
        ...
        and any(Path(a).name in {'run_stage1.py','run_stage2.py','train_job.py','train_candidate.py','run_trial.py','train_capture.py'}
                for a in p.info['cmdline'] or [])]
    assert not conflicts, conflicts
    assert shutil.disk_usage(BASE).free > 20*1024**3
    write_new(BASE/'stage2_owner.json',{'pid':os.getpid(),'create_time':psutil.Process().create_time(),
    ...
    for seed,arm in ((1,'control'),(1,'independent'),(3,'independent'),(3,'control')):
        previous = BASE/f'seed{seed}_{arm}_stage1'
        ...
        folder = BASE/f'seed{seed}_{arm}_stage2'
        folder.mkdir()
        prefixes = {}
        ...
        if '--opponent-greedy-mixture' in argv:
        ...
        for key,value in {'--resume':parent_path,'--run-dir':folder,'--out':folder/'latest.pt',
            '--total-environment-hands':original_physical+1048576,'--max-runtime-seconds':7200,
        ...
        if arm == 'independent' and '--independent-observable-critic' not in argv: argv.append('--independent-observable-critic')
        ...
        write_new(folder/'job_contract.json',{'parent':str(parent_path),'parent_sha256':item['sha256'],
            'sources':sources,'arm':arm,'seed':seed,'stage':2,'prefixes':prefixes,
        ...
        while True:
            try: code = child.wait(timeout=60); break
                ...
                count = stage1_hands + sum(r['new_physical_hands'] for r in results) + row['environment_hand_accounting']['completed_hands']-physical
        ...
        assert actual >= original_physical+1048576-physical
        ...
        logger('--count',f"training_hands={stage1_hands+sum(r['new_physical_hands'] for r in results)}",'--artifact',str(folder/'terminal_training.json'))
        del final
    write_new(BASE/'stage2_training_terminal.json',{'status':'TRAINED_REVIEW_AND_EVALUATION_REQUIRED','runs':results})
    print('Stage2 training complete; stopped for fixed evaluation.',flush=True)
