"""Run only the fixed stage2 evaluation after all training reviews pass."""
import gzip
import json
import os
...
def main():
    ...
    for seed in (1,3):
        for arm in ('control','independent'):
            checkpoint = BASE/f'seed{seed}_{arm}_stage2/latest.pt'
            out = BASE/f'eval_seed{seed}_{arm}_stage2'
            job = BASE/f'evaluation_job_seed{seed}_{arm}_stage2'
            ...
            argv += ['--pairs-per-anchor','1024','--seed',str(contract['eval_seeds'][f'{seed}_2']),
    ...
    write_new(BASE/'stage2_evaluation_execution.json',{'input_sha256':inputs,'jobs':jobs,'expected_hands':131072})
    completed = 0
    for spec in jobs:
        ...
        while True:
            try: code = child.wait(timeout=60); break
                ...
                count = 131072 + completed + 4*completed_raw_rows(out/'common_deck_pairs.jsonl.gz')
                subprocess.run([sys.executable,str(ROOT/'research/experiment_log.py'),'update',BASE.name,
        ...
        completed += 32768
        subprocess.run([sys.executable,str(ROOT/'research/experiment_log.py'),'update',BASE.name,
            '--count',f'evaluation_hands={131072+completed}','--artifact',str(out/'summary.json')],check=True,capture_output=True)
    for name,digest in inputs.items(): assert sha(Path(name)) == digest
    write_new(BASE/'stage2_evaluation_terminal.json',{'completed_hands':completed,'status':'RAW_REVIEW_REQUIRED'})
