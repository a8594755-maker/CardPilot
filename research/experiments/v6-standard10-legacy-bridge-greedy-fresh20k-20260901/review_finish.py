"""Independent terminal review of frozen bridge fresh20k."""
import json
...
def main():
    ...
    for index in range(1, 9):
        ...
        rows = [
            ...
            (BASE / "sessions" / f"s{index:02d}/hands.jsonl").read_text(encoding="utf-8").splitlines()
        ]
        assert len(rows) == 2500
        assert [row["successful_hand"] for row in rows] == list(range(1, 2501))
        assert all(
            row["model_sha256"] == MODEL_SHA and row["strict_policy_execution"]
            and row["session_id"] == session_id(index)
            and row["policy_seed"] == 2026112200 + index
            and row["policy_mode"] == "greedy" and row["policy_temperature"] == 0
            for row in rows
        )
        for row in rows:
    ...
    decision = (
        "ADMIT_BRIDGE_GREEDY_FRESH100K"
        if statistics["supports_separate_fresh100k"]
        else "REJECT_DIRECT_BRIDGE_QUALIFICATION"
    )
    reviewed = {
        "status": "PASS", "decision": decision,
        "model_sha256": MODEL_SHA, "observation_bridge_contract": BRIDGE,
        "statistics": statistics, "prior_audits_checked": prior_audits,
    ...
    (BASE / "result_summary.md").write_text(
        "# Standard10 legacy-observation bridge generic-greedy fresh20k\n\n"
        ...
        f"{statistics['positive_sessions']}/8. The point estimate and both intervals are negative, "
        "so no fresh100k qualification is admitted.\n",
        encoding="utf-8",
    )
