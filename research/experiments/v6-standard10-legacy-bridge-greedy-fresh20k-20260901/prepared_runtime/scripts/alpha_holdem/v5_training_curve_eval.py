...
def paired_comparison(
    *,
    ...
    source_outcomes = outcome_lookup[(source["candidate"], anchor_label)]
    target_outcomes = outcome_lookup[(target["candidate"], anchor_label)]
    overall_delta, overall_delta_ci95 = paired_delta(
        source_outcomes["overall_bb_per_hand"],
        target_outcomes["overall_bb_per_hand"],
    )
    bb_delta, bb_delta_ci95 = paired_delta(
        source_outcomes["bb_bb_per_hand"],
        target_outcomes["bb_bb_per_hand"],
    )
    sb_delta, sb_delta_ci95 = paired_delta(
        source_outcomes["sb_bb_per_hand"],
        target_outcomes["sb_bb_per_hand"],
    )
    return {
        "anchor": anchor_label,
        "from_candidate": source["candidate"],
        "to_candidate": target["candidate"],
        "from_total_hands": source["checkpoint_total_hands"],
        "to_total_hands": target["checkpoint_total_hands"],
        ...
        "structural_label": structural_label(overall_delta, bb_delta, sb_delta),
    }
...
def build_summary(
    *,
    ...
    baseline_candidate_labels: list[str] | None = None,
) -> dict[str, Any]:
            ...
            rows.append(
                {
                    ...
                    "position_street_policy_adapter_hidden": checkpoint.get(
                        "position_street_policy_adapter_hidden", 0
                    ),
                    "position_street_raw_policy_adapter_hidden": checkpoint.get(
                        "position_street_raw_policy_adapter_hidden", 0
                    ),
    ...
    baseline_labels = list(baseline_candidate_labels or [])
    baseline_comparisons: list[dict[str, Any]] = []
    for baseline_candidate_label in baseline_labels:
        for anchor_label, _ in anchors:
            anchor_rows = [row for row in rows if row["anchor"] == anchor_label]
            source_rows = [
                row
                for row in anchor_rows
                if row["candidate"] == baseline_candidate_label
            ]
            if len(source_rows) != 1:
                raise ValueError(
                    f"baseline candidate {baseline_candidate_label!r} has "
                    f"{len(source_rows)} rows for anchor {anchor_label!r}"
                )
            for target in anchor_rows:
                if target["candidate"] == baseline_candidate_label:
                    continue
                ...
                comparison["baseline_candidate"] = baseline_candidate_label
    ...
    return {
        ...
        "interpretation_contract": {
            ...
            "fixed_baseline_delta_ci": (
                "When baseline_candidate is set, every candidate is compared "
                "directly with that frozen baseline using the same per-deck outcomes."
            ),
        },
        "evaluator": {
        ...
        "baseline_candidates": baseline_labels,
        "anchors": [
...
def write_markdown(summary: dict[str, Any], path: Path) -> None:
    ...
    for baseline_label in summary.get("baseline_candidates", []):
        lines.extend(
            [
                "",
                f"## Fixed baseline: {baseline_label}",
                "",
                "| anchor | baseline | candidate | overall delta (CI95) | BB delta (CI95) | SB delta (CI95) | diagnosis |",
                "|---|---|---|---:|---:|---:|---|",
            ]
        )
        for row in summary["baseline_comparisons"]:
            if row["baseline_candidate"] == baseline_label:
                lines.append(
                    "| {anchor} | {source} | {target} | {overall:+.2f} (+/-{overall_ci:.2f}) | {bb:+.2f} (+/-{bb_ci:.2f}) | {sb:+.2f} (+/-{sb_ci:.2f}) | {label} |".format(
                        anchor=row["anchor"],
                        source=row["from_candidate"],
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="append",
        help=(
            "Candidate label used for direct paired comparisons against every "
            "other candidate under each fixed anchor; repeat for multiple baselines."
        ),
    )
    ...
    baseline_labels = list(args.baseline_candidate_label or [])
    if len(set(baseline_labels)) != len(baseline_labels):
        raise ValueError("baseline candidate labels must be unique")
    missing_baselines = sorted(set(baseline_labels).difference(labels))
    if missing_baselines:
        raise ValueError(
            "--baseline-candidate-label must name a candidate: "
            f"{missing_baselines}"
        )
    if args.pairs <= 0:
    ...
    summary = build_summary(
        ...
        baseline_candidate_labels=baseline_labels,
    )
