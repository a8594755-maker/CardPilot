...
def main():
 ...
 assert ck['architecture']=='position_sequence_residual_v1' and ck['source_weights_sha256']==run.SOURCE_SHA and ck['parent_weights_sha256']==run.PARENT_SHA and run.sha(run.PARENT/'latest.pt')==run.PARENT_SHA
 assert ck['epoch']==16 and ck['optimizer_steps']==4096 and ck['total_hands']==ck['new_training_hands']==0
 ...
 assert all(float(x['step'])==4096 for x in ck['optimizer']['state'].values())
 rows=[json.loads(x) for x in (BASE/'training_metrics.jsonl').read_text().splitlines()]
 assert len(rows)==2048 and [x['step'] for x in rows]==list(range(2049,4097)) and all(x['epoch'] in range(9,17) for x in rows) and all(x['rows']==1024 and math.isfinite(x['loss']) and math.isfinite(x['gradient_norm']) for x in rows)
 ...
 p=np.load(BASE/'development_probabilities.npy');sourcep=np.load(run.PARENT/'development_probabilities.npy')
 ...
 report=dict(status='PASS',decision=decision,treatment=treatment,paired_source_minus_treatment=paired,improvement=improvement,model_sha256=run.sha(BASE/'latest.pt'),source_weights_sha256=run.SOURCE_SHA,parent_weights_sha256=run.PARENT_SHA,adapter_parameter_tensors=len(ck['adapter']),optimizer_steps=4096,optimizer_rows_processed=2097152,model_state_queries=e['model_state_queries'],qualification=qual,new_training_hands=0,evaluation_hands=0,strength_hands=0,slumbot_hands=0,goal_achieved=False,wall_time_seconds=e['wall_time_seconds'],review_wall_time_seconds=time.monotonic()-start,interpretation='Sequence continuation was selected on a known development cohort and is not confirmatory strength evidence.')
 run.write('reviewed_analysis.json',report);(BASE/'result_summary.md').write_text(f'# Sequence continuation development\n\n{decision}\n\nTV {treatment["mean"]:.6f}; improvement {improvement:.6f}; paired {paired}.\n\n0newhands/strength/Slumbot. {report["interpretation"]}\n')
