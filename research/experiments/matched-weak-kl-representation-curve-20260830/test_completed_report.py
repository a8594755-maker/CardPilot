...
def config(arm):
    return dict(source_policy_kl_coef=.01, seed=20260911, worker_seed_base=2026091100,
...
def test_independent_statistics_and_adjusted_gate():
    ...
    for row in rows: row['bonferroni_lower'] = -.01
    rows[0]['ci95_lower'] = .1
...
def test_configuration_and_executed_coefficient_checked(kind):
    cfg, rows = config('heads'), metrics(.01)
    report.validate_training(cfg, rows, 'heads')
    ...
    with pytest.raises(ValueError): report.validate_training(cfg, rows, 'heads')
...
def test_actual_adam_steps_include_partial_batches():
    ...
    report.validate_steps(result, [6]*10, 10)
    report.validate_steps(result, [6]*86, 86)
    with pytest.raises(ValueError): report.validate_steps(result, [6]*10, 86)
    with pytest.raises(ValueError): report.validate_steps(result, [4]*10, 10)
...
def test_independent_cell_review_rejects_corruption(kind):
    ...
    assert len(report.review_cell(document)) == 4
...
def test_full_synthetic_report_writes_and_refuses_overwrite(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(report, 'select_midpoint', lambda run: run/'mid.pt')
    ...
    for arm, coef, delta in [('heads', .01, .1), ('full', .01, .2)]:
        run = tmp_path/arm
        run.mkdir()
        rows = metrics(coef)
        checkpoint = dict(model={k: v if arm=='heads' and k.startswith('trunk.') else v+delta for k, v in model.items()},
            iteration=2, total_hands=3000, optimizer=dict(param_groups=[{'lr': 0.}],
                state={i: dict(step=torch.tensor(6.), exp_avg=torch.ones(1), exp_avg_sq=torch.ones(1)) for i in range(10 if arm=='heads' else 86)}),
            environment_hand_accounting=dict(completed_hands=525000,prefix_complete=True))
        torch.save(checkpoint, run/'latest.pt')
        selection[f'{arm}_final'] = dict(path=str(run/'latest.pt'), sha256=report.sha(run/'latest.pt'), physical_hands=525000, iteration=2)
        mid_ckpt=copy.deepcopy(checkpoint)
        mid_ckpt['environment_hand_accounting']['completed_hands']=265000
        mid_ckpt['iteration']=1
        torch.save(mid_ckpt,run/'mid.pt')
        selection[f'{arm}_mid'] = dict(path=str(run/'mid.pt'),sha256=report.sha(run/'mid.pt'),physical_hands=265000,iteration=1)
        manifest = dict(config=config(arm), iteration=2, total_hands=3000,
            environment_hand_accounting=dict(completed_hands=525000, prefix_complete=True))
    ...
    for name, candidate in selection.items():
        documents[name] = doc(candidate['sha256'], point=.01 if name == 'full_final' else 0.)
    ...
    comparisons = {label: evaluate.compare(documents[c],documents[t]) for label,c,t in evaluate.CONTRASTS}
    analysis = dict(status='PASS', evaluation_hands=163840, cell_hashes=hashes, comparisons=comparisons,
        admits_independent_confirmation=True)
    record = dict(metrics={'pilot_evaluation_complete': 1,'heads_exit_code':0,'full_exit_code':0}, accounting={'evaluation_hands': 163840, 'new_training_hands': 1050000})
    ...
    assert result['decision'] == 'REPRESENTATION_SCOPE_ADMITS_CONFIRMATION'
    assert result['arms']['heads']['unchanged_representation_tensors'] == 76
    assert result['arms']['full']['unchanged_representation_tensors'] == 0
    assert result['arms']['full']['optimizer_state_tensors'] == 86
