...
CANDIDATES = ['source', 'heads_mid', 'heads_final', 'full_mid', 'full_final']
CONTRASTS = [('full_final_vs_heads_final', 'heads_final', 'full_final'),
             ('full_final_vs_source', 'source', 'full_final'),
             ('heads_final_vs_source', 'source', 'heads_final'),
             ('full_mid_vs_heads_mid', 'heads_mid', 'full_mid'),
             ('heads_mid_vs_source', 'source', 'heads_mid'),
             ('full_mid_vs_source', 'source', 'full_mid'),
             ('full_final_vs_full_mid', 'full_mid', 'full_final'),
             ('heads_final_vs_heads_mid', 'heads_mid', 'heads_final')]
...
def compare(control, treatment):
    ...
    rows = []
    z_adjusted = statistics.NormalDist().inv_cdf(1-.05/8)
...
def admission(primary, versus_source):
    valid = len(primary) == len(versus_source) == 4 and all(r['ood_valid'] for r in [*primary, *versus_source])
    return (valid and all(r['delta_bb100'] > 0 for r in primary)
            and sum(r['bonferroni_lower'] > 0 for r in primary) >= 2
            and any(r['bonferroni_lower'] > 0 and r['anchor'] in ('standard10', 'heldout_weak') for r in primary)
...
def main():
    ...
    for name in CANDIDATES:
        ...
        command = [str(snapshot / 'v5_mirror_eval.py'), '--candidate', candidate['path'],
                   '--candidate-label', f'representation_curve_{name}', '--policy-mode', MODE,
    ...
    admitted = admission(comparisons['full_final_vs_heads_final'], comparisons['full_final_vs_source'])
    assert hands == 163840
    ...
    result = dict(status='PASS', evaluation_hands=hands, seed=SEED, comparisons=comparisons,
                  ...
                  adjusted_secondary_gate=(admitted and any(r['bonferroni_lower'] > 0 for r in comparisons['full_final_vs_heads_final'])),
                  cell_hashes={name: sha(path) for name, path in paths.items()},
                  claim_scope='matched_representation_curve_internal_not_slumbot')
    ...
    log('--artifact', str(output), '--metric', 'pilot_evaluation_complete=1',
        ...
        '--note', 'Five preregistered frozen curve candidates evaluated on four opponents after both matched scope arms. Fourth anchor excluded from this training pool, not an unseen historical family. Complete raw evidence and independent CI recomputation passed. Inspect numerical/Adam health and result before finish; no claim of Slumbot success.')
