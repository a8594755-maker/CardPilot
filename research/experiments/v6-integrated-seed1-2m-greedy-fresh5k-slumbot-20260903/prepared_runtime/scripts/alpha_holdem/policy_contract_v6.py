...
def validate_resume(args, checkpoint):
    if args.env_version in PHYSICAL_V6_ENVIRONMENTS:
        if getattr(args, 'v6_rebind_legacy_weights', False):
            ...
            if getattr(args, 'ppo_replay_buffer_iterations', 0) and (
                checkpoint.get('ppo_replay_entries')
                or int(checkpoint.get('ppo_replay_cumulative_rows', 0)) > 0
            ):
                raise ValueError(
                    'Legacy migration cannot import old-contract replay entries'
                )
        else:
