        ...
        for start in range(0, n, mini_batch_size):
            ...
            if use_reference_policy_kl:
                reference_probs = F.softmax(
                    reference_logits / float(reference_policy_temperature), dim=-1
                )
                reference_current_probs = F.softmax(
                    logits / float(reference_policy_temperature), dim=-1
                )
                ...
                reference_policy_kl_rows = (
                    ...
                    * (
                        ...
                        - reference_current_probs.clamp_min(1e-8).log()
                    )
                ).sum(dim=-1)
