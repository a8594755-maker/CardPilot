...
class Smoke:
    ...
    def __init__(self):
        ...
        require(not any((BASE/f'seed{s}_smoke_v2').exists() for s in EXPECTED), 'existing attempt; no retry')
        write_new(BASE/'smoke_ownership_v2.json', {'pid':os.getpid(),
        ...
        write_new(BASE/'smoke_input_contract_v2.json', {'input_sha256':self.inputs,
    ...
    def tick(self, force=False):
        ...
        for seed in EXPECTED:
            folder=BASE/f'seed{seed}_smoke_v2'
    ...
    def run_seed(self, seed):
        ...
        expected_pool=normalized_pool(parent,int(argv[argv.index('--pool-history-limit')+1]))
...
def main():
    ...
    for seed in (1,3):
        smoke.run_seed(seed)
    write_new(BASE/'smoke_result_v2.json', {'passed':True, 'runs':smoke.results,
        'new_physical_hands':sum(r['new_physical_hands'] for r in smoke.results),
        'evaluation_hands':0, 'final_qualification_hands':0})
    logger('--artifact', str(BASE/'smoke_result_v2.json'), '--metric', 'smoke_complete=true')
