...
def main():
    ...
    # This outcome-independent helper was added after launch and is not imported
    # by the live evaluator. Its own source provenance is captured separately.
    provenance = BASE/'deployment_readiness_code'
    if provenance.exists():
        raise ValueError('Preserve existing readiness provenance; do not rerun automatically')
    ...
    arguments = [str(script), '--model', str(checkpoint), '--hands', '0', '--device', 'cpu',
        ...
        '--policy-seed', '2026091300', '--strict-policy-execution',
    ...
    expected = dict(model_sha256=digest, requested_hands=0, successful_hands=0, dry_run=True,
        ...
        temperature=1., starting_stack_bb=200., policy_seed=2026091300,
