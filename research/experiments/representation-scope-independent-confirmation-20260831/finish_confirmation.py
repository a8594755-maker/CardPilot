...
def gate(primary, source):
    return (len(primary) == len(source) == 4 and all(r['ood_valid'] for r in [*primary, *source])
...
def main():
    ...
    for label, _, digest in CANDIDATES:
        ...
        documents[label] = doc
    if hands != 196608: raise ValueError('Wrong actual evidence count')
    result = analyze(documents)
    for key, rows in result['comparisons'].items():
        assert_contrasts_match(rows, stored['comparisons'][key])
    passed = gate(result['comparisons']['full_vs_heads'], result['comparisons']['full_vs_source'])
    ...
    verify_inputs()
    verify_sources(copies)
    lines = ['# Representation-scope independent confirmation', '', f"Decision: `{result['decision']}`.", '',
        ...
        '|---|---:|---|---|']
    for r in result['comparisons']['full_vs_heads']:
    ...
    lines += ['', 'All raw pair/seat statistics, OOD count checks, frozen input/source hashes and paired contrasts passed post-exit review.', '',
        *['- '+text for text in result['limitations']], '',
        'Next: preregister one fixed20k strict native-sampled Slumbot pilot of exactly this full checkpoint.' if passed else
    ...
    subprocess.run([sys.executable, 'research/experiment_log.py', 'finish', ID, '--status', 'COMPLETED',
        '--count', 'new_training_hands=0', '--count', 'evaluation_hands=196608',
        '--summary', f"Independent frozen representation-scope confirmation completed196608internal hands with intact evidence; {result['decision']}.",
        '--conclusion', contrasts+'. No discovery pooling or Slumbot success claim.',
        '--decision', result['decision'], '--next-step',
        'Preregister fixed20k strict sampled Slumbot pilot of this exact full checkpoint.' if passed else
