...
ROOT = BASE.parents[2]
TRAIN = ROOT/'research/experiments/v6-fictitious-response-phase2-20260831'
CORPUS = ROOT/'research/experiments/v6-common-state-retention-diagnostic-20260831/attempt01/states.jsonl'
SOURCE_SHA = 'a73dd73af04e990bdff9892ba2b3eeebf96ded26ba6d4f099597feb262329d38'
PRIOR_SHA = 'cd7ce2b27cf3a86e96141432c92c6f92a3eb923dce519d1f1128af5acbb2364b'
RESPONSE1_SHA = '58ef62e875ce8ad1ecf518a1a65c8f54ba711b3bc1ea1a3ab3a00ed1e1a3540d'
FINAL_SHA = '393fb9e119ba73864ee76f6732c468d3e09c1329da3625f167f53de8321eea17'  # Independently reviewed response2 final only.
REVIEW_SHA = '03d64e4849d67b938c15d43874aa0061c9976a2f2d661cea707a402a8b980bae'
EPOCHS, TEACHERS = 8, 3
...
def verify(copies, inputs):
    ...
    for row in [*inputs['teachers'], inputs['initializer']]:
...
def validate_response_review(review, digest):
    ...
    assert review['new_training_hands'] >= 2097152 and review['evaluation_hands'] == 262144
...
def require_admission():
    ...
    assert sha(TRAIN/'frozen/prior.pt') == PRIOR_SHA
    assert sha(TRAIN/'frozen/response1.pt') == RESPONSE1_SHA
...
def teacher_specification():
    # The fitted phase1 average initializes the student only, not a fourth teacher.
    return [('prior', PRIOR_SHA, 'warmstart_prior'),
            ('response1', RESPONSE1_SHA, 'response1'),
            ('final', FINAL_SHA, 'response2')]
...
def freeze_teachers():
    ...
    (BASE/'teachers').mkdir()
    teachers = []
    for index, (name, expected, role) in enumerate(teacher_specification()):
        ...
        teachers.append(dict(index=index, role=role,
            source=str(path), path=str(target), sha256=sha(target), probability=1/TEACHERS))
    initializer_source, initializer_path = TRAIN/'frozen/average.pt', BASE/'initializer.pt'
    assert sha(initializer_source) == SOURCE_SHA
    shutil.copy2(initializer_source, initializer_path)
    initializer = dict(source=str(initializer_source), path=str(initializer_path), sha256=sha(initializer_path))
    assert initializer['sha256'] == SOURCE_SHA
    ...
    result = dict(teachers=teachers, initializer=initializer, dependencies=dependencies, collection_hands=TRAIN_HANDS,
        supervised_validation_hands=VALID_HANDS, slumbot_hands=0, source_training_hands_recounted=0,
        normal_form_mixture_weights=[1/TEACHERS]*TEACHERS, phase=2, response_review_sha256=REVIEW_SHA)
    write('input_manifest.json', result)
    log('--artifact', BASE/'input_manifest.json', '--artifact', initializer_path, *[v for row in teachers for v in ('--artifact', row['path'])])
    return result
...
def fit_student(inputs, reservoir):
    ...
    torch.manual_seed(2026102004)
    torch.cuda.manual_seed_all(2026102004)
    student, source, _ = load_policy(inputs['initializer']['path'], 'cuda')
    ...
    with (BASE/'training_metrics.jsonl').open('x') as handle:
        for epoch in range(1, EPOCHS+1):
            ...
            checkpoint = dict(**METADATA, model={k: v.detach().cpu().clone() for k, v in student.state_dict().items()},
                optimizer=optimizer.state_dict(), config={**source.get('config', {}), 'training_algorithm':'fictitious_average_phase2_distillation_v1',
                'lr':1e-4, 'seed':2026102004}, critic_contract=source.get('critic_contract', 'critic_v2'),
                norm_layer='gn', separate_preflop_head=True, epoch=epoch, iteration=epoch, total_hands=TRAIN_HANDS,
                optimizer_steps=steps, run_id=BASE.name, training_algorithm='fictitious_average_phase2_distillation_v1',
...
def main():
    ...
    try:
        ...
        reservoir = Reservoir(262144, 2026102003)
        ...
        write('execution.json', execution)
        training = collect(models, seed=2026102001, hands=TRAIN_HANDS, out=BASE/'training_hands.jsonl', reservoir=reservoir,
        ...
        write('execution.json', execution)
        validation = collect(models, seed=2026102002, hands=VALID_HANDS, out=BASE/'validation_hands.jsonl',
        ...
        write('execution.json', execution)
        train_audit = audit_trace(BASE/'training_hands.jsonl', seed=2026102001, hands=TRAIN_HANDS, teacher_count=TEACHERS,
                                  reservoir=reservoir, models=models, model_replay_hands=64)
        valid_audit = audit_trace(BASE/'validation_hands.jsonl', seed=2026102002, hands=VALID_HANDS, teacher_count=TEACHERS)
