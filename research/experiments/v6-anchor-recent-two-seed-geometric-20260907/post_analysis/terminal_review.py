...
def main():
    ...
    require(owner['pid'] == 48468 and abs(owner['create_time']-1788760722.5186431) < .001, 'owner identity')
    ...
    require(result['phase'] == 'FIXED_1M_COMPLETE_NEEDS_EXTERNAL_CALIBRATION', 'wrong boundary')
    ...
    for row in result['training']:
        ...
        keys = [k for k in old.INITIAL_KEYS if k != 'pool_strategy'] + [
            'adaptive_opponent_ema_rewards', 'adaptive_opponent_weights', 'adaptive_opponent_observations']
        require(all(old.equal_tree(parent[k], initial[k]) for k in keys), 'initial state changed')
        ...
        strategy = 'anchor-latest' if row['arm'] == 'recent' else 'loss-kbest'
        ...
        for seed in (1,3):
            ...
            for arm in ('control','recent'):
                ...
                for p in (raw, folder/'summary.json'):
                    hashes[str(p)] = sha(p)
            contrast = old.compare_arms(arms['control'], arms['recent'], seen)
            expected = reported['seeds'][str(seed)]
            old.check_reported(contrast['connected_minus_detached'], expected['recent_minus_control'])
            for group in ('endpoint_minus_parent','absolute_vs_anchors'):
                for a,b in (('detached','control'),('connected','recent')):
            ...
            stages[str(stage)][str(seed)] = {'recent_minus_control':contrast['connected_minus_detached'],
                **{g:{b:contrast[g][a] for a,b in (('detached','control'),('connected','recent'))}
