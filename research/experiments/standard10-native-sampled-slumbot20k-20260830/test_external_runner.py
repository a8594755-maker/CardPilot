...
spec = importlib.util.spec_from_file_location('sampled_external_runner', BASE/'run_external.py')
run = importlib.util.module_from_spec(spec)
spec.loader.exec_module(run)


def test_client_is_fixed_native_sample_and_explicit_seed():
    item = dict(requested_hands=2500, policy_seed=2026090501, raw_hands='raw', dump='dump', result='result')
    ...
    assert args[args.index('--policy-seed')+1] == '2026090501'
