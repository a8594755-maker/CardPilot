"""Immutable fixed20k native-sampled Slumbot pilot. No automatic recovery."""
import json
import math
...
def client_args(script, checkpoint, spec, hands=None, result=None):
    return [str(script), '--model', str(checkpoint), '--strategy', 'model', '--device', 'cpu',
        ...
        '--policy-mode', 'sample', '--temperature', '1', '--policy-seed', str(spec['policy_seed']),
...
def main():
    ...
    readiness = json.loads((ROOT/'research/experiments/stochastic-slumbot-evidence-readiness-20260830/readiness_analysis.json').read_text())
    if readiness['status'] != 'PASS' or readiness['actual_slumbot_hands'] != 0 or sha(SOURCE) != DIGEST:
        raise ValueError('Readiness/source gate failed')
    validated_manifest = ROOT/'research/experiments/stochastic-slumbot-evidence-readiness-20260830/final_code/copy_manifest.json'
    ...
    (BASE/'frozen').mkdir()
    checkpoint = BASE/'frozen/standard10.pt'
    shutil.copy2(SOURCE, checkpoint)
    verified_sources(copies, checkpoint)
    session_dir = BASE/'sessions'
    session_dir.mkdir()
    specs = [dict(id=f'part{i:02d}', policy_seed=2026090500+i, requested_hands=TARGET,
    ...
    if dry['successful_hands'] != 0 or dry['model_sha256'] != DIGEST or dry['policy_seed'] != specs[0]['policy_seed']:
    ...
    def progress(force=False):
        ...
        if force or (count != last_count and time.monotonic()-last_log >= 15):
            log('--count', f'evaluation_hands={count}', '--artifact', ledger_path,
        ...
        for spec in specs:
            ...
            while job['counter'].scan() < 1 or not Path(spec['dump']).exists() or Path(spec['dump']).stat().st_size == 0:
                ...
                if process.poll() is not None:
                    raise RuntimeError('Client exited before first-hand readiness')
