...
from alpha_holdem.slumbot_transport import post_json, close_transport, TRANSPORT_MODE
...
def new_hand(token: str | None) -> dict:
    ...
    if token:
        data['token'] = token
    return post_json(f'https://{HOST}/slumbot/api/new_hand', data)


def act(token: str, incr: str) -> dict:
    return post_json(f'https://{HOST}/slumbot/api/act', {'token': token, 'incr': incr})
...
def decide_action(
    ...
    if (
        ...
        or int(getattr(model, 'position_value_adapter_hidden', 0)) > 0
    ):
...
def dump_hand_records(
    ...
    policy_execution_issues: list[str] | None = None,
) -> None:
    ...
    for move_idx, (mv, pos, amt, prefix, st) in enumerate(_walk_action_string(action_str)):
        ...
        if policy_execution_issues is not None:
            record['hand_policy_execution_clean'] = not policy_execution_issues
            record['hand_policy_execution_issues'] = list(policy_execution_issues)
...
def play_hand(
    ...
    dump_fp=None,
    hand_idx: int = 0,
    strict_policy_execution: bool = False,
):
    r = new_hand(token)
    ...
    policy_execution_issues: list[str] = []
    ...
    while True:
        ...
        if 'error' in state:
            raise RuntimeError(f'Invalid Slumbot action trace: {state["error"]}')

        # Our turn?
        if state['pos'] != client_pos:
            if strict_policy_execution:
                raise RuntimeError('Out-of-turn API state; strict execution forbids fallback')
            policy_execution_issues.append('out_of_turn_fallback')
            ...
            try:
                ...
                raise RuntimeError(
                    f'Could not repair out-of-turn Slumbot state: {e}'
                ) from e
        ...
        try:
            ...
            if strict_policy_execution:
                raise
            policy_execution_issues.append('http_action_fallback')
            ...
            try:
                ...
                raise RuntimeError(
                    'Slumbot rejected both the network action and the legal '
                    'fallback; no completed hand may be counted'
                ) from fallback_error
...
def configure_policy_rng(seed: int | None) -> None:
    """Seed action RNGs only when explicitly requested; leave defaults untouched."""
    if seed is None:
        return
    if type(seed) is not int or not 0 <= seed < 2**63:
        raise ValueError('policy-seed must be in [0, 2**63)')
    random.seed(seed)
...
def main():
    ...
    parser.add_argument('--strict-policy-execution', action='store_true',
                        help='Abort on any hand error; never fallback, skip, retry, or supplement a hand.')
    parser.add_argument('--policy-seed', type=int, default=None,
                        help='Optional explicit RNG seed applied after model loading, before the first hand.')
    parser.add_argument('--device', default='cpu')
    parser.add_argument('--torch-threads', type=int, default=1,
                        help='CPU intra-op threads per evaluator process.')
    parser.add_argument('--torch-interop-threads', type=int, default=1,
                        help='CPU inter-op threads per evaluator process.')
    ...
    if args.torch_threads <= 0 or args.torch_interop_threads <= 0:
        raise SystemExit('Torch thread counts must be positive')
    ...
    def load_checkpoint_model(path):
        ...
        def build_checkpoint_actor(actor_checkpoint):
            ...
            actor_position_street_policy_adapter_hidden = int(
                actor_state[
                    'position_street_policy_adapters.0.0.weight'
                ].shape[0]
                if 'position_street_policy_adapters.0.0.weight' in actor_state
                else 0
            )
            actor_position_street_policy_adapter_cap = float(
                actor_checkpoint.get(
            ...
            actor_position_street_raw_policy_adapter_hidden = int(
                actor_state[
                    'position_street_raw_policy_adapters.0.0.weight'
                ].shape[0]
                if (
                    'position_street_raw_policy_adapters.0.0.weight'
                    in actor_state
            ...
            actor_position_value_adapter_hidden = int(
                actor_checkpoint.get('position_value_adapter_hidden')
                or (actor_checkpoint.get('config') or {}).get(
                ...
                or (
                    ...
                    if 'position_value_adapters.0.0.weight' in actor_state
                    else 0
                )
            )
            actor_action_q_hidden = int(
                actor_state['action_q_head.0.weight'].shape[0]
                if 'action_q_head.0.weight' in actor_state
                else 0
            )
            actor_position_adapter_postflop_only = bool(
                actor_checkpoint.get('position_adapter_postflop_only')
                or (actor_checkpoint.get('config') or {}).get(
                    'position_adapter_postflop_only'
                )
            )
            actor_position_adapter_min_street = int(
                actor_checkpoint.get('position_adapter_min_street')
                or (actor_checkpoint.get('config') or {}).get(
                    'position_adapter_min_street'
                )
                or 1
            )
            actor_position_adapter_max_street = int(
                actor_checkpoint.get('position_adapter_max_street')
                or (actor_checkpoint.get('config') or {}).get(
                    'position_adapter_max_street'
                )
                or 3
            )
            actor_position_adapter_forbid_new_folds = bool(
                actor_checkpoint.get(
            ...
            actor_position_adapter_forbid_new_allins = bool(
                actor_checkpoint.get(
            ...
            actor_position_adapter_forbid_new_allins_min_street = int(
                actor_checkpoint.get(
            ...
            actor_position_adapter_forbid_new_allins_max_street = int(
                actor_checkpoint.get(
            ...
            actor_uses_hybrid_network = bool(
                actor_action_q_hidden > 0
                or actor_position_street_policy_adapter_hidden > 0
                or actor_position_street_raw_policy_adapter_hidden > 0
            )
            actor_class = AlphaHoldemNet
            if actor_uses_hybrid_network:
                from alpha_holdem.network_hybrid_h1 import (
                    AlphaHoldemNet as ActionQAlphaHoldemNet,
                )

                actor_class = ActionQAlphaHoldemNet
            actor = actor_class(
                ...
                position_value_adapter_hidden=(
                    actor_position_value_adapter_hidden
                ),
                position_adapter_postflop_only=(
                    actor_position_adapter_postflop_only
                ),
                position_adapter_min_street=(
                    actor_position_adapter_min_street
                ),
                position_adapter_max_street=(
                    actor_position_adapter_max_street
                ),
                position_adapter_forbid_new_folds=(
                    actor_position_adapter_forbid_new_folds
                ),
                position_adapter_forbid_new_allins=(
                    actor_position_adapter_forbid_new_allins
                ),
                position_adapter_forbid_new_allins_min_street=(
                    actor_position_adapter_forbid_new_allins_min_street
                ),
                position_adapter_forbid_new_allins_max_street=(
                    actor_position_adapter_forbid_new_allins_max_street
                ),
                critic_contract=str(
                ...
                **(
                    {
                        'action_q_hidden': actor_action_q_hidden,
                        'action_q_dueling': bool(
                            actor_checkpoint.get('action_q_dueling')
                            or (actor_checkpoint.get('config') or {}).get(
                                'action_q_dueling'
                            )
                        ),
                        'position_street_policy_adapter_hidden': (
                            actor_position_street_policy_adapter_hidden
                        ),
                        'position_street_policy_adapter_cap': (
                            actor_position_street_policy_adapter_cap
                        ),
                        'position_street_raw_policy_adapter_hidden': (
                            actor_position_street_raw_policy_adapter_hidden
                        ),
                    }
                    if actor_uses_hybrid_network
                    else {}
        ...
        if checkpoint.get('architecture') in {'dual_seat_v1', 'dual_seat_v2'}:
            def load_seat_actor(prefix):
                ...
                seat_name = (
                    'sb' if prefix.startswith('sb_model.') else 'bb'
                )
                seat_config = checkpoint.get(
                    f'{seat_name}_actor_config'
                )
                seat_checkpoint = {
                    **checkpoint,
                    **(
            ...
            if checkpoint.get('architecture') == 'dual_seat_v1':
                ...
                from alpha_holdem.network_dual_seat_v2 import (
                    DualSeatAlphaHoldemNetV2,
                )

                net = DualSeatAlphaHoldemNetV2(
                    sb_model=load_seat_actor('sb_model.'),
                    bb_model=load_seat_actor('bb_model.'),
                    bb_street_min=checkpoint.get('bb_street_min'),
                    bb_street_max=checkpoint.get('bb_street_max'),
                ).to(device)
            net.eval()
        ...
        position_street_policy_adapter_hidden = int(
            model_state[
                'position_street_policy_adapters.0.0.weight'
            ].shape[0]
            if 'position_street_policy_adapters.0.0.weight' in model_state
            else 0
        )
        position_street_policy_adapter_cap = float(
            checkpoint.get('position_street_policy_adapter_cap')
            or (checkpoint.get('config') or {}).get(
                'position_street_policy_adapter_cap'
            )
            or 0.0
        )
        position_street_raw_policy_adapter_hidden = int(
            model_state[
                'position_street_raw_policy_adapters.0.0.weight'
            ].shape[0]
            if (
        ...
        position_value_adapter_hidden = int(
            checkpoint.get('position_value_adapter_hidden')
            or (checkpoint.get('config') or {}).get(
                'position_value_adapter_hidden'
            )
            or (
                ...
                if 'position_value_adapters.0.0.weight' in model_state
                else 0
            )
        )
        position_adapter_postflop_only = bool(
            checkpoint.get('position_adapter_postflop_only')
            or (checkpoint.get('config') or {}).get(
                'position_adapter_postflop_only'
            )
        )
        position_adapter_min_street = int(
            checkpoint.get('position_adapter_min_street')
            or (checkpoint.get('config') or {}).get(
                'position_adapter_min_street'
            )
            or 1
        ...
        position_adapter_max_street = int(
            checkpoint.get('position_adapter_max_street')
            or (checkpoint.get('config') or {}).get(
                'position_adapter_max_street'
            )
            or 3
        )
        position_adapter_forbid_new_folds = bool(
            checkpoint.get('position_adapter_forbid_new_folds')
            or (checkpoint.get('config') or {}).get(
                'position_adapter_forbid_new_folds'
            )
        )
        position_adapter_forbid_new_allins = bool(
            checkpoint.get('position_adapter_forbid_new_allins')
            or (checkpoint.get('config') or {}).get(
                'position_adapter_forbid_new_allins'
            )
        )
        position_adapter_forbid_new_allins_min_street = int(
            checkpoint.get(
                'position_adapter_forbid_new_allins_min_street'
            )
            or (checkpoint.get('config') or {}).get(
        ...
        position_adapter_forbid_new_allins_max_street = int(
            checkpoint.get(
                'position_adapter_forbid_new_allins_max_street'
            )
            or (checkpoint.get('config') or {}).get(
        ...
        action_q_hidden = int(
            model_state['action_q_head.0.weight'].shape[0]
            if 'action_q_head.0.weight' in model_state
            else 0
        )
        uses_hybrid_network = bool(
            action_q_hidden > 0
            or position_street_policy_adapter_hidden > 0
            or position_street_raw_policy_adapter_hidden > 0
        )
        model_class = AlphaHoldemNet
        if uses_hybrid_network:
            from alpha_holdem.network_hybrid_h1 import (
                AlphaHoldemNet as ActionQAlphaHoldemNet,
            )

            model_class = ActionQAlphaHoldemNet
        model_kwargs = dict(
            num_actions=NUM_ACTIONS,
            norm_layer=norm,
        ...
        if uses_hybrid_network:
            model_kwargs.update(
                action_q_hidden=action_q_hidden,
                action_q_dueling=bool(
                    checkpoint.get('action_q_dueling')
                    or (checkpoint.get('config') or {}).get(
                        'action_q_dueling'
                    )
                ),
    ...
    configure_policy_rng(args.policy_seed)
    model_sha256 = None
    if Path(args.model).is_file():
        digest = hashlib.sha256()
        with Path(args.model).open('rb') as model_file:
    ...
    if args.hands <= 0:
        ...
        if args.result_json:
            ...
            result = {
                ...
                'strict_policy_execution': bool(args.strict_policy_execution),
                'http_transport': TRANSPORT_MODE,
                'temperature': float(args.temperature),
                'starting_stack_bb': STACK_SIZE / BIG_BLIND,
    ...
    for h in range(args.hands):
        try:
            token, winnings = play_hand(
                ...
                dump_fp=dump_fp,
                hand_idx=h,
                strict_policy_execution=args.strict_policy_execution,
            )
            ...
            if hand_results_fp is not None:
                ...
                hand_results_fp.write(json.dumps({
                    ...
                    'model_sha256': model_sha256,
                    'policy_mode': policy_mode,
                    'policy_temperature': float(args.temperature),
                }) + '\n')
                hand_results_fp.flush()
        except Exception as e:
            if args.strict_policy_execution:
                for stream in (dump_fp, hand_results_fp):
                    if stream is not None:
                        stream.close()
                close_transport()
                # No server payload/token in the error log; no result summary
                # can masquerade as a completed fixed-count run after failure.
                raise RuntimeError(
                    f'Strict hand {h+1} failed ({type(e).__name__}); '
                    'no fallback, retry, skip, or further hand requested'
                ) from None
    ...
    hands_per_minute = n/elapsed*60 if elapsed > 0 else 0.0
    print(f'  Time:         {elapsed/60:.1f} min ({hands_per_minute:.1f} hands/min)')
    print('=' * 60)
