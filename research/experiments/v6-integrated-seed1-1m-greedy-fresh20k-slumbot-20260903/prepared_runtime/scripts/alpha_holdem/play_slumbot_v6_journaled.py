...
def run_session(model, model_path, model_sha256, *, hands, seed, session_id, out_dir,
                ...
                hand_start_fn=None, hand_end_fn=None):
    ...
    try:
        ...
        for index in range(1, hands+1):
            attempted = index
            hand_start_metadata = hand_start_fn(index) if hand_start_fn is not None else None
            phase = 'hand_start'
            journal.append('hand_start', hand=index, session_policy_state=hand_start_metadata)
            ...
            while True:
                ...
                if terminal is not None:
                    hand_end_metadata = hand_end_fn(index, public) if hand_end_fn is not None else None
                    ...
                    if hand_end_fn is not None:
                        record['session_policy_state'] = hand_end_metadata
                    phase = 'hand_commit'
...
def main():
    ...
    hand_start_fn = hand_end_fn = None
    if args.observation_bridge == 'legacy-v4':
        ...
        model_hash = model.sha256
        tracker = SessionContextTracker(model.cold_start_hands)
        def decision_fn(policy, response, *, uniform, device, policy_mode):
        ...
        hand_start_fn, hand_end_fn = tracker.start_hand, tracker.finish_hand
        frozen_artifacts = {str(model.base_path): model.base_sha256}
        execution_metadata = dict(
            ...
            context_cold_start_hands=model.cold_start_hands,
            context_reliability_power=float(model.checkpoint['reliability_power']),
        )
    else:
        if args.base_model:
            parser.error('--base-model is only valid for --observation-bridge dual-contract')
    ...
    summary = run_session(model, args.model, model_hash, hands=args.hands, seed=args.seed,
        ...
        hand_start_fn=hand_start_fn, hand_end_fn=hand_end_fn)
