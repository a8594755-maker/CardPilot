...
class HUNLEnvironmentV6:
    ...
    def __init__(self, starting_stack=200.0, observation_style="v6"):
        ...
        if observation_style not in {"v6", "legacy_v4"}:
            raise ValueError("observation_style must be v6 or legacy_v4")
    ...
    def _get_obs(self):
        if self.observation_style == "legacy_v4":
            from alpha_holdem.legacy_observation_bridge_v6 import legacy_observation_from_state
            obs, legacy_table = legacy_observation_from_state(self.state.core)
            mask, table = action_table(self.state.core)
            legacy_actions = {action for action in legacy_table if action is not None}
            physical_actions = {action for action in table if action is not None}
            if not legacy_actions or not legacy_actions.issubset(physical_actions):
                raise ValueError("Restricted legacy action space is not v6-table exact")
            table = legacy_table
        else:
