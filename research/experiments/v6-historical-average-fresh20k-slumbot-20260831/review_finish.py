...
def main():
    ...
    for i,item in enumerate(audit['results'],1):
        ...
        assert item['session_id']==f'v6_historical_average_fresh20k_20260831_s{i:02d}' and item['policy_seed']==2026101200+i
    ...
    text='# Frozen historical-average epoch12 fresh20k Slumbot pilot\n\n'
    ...
    text+='All clients exited0. Complete journal/terminal/server-counter/frozen-model replay audit and independent raw arithmetic passed. Token chains are disjoint across all8sessions and all excluded corrected-v6 cohorts (source16probe,source20k,physical-final20k,and both selfplay-share20k arms). No proof of server RNG independence is claimed. No probe,internal,or old-contract hands are pooled.\n\n'
    ...
    subprocess.run([sys.executable,str(ROOT/'research/experiment_log.py'),'finish',BASE.name,'--status','COMPLETED',
        '--summary',f'Exactly20k fresh frozen historical-average epoch12 hands: {result["bb_per_100"]:+.4f}bb/100; raw95%CI{result["raw_hand_ci95"]}; complete evidence PASS.',
