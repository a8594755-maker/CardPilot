...
CONFIRMATION = ROOT/'research/experiments/weak-source-kl-independent-confirmation-20260830'
SOURCE = ROOT/'research/experiments/weak-source-kl-pilot-20260830/frozen/weak.pt'
DIGEST = 'a68ac47aad7fa943f0e784e1c14be3d742fac7390a2cd853ba6cb971b429019b'
...
def validate_confirmation(record, review, readiness):
    if (record['status'] != 'COMPLETED' or record['metrics'].get('post_exit_review_pass') != 1
            ...
            or review['decision'] != 'WEAK_KL_REPLICATION_PASSED'
            or review['seed'] != 20260909 or review['evaluation_hands'] != 147456
            or review['new_training_hands'] != 0 or review['slumbot_hands'] != 0
            or review['candidate_hashes']['weak'] != DIGEST
...
def main():
    ...
    (BASE/'frozen').mkdir()
    checkpoint = BASE/'frozen/weak.pt'
    shutil.copy2(SOURCE, checkpoint)
    verified_sources(copies, checkpoint)
    session_dir = BASE/'sessions'
    session_dir.mkdir()
    specs = [dict(id=f'part{i:02d}', policy_seed=2026091000+i, requested_hands=TARGET,
