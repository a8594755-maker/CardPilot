...
def main():
    ...
    if [s['policy_seed'] for s in manifest['sessions']] != list(range(2026091001, 2026091009)):
    ...
    lines = ['# Frozen weak-KL native-sampled Slumbot pilot', '', f'Decision: `{status}`.', '',
        ...
        *[f"| {row['id']} | {row['hands']} | {row['bb_per_100']:+.4f} |" for row in sessions], '',
        f'Policy: unchanged weak-KL final checkpoint, native sample/temp1, SHA256 `{DIGEST}`.',
