...
def training_metadata(args):
    ...
    if args.env_version != 'v6':
        if rebind:
            raise ValueError('Legacy-weight rebinding requires env_version=v6')
        return {}
    ...
    if getattr(args, 'pool_strategy', '') == 'elo-kbest':
        raise ValueError('The legacy ELO tournament evaluator is not v6-qualified')
    ...
    return {**METADATA, 'action_space_version': '9slot_pot_fraction_chips_v1',
...
def validate_resume(args, checkpoint):
    if args.env_version == 'v6':
        if getattr(args, 'v6_rebind_legacy_weights', False):
            ...
            if checkpoint.get('env_version') == 'v6':
    ...
    elif checkpoint.get('env_version') == 'v6' or checkpoint.get('policy_contract') == CONTRACT_VERSION:
