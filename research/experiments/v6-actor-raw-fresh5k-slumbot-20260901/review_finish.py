"""Independent raw-hand and evidence review for the fixed fresh5k cohort."""
import json
...
def main():
    ...
    for index in range(1, 9):
        ...
        assert all(row["policy_mode"] == "sample" and row["policy_seed"] == 2026104400 + index for row in rows)
        assert all(row["terminal_validation"]["status"] == "PASS" for row in rows)
    ...
    reviewed = {"status": "PASS", "decision": decision, "model_sha256": MODEL_SHA,
                "statistics": statistics, "prior_audits_checked": old_audits,
    ...
    (BASE / "result_summary.md").write_text(
        "# Raw terminal actor fresh5k result\n\n"
        f"Decision: `{decision}`. Frozen strict-sampled policy: {statistics['bb_per_100']:+.6f} bb/100; "
        f"raw-hand 95% CI [{ci[0]:+.6f}, {ci[1]:+.6f}], session-t7 CI "
        f"[{session_ci[0]:+.6f}, {session_ci[1]:+.6f}], positive sessions "
        f"{statistics['positive_sessions']}/8. Independent review used all 5,000 durable raw hands. "
        "These are pilot hands only and are not pooled into qualification. Goal not achieved.\n",
        encoding="utf-8",
    )
