"""Eight fixed fresh625 strict-sampled Slumbot sessions; never retry."""
...
CLIENT = READY / "execution_code/source_files/scripts/alpha_holdem/play_slumbot_v6_journaled.py"
AUDITOR = READY / "execution_code/source_files/scripts/alpha_holdem/audit_slumbot_v6_session.py"
...
def session_id(index): return f"v6_actor_raw_fresh5k_20260901_s{index:02d}"
def session_seed(index): return 2026104400 + index
def session_command(index):
    return [str(CLIENT), "--model", str(BASE / "frozen/final.pt"), "--hands", "625",
...
def main():
    forbidden = ("execution.json", "execution_code", "frozen", "sessions")
    ...
    try:
        ...
        assert report["status"] == "PASS" and report["sessions"] == 8 and report["successful_hands"] == 5000
        ...
        write(BASE / "completed_analysis.json", {"status": "COMPLETED_PENDING_REVIEW", "decision": decision,
              ...
              "new_training_hands": 0, "evaluation_hands": 5000, "slumbot_hands": 5000, "goal_achieved": False})
