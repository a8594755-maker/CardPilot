...
$runDir = "$experiment/seed2"
$checkpoint = "$runDir/latest.pt"
$assignment = "$runDir/opponent_assignments.jsonl"
$preflightPath = "$experiment/seed2_resume_preflight_iter151.json"
...
if ($actualSha -ne $preflight.checkpoint.sha256) {
    throw "Interrupted seed2 checkpoint SHA256 mismatch"
}
if ([int64]$preflight.fixed_deal_recovery.deal_start_index -ne 31500000) {
...
$arguments = @(
    ...
    '--worker-seed-base', '2026300200',
    '--fixed-training-deal-stream',
    '--fixed-training-deal-start-index', '31500000',
    ...
    '--run-id', 'v6_nashpg_static_seed2_20260903',
    '--run-dir', $runDir,
    '--out', $checkpoint,
    '--seed', '20263002',
...
if ($LASTEXITCODE -ne 0) {
    throw "Seed2 recovery continuation failed with exit code $LASTEXITCODE"
}
