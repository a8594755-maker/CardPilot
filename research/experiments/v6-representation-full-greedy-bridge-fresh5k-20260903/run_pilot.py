"""Eight fixed fresh625 full-representation greedy-bridge Slumbot sessions."""
...
ROOT = BASE.parents[2]
PARENT_MODULE = (
    ROOT / "research/experiments/v6-actor-raw-fresh5k-slumbot-20260901/run_pilot.py"
)
SOURCE = (
    ROOT / "research/experiments/representation-full-strict-sampled-slumbot20k-20260831/frozen/full.pt"
)
SOURCE_SHA = "ff2b9e6fe188ac89aa3ff4b632b70a195a46e8e6a73f1b94a75fcfbbf543c17e"
...
def load_parent():
    spec = importlib.util.spec_from_file_location("representation_fresh5k_parent", PARENT_MODULE)
...
def session_id(index):
    return f"v6_representation_full_greedy_bridge_fresh5k_20260903_s{index:02d}"


def session_seed(index):
    return 2026264000 + index
...
def main():
    ...
    def batched_log(*args):
        if len(args) % 2:
            raise ValueError("experiment update arguments must be option/value pairs")
        for offset in range(0, len(args), 32):
            original_log(*args[offset:offset + 32])
