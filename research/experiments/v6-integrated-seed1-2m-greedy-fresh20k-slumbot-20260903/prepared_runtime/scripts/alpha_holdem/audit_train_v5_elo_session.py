...
def main() -> None:
    ...
    checkpoint_environment = checkpoint.get('environment_hand_accounting') or {}
    manifest_environment = manifest.get('environment_hand_accounting') or {}
    physical_hands = int(checkpoint_environment.get('completed_hands', -1))
    if checkpoint_environment != manifest_environment:
        raise RuntimeError('checkpoint/manifest physical hand accounting mismatch')
    if args.expected_target_environment_hands > 0:
        if int(config.get('total_environment_hands', -1)) != int(
            args.expected_target_environment_hands
        ):
            raise RuntimeError('configured physical hand target mismatch')
        if physical_hands < int(args.expected_target_environment_hands):
            raise RuntimeError('actual physical hands are below target')
        if not bool(checkpoint_environment.get('prefix_complete')):
            raise RuntimeError('physical environment-hand prefix is incomplete')
    ...
    adaptive_enabled = bool(config.get('adaptive_opponent_league'))
    if adaptive_enabled:
        if adaptive_lengths != [len(final_pool)] * 3:
            raise RuntimeError('adaptive state does not match final pool membership')
        adaptive_state_contract = 'active_matches_terminal_pool'
    else:
        # Non-adaptive assignment never consumes these arrays.  They are
        # initialized once from the rollout-start pool and intentionally are
        # not reconciled as ELO membership changes.
        if adaptive_lengths != [args.expected_initial_pool_size] * 3:
            raise RuntimeError(
                'inactive adaptive state differs from initial pool shape'
            )
        rewards = checkpoint.get('adaptive_opponent_ema_rewards') or []
        weights = checkpoint.get('adaptive_opponent_weights') or []
        observations = checkpoint.get('adaptive_opponent_observations') or []
        if any(float(value) != 0.0 for value in rewards) or any(
            int(value) != 0 for value in observations
        ):
            raise RuntimeError('inactive adaptive state contains observations')
        if weights and abs(sum(float(value) for value in weights) - 1.0) > 1e-12:
            raise RuntimeError('inactive adaptive weights are not normalized')
        adaptive_state_contract = 'inactive_initial_pool_sentinel'
    ...
    result = {
        ...
        'actual_environment_hands': physical_hands,
        'target_environment_hands': int(
            args.expected_target_environment_hands
            or config.get('total_environment_hands')
            or 0
        ),
        'actual_transition_bearing_hands': final_hands,
        'target_transition_bearing_hands': int(args.expected_target_hands),
        ...
        'adaptive_state_contract': adaptive_state_contract,
