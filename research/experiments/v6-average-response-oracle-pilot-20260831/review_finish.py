...
def main():
    ...
    assert len(execution['children']) == 25
    ...
    for label in run.LABELS:
        for opponent in run.OPPONENTS:
            ...
            absolute[f'{label}_{opponent}'] = {k:v for k,v in interval(cell).items() if k != 'family18_ci95'}
    ...
    report = dict(status='PASS', decision=decision, reviewed_at=datetime.now(timezone.utc).isoformat(),
        ...
        absolute=absolute, paired_response_minus_initial=effects, training_health=health,
        independently_regenerated_decks=run.PAIRS, normal_child_exits=25, code_source_copy_pairs=len(copies),
    ...
    for name, row in effects.items():
        lines.append(f'| {name} | {row["bb_per_100"]:+.5f} | {row["ci95"]} | {row["family18_ci95"]} |')
    lines += ['', 'One approximate-response component, not full NFSP, an exact best response, equilibrium, or external qualification. Known anchor families are not novel adversaries. Initial SL optimizer and all prior evidence preserved.']
