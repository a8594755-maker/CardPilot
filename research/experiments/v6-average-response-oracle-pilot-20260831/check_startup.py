...
def main():
    ...
    assert checkpoint['iteration'] == 4 and checkpoint['run_id'] == 'v6_average_response_20260831'
    ...
    assert config['seed'] == run.SEED and config['worker_seed_base'] == 2026101300
    ...
    assert run.sha(run.PARENT/'frozen/final.pt') == run.MODEL_SHA
    report = dict(status='PASS', checked_at=datetime.now(timezone.utc).isoformat(),
        ...
        changed_finite_model_tensors=changed, new_ppo_optimizer_states=86, preserved_source_sl_optimizer_states=80,
