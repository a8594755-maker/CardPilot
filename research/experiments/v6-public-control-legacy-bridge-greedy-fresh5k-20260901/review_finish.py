"""Independent raw/evidence review for matched-control bridge fresh5k."""
import json
...
def main():
    ...
    for index in range(1, 9):
        ...
        assert all(row["policy_mode"] == "greedy" and row["policy_temperature"] == 0 and row["policy_seed"] == 2026137200 + index for row in rows)
        for row in rows:
    ...
    (BASE / "result_summary.md").write_text(
        "# Matched-control legacy-bridge greedy fresh5k result\n\n"
        ...
        f"{statistics['positive_sessions']}/8. All actions passed bridge replay audit. "
        "Pilot hands are not qualification hands; Goal not achieved.\n", encoding="utf-8")
