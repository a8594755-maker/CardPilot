"""Raw-hand and equal-session statistics for the legacy-bridge fresh5k gate."""
import math
import statistics

T7 = 2.3646242515927853
STANDARD10_BB_PER_100 = -11.4275


def summarize(sessions, hands_per_session=625):
    if len(sessions) != 8 or any(len(x) != hands_per_session for x in sessions):
    ...
    session_means = [math.fsum(x) / len(x) for x in sessions]
    ...
    return {
        ...
        "positive_sessions": positive_sessions, "session_se": session_se,
        "session_t7_ci95": session_ci,
        "standard10_historical_bb_per_100": STANDARD10_BB_PER_100,
        "point_delta_vs_standard10_historical": mean - STANDARD10_BB_PER_100,
