...
sys.path.insert(0,str(pilot.ROOT/'scripts/alpha_holdem'))
...
def test_only_selfplay_share_and_run_identity_differ():
    ...
    for cmd in [a,b]:
        ...
        assert cmd[:2]==['-u',str(pilot.BASE/'execution_code/source_files/scripts/alpha_holdem/train_v5.py')]
        assert cmd[2]=='--device'
        assert 'scripts/alpha_holdem/train_v5.py' not in cmd
...
def test_exposure_is_assignment_slots_not_hand_fraction():
    ...
    for i in range(3):
        ...
        rows.append(dict(applies_to_iteration=i+1,group_metadata=groups['groups'],
