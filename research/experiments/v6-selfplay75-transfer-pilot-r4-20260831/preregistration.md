# R4 pre-environment launcher correction

R3 passed13tests but argparse rejected the trainer command before environment
initialization: the runtime substitution replaced Python's -u instead of the
script token. Production directory absent, zero hands. R4 preserves -u and
replaces the actual script position, with exact argv-layout assertions. All
prior originals/copies/errors remain untouched. Same still-unconsumed seeds and
model settings, new r4 run IDs; no training stream retry or data replay.
...
python research/experiments/v6-selfplay75-transfer-pilot-r4-20260831/run_pilot.py
