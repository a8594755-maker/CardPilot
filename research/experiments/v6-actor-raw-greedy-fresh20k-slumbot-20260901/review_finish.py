"""Independent raw/evidence review for raw-actor greedy fresh20k."""
import json
...
MODEL_SHA = "9fd38ad30ba25f46a80d2a103b55bb5a64affb7979a2fa47504cc187d38a2bf4"
...
def main():
    ...
    assert audit["status"] == "PASS" and audit["successful_hands"] == 20000
    ...
    for index in range(1, 9):
        ...
        assert len(rows) == 2500 and [row["successful_hand"] for row in rows] == list(range(1, 2501))
        assert all(row["model_sha256"] == MODEL_SHA and row["strict_policy_execution"] for row in rows)
        assert all(row["policy_mode"] == "greedy" and row["policy_temperature"] == 0 and row["policy_seed"] == 2026111300 + index for row in rows)
        for row in rows:
    ...
    for path in sorted((ROOT / "research/experiments").glob("*/*combined_audit.json")):
        ...
        if report.get("status") != "PASS": continue
        audits += 1
    ...
    reviewed = {"status": "PASS", "decision": decision, "model_sha256": MODEL_SHA,
                "statistics": statistics, "prior_audits_checked": audits,
                "prior_token_hashes_checked": len(old_tokens), "slumbot_hands": 20000,
                "goal_achieved": False}
    ...
    (BASE / "result_summary.md").write_text(
        "# Raw terminal actor generic-greedy fresh20k result\n\n"
        f"Decision: `{decision}`. Frozen greedy policy: {statistics['bb_per_100']:+.6f} bb/100; "
        f"raw-hand 95% CI [{ci[0]:+.6f}, {ci[1]:+.6f}], session-t7 CI "
        f"[{session_ci[0]:+.6f}, {session_ci[1]:+.6f}], positive sessions "
        f"{statistics['positive_sessions']}/8, unpaired point delta versus prior fresh5k "
        f"{statistics['point_delta_vs_prior_fresh5k']:+.6f} bb/100. All actions were audited legal "
        "learned-logit argmax decisions. Confirmation hands are not qualification hands; Goal not achieved.\n",
        encoding="utf-8")
