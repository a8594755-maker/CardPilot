...
if (Test-Path -LiteralPath $auditOut) {
    ...
    & $python scripts/alpha_holdem/audit_v6_static_current_kl_1m_training.py `
        --experiment-dir $experiment `
        --base models/baseline/standard10/latest.pt `
        --min-environment-hands 2097152 `
        --expected-archive-iteration 256 `
        --expected-archive-iteration 320 `
        --expected-archive-iteration 384 `
        --schema cardpilot.static_current_kl_2m_training_audit.v1 `
        --out $auditOut
...
& $python scripts/alpha_holdem/aggregate_v6_static_current_kl_1m.py `
    ...
    --expected-prior-raw-rows 98304 `
    --eval-seed-base 20263180 `
    --drift-seed-base 20263190 `
    --scale-label 2M `
    --schema cardpilot.static_current_kl_2m_aggregate.v1 `
