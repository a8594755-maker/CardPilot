...
def main():
 ...
 assert sha(SOURCE/'latest.pt')==SOURCE_SHA and read(SOURCE/'experiment.json')['status']=='COMPLETED'
 ...
 try:
  capture();inputs=[SOURCE/'latest.pt',DATA/'reservoir.pt',REACH/'training_relabel/reach_targets.npy',ACTORS/'retained_actors.npy',REACH/'validation_relabel/validation_arrays.npz',REACH/'validation_relabel/validation_targets.npy',REACH/'validation_relabel/validation_metadata.npz']
  ...
  vm=np.load(REACH/'validation_relabel/validation_metadata.npz');va=arrays_actor(raw,vm['actors']);vq=np.load(REACH/'validation_relabel/validation_targets.npy');sourcep=np.load(SOURCE/'development_probabilities.npy')
  model=build('cuda');epoch0=infer(model,va,count=False);assert np.allclose(epoch0,sourcep,atol=2e-5,rtol=0)
  ...
  opt=torch.optim.Adam(params,lr=1e-4);gen=np.random.default_rng(2026102801);rows=[];steps=0
  ...
  adapter=model.adapter_state();torch.save(dict(architecture='position_sequence_residual_v1',adapter=adapter,source_weights_sha256=SOURCE_SHA,seed=2026102801,optimizer=opt.state_dict(),epoch=8,optimizer_steps=2048,total_hands=0,new_training_hands=0),BASE/'latest.pt')
  ...
  write('analysis.json',dict(status='COMPLETED_PENDING_REVIEW',decision=decision,treatment=treatment,paired_source_minus_treatment=paired,improvement=improvement,model_sha256=sha(BASE/'latest.pt'),source_weights_sha256=SOURCE_SHA,adapter_parameter_tensors=len(adapter),optimizer_steps=2048,optimizer_rows_processed=2097152,model_state_queries=QUERIES,qualification=qualification,new_training_hands=0,new_hands=0,strength_hands=0,slumbot_hands=0,goal_achieved=False));success=True
