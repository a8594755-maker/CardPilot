...
import resume_contract as observable_resume
...
from alpha_holdem.fixed_deal_attempt import (
    allocate_attempt, attempt_deck, attempt_deal_identity, load_attempt,
)
from alpha_holdem.managed_checkpoint_io import atomic_torch_save
from alpha_holdem.preflop_gradient_contract import (
    validate_config as validate_preflop_gradient_config,
    validate_resume as validate_preflop_gradient_resume,
)
try:
...
def public_opponent_hand_seed(worker_seed: int, hand_index: int) -> int:
    """Stable RNG seed for public-model assignment and action sampling."""
    payload = f'PUBLIC_OPPONENT_V1:{int(worker_seed)}:{int(hand_index)}'.encode('ascii')
    return int.from_bytes(hashlib.sha256(payload).digest()[:8], 'big')
...
def restore_group_assignment_rng_from_evidence(
    ...
    *,
    ...
    if replay_origin is None:
        ...
        rng.setstate(replay_origin['python_rng_state'])
    previous_sha = None
    expected_iteration = 1 if replay_origin is None else int(replay_origin['first_iteration'])
...
def run_elo_survivor_tournament(
    *,
    ...
    device: str,
    k_factor: float,
    env_version: str = 'v55preflopv2v4obs',
) -> dict:
    ...
    physical_v6 = env_version in ('v6', 'v6legacyv4obs')
    if physical_v6:
        try:
        ...
        observation_style = (
            'legacy_v4' if env_version == 'v6legacyv4obs' else 'v6'
        )
    else:
        try:
    ...
    for row in ordered:
        ...
        row['model'].eval()
        if physical_v6:
            policies[competitor_id] = row['model']
        else:
    ...
    return {
        ...
        'evaluation_contract': (
            ...
            if matches else (
                ...
                if env_version == 'v6legacyv4obs'
                else (
                    'physical_v6_native_observation_v1'
                    if env_version == 'v6'
                    else 'legacy_v55preflopv2v4obs_v1'
                )
            )
        ),
...
def transition_digest(t) -> str:
    ...
    if len(t) > 16:
        h.update(struct.pack('<d', float(t[16])))
    return h.hexdigest()
...
def fixed_training_deck(worker_seed: int, env_index: int, deal_index: int,
                        namespace: str = '') -> list[int]:
    """Deterministic per-worker/env/deal deck for controlled same-start arms."""
    if namespace:
        return attempt_deck(worker_seed, env_index, deal_index, namespace=namespace)
...
PUBLIC_OPPONENT_ACCOUNTING_SCHEMA = 'cardpilot.public_opponent_accounting.v1'
PROCEDURAL_COUNTER_STRIDE = (
    3 + len(PROCEDURAL_OPPONENT_PROFILES) + NUM_ACTIONS
)
PUBLIC_OPPONENT_COUNTER_STRIDE = 2 + NUM_ACTIONS
...
def count_completed_public_opponent_hand(counters, worker_id, *, action_counts):
    if counters is None:
        return
    action_counts = [int(value) for value in action_counts]
    if len(action_counts) != NUM_ACTIONS or any(value < 0 for value in action_counts):
        raise ValueError('invalid public-opponent per-hand action counts')
    offset = int(worker_id) * PUBLIC_OPPONENT_COUNTER_STRIDE
    with counters.get_lock():
        counters[offset] += 1
        counters[offset + 1] += sum(action_counts)
        for slot, count in enumerate(action_counts):
            counters[offset + 2 + slot] += count


def initial_public_opponent_accounting(checkpoint, *, reset_hand_counter):
    empty = {
        'schema': PUBLIC_OPPONENT_ACCOUNTING_SCHEMA,
        'hands': 0,
        'decisions': 0,
        'action_counts': [0 for _ in range(NUM_ACTIONS)],
        'prefix_complete': True,
    ...
    if checkpoint is None or reset_hand_counter:
        return empty
    previous = checkpoint.get('public_opponent_accounting')
    if previous is not None:
        return dict(previous)
    previous_fraction = float(
        (checkpoint.get('config') or {}).get('public_opponent_fraction', 0.0)
    )
    return empty if previous_fraction == 0.0 else {**empty, 'prefix_complete': False}


def public_opponent_accounting_snapshot(counters, base=None):
    base = base or {
        'schema': PUBLIC_OPPONENT_ACCOUNTING_SCHEMA,
        'hands': 0,
        'decisions': 0,
    ...
    workers = []
    for offset in range(0, len(raw), PUBLIC_OPPONENT_COUNTER_STRIDE):
        values = raw[offset:offset + PUBLIC_OPPONENT_COUNTER_STRIDE]
        hands += int(values[0])
        decisions += int(values[1])
        actions = [a + int(b) for a, b in zip(actions, values[2:])]
        workers.append({
            'worker_id': offset // PUBLIC_OPPONENT_COUNTER_STRIDE,
            'hands': int(values[0]),
            'decisions': int(values[1]),
        })
    result = {
        **base,
        'hands': int(base['hands']) + hands,
        'decisions': int(base['decisions']) + decisions,
        'action_counts': [
            int(a) + int(b) for a, b in zip(base['action_counts'], actions)
        ],
        'session_hands': hands,
        'session_decisions': decisions,
        'session_action_counts': actions,
        'session_worker_counts': workers,
        'inference_bypasses': decisions,
    }
    if (
        sum(actions) != decisions
        or sum(result['action_counts']) != int(result['decisions'])
    ):
        raise ValueError('inconsistent public-opponent accounting totals')
    return result
...
def worker_process_v5(
    ...
    public_opponent_checkpoint='',
    public_opponent_fraction=0.0,
    public_opponent_counters=None,
    record_opponent_identity=False,
    fixed_training_deal_namespace='',
):
    """
    ...
    public_opponent_policy = None
    if public_opponent_checkpoint:
        # Each rollout worker performs tiny batch-1 CPU inference.  Leaving the
        # process-local PyTorch thread pool at its machine-wide default causes
        # severe oversubscription when several workers select this opponent.
        import torch as _torch
        _torch.set_num_threads(1)
        try:
            _torch.set_num_interop_threads(1)
        except RuntimeError:
            # A caller may have initialized inter-op parallelism before invoking
            # the worker directly in a test process.  In spawned production
            # workers this setting is applied before model construction.
            pass
        from alpha_holdem.public_opponent_v6 import load_public_opponent
        public_opponent_policy = load_public_opponent(public_opponent_checkpoint)

    _sys.path.insert(0, _os.path.join(_os.path.dirname(__file__), '..'))
    if env_version in ('v6', 'v6legacyv4obs'):
        ...
        while not stop_event.is_set():
            ...
            if is_mirror_replay:
                ...
                if fixed_training_deal_stream:
                    ...
                    if fixed_training_deal_namespace:
                        deal_identity = attempt_deal_identity(
                            fixed_training_deal_namespace, worker_seed, 0, deal_index)
                else:
                    obs = env.reset()
            ...
            public_rng = None
            public_opponent_active = False
            public_hand_action_counts = [0 for _ in range(NUM_ACTIONS)]
            if not is_self_play and public_opponent_fraction > 0.0:
                if public_opponent_policy is None:
                    raise RuntimeError('public-opponent fraction requires a loaded checkpoint')
                public_seed = public_opponent_hand_seed(worker_seed, hands_played)
                public_rng = _np.random.default_rng(public_seed)
                public_opponent_active = bool(
                    public_rng.random() < public_opponent_fraction
                )
            if not is_self_play and procedural_opponent_fraction > 0.0:
                ...
                if (
                    not public_opponent_active
                    and procedural_rng.random() < procedural_opponent_fraction
                ):
            ...
            while not done and not stop_event.is_set():
                ...
                if public_opponent_active and not is_hero:
                    from alpha_holdem.public_opponent_v6 import decide as public_opponent_decide
                    _action, public_metadata = public_opponent_decide(
                        public_opponent_policy,
                        env.state.core,
                        public_rng,
                        execution_legal_mask=lm,
                    )
                    action_idx = int(public_metadata['selected_action_slot'])
                    if lm[action_idx] <= 0:
                        raise RuntimeError('public opponent selected an illegal action slot')
                    log_prob = 0.0
                    value = 0.0
                    public_hand_action_counts[action_idx] += 1
                elif procedural_style is not None and not is_hero:
            ...
            if public_opponent_active:
                count_completed_public_opponent_hand(
                    public_opponent_counters,
                    worker_id,
                    action_counts=public_hand_action_counts,
                )
            elif procedural_style is not None:
            ...
            for p in (0, 1):
                ...
                for i, row in enumerate(buf):
                    ...
                    if record_opponent_identity:
                        if not centralized_critic:
                            transition = transition + (
                                float('nan'),
                                float('nan'),
                            )
                        transition = transition + (float(current_opp_id),)
    ...
    def start_hand(e):
        ...
        if s.mirror_deck is not None:
            ...
            if fixed_training_deal_stream:
                ...
                if fixed_training_deal_namespace:
                    s.deal_identity = attempt_deal_identity(
                        fixed_training_deal_namespace, worker_seed, e, deal_index)
            else:
                s.obs = s.env.reset()
...
class OpponentPool:
    ...
    def load_from_checkpoint(self, snapshots, candidate_history=None):
        ...
        if isinstance(candidate_history, list):
            ...
            # Rejected candidates are absent from ``self.snapshots`` but their
            # IDs remain part of the run's provenance.  Advancing only past
            # active IDs reuses rejected identities after resume and makes the
            # candidate/assignment history ambiguous.  Recover the monotonic
            # allocator from every valid serialized history ID as well.
            for item in self.candidate_history:
                try:
                    ...
                    continue
                if candidate_id >= 0:
                    self.next_id = max(self.next_id, candidate_id + 1)
        else:
...
def main():
    ...
    parser.add_argument('--torch-threads', type=int, default=0,
                        help='Optional main-process Torch CPU thread bound for diagnostics.')
    parser.add_argument('--managed-deal-attempts', action='store_true',
                        help='Reserve a durable fresh v2 deal namespace before each worker start.')
    parser.add_argument('--deal-attempt-registry', default='research/deal_attempt_registry')
    parser.add_argument('--assignment-replay-origin-checkpoint', default='',
                        help='Explicit migration of an initial managed checkpoint into an assignment replay origin.')
    parser.add_argument('--deal-attempt-namespace', default='',
                        help='Optional explicit single-use namespace; reused receipts fail closed.')
    ...
    parser.add_argument('--env-version', choices=('v6', 'v6legacyv4obs', 'v55', 'v4', 'v55cap1', 'v55cap1v4obs', 'v55v4obs', 'v55pfv2v4obs', 'v55preflopv2v4obs'), default='v55',
    ...
    parser.add_argument(
        '--source-greedy-risk-max-weight',
        type=float,
        default=1.0,
        help=(
            'Default 1 preserves the unweighted source-margin loss. Values '
            'above 1 linearly upweight rows by public pot/effective-stack '
            'fraction, without using outcomes or private opponent state.'
        ),
    )
    parser.add_argument(
        '--source-greedy-risk-full-pot-fraction',
        type=float,
        default=0.15,
        help=(
            'Pot/effective-stack fraction at which source-greedy risk weight '
            'reaches --source-greedy-risk-max-weight.'
        ),
    )
    parser.add_argument(
        '--source-greedy-risk-scale-release',
        action='store_true',
        help=(
            'Default off. Multiply the sampled-advantage release threshold '
            'by the same public risk weight used for source-margin credit.'
        ),
    )
    ...
    parser.add_argument('--source-policy-kl-coef', type=float, default=0.0,
                        help='Discovery mode: explicit-direction KL regularization toward '
                             'a fixed or moving reference on rollout states. Requires --resume.')
    parser.add_argument(
        '--source-policy-kl-direction',
        choices=('reference_to_current', 'current_to_reference'),
        default='reference_to_current',
        help=(
            'KL direction. reference_to_current preserves legacy '
            'KL(reference||current) behavior; current_to_reference enables '
            'the NashPG-style direction.'
        ),
    )
    parser.add_argument(
        '--source-policy-reference-refresh-updates',
        type=int,
        default=0,
        help=(
            'Refresh the reference from the post-update current policy every N '
            'completed trainer PPO calls (0 keeps the legacy fixed reference).'
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--opponent-seat-gradient-diagnostic-minibatches',
        type=int,
        default=0,
        help=(
            'Opt-in non-mutating objective geometry probes. Records transition '
            'opponent identity and measures fixed-anchor, dynamic-snapshot, '
            'self-play, seat, and source-KL gradients on this many initial '
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=(
            'Replace each policy-head actor gradient with a norm-matched '
            'minimum-norm convex combination of supported opponent-seat PPO '
            'and source-KL gradients. Restricted to identity-complete '
            'single-rollout all-policy-heads-only training.'
        ),
    )
    ...
    parser.add_argument(
        '--preflop-trunk-gradient',
        action='store_true',
        help=(
            'Opt-in learned preflop actor gradients to the shared trunk. '
            'Requires full scope, separate head, model preflop and no teacher loss. '
            'A resumed checkpoint must explicitly agree; critic routing is unchanged.'
        ),
    )
    parser.add_argument(
        '--preflop-head-lr',
    ...
    parser.add_argument(
        '--elo-seed-initial-policy',
        action='store_true',
        help=(
            'For a fresh elo-kbest run, insert the exact initialized hero as '
            'historical snapshot id0 before rollout. This makes the first '
            'learned snapshot tournament comparative and checkpoint-auditable.'
        ),
    )
    ...
    parser.add_argument(
        '--public-opponent-checkpoint',
        default='',
        help=(
            'Frozen public-state opponent model used only inside workers. '
            'Requires physical-v6 single-rollout mode.'
        ),
    )
    parser.add_argument(
        '--public-opponent-fraction',
        type=float,
        default=0.0,
        help=(
            'Among external-opponent hands, replace this bounded fraction with '
            'the worker-local frozen public opponent model.'
        ),
    )
    ...
    observable_resume.validate_config(vars(args))
    args.fixed_training_deal_namespace = ''
    if args.managed_deal_attempts and not args.fixed_training_deal_stream:
        parser.error('--managed-deal-attempts requires --fixed-training-deal-stream')
    if args.resume and args.fixed_training_deal_stream and not args.managed_deal_attempts:
        parser.error('fixed-deal resume requires --managed-deal-attempts; legacy restart would reuse its deal prefix')
    if args.resume and args.managed_deal_attempts and (args.reset_hand_counter or args.reset_optimizer):
        parser.error('managed continuation requires --no-reset-optimizer and inherited hand counters')
    if args.deal_attempt_namespace and not args.managed_deal_attempts:
        parser.error('--deal-attempt-namespace requires --managed-deal-attempts')
    if args.assignment_replay_origin_checkpoint and not (
        args.managed_deal_attempts and args.resume_assignment_state_from_provenance
    ):
        parser.error('assignment replay origin requires managed provenance resume')
    ...
    if args.pool_strategy == 'elo-kbest':
        ...
        if (
            args.elo_seed_initial_policy
            and len(initial_opponent_paths) + 1 > args.k_best
        ):
            parser.error(
                '--k-best must fit all initial opponents plus the initialized '
                'hero snapshot'
            )
        if (
            args.env_version in ('v6', 'v6legacyv4obs')
            and not args.resume
            and not initial_opponent_active
            and not args.elo_seed_initial_policy
        ):
            parser.error(
                'fresh physical-v6 elo-kbest requires '
                '--elo-seed-initial-policy or initial opponents'
            )
    elif args.elo_seed_initial_policy:
        parser.error('--elo-seed-initial-policy requires elo-kbest')
    ...
    if args.source_greedy_risk_max_weight < 1.0:
        parser.error('--source-greedy-risk-max-weight must be at least one')
    if args.source_greedy_risk_full_pot_fraction <= 0.0:
        parser.error('--source-greedy-risk-full-pot-fraction must be positive')
    if (
        args.source_greedy_risk_max_weight > 1.0
        and args.source_greedy_margin_coef <= 0.0
    ):
        parser.error(
            'risk-weighted source greedy preservation requires '
            '--source-greedy-margin-coef > 0'
        )
    if (
        args.source_greedy_risk_scale_release
        and args.source_greedy_risk_max_weight <= 1.0
    ):
        parser.error(
            '--source-greedy-risk-scale-release requires '
            '--source-greedy-risk-max-weight > 1'
        )
    ...
    if not 0.0 <= args.public_opponent_fraction <= 1.0:
        parser.error('--public-opponent-fraction must be in [0, 1]')
    if args.public_opponent_fraction > 0.0:
        if not args.public_opponent_checkpoint:
            parser.error('--public-opponent-fraction requires --public-opponent-checkpoint')
        if args.env_version not in ('v6', 'v6legacyv4obs'):
            parser.error('--public-opponent-fraction requires a physical-v6 environment')
        if args.rollout_mode != 'single':
            parser.error('--public-opponent-fraction currently requires --rollout-mode single')
        if args.worker_seed_base is None:
            parser.error('--public-opponent-fraction requires --worker-seed-base')
        if args.adaptive_opponent_league:
            parser.error('--public-opponent-fraction is incompatible with --adaptive-opponent-league')
        if args.paired_seat_average_returns:
            parser.error('--public-opponent-fraction is incompatible with --paired-seat-average-returns')
        if args.procedural_opponent_fraction > 0.0:
            parser.error('public and procedural opponent fractions are mutually exclusive')
        if not (fixed_opponent_active or initial_opponent_active):
            parser.error('--public-opponent-fraction requires fixed or initial opponents')
    if args.procedural_opponent_fraction > 0.0:
        if args.rollout_mode != 'single':
            parser.error(
    ...
    if args.source_policy_reference_refresh_updates < 0:
        parser.error('--source-policy-reference-refresh-updates must be >= 0')
    if args.source_policy_reference_refresh_updates > 0 and (
        args.source_policy_kl_coef <= 0.0
        or args.source_policy_kl_direction != 'current_to_reference'
        or args.source_policy_reference_checkpoint
        or args.source_greedy_margin_coef > 0.0
    ):
        parser.error(
            'moving source-policy reference requires positive KL, '
            '--source-policy-kl-direction current_to_reference, no explicit '
            'reference checkpoint, and no source greedy margin'
        )
    if (
    ...
    if args.opponent_seat_gradient_diagnostic_minibatches < 0:
        parser.error(
            '--opponent-seat-gradient-diagnostic-minibatches must be >= 0'
        )
    if args.opponent_seat_gradient_diagnostic_minibatches and (
        args.rollout_mode != 'single'
        or args.procedural_opponent_fraction > 0.0
        or args.public_opponent_fraction > 0.0
    ):
        parser.error(
            'opponent-seat gradient diagnostics currently require single '
            'rollout with learned pool/self-play opponents'
        )
    if args.opponent_seat_mgda and (
        args.rollout_mode != 'single'
        or args.procedural_opponent_fraction > 0.0
        or args.public_opponent_fraction > 0.0
        or not args.all_policy_heads_only_training
        or args.action_q_advantage
        or args.critic_head_only_gradient
        or args.source_policy_kl_coef <= 0.0
    ):
        parser.error(
            'opponent-seat MGDA requires single learned-opponent rollout, '
            'all-policy-heads-only training, source-policy KL, and no Action-Q '
            'or critic-head-only gradient routing'
        )
    ...
    try:
        validate_preflop_gradient_config(vars(args))
    except ValueError as exc:
        parser.error(str(exc))
    ...
    model = AlphaHoldemNet(
        ...
        preflop_trunk_gradient=args.preflop_trunk_gradient,
    ...
    actor_ema_updates = 0
    moving_reference_completed_updates = 0
    moving_reference_last_refresh_update = 0
    moving_reference_round = 0
    moving_reference_activation_iteration = 0
    moving_reference_resume_rng_state = None
    managed_attempt_state = None
    managed_resume_rng_state = None
    assignment_replay_origin = None
    moving_reference_update_in_progress = False
    ...
    public_accounting_base = initial_public_opponent_accounting(
        None, reset_hand_counter=False
    )
    shutdown_accounting = None

    def current_environment_hand_accounting():
        if shutdown_accounting is not None:
            return copy.deepcopy(shutdown_accounting['environment'])
    ...
    def current_procedural_opponent_accounting():
        if shutdown_accounting is not None:
            return copy.deepcopy(shutdown_accounting['procedural'])
        return procedural_opponent_accounting_snapshot(
            procedural_accounting_base, procedural_opponent_counters
        )

    def current_public_opponent_accounting():
        if shutdown_accounting is not None:
            return copy.deepcopy(shutdown_accounting['public'])
        return public_opponent_accounting_snapshot(
            public_opponent_counters, public_accounting_base
        )
    ...
    def checkpoint_payload() -> dict:
        payload = {
            'fixed_deal_attempt': managed_attempt_state,
            'assignment_replay_origin': assignment_replay_origin,
            'main_process_rng_state': (
                capture_training_rng_state() if args.managed_deal_attempts else None),
            ...
            'public_opponent_accounting': current_public_opponent_accounting(),
            'iteration': iteration,
            ...
            **v6_metadata,
            ...
            'preflop_trunk_gradient': bool(args.preflop_trunk_gradient),
            'preflop_trunk_gradient_migration': (
                ckpt.get('preflop_trunk_gradient_migration') if ckpt else None
            ),
        ...
        if args.source_policy_reference_refresh_updates > 0:
            if reference_policy is None:
                raise RuntimeError('moving source-policy reference is not initialized')
            payload['moving_source_policy_reference'] = (
                moving_reference_checkpoint_state(
                    reference_policy,
                    direction=args.source_policy_kl_direction,
                    refresh_interval_updates=(
                        args.source_policy_reference_refresh_updates
                    ),
                    completed_updates=moving_reference_completed_updates,
                    last_refresh_update=moving_reference_last_refresh_update,
                    reference_round=moving_reference_round,
                    activation_iteration=(
                        moving_reference_activation_iteration
                    ),
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        observable_resume.before_load(model, optimizer, ckpt, vars(args))
        if ckpt.get('fixed_deal_attempt') and not args.managed_deal_attempts:
            raise RuntimeError('managed checkpoint cannot resume with legacy deal mode')
        if ckpt.get('fixed_deal_attempt'):
            prior_attempt = ckpt['fixed_deal_attempt']
            prior_receipt = load_attempt(Path(prior_attempt['path']), prior_attempt['sha256'])
            if prior_receipt != prior_attempt['receipt'] or prior_receipt['namespace'] != ckpt['config'].get('fixed_training_deal_namespace'):
                raise RuntimeError('managed parent receipt/configuration mismatch')
        validate_preflop_gradient_resume(vars(args), ckpt)
        ...
        if args.critic_contract == CRITIC_V2 and source_critic_contract == CRITIC_V1:
            ...
            if not args.reset_optimizer:
                ...
                observable_resume.after_load(model, optimizer, ckpt, vars(args))
        ...
        if args.ppo_replay_buffer_iterations > 0:
            ...
            if serialized_replay is not None:
                ...
                args.v6_rebind_legacy_weights
                and args.reset_hand_counter
                and args.reset_optimizer
            ):
                print(
                    'Initialized empty complete-hand PPO replay buffer at '
                    'the explicit physical-v6 contract-rebind boundary'
                )
            else:
    ...
    if (
        resume_is_same_run
        and ckpt.get('moving_source_policy_reference') is not None
        and args.source_policy_reference_refresh_updates <= 0
    ):
        raise RuntimeError(
            'same-run moving-reference resume cannot disable reference refresh'
        )
    ...
    if args.elo_seed_initial_policy and ckpt is None:
        expected_existing = len(initial_opponent_paths)
        if pool.size() != expected_existing:
            raise RuntimeError(
                'initial-policy ELO seeding found an unexpected seeded pool '
                f'size: {pool.size()} != {expected_existing}'
            )
        initial_snap = pool.add(
            model.state_dict(),
            hands=0,
            iteration=0,
            selection_score=1500.0,
            score_components={
                'kind': 'initial_random_policy',
                'env_version': args.env_version,
                'norm_layer': args.norm_layer,
            },
        )
        print(
            'Seeded ELO pool with exact initialized hero: '
            f'id={initial_snap["id"]} state_sha256='
            f'{lg002_state_dict_sha256(initial_snap["state_dict"])}'
        )
    ...
    if (
        ...
        reference_policy = observable_resume.strip_reference_encoder(copy.deepcopy(model)).to(device)
        ...
        if args.source_policy_reference_refresh_updates > 0:
            serialized_moving_reference = ckpt.get(
                'moving_source_policy_reference'
            )
            source_refresh_interval = int(
                (ckpt.get('config') or {}).get(
                    'source_policy_reference_refresh_updates', 0
                )
            )
            if serialized_moving_reference is None:
                if source_refresh_interval > 0:
                    raise RuntimeError(
                        'source checkpoint declares moving-reference training '
                        'but has no serialized reference state'
                    )
                reference_label = 'new moving reference from exact resume actor'
                moving_reference_activation_iteration = int(iteration)
            else:
                if args.reset_optimizer or args.reset_hand_counter:
                    raise RuntimeError(
                        'moving-reference continuation cannot reset optimizer '
                        'or hand counter'
                    )
                restored_moving = restore_moving_reference_checkpoint(
                    reference_policy,
                    serialized_moving_reference,
                    direction=args.source_policy_kl_direction,
                    refresh_interval_updates=(
                        args.source_policy_reference_refresh_updates
                    ),
                )
                moving_reference_completed_updates = restored_moving[
                    'completed_updates'
                ]
                moving_reference_last_refresh_update = restored_moving[
                    'last_refresh_update'
                ]
                moving_reference_round = restored_moving['reference_round']
                moving_reference_activation_iteration = restored_moving[
                    'activation_iteration'
                ]
                if restored_moving['trainer_iteration'] != int(iteration):
                    raise RuntimeError(
                        'moving-reference state is stale relative to trainer '
                        f'iteration: state={restored_moving["trainer_iteration"]} '
                        f'checkpoint={iteration}'
                    )
                moving_reference_resume_rng_state = restored_moving['rng_state']
                if (
                    pool.size() > 0
                    and not args.resume_assignment_state_from_provenance
                ):
                    raise RuntimeError(
                        'moving-reference pool resume requires '
                        '--resume-assignment-state-from-provenance so the '
                        'pending opponent and Python RNG clock remain exact'
                    )
                reference_label = (
                    'serialized moving reference '
                    f'round={moving_reference_round} '
                    f'completed_updates={moving_reference_completed_updates}'
                )
        ...
        print(
            ...
            f'kl_direction={args.source_policy_kl_direction} '
            f'refresh_updates={args.source_policy_reference_refresh_updates} '
            f'greedy_margin_coef={args.source_greedy_margin_coef:g} '
            f'risk_max_weight={args.source_greedy_risk_max_weight:g} '
            f'risk_full_pot_fraction='
            f'{args.source_greedy_risk_full_pot_fraction:g} '
            f'risk_scale_release={args.source_greedy_risk_scale_release} '
            f'({reference_label})'
        )
    ...
    if args.managed_deal_attempts:
        parent_sha = sha256_path(Path(args.resume)) if args.resume else None
        managed_attempt_state = allocate_attempt(
            Path(args.deal_attempt_registry), run_id=args.run_id,
            training_seed=args.seed, worker_seed_base=args.worker_seed_base,
            workers=args.workers, envs_per_worker=args.rollout_envs_per_worker,
            parent_checkpoint_sha256=parent_sha,
            namespace=args.deal_attempt_namespace or None,
        )
        args.fixed_training_deal_namespace = managed_attempt_state['receipt']['namespace']
        managed_resume_rng_state = ckpt.get('main_process_rng_state') if ckpt else None
        managed_attempt_state['main_rng_restored'] = managed_resume_rng_state is not None
        managed_attempt_state['legacy_rng_bootstrap'] = bool(ckpt) and managed_resume_rng_state is None
        if managed_resume_rng_state is not None:
            restore_training_rng_state(managed_resume_rng_state)
        assignment_replay_origin = ckpt.get('assignment_replay_origin') if ckpt else None
        if args.assignment_replay_origin_checkpoint:
            if assignment_replay_origin is not None:
                raise RuntimeError('cannot override a serialized assignment replay origin')
            origin_path = Path(args.assignment_replay_origin_checkpoint)
            origin_checkpoint = torch.load(origin_path, map_location='cpu', weights_only=False)
            if not origin_checkpoint.get('fixed_deal_attempt') or not origin_checkpoint.get('main_process_rng_state'):
                raise RuntimeError('assignment origin must be a managed initial checkpoint')
            for key in ('run_id', 'seed', 'workers', 'opponent_groups', 'self_play_fraction'):
                if origin_checkpoint['config'].get(key) != vars(args).get(key):
                    raise RuntimeError(f'assignment origin configuration mismatch: {key}')
            origin_records = [json.loads(line) for line in Path(
                args.opponent_assignment_provenance_file).read_text(encoding='utf-8').splitlines() if line.strip()]
            if not origin_records or origin_records[0]['applies_to_iteration'] != int(origin_checkpoint['iteration']) + 1:
                raise RuntimeError('assignment origin iteration does not precede first evidence row')
            if origin_records[0]['total_hands_before_iteration'] != origin_checkpoint['total_hands']:
                raise RuntimeError('assignment origin hand counter does not match first evidence row')
            assignment_replay_origin = {
                'first_iteration': int(origin_checkpoint['iteration']) + 1,
                'python_rng_state': origin_checkpoint['main_process_rng_state']['python'],
                'migration_checkpoint': str(origin_path.resolve()),
                'migration_checkpoint_sha256': sha256_path(origin_path),
            }
        elif not args.resume_assignment_state_from_provenance:
            assignment_replay_origin = {
                'first_iteration': int(iteration) + 1,
                'python_rng_state': random.getstate(),
                'migration_checkpoint': None,
                'migration_checkpoint_sha256': None,
            }
        print(f"Managed deal attempt: {args.fixed_training_deal_namespace} (statistical continuation)")

    def save_completed_checkpoint(path):
    ...
    if not args.resume:
        ...
        if args.overwrite or not init_path.exists():
            (atomic_torch_save if args.managed_deal_attempts else torch.save)({
                'fixed_deal_attempt': managed_attempt_state,
                'assignment_replay_origin': assignment_replay_origin,
                'main_process_rng_state': (
                    capture_training_rng_state() if args.managed_deal_attempts else None),
        ...
        save_completed_checkpoint(args.out)
        print(
    ...
    if args.resume_assignment_state_from_provenance:
        ...
        early_restored = restore_group_assignment_rng_from_evidence(
            ...
            replay_origin=assignment_replay_origin,
        )
    ...
    public_opponent_counters = mp.Array(
        'q', PUBLIC_OPPONENT_COUNTER_STRIDE * W, lock=True
    )
        ...
        if args.rollout_mode == 'multi':
            ...
            p = mp.Process(
                ...
                args=(w, obs_shm.name, result_shm.name, status_shm.name,
                      ...
                      procedural_opponent_counters,
                      args.public_opponent_checkpoint,
                      args.public_opponent_fraction,
                      public_opponent_counters,
                      bool(
                          args.opponent_seat_gradient_diagnostic_minibatches
                          or args.opponent_seat_mgda
                      ), args.fixed_training_deal_namespace),
                daemon=True,
            )
        p.start()
    ...
    print(
        f'Public opponent: fraction={args.public_opponent_fraction} '
        f'checkpoint={args.public_opponent_checkpoint or "disabled"}'
    )
    print(
    ...
    if managed_resume_rng_state is not None:
        restore_training_rng_state(managed_resume_rng_state)
        print('Restored managed main-process RNG state')
    elif moving_reference_resume_rng_state is not None:
        # Restore before assignment evidence replay. The evidence chain is
        # authoritative for a pool run's pending assignment and Python RNG;
        # Torch/NumPy are untouched by that replay and remain checkpoint exact.
        restore_training_rng_state(moving_reference_resume_rng_state)
        print('Restored moving-reference main-process RNG state')
    ...
    try:
        ...
        while continue_training:
            ...
            if iter_hands >= args.hands_per_iter and len(iter_transitions) > 0:
                ...
                moving_reference_update_in_progress = bool(
                    args.source_policy_reference_refresh_updates > 0
                    or args.managed_deal_attempts
                )
                stats = trinal_clip_ppo_update(
                    ...
                    source_greedy_risk_max_weight=(
                        args.source_greedy_risk_max_weight
                    ),
                    source_greedy_risk_full_pot_fraction=(
                        args.source_greedy_risk_full_pot_fraction
                    ),
                    source_greedy_risk_scale_release=(
                        args.source_greedy_risk_scale_release
                    ),
                    ...
                    reference_policy_kl_direction=(
                        args.source_policy_kl_direction
                    ),
                    ...
                    opponent_seat_gradient_diagnostic_minibatches=(
                        args.opponent_seat_gradient_diagnostic_minibatches
                    ),
                    opponent_seat_mgda=args.opponent_seat_mgda,
                ...
                if args.source_policy_reference_refresh_updates > 0:
                    moving_advance = advance_moving_reference(
                        model,
                        reference_policy,
                        refresh_interval_updates=(
                            args.source_policy_reference_refresh_updates
                        ...
                        completed_updates=moving_reference_completed_updates,
                        last_refresh_update=(
                            moving_reference_last_refresh_update
                        ),
                        reference_round=moving_reference_round,
                    )
                    moving_reference_completed_updates = moving_advance[
                        'completed_updates'
                    ]
                    moving_reference_last_refresh_update = moving_advance[
                        'last_refresh_update'
                    ]
                    moving_reference_round = moving_advance['reference_round']
                    stats['moving_reference_refreshed'] = bool(
                        moving_advance['refreshed']
                    )
                    stats['moving_reference_completed_updates'] = int(
                        moving_reference_completed_updates
                    )
                    stats['moving_reference_last_refresh_update'] = int(
                        moving_reference_last_refresh_update
                    )
                    stats['moving_reference_round'] = int(
                        moving_reference_round
                    )
                # Managed checkpoints include league/replay state as well as
                # optimizer state. Keep the guard through those finalizations.
                moving_reference_update_in_progress = bool(args.managed_deal_attempts)
                if actor_ema_state is not None:
                    update_actor_ema(
                        model,
                        actor_ema_state,
                ...
                score_components = {
                    ...
                    'source_greedy_risk_max_weight': float(
                        stats.get('source_greedy_risk_max_weight', 1.0)
                    ),
                    'source_greedy_risk_full_pot_fraction': float(
                        stats.get('source_greedy_risk_full_pot_fraction', 0.15)
                    ),
                    'source_greedy_risk_scale_release': bool(
                        stats.get('source_greedy_risk_scale_release', False)
                    ),
                    'source_greedy_risk_weight_mean': float(
                        stats.get('source_greedy_risk_weight_mean', 1.0)
                    ),
                    'source_greedy_risk_weight_saturated_fraction': float(
                        stats.get(
                            'source_greedy_risk_weight_saturated_fraction', 0.0
                        )
                    ),
                ...
                with h1_metrics_jsonl_path.open('a', encoding='utf-8', newline='\n') as f:
                    f.write(json.dumps({
                        ...
                        'fixed_training_deal_namespace': args.fixed_training_deal_namespace,
                        'deal_attempt_receipt_sha256': (
                            managed_attempt_state['sha256'] if managed_attempt_state else None),
                        ...
                        'public_opponent_accounting': (
                            current_public_opponent_accounting()
                        ),
                        ...
                        'opponent_seat_gradient_diagnostics': stats.get(
                            'opponent_seat_gradient_diagnostics', []
                        ),
                        'opponent_seat_mgda_updates': stats.get(
                            'opponent_seat_mgda_updates', []
                        ),
                        ...
                        'reference_policy_kl_direction': stats.get(
                            'reference_policy_kl_direction',
                            args.source_policy_kl_direction,
                        ),
                        'moving_reference_refresh_updates': int(
                            args.source_policy_reference_refresh_updates
                        ),
                        'moving_reference_completed_updates': int(
                            stats.get(
                                'moving_reference_completed_updates',
                                moving_reference_completed_updates,
                            )
                        ),
                        'moving_reference_last_refresh_update': int(
                            stats.get(
                                'moving_reference_last_refresh_update',
                                moving_reference_last_refresh_update,
                            )
                        ),
                        'moving_reference_round': int(
                            stats.get(
                                'moving_reference_round', moving_reference_round
                            )
                        ),
                        'moving_reference_refreshed': bool(
                            stats.get('moving_reference_refreshed', False)
                        ),
                        ...
                        'source_greedy_risk_weight_saturated_fraction': float(
                            stats.get(
                                'source_greedy_risk_weight_saturated_fraction',
                                0.0,
                            )
                        ),
                        'source_greedy_margin_eligible_rows': int(
                ...
                if (
                    ...
                    save_completed_checkpoint(archive_path)
                ...
                while (
                    ...
                    save_completed_checkpoint(milestone_path)
                    print(
        ...
        stop_event.set()
        time.sleep(0.25)
        # Give CPU-local opponent inference enough time to observe stop_event.
        # Snapshot every shared counter while all surviving workers still own
        # valid locks; a process terminated while holding an mp.Array lock would
        # otherwise make the terminal checkpoint wait forever.
        shutdown_deadline = time.monotonic() + 30.0
        for p in procs:
            p.join(timeout=max(0.0, shutdown_deadline - time.monotonic()))
        shutdown_accounting = {
            'environment': environment_hand_accounting_snapshot(
                environment_accounting_base, environment_hand_counters, total_hands
            ),
            'procedural': procedural_opponent_accounting_snapshot(
                procedural_accounting_base, procedural_opponent_counters
            ),
            'public': public_opponent_accounting_snapshot(
                public_opponent_counters, public_accounting_base
            ),
        }
        for p in procs:
    ...
    if moving_reference_update_in_progress:
        # A Ctrl-C inside the multi-minibatch PPO call leaves model/optimizer
        # partially advanced. Preserve the preceding complete checkpoint
        # instead of mislabeling a half update as resumable.
        write_manifest(
            'interrupted_during_ppo_update_checkpoint_preserved',
            total_hands=total_hands,
            iteration=iteration,
            checkpoint=str(out_path),
        )
        print(
            'Interrupted during guarded PPO update; preserved the '
            'preceding complete checkpoint without overwriting it.'
        )
        return
    save_completed_checkpoint(args.out)
    if args.pool_strategy == 'elo-kbest':
