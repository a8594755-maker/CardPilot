...
from alpha_holdem.policy_contract_v6 import CONTRACT_VERSION, action_table as v6_action_table
...
BRIDGE_CONTRACT = "hunl_v6_physical_legacy_v4_observation_bridge_v1"
RANKS = "23456789TJQKA"
SUITS = "cdhs"
...
def load_policy(path: str | Path, device: str = "cpu") -> LegacyObservationBridgePolicy:
    ...
    if checkpoint.get("raise_action_mapping") != "preflop_pot_fraction_v2":
        raise ValueError("Legacy observation bridge requires preflop_pot_fraction_v2")
    for key in (
...
def legacy_observation(policy: LegacyObservationBridgePolicy, state: ChipState) -> tuple[dict, list[str | None]]:
    ...
    if bool(getattr(policy.model, "requires_position_feature", False)):
