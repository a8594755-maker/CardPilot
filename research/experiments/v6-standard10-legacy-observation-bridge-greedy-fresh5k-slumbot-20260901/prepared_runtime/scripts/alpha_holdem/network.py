...
class AlphaHoldemNet(nn.Module):
    ...
    def __init__(
        ...
        self.critic_contract = str(critic_contract)
        ...
        self.preflop_raw_action_scale = float(preflop_raw_action_scale)
        self.preflop_raw_gate = str(preflop_raw_gate)
        if self.preflop_raw_gate not in {'none', 'v4_bb_vs_open_v1'}:
            raise ValueError(
                f'unsupported preflop_raw_gate: {self.preflop_raw_gate}'
            )
        ...
        # Consumers that construct policy-visible observations need an
        # explicit contract for the public seat feature.  The legacy network
        # previously exposed no such flag even when a position policy/value
        # adapter was configured, so loader warm-up forwards could
        # incorrectly supply only the two legacy extra features.
    ...
    def _init_trunk(self, card_flat: int, action_flat: int):
        ...
        if self.position_adapter_hidden > 0:
            # The third extra feature is the player's observed HUNL seat:
            # 0=BB/OOP, 1=SB/IP. Independent learned experts prevent a BB
            # gradient from perturbing the SB branch (and vice versa). The
            # zero-initialized outputs make both branches exactly behavior
            # preserving before optimization.
        ...
        if self.critic_contract == CRITIC_V1:
            ...
            # Match the trainer's critic_v2 construction without perturbing
            # the actor/global RNG used by evaluation.
    ...
    def forward(
        ...
        # The legacy trunk consumes the original two normalized stack values.
        # Optional features appended after them are isolated to their adapters,
        # preserving every existing checkpoint's trunk semantics.
        ...
        if self.trunk is None:
            ...
            if self.position_policy_adapters is not None:
                self.position_policy_adapters = self.position_policy_adapters.to(device)
                ...
                raw_preflop = torch.cat(
                    [
                        ...
                        (
                            ...
                            * self.preflop_raw_action_scale
                        ),
                        extra_info[:, :2],
                    ],
                    dim=1,
                )
                raw_delta = self.preflop_raw_policy_adapter(raw_preflop)
                if self.preflop_raw_gate == 'v4_bb_vs_open_v1':
                    flat_action = action_info.flatten(start_dim=1)
                    gate = (
                        (flat_action[:, 9] > 0.5)
                        & (flat_action[:, 15] > 0.5)
                        & (
                    ...
                    raw_delta = raw_delta * gate.to(raw_delta.dtype).unsqueeze(-1)
                preflop_logits = preflop_logits + raw_delta
        ...
        if self.position_policy_adapters is not None:
            if extra_info.shape[1] < 3:
                raise ValueError(
                    "position_policy_adapters require extra_info[:, 2] seat feature"
                )
        ...
        if self.position_value_adapters is not None:
            if extra_info.shape[1] < 3:
                raise ValueError(
                    "position_value_adapters require extra_info[:, 2] seat feature"
                )
