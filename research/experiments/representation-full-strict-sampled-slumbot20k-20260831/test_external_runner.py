...
def test_client_is_fixed_native_sample_and_explicit_seed():
    item = dict(requested_hands=2500, policy_seed=2026091401, raw_hands='raw', dump='dump', result='result')
    ...
    assert args[args.index('--policy-seed')+1] == '2026091401'
...
def confirmation_inputs():
    record = dict(status='COMPLETED', metrics={'post_exit_review_pass': 1}, accounting={'evaluation_hands': 196608})
    review = dict(status='PASS', replication_passed=True, decision='REPRESENTATION_SCOPE_REPLICATION_PASSED',
        seed=20260913, evaluation_hands=196608, new_training_hands=0, slumbot_hands=0,
        candidate_hashes={'full': run.DIGEST})
...
def test_confirmation_gate_rejects_unqualified_external_launch(bad):
    ...
    elif bad == 'wrong_checkpoint': review['candidate_hashes']['full'] = 'different'
