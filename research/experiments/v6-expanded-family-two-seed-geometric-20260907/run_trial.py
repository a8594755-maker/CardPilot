"""Fixed matched opponent execution-mixture trial; single owner, no automatic retries."""
...
PARENTS = ROOT/'research/experiments/v6-opponent-execution-mixture-two-seed-geometric-20260907'
SMOKE = ROOT/'research/experiments/v6-expanded-family-worker-smoke-20260907'
...
WRAPPER = PARENTS/'train_candidate.py'
sys.path.insert(0,str(OLD))
import run_actor_control as ctl
require,read,sha,write_new = ctl.require,ctl.read,ctl.sha,ctl.write_new
EXPECTED = {1:'a53a452603d02021a582a46baba25d05b655a61321ab53eaae62fcbfc9ede0d5',3:'ae0a4009fcbbd8cddef04649a53f009b8db3cdffab0f144ab843102b427e69a8'}
INITIAL = {1:14695135,3:14694500}
DOSES = {1:262144,2:1048576}
STRATEGIES = {'control':'anchor-latest','expanded':'anchor-latest'}
MIXTURES = {'control':0.0,'expanded':0.0}
CAPACITY = {'control':5,'expanded':9}
ORDER = [(1,'control'),(1,'expanded'),(3,'expanded'),(3,'control')]
EVAL_SEEDS = {(seed,stage):20266200+seed*10+stage for seed in (1,3) for stage in (1,2)}
...
def parent_path(seed,arm,stage):
    ...
    return (PARENTS/f'seed{seed}_control_stage2/latest.pt' if arm=='control' else SMOKE/f'derived/seed{seed}_recent_stage2/latest.pt') if stage==1 else directory(seed,arm,1)/'latest.pt'
...
def training_command(seed,arm,stage):
    ...
    argv = read((PARENTS/f'seed{seed}_control_stage2')/'command.json')
    argv[0],argv[2] = sys.executable,str(WRAPPER)
    for flag,value in {'--resume':path,'--run-dir':run,'--out':run/'latest.pt',
        '--pool-strategy':STRATEGIES[arm], '--k-best':CAPACITY[arm],'--total-environment-hands':INITIAL[seed]+DOSES[stage],
    ...
    ctl.execution.set_option(argv,'--opponent-greedy-mixture',str(MIXTURES[arm]))
    return argv
...
class Controller:
    ...
    def __init__(self):
        ...
        require(qualification['passed'] and qualification['tests_passed'] >= 4 and qualification['real_worker_smoke_passed'],'controller not qualified')
    ...
    def train(self, seed, arm, stage):
        ...
        digest = sha(path)
        require(digest == (read(BASE/'qualification.json')['parent_sha256'][f'{seed}_{arm}'] if stage == 1 else read(path.parent/'verification.json')['checkpoint_sha256']), 'parent hash')
        if stage == 2:
            previous = read(path.parent/'verification.json')
            require(sha(path.parent/'mixture_runtime.json') == previous['mixture_runtime_sha256'], 'parent runtime changed')
            require(read(path.parent/'mixture_runtime.json')['greedy_weight'] == MIXTURES[arm], 'parent mixture changed')
        ...
        write_new(run/'parent_contract.json', {'path':str(path), 'sha256':digest,
            ...
            'opponent_greedy_mixture':MIXTURES[arm], 'prior_mixture_runtime':str(path.parent/'mixture_runtime.json') if stage == 2 else None,
            'prior_mixture_runtime_sha256':sha(path.parent/'mixture_runtime.json') if stage == 2 else None})
        ...
        def capture(line):
            ...
            for key in keys:
                require(state_equal(normalized_pool(parent,200)[key], initial[key]), 'initial state changed: '+key)
            ...
            write_new(run/'initial_resume_gate.json', {'passed':True, 'exact_keys':keys,
                'intentional_change':'capacity5 versus capacity9 expanded fixed families; sampled execution0; managed new namespace',
        ...
        require(runtime['greedy_weight'] == MIXTURES[arm] and runtime['parent_sha256'] == digest, 'mixture runtime mismatch')
        require(observed['clean_return'] and observed['runtime_sha256'] == sha(run/'mixture_runtime.json'), 'mixture observation mismatch')
        require(observed['opponent_forward_rows'] > 0, 'no real opponent forwards')
        ...
        for original, actual, greedy, mask in zip(batch['original_probabilities'],batch['mixture_probabilities'],batch['greedy_actions'],batch['legal_masks']):
            require(all(abs(p - ((1-MIXTURES[arm])*q + (MIXTURES[arm] if i == greedy else 0))) < 1e-6 for i,(p,q) in enumerate(zip(actual,original))), 'actual mixture formula')
        ...
        if arm in STRATEGIES:
            ...
            require(len(final['pool_snapshots'])==CAPACITY[arm], 'pool capacity changed')
        ...
        result = {'passed':True, 'seed':seed, 'arm':arm, 'stage':stage, 'new_physical_hands':new_physical,
            ...
            'mixture_runtime_sha256':sha(run/'mixture_runtime.json'), 'mixture_observation_sha256':sha(run/'mixture_observation.json'),
    ...
    def evaluate(self,stage):
        ...
        for seed in (1,3):
            ...
            for arm in STRATEGIES:
                ...
                original = parent_path(seed,'control',1)
            ...
            contrast = ctl.evidence.summarize_rows(ctl.evidence.join_arms(arms['control'],arms['expanded']))
            endpoints = {arm:ctl.evidence.summarize_rows(rows) for arm,rows in arms.items()}
            results[str(seed)] = {'expanded_minus_control':contrast,'endpoint_minus_parent':endpoints,
    ...
    def run(self):
        for stage in DOSES:
            ...
            self.phase = 'FIXED_1M_COMPLETE_RESEARCH_REVIEW'
