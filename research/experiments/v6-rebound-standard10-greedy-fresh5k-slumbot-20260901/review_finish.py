"""Independent raw-hand review for rebound Standard10 current-v6 fresh5k."""
...
def main():
    ...
    for index in range(1, 9):
        ...
        assert all(
            ...
            and row["policy_seed"] == 2026112000 + index
    ...
    decision = (
        "ADMIT_REBOUND_GREEDY_FRESH20K"
        if statistics["supports_separate_fresh20k"]
        else "REBOUND_GREEDY_CALIBRATION_NOT_POSITIVE"
    )
    reviewed = {
        "status": "PASS",
        "decision": decision,
    ...
    (BASE / "result_summary.md").write_text(
        "# Rebound Standard10 current-v6 generic-greedy fresh5k\n\n"
        ...
        f"{statistics['positive_sessions']}/8. Unpaired point delta versus historical old-contract "
        f"greedy {statistics['point_delta_vs_historical_old_contract']:+.6f}. All actions and "
        "evidence passed audit. Pilot hands are not qualification hands.\n",
        encoding="utf-8",
    )
