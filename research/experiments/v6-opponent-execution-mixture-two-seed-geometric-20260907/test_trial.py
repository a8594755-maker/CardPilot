...
class TrialTests(unittest.TestCase):
    def test_matched_parent(self):
        for seed in (1,3):
            self.assertEqual(t.parent_path(seed,'control',1),t.parent_path(seed,'mixture',1))
    ...
    def test_only_execution_mixture_differs(self):
        for seed in (1,3):
            for stage in t.DOSES:
                commands=[]
                for arm in t.STRATEGIES:
                    argv=t.training_command(seed,arm,stage)
                    for flag in ('--opponent-greedy-mixture','--run-dir','--out','--resume','--opponent-assignment-provenance-file'):
                        self.assertEqual(argv.count(flag),1)
    ...
    def test_invalid_cell(self):
        for cell in ((2,'mixture',1),(1,'unknown',1),(1,'mixture',3)):
