...
def main():
    ...
    require(owner['pid'] == 19632 and abs(owner['create_time']-1788783925.6140864) < .001, 'owner identity')
    ...
    for row in result['training']:
        ...
        weight = {'control':0., 'mixture':.5}[row['arm']]
        ...
        require(runtime['wrapper_sha256'] == sha(BASE/'train_candidate.py'), 'mixture wrapper changed')
        if row['stage'] == 2:
        ...
        for seed in (1,3):
            ...
            for arm in ('control','mixture'):
                ...
                for p in (raw, folder/'summary.json'):
                    hashes[str(p)] = sha(p)
            contrast = old.compare_arms(arms['control'], arms['mixture'], seen)
            expected = reported['seeds'][str(seed)]
            old.check_reported(contrast['connected_minus_detached'], expected['mixture_minus_control'])
            for group in ('endpoint_minus_parent','absolute_vs_anchors'):
                for a,b in (('detached','control'),('connected','mixture')):
            ...
            stages[str(stage)][str(seed)] = {'mixture_minus_control':contrast['connected_minus_detached'],
                **{g:{b:contrast[g][a] for a,b in (('detached','control'),('connected','mixture'))}
