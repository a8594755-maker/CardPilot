...
INITIAL = {1:13645045,3:13641556}
DOSES = {1:262144,2:1048576}
STRATEGIES = {'control':'anchor-latest','mixture':'anchor-latest'}
MIXTURES = {'control':0.0,'mixture':0.5}
ORDER = [(1,'control'),(1,'mixture'),(3,'mixture'),(3,'control')]
EVAL_SEEDS = {(seed,stage):20265400+seed*10+stage for seed in (1,3) for stage in (1,2)}
...
def parent_path(seed,arm,stage):
    ...
    return PARENTS/f'seed{seed}_recent_stage2/latest.pt' if stage == 1 else directory(seed,arm,1)/'latest.pt'
...
def training_command(seed,arm,stage):
    ...
    argv = read((PARENTS/f'seed{seed}_recent_stage2')/'command.json')
    ...
    argv += ['--opponent-greedy-mixture', str(MIXTURES[arm])]
    return argv
...
class Controller:
    ...
    def __init__(self):
        ...
        require(qualification['passed'] and qualification['tests_passed'] >= 7 and qualification['runtime_tests_passed'] == 7,'controller not qualified')
    ...
    def evaluate(self,stage):
        ...
        for seed in (1,3):
            ...
            contrast = ctl.evidence.summarize_rows(ctl.evidence.join_arms(arms['control'],arms['mixture']))
            endpoints = {arm:ctl.evidence.summarize_rows(rows) for arm,rows in arms.items()}
            results[str(seed)] = {'mixture_minus_control':contrast,'endpoint_minus_parent':endpoints,
