...
def test_assignment_rng_resume_replays_native_uniform_fixed_pool():
    seed = 20261121
    generator = random.Random(seed)
    records = []
    previous_sha = None
    for iteration in (1, 2):
        ...
        record = build_assignment_provenance_record(
            run_id='uniform-fixed-pool-resume-test',
            applies_to_iteration=iteration,
    ...
    result = restore_group_assignment_rng_from_evidence(
        records,
        [
            {'iteration': 1, 'adaptive_opponent_league': []},
            {'iteration': 2, 'adaptive_opponent_league': []},
        ],
        rng=restored_rng,
        seed=seed,
        worker_count=12,
    ...
    assert result['pending_assignments'] is None
    assert restored_rng.getstate() == generator.getstate()
