"""Resume one MGDA checkpoint for a fresh geometric continuation replicate."""
...
from alpha_holdem.v6_dual_contract_mgda_fresh_curve import collect_chunk, summarize_delta
...
from alpha_holdem.v6_mgda_source_objective_continuation import panel_metric, train_arm
...
def main() -> None:
    ...
    expected_sha = next(
        row["sha256"]
        for row in parent_summary["checkpoints"]
        if Path(row["path"]).name == start_path.name
    )
    if (
        expected_sha != start_sha
        or start["base_sha256"] != base_sha
        or start["dose_hands"] != 8192
    ):
        raise RuntimeError("invalid start checkpoint lineage")
    ...
    for name in ("earlier", "later"):
        ...
        models[name] = model
    optimizer = torch.optim.Adam(models["later"].trainable_parameters(), lr=3e-4)
    ...
    schedule_path.write_text(
        json.dumps(
            [[selection.tolist() for selection in step] for step in schedules],
            indent=2,
        )
        + "\n",
        encoding="utf-8",
    )
    checkpoint_path = args.out_dir / "treatment_residual_12288.pt"
    torch.save(
        {
            ...
            "residual_state_dict": residual_state(models["later"]),
    ...
    with gzip.open(evaluation_path, "xt", encoding="utf-8", newline="\n") as handle:
        for anchor_index, anchor in enumerate(opponents):
            rng = random.Random(args.seed + 60_000_019 * (anchor_index + 1))
    ...
    disagreement_growth = panel_metrics["later"]["greedy_disagreement"] - panel_metrics["earlier"]["greedy_disagreement"]
    positive_geometry = float(np.mean([row["reward_worst_alignment"] > 0 for row in metrics]))
    gates = {
        "exact_resume_lineage": start_sha == expected_sha,
        "actual_hands_exact": len(hands) == args.continuation_hands,
        "positive_reward_alignment_at_least_75pct": positive_geometry >= 0.75,
    ...
    summary = {
        "schema": "cardpilot.mgda_single_continuation.v1",
        "status": "COMPLETED",
        "base_sha256": base_sha,
        "start_checkpoint_sha256": start_sha,
        "checkpoint": {"path": str(checkpoint_path.resolve()), "sha256": checkpoint_sha},
        "actual_environment_training_hands": len(hands),
        "transition_rows": len(transitions),
        "update_rows": args.steps * args.group_batch_size * 6,
        "positive_reward_alignment_fraction": positive_geometry,
        ...
        "gates": gates,
        "admit_beyond_12k": admit,
        "decision": "ADMIT_BEYOND_12K" if admit else "HOLD_MGDA_AT_12K",
