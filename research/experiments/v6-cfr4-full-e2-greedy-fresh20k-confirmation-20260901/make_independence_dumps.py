#!/usr/bin/env python3
"""Convert the frozen raw hand evidence into independence-audit dumps."""
            ...
            rows.append(
                {
                    ...
                    "action_move": terminal["action"],
                    "action_amount": 0,
                    "winnings_hero": hand["winnings_chips"],
                }
            )
        if len(rows) != 2500:
            raise ValueError(f"Expected 2,500 complete hands in {source}; got {len(rows)}")
        target.write_text(
            "".join(
