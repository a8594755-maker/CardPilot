...
def decision(summary):
    ...
    return 'ADMIT_SEPARATE_FRESH100K' if summary['bb_per_100'] > 0 else 'SAMPLED_BASELINE_NONPOSITIVE'
...
def main():
    ...
    if [s['policy_seed'] for s in manifest['sessions']] != list(range(2026090601, 2026090609)):
    ...
    result = dict(status='PASS', decision=status, policy_sha256=DIGEST, raw=summary,
        ...
        review_script_sha256=sha(__file__), input_sha256=audit['input_sha256'])
    result['verified_policy_metadata'] = expected_policy_metadata
    result['execution_contract'] = expected_execution
    lines = ['# Frozen native-sampled Standard10 Slumbot baseline', '', f'Decision: `{status}`.', '',
        ...
        *[f"| {row['id']} | {row['hands']} | {row['bb_per_100']:+.4f} |" for row in sessions], '',
        f'Policy: unchanged Standard10, native sample/temp1, SHA256 `{DIGEST}`.',
