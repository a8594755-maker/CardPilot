...
def main():
    ...
    command = ['python', (BASE/'review_completed_baseline_pathfix.py').relative_to(ROOT).as_posix()]
    ...
    paths = [BASE/name for name in ['completed_analysis.json', 'result_summary.md',
        'review_completed_baseline_pathfix.py', 'finish_review_corrected.py']]
    subprocess.run([sys.executable, 'research/experiment_log.py', 'update', BASE.name,
    ...
    subprocess.run([sys.executable, 'research/experiment_log.py', 'finish', BASE.name,
        '--status', 'COMPLETED', '--count', 'new_training_hands=0', '--count', 'evaluation_hands=20000',
        '--summary', f'Exactly20000fresh strict native-sampled Standard10 hands: {summary["bb_per_100"]:+.4f}bb/100,95%CI[{summary["lower_bound_bb_per_100"]:+.4f},{summary["upper_bound_bb_per_100"]:+.4f}]. All8clients exited0; strict audit and independent chip-space review passed.',
