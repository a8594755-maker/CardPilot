...
def main():
 ...
 source=torch.load(run.PREV/'unweighted_tv.pt',map_location='cpu',weights_only=False);final=torch.load(BASE/'latest.pt',map_location='cpu',weights_only=False)
 base=[k for k in final['model'] if not k.startswith('position_policy_adapters.')];adapter=[k for k in final['model'] if k.startswith('position_policy_adapters.')]
 assert len(base)==86 and len(adapter)==8 and all(torch.equal(final['model'][k],source['model'][k]) for k in base)
 assert all(torch.isfinite(final['model'][k]).all() for k in adapter) and final['position_adapter_hidden']==128
 assert len(final['optimizer']['state'])==8 and all(float(x['step'])==2048 for x in final['optimizer']['state'].values())
 ...
 p=np.load(BASE/'development_probabilities.npy');sourcep=np.load(run.PREV/'unweighted_tv_development_probabilities.npy')
 ...
 report=dict(status='PASS',decision=decision,treatment=treatment,paired_source_minus_treatment=paired,improvement=improvement,model_sha256=run.sha(BASE/'latest.pt'),adapter_tensors=8,frozen_base_tensors=86,optimizer_steps=2048,optimizer_rows_processed=2097152,model_state_queries=e['model_state_queries'],qualification=qual,new_training_hands=0,evaluation_hands=0,strength_hands=0,slumbot_hands=0,goal_achieved=False,wall_time_seconds=e['wall_time_seconds'],review_wall_time_seconds=time.monotonic()-start,interpretation='Explicit-position adapter result uses a known development cohort and is not confirmatory strength evidence.')
 run.write('reviewed_analysis.json',report);(BASE/'result_summary.md').write_text(f'# Explicit-position development\n\n{decision}\n\nTV {treatment["mean"]:.6f}; improvement {improvement:.6f}; paired {paired}.\n\n0newhands/strength/Slumbot. {report["interpretation"]}\n')
