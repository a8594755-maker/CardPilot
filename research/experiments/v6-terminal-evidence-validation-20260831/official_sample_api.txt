...
#   /slumbot/api/login
#   /slumbot/api/new_hand
#   /slumbot/api/act
# To initiate a new hand, send a request to /slumbot/api/new_hand.  To take an action, send a
# request to /slumbot/api/act.
#
# The body of a sample request to /slumbot/api/new_hand:
#   {"token": "a2f42f44-7ff6-40dd-906b-4c2f03fcee57"}
# The body of a sample request to /slumbot/api/act:
#   {"token": "a2f42f44-7ff6-40dd-906b-4c2f03fcee57", "incr": "c"}
#
# A sample response from /slumbot/api/new_hand or /slumbot/api/act:
#   {'old_action': '', 'action': 'b200', 'client_pos': 0, 'hole_cards': ['Ac', '9d'], 'board': [], 'token': 'a2f42f44-7ff6-40dd-906b-4c2f03fcee57'}
#
# Note that if the bot is first to act, then the response to /slumbot/api/new_hand will contain the
# bot's initial action.
#
# A token should be passed into every request.  With the exception that on the initial request to
# /slumbot/api/new_hand, the token may be missing.  But all subsequent requests should contain a
# token.  The token can in theory change over the course of a session (usually only if there is a
# long pause) so always check if there is a new token in a response and use it going forward.
#
...
#
# A sample curl command you can use to verify your connection to the server is:
#
#   curl -i -X POST -d '{}' -H "Content-Type: application/json" https://slumbot.com/slumbot/api/new_hand
...
def PlayHand(token):
    r = NewHand(token)
    # We may get a new token back from /slumbot/api/new_hand
...
# Note that you need to register your username on the website before you can log in via the API.
def Login(username, password):
    ...
    token = r.get('token')
    if not token:
        print('Did not get token in response to /slumbot/api/login')
        sys.exit(-1)
    return token
