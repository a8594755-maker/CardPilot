...
def test_phase1_preregistered_schedule_and_identical_qualified_collector():
    import run_distillation as run
    assert run.EPOCHS == 4 and run.TEACHERS == 2
...
def test_real_parent_admission_after_hash_freeze():
    import run_distillation as run
    assert isinstance(run.FINAL_SHA, str) and isinstance(run.REVIEW_SHA, str)
...
def test_response_admission_uses_primary_ci_and_exact_final_not_other_checkpoints():
    ...
    report = dict(status='PASS', decision='ADMIT_SEPARATE_AVERAGE_UPDATE',
        training_health={'checkpoint_sha256':digest}, new_training_hands=1048576,
        evaluation_hands=196608, primary={'ci95':[1,5]}, slumbot_hands=0,
...
def test_two_teacher_independent_seat_sampling_preserves_deck_stream():
    rows = [hand_spec(2026101501, i, 2) for i in range(4096)]
    counts = {pair:0 for pair in [(0,0),(0,1),(1,0),(1,1)]}
    for index, (deck, teachers) in enumerate(rows):
        assert deck == hand_spec(2026101501, index, 17)[0]
        counts[tuple(teachers)] += 1
    assert all(850 < v < 1200 for v in counts.values())
