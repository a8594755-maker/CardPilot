...
def main():
    ...
    baseline, endpoint = [np.load(BASE/name) for name in ('validation_source_ce.npy', 'validation_epoch04_ce.npy')]
    ...
    report = dict(status='PASS', decision=decision, new_training_hands=run.TRAIN_HANDS,
        ...
        interpretation='Phase1 behavior average fit of the prior and separately learned response. No reward training in this record, exact realization equivalence, equilibrium guarantee or measured strength gain.',
        wall_time_seconds=execution['wall_time_seconds'])
    run.write('reviewed_analysis.json', report)
    (BASE/'result_summary.md').write_text(f'# Phase1 learned average update\n\n{decision}\n\n'
        f'262144new native200bb training-data hands;8192separate supervised-validation hands;0Slumbot/strength hands. '
        f'2whole-hand teachers at half/half,262144reservoir decisions,4epochs/1024Adam steps.\n\n'
    ...
    subprocess.run([sys.executable, str(run.ROOT/'research/experiment_log.py'), 'finish', BASE.name,
        '--status', 'COMPLETED', '--summary', f'Phase1 average-policy fit completed262144training-data hands and8192supervised validation hands: {decision}.',
        '--conclusion', report['interpretation'], '--decision', decision,
        '--next-step', 'Freeze epoch4 for separate average assessment and next response-phase planning; no external or100k admission.' if ci[0] > 0 else 'Analyze failed phase1 average fit before the next response phase.',
