...
from alpha_holdem.train_mp3_hybrid_h1 import (
    source_greedy_public_risk_weights,
...
def test_public_risk_weights_recover_pot_fraction_and_default_is_exact():
    extras = torch.tensor([
        [0.95, 0.95, 0.0],
        [0.50, 0.50, 1.0],
    ])
    default = source_greedy_public_risk_weights(extras)
    weighted = source_greedy_public_risk_weights(
        extras,
        max_weight=5.0,
        full_weight_pot_fraction=0.20,
    )
    assert torch.equal(default, torch.ones(2))
    assert torch.allclose(weighted, torch.tensor([3.0, 5.0]))


def test_risk_weighting_changes_relative_margin_credit_not_loss_scale():
    logits = torch.tensor([
        [0.0, 0.0],
        [-0.2, 0.0],
    ], requires_grad=True)
    ...
    unweighted = source_greedy_margin_preservation_loss(
        *common,
        max_margin=0.1,
        release_advantage=1.0,
    )[0]
    weighted = source_greedy_margin_preservation_loss(
        *common,
        max_margin=0.1,
        release_advantage=1.0,
        row_weights=torch.tensor([1.0, 4.0]),
    )[0]
    assert torch.isclose(unweighted, torch.tensor(0.2))
    assert torch.isclose(weighted, torch.tensor(0.26))


def test_risk_scaled_release_requires_proportionally_stronger_evidence():
    logits = torch.tensor([[0.0, 0.1], [0.0, 0.1]], requires_grad=True)
    reference = torch.tensor([[0.5, 0.0], [0.5, 0.0]])
    loss, eligible, released, violations = source_greedy_margin_preservation_loss(
        ...
        release_advantage=1.0,
        row_weights=torch.tensor([1.0, 4.0]),
        scale_release_by_row_weight=True,
    )
    assert released.item() == 1
    assert eligible.item() == violations.item() == 1
    assert torch.isclose(loss, torch.tensor(0.2))


def test_risk_scaled_release_validation_requires_weights():
    logits = torch.tensor([[0.0, 0.1]], requires_grad=True)
