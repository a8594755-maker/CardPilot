...
def main():
    ...
    assert not any(Path(a).name in ('train_v5.py','run_diagnostic.py','run_pilot.py','review_finish.py') for p in psutil.process_iter(['pid','name','cmdline']) if p.pid!=os.getpid() for a in p.info['cmdline'] or [])
