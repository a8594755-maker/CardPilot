"""Independent raw review and same-record finish input for recovery fresh5k."""
import json
...
def main():
    ...
    for index in range(1, 9):
        ...
        assert all(row["policy_mode"] == "greedy" and row["policy_temperature"] == 0 and row["policy_seed"] == 2026137100 + index for row in rows)
        for row in rows:
    ...
    (BASE / "result_summary.md").write_text(
        "# Shallow matched-control greedy fresh5k recovery result\n\n"
        ...
        f"{statistics['positive_sessions']}/8. All actions passed the journal audit. "
