"""Evaluate only the new treatment against the prior audited common-deck control."""
...
EXPECTED_TREATMENT_SHA = "b3d2166eae7c7c3621b75c00688ae0c9ee8efe4b3c9323aa481a8038deb158c4"
ANCHORS = {
...
def main():
    ...
    for name, anchor in ANCHORS.items():
        ...
        rows.append({
            ...
            "prior_control_pairs_path": str(control_path),
            "prior_control_pairs_sha256": sha256(control_path),
            "treatment_pairs_path": str(treatment_path),
            "treatment_pairs_sha256": sha256(treatment_path),
        })
    pooled = np.asarray(pooled, dtype=np.float64)
    mean, interval = stats(pooled)
    report = {
        "schema": "cardpilot.source_greedy_preservation_smoke_delta.v1",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "seed": SEED, "pairs_per_anchor": PAIRS, "policy_mode": "greedy",
        "control_sha256": EXPECTED_CONTROL_SHA,
        "treatment_sha256": EXPECTED_TREATMENT_SHA,
        "anchors": rows,
        "pooled": {
            "pairs": len(pooled), "treatment_minus_control_bb100": mean,
            "paired_ci95": interval,
