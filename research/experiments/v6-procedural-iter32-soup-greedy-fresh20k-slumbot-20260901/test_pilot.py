"""Admission tests for the fixed fresh20k journaled cohort."""
...
os.environ["JOURNAL_TEST_OUTPUT"] = str(
    Path(tempfile.gettempdir()) / f"cardpilot_procedural_soup_fresh20k_{uuid.uuid4().hex}"
)
...
def test_fixed_confirmation_statistics_gate():
    from pilot_stats import summarize

    passing = summarize([[10] * 2500 for _ in range(8)])
    negative = summarize([[-10] * 2500 for _ in range(8)])
    assert passing["supports_separate_fresh100k"]
    assert not negative["supports_separate_fresh100k"]
