...
def validate_decision(row, response, rng, model, policy_mode):
    ...
    if model is not None:
        _, info = external_decision(model, response, uniform=uniform, device='cpu', policy_mode=policy_mode)
    ...
    for event in events[1:]:
        ...
        if kind == 'hand_start':
            ...
            if expected_method == 'new_hand':
                ...
                validate_decision(event.get('decision'), last_response, rng, model, start['policy_mode'])
                decision_count += 1
...
def audit_sessions(directories, *, model=None, expected_model_sha256=None):
    results = [audit_session(p, model=model, expected_model_sha256=expected_model_sha256) for p in directories]
...
def main():
    ...
    result = (audit_sessions(args.session_dir, model=model, expected_model_sha256=expected) if len(args.session_dir) > 1 else
