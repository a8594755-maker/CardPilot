...
def independent_estimate(groups):
    ...
    return dict(bb_per_100=mean,raw_hand_ci95=[mean-1.96*se,mean+1.96*se],raw_hand_se=se,
        ...
        session_t7_ci95=[group_mean-2.3646242510102993*group_se,group_mean+2.3646242510102993*group_se],
        session_t7_ci95_high_precision=[group_mean-2.3646242515927853*group_se,group_mean+2.3646242515927853*group_se])
...
def main():
    ...
    assert audit['model_sha256']==run.SOURCE_SHA and audit['token_chains_disjoint']
    probe_audit=run.read(run.READY/'combined_audit.json')
    probe_tokens={h for s in probe_audit['results'] for h in s['token_sha256']}
    seen=set()
    groups=[]
    observed=[]
    for i,item in enumerate(audit['results'],1):
        ...
        assert item['session_id']==f'v6_source_fresh20k_20260831_s{i:02d}' and item['policy_seed']==2026092900+i
        ...
        assert tokens and not (tokens&seen) and not (tokens&probe_tokens)
        seen.update(tokens)
        assert summary['status']=='COMPLETED' and summary['successful_hands']==2500 and summary['frozen_identity_verified']
        assert summary['model_sha256']==run.SOURCE_SHA and summary['policy_mode']=='sample' and summary['policy_temperature']==1
        ...
        with (directory/'hands.jsonl').open() as handle:
            for index,line in enumerate(handle,1):
                ...
                assert row['model_sha256']==run.SOURCE_SHA and row['policy_seed']==item['policy_seed']
    ...
    supports=result['bb_per_100']>0 and result['raw_hand_ci95'][0]>0 and result['session_t7_ci95'][0]>0
    # Retain the immutable preregistered constant, and independently ensure its
    # 5.825e-10 approximation error cannot change a zero-boundary decision.
    assert (result['session_t7_ci95'][0]>0)==(result['session_t7_ci95_high_precision'][0]>0), 'Numerically ambiguous zero boundary'
    ...
    decision='SUPPORTS_SEPARATE_100K_CONFIRMATION' if supports else 'SOURCE_BASELINE_NOT_POSITIVE_CONFIDENTLY'
    assert writer['decision']==decision
    report=dict(status='PASS',reviewed_at=datetime.now(timezone.utc).isoformat(),decision=decision,
        model_sha256=run.SOURCE_SHA,new_training_hands=0,evaluation_hands=20000,slumbot_hands=20000,
        statistics=result,supports_separate_100k_confirmation=supports,qualification_admitted=False,goal_achieved=False,
        probe_hands_pooled=0,old_baseline_hands_pooled=0,probe_token_chains_disjoint=True,server_rng_independence_proven=False,
        clients_exited_normally=8,full_model_session_audit_exit=0,sessions=observed,
        protected_matrix_source_pairs_verified=77,client_runtime_source_pairs_verified=74)
    ...
    text='# Corrected-v6 source fresh20k Slumbot baseline\n\n'
    text+=f'Decision: {decision}. Exactly20,000fresh hands in8complete2500hand sessions.\n\n'
    text+=f'Frozen source: {run.SOURCE_SHA}.\n\nbb/100: {result["bb_per_100"]:+.4f}; raw-hand95%CI: {result["raw_hand_ci95"]}; session-t7 95%CI: {result["session_t7_ci95"]}.\n\n'
    text+='All clients exited0. Complete journal/terminal/server-counter/frozen-model replay audit and independent raw arithmetic passed. Token chains are disjoint across all8sessions and the excluded16hand probe. No proof of server RNG independence is claimed. No probe,internal,or old-contract hands are pooled.\n\n'
    text+='This is source calibration,not a100k Goal result or a promotion of any pending/failed learned candidate. No automatic extension or policy substitution. The existing league matrix and77protected source/copy pairs remain unchanged.\n'
    ...
    subprocess.run([sys.executable,str(ROOT/'research/experiment_log.py'),'finish',BASE.name,'--status','COMPLETED',
        '--summary',f'Exactly20k fresh corrected-v6 source hands: {result["bb_per_100"]:+.4f}bb/100; raw95%CI{result["raw_hand_ci95"]}; complete evidence PASS.',
        '--conclusion','Independent raw and session-cluster analysis of one immutable v6 source; no probe/old-contract pooling or100k completion.',
        '--decision',decision,'--next-step',
        'Preregister independent fresh100k confirmation of this exact source; do not pool baseline/probe hands.' if supports else
        'Use this corrected-v6 external baseline to interpret the ongoing fixed league matrix,then select the next evidence-driven learned-weight experiment.',
