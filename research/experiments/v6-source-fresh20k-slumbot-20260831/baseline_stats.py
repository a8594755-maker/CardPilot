"""Raw chip arithmetic for the fixed equal-eight-session baseline."""
import math
import statistics
...
def summarize(sessions,hands_per_session=2500):
    ...
    if any(type(v) is not int or abs(v)>20000 for v in chips): raise ValueError('Invalid terminal chips')
    if len(chips)<2: raise ValueError('Insufficient hands')
    ...
    if not math.isclose(mean,group_mean,rel_tol=1e-12,abs_tol=1e-12): raise ValueError('Unequal session weighting')
    raw_ci=[mean-1.96*raw_se,mean+1.96*raw_se]
    group_ci=[group_mean-T7*group_se,group_mean+T7*group_se]
    return dict(hands=len(chips),sessions=8,bb_per_100=mean,raw_hand_se=raw_se,raw_hand_ci95=raw_ci,
                session_means_bb_per_100=means,session_se=group_se,session_t7_ci95=group_ci,
                supports_separate_100k_confirmation=mean>0 and raw_ci[0]>0 and group_ci[0]>0,
