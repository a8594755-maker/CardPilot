...
describe('P0.7: Variant scaffolding', () => {
  ...
  it('double-board hand resolves with two boards while keeping runCount=1', () => {
    ...
    assert.ok(
      sr!.doubleBoardPayouts && sr!.doubleBoardPayouts.length === 2,
      'double-board payouts should be captured',
    );
...
describe('P0.2: Run It Twice', () => {
  it('two boards are dealt from same deck (no reshuffle)', () => {
    ...
    assert.equal(
      uniqueCards.size,
      allCards.length,
      'all dealt cards must be unique (no duplicates)',
    );
  });
  ...
  it('does not enter RUN_IT_TWICE_PROMPT when all-in happens on the river', () => {
    ...
    assert.notEqual(
      s.street,
      'RUN_IT_TWICE_PROMPT',
      'river all-in closure must not prompt run-it-twice',
    );
    assert.equal(
      t.isRunoutPending(),
      true,
      'river all-in closure should proceed directly to showdown runout path',
    );
  });
});
...
describe('P0.3: Showdown visibility', () => {
  ...
  it('all-in runout uses showdown decision phase (not forced reveal)', () => {
    ...
    assert.equal(
      s.showdownPhase,
      'decision',
      'all-in runout should defer to showdown decision phase',
    );
  });
  ...
  it('river called showdown still forces all contenders to reveal', () => {
    ...
    s = t.getPublicState();
    assert.ok(
      s.actorSeat !== null && s.legalActions?.canCall,
      'opponent should be able to call on river',
    );
    ...
    for (const seat of contenders) {
      assert.ok(
        s.revealedHoles?.[seat],
        `contender seat ${seat} should be revealed after river call`,
      );
    }
  });
  ...
  it('finalizeShowdownReveals auto-mucks losers and reveals winners', () => {
    ...
    for (const w of winners) {
      assert.ok(s.revealedHoles?.[w.seat], `winner seat ${w.seat} should have revealed cards`);
    }
  });
});
