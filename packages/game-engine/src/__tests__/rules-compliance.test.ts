...
function blindSeat(
  s: ReturnType<GameTable['getPublicState']>,
  type: 'post_sb' | 'post_bb',
): number {
...
describe('Re-raise eligibility regressions', () => {
  ...
  it('open -> BB 3-bet -> opener can 4-bet', () => {
    ...
    assert.equal(
      ...
      true,
      'opener should be able to 4-bet after a full 3-bet',
    );
  });
...
describe('Full Raise Rule: short all-in does NOT reopen betting', () => {
  it('classic scenario: Raise → Call → Short all-in → original raiser CANNOT re-raise', () => {
    ...
    assert.equal(
      s.actorSeat,
      raiserSeat,
      'action should return to original raiser after short all-in',
    );
    assert.equal(
      s.legalActions?.canRaise,
      false,
    ...
    assert.equal(
      s.legalActions?.canCall,
      true,
      'player should still be able to call the short all-in',
    );
    ...
    if (secondPlayer.stack + secondPlayer.streetCommitted <= 200) {
      ...
      if (s.actorSeat !== null) {
        ...
        assert.equal(
          la!.canRaise,
          true,
          'player should be able to re-raise after full all-in raise',
        );
      }
    } else {
      ...
      if (thirdPlayer.stack + thirdPlayer.streetCommitted <= 200) {
        ...
        if (s.actorSeat !== null) {
          ...
          assert.equal(
            la!.canRaise,
            true,
            'original raiser should be able to re-raise after full all-in',
          );
        }
      }
    }
  ...
  it('cumulative short all-ins reopen betting once full raise amount is reached', () => {
    ...
    assert.equal(
      shortOne.stack + shortOne.streetCommitted,
      35,
      'first short stack should have total 35 chips',
    );
    ...
    assert.equal(
      shortTwo.stack + shortTwo.streetCommitted,
      50,
      'second short stack should have total 50 chips',
    );
  ...
  it('conservation holds across 10 hands with short all-ins', () => {
    ...
    for (let hand = 0; hand < 10; hand++) {
      const eligible = t
        .getPublicState()
        .players.filter((p) => p.stack > 0 && p.status === 'active');
  ...
  it('run-it-twice split is deterministic: run1 gets ceil(totalPaid/2), run2 gets floor(totalPaid/2)', () => {
    ...
    const run1Total = Object.values(sr!.payoutsBySeatByRun![0]).reduce(
      (sum, amount) => sum + amount,
      0,
    );
    const run2Total = Object.values(sr!.payoutsBySeatByRun![1]).reduce(
      (sum, amount) => sum + amount,
      0,
    );
...
describe('New Player Blind Policies', () => {
  ...
  it('postDeadBlind deducts chips and carries dead blind into next hand pot', () => {
    ...
    assert.equal(
      s.actions.some((a) => a.seat === 3 && a.type === 'post_dead_blind' && a.amount === 10),
      true,
    );
    assert.equal(contrib.get(3), 15, 'seat 3 should have dead blind + live SB in contribution map');
    assert.equal(
      seat3.streetCommitted,
      5,
      'dead blind should be non-live and not count toward streetCommitted',
    );
  });
  ...
  it('run-it-twice odd chip goes to Run #1', () => {
    for (let trial = 0; trial < 10; trial++) {
      ...
      if (sr && sr.runCount === 2 && sr.payoutsBySeatByRun) {
        ...
        assert.ok(
          r1Total >= r2Total,
          `Run 1 (${r1Total}) should get >= Run 2 (${r2Total}) [odd chip to run 1]`,
        );
      }
      t.clearHand();
    }
...
describe('sit out excludes player from dealing', () => {
  ...
  it('sit in allows player to be dealt next hand', () => {
    ...
    const s = t.getPublicState();
    assert.equal(
      s.players.find((p) => p.seat === 3)!.inHand,
      true,
      're-activated player should be dealt in',
    );
  });
...
describe('host self-rebuy via addStack', () => {
  it('addStack credits chips between hands (not mid-hand)', () => {
    ...
    const s = t.getPublicState();
    assert.equal(
      s.players.find((p) => p.seat === 1)!.stack,
      1000,
      'stack should increase by rebuy amount',
    );
  });
  ...
  it('addStack does not deadlock when host rebuys themselves', () => {
    ...
    const s = t.getPublicState();
    assert.equal(
      s.players.find((p) => p.seat === 1)!.inHand,
      true,
      'host should be dealt in after rebuy',
    );
