...
function estimateBtnCalling3bet(hand: string, stackBB: number): number {
  ...
  // Pairs
  if (h.pair) {
    if (high >= 12) return 0.50; // AA: mix call/4-bet
    if (high >= 11) return 0.60; // KK: mix call/4-bet
    if (high >= 10) return 0.90; // QQ: mostly call
    if (high >= 9)  return 0.85; // JJ: mostly call
    if (high >= 8)  return 0.75; // TT: call
    if (high >= 7)  return 0.50; // 99: mix call/fold
  ...
  // Suited hands
  if (h.suited) {
    if (high === 12) { // Ace-x suited
      if (low >= 10) return 0.90; // AKs, AQs, AJs: strong call
      if (low >= 8)  return 0.60; // ATs, A9s: mixed
      if (low <= 5)  return 0.35; // A2s-A5s: blocker + nut flush draw
      return 0.20; // A6s-A8s: marginal
    }
    if (high === 11 && low >= 10) return 0.70; // KQs: call
    if (high === 11 && low >= 9)  return 0.40; // KJs: mixed
    // Suited connectors (implied odds in 3-bet pots)
    if (isConnected(hand) && low >= 5) return 0.30; // 76s-T9s
    if (isOneGap(hand) && low >= 5)   return 0.15; // 86s, 97s, T8s
    return 0;
  }

  // Offsuit hands
  if (high === 12) { // Ace-x offsuit
    if (low >= 11) return 0.85; // AKo: strong call
    if (low >= 10) return 0.45; // AQo: mixed
    if (low >= 9)  return 0.15; // AJo: mostly fold
    return 0;
  }
  if (high === 11 && low >= 10) return 0.20; // KQo: sometimes call
...
function estimateSbColdCall(hand: string): number {
  ...
  if (h.pair) {
    ...
    if (high >= 5)  return 0.60; // 55-99: cold-call for set mining
    return 0.30; // 22-44: sometimes cold-call
  }

  if (h.suited) {
    if (high === 12) { // Axs
      if (low >= 11) return 0;     // AKs: should 3-bet
      if (low >= 9)  return 0.40;  // ATs-AQs: mixed 3-bet/call
      return 0.50; // A2s-A9s: good cold-call (nut flush potential)
    }
    if (high === 11 && low >= 9) return 0.40; // KJs, KQs: mixed
    ...
    if (high === 10 && low >= 8) return 0.30; // T9s, T8s: playable
    return 0;
  }

  // Offsuit: very few cold-calls from SB
  if (high === 12 && low >= 10) return 0.15; // AQo, AJo, ATo: sometimes
  if (high === 11 && low >= 10) return 0.10; // KQo, KJo: rarely
  return 0;
}
...
function estimateBtnColdCallVsCo(hand: string): number {
  ...
  if (h.pair) {
    ...
    if (high >= 10) return 0.40; // QQ, TT-JJ: mix call/3-bet
    if (high >= 4)  return 0.70; // 44-99: flat call
  ...
  if (h.suited) {
    if (high === 12) { // Axs
      ...
      if (low >= 9)  return 0.50; // ATs-AQs: mix
      return 0.55; // A2s-A9s: cold-call (suited playability)
    }
    if (high === 11) {
      if (low >= 9) return 0.50; // KJs, KQs: mix
      if (low >= 7) return 0.35; // K8s-KTs
      return 0;
    }
    if (high === 10 && low >= 7) return 0.45; // T8s-TJs
    if (isConnected(hand) && low >= 3) return 0.60; // 54s-T9s
    if (isOneGap(hand) && low >= 4)   return 0.40; // 75s, 86s, 97s
    if (isTwoGap(hand) && low >= 5)   return 0.20; // 85s, 96s, T7s
    return 0;
  }
...
export function adjustBBMultiWayDefense(
  ...
  return entries.map(entry => {
    ...
    if (isPair(hand) && lowRank(hand) <= 4) {
      boost = 0.40 * boostFactor; // 22-55: strong boost
...
export type EstimatedSpot =
  | 'BTN_vs_BB_facing_3bet'
  | 'SB_vs_BTN_cold_call'
  | 'BTN_vs_CO_cold_call';
 ...
 */
export function generateEstimatedRange(
  spot: EstimatedSpot,
  stackBB = 100,
): GtoWizardRangeEntry[] {
