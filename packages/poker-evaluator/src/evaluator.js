...
import { RANKS, parseCard } from "./card-utils.js";
import { HandRank, HAND_RANK_NAMES } from "./hand-rank.js";
/**
 * 從 7 張牌中找出最強的 5 張組合
 */
...
function evaluate5CardHand(cards) {
    ...
    if (isFlushHand && isStraightHand.isStraight) {
        const rank = isStraightHand.highCard === 'A'
    ...
    if (trips) {
        const pair = findOfAKind(parsed.filter(c => c.rank !== trips.rank), 2);
        if (pair) {
            return {
    ...
    if (pair1) {
        const pair2 = findOfAKind(parsed.filter(c => c.rank !== pair1.rank), 2);
        if (pair2) {
...
function isStraight(cards) {
    ...
    if (uniqueRanks.includes(0) && uniqueRanks.includes(9) &&
        uniqueRanks.includes(10) && uniqueRanks.includes(11) && uniqueRanks.includes(12)) {
...
function findOfAKind(cards, count) {
    const counts = {};
    for (const c of cards) {
...
function calculateValue(rank, ranks) {
