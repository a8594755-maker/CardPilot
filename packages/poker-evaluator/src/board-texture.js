...
import { parseCard, RANKS } from "./card-utils.js";
/**
...
export function analyzeBoardTexture(board) {
    ...
    let straightDraws = 0;
    if (gaps.some(g => g === 1))
        straightDraws += 2; // Connected cards
    if (gaps.some(g => g === 2))
        straightDraws += 1; // One-gap
    // High card
    ...
    if (highCardValue <= 3)
        wetness -= 0.1; // High cards = less draws
 ...
 */
export function classifyHandOnBoard(heroHand, board) {
