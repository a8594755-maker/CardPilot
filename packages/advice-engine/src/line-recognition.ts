...
export function detectPreflopAggressor(actions: HandAction[]): PfaDetection {
  const preflopActions = filterStreet(actions, 'PREFLOP');
  const raises = preflopActions.filter((a) => a.type === 'raise' || a.type === 'all_in');
...
function detectPreflopLines(
  ...
  const allRaises = preflopActions.filter((a) => a.type === 'raise' || a.type === 'all_in');
  ...
  const heroRaises = heroActions.filter((a) => a.type === 'raise' || a.type === 'all_in');

  for (const heroRaise of heroRaises) {
    const priorRaisesAll = allRaises.filter((r) => r.at < heroRaise.at);
    ...
    if (priorRaisesAll.length === 1) {
      ...
      const coldCalls = preflopActions.filter(
        (a) =>
          a.type === 'call' && a.seat !== heroSeat && a.at > openRaise.at && a.at < heroRaise.at,
      );
...
function detectPostflopLines(input: {
  ...
  const { streetActions, allActions, heroSeat, pfaSeat, street, tags } = input;
  ...
  if (heroIsPfa && street === 'FLOP') {
    const heroBets = heroActions.filter((a) => a.type === 'raise' || a.type === 'all_in');
  ...
  if (heroIsPfa && street === 'TURN') {
    ...
    const heroFlopBets = heroFlopActions.filter((a) => a.type === 'raise' || a.type === 'all_in');

    if (heroFlopChecks.length > 0 && heroFlopBets.length === 0) {
      const heroTurnBets = heroActions.filter((a) => a.type === 'raise' || a.type === 'all_in');
  ...
  if (street === 'TURN' && !heroIsPfa) {
    const flopActions = filterStreet(allActions, 'FLOP');
    const heroFlopCalls = flopActions.filter((a) => a.seat === heroSeat && a.type === 'call');
  ...
  for (const action of heroActions) {
    if ((action.type === 'raise' || action.type === 'all_in') && action.amount > 0) {
  ...
  if (street === 'RIVER' && !heroIsPfa) {
    const heroRiverBets = heroActions.filter((a) => a.type === 'raise' || a.type === 'all_in');
    if (heroRiverBets.length > 0) {
      const flopActions = filterStreet(allActions, 'FLOP');
      const turnActions = filterStreet(allActions, 'TURN');
      const heroFlopChecks = flopActions.filter((a) => a.seat === heroSeat && a.type === 'check');
      const heroTurnChecks = turnActions.filter((a) => a.seat === heroSeat && a.type === 'check');
      const heroFlopBets = flopActions.filter(
        (a) => a.seat === heroSeat && (a.type === 'raise' || a.type === 'all_in'),
      );
      ...
      if (
        ...
        (turnActions.length === 0 || (heroTurnChecks.length > 0 && heroTurnBets.length === 0))
      ) {
        tags.add('LEAD_RIVER');
      }
    }
...
function estimatePotAtAction(allActions: HandAction[], targetAction: HandAction): number {
  let pot = 0;
