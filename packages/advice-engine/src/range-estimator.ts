...
import type { Street, PlayerActionType } from '@cardpilot/shared-types';
...
export class RangeEstimator {
  ...
  narrowRangePostflop(
    ...
    for (const [hand, weight] of currentRange) {
      ...
      if (betSize && betSize > 0.75 && action === 'raise') {
        const isNutted = this.isLikelyNutted(hand);
  ...
  private getOpeningRange(position: string): HandRange {
    ...
    const freq = openingFrequencies[position] || 0.2;
  ...
  private get3BetRange(_position: string, _facing: string): HandRange {
    const range = new Map<string, number>();
    const allHands = this.getAllHands();
    const sortedHands = this.sortHandsByStrength(allHands);
  ...
  private getCallingRange(_position: string, _facing: string): HandRange {
    const range = new Map<string, number>();
    const allHands = this.getAllHands();
    const sortedHands = this.sortHandsByStrength(allHands);

    // Calling range: medium strength, speculative hands
    const startCutoff = Math.floor(sortedHands.length * 0.1);
  ...
  private isLikelyNutted(hand: string): boolean {
