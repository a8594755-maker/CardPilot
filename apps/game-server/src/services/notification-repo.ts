...
export class NotificationRepo {
  ...
  constructor() {
    ...
    logInfo({
      ...
      message: this.db
        ? 'NotificationRepo connected to Supabase'
        : `NotificationRepo running in offline mode${disabled ? ' (DISABLED via env)' : ' (no Supabase)'}`,
    });
  }
  ...
  async list(
    ...
    let query = this.db
      .from('notifications')
      .select(
        'id, user_id, type, club_id, ref_id, title, body, metadata, is_read, read_at, created_at',
      )
      .eq('user_id', userId);
