...
export class ChatRepo {
  ...
  constructor() {
    ...
    logInfo({
      ...
      message: this.db
        ? 'ChatRepo connected to Supabase'
        : `ChatRepo running in offline mode${disabled ? ' (DISABLED via env)' : ' (no Supabase)'}`,
    });
  }
  ...
  async getHistory(
    ...
    let query = this.db
      .from('chat_messages')
      .select(
        'id, club_id, table_id, sender_user_id, sender_display_name, message_type, content, mentions, deleted_at, deleted_by, created_at',
      )
      .eq('club_id', clubId)
      .is('deleted_at', null)
  ...
  async getUnreads(clubId: string, userId: string): Promise<ChatUnreadCount[]> {
    ...
    const settled = await Promise.all(
      countTasks.map(async (task) => {
        ...
        if (error) {
          logWarn({
            event: 'chat_repo.getUnreads.count_failed',
