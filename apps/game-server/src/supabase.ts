...
export class SupabasePersistence {
  ...
  constructor() {
    ...
    console.log(
      `[supabase] admin=${!!this.admin} authClient=${!!this.authClient} url=${url ? 'set' : 'missing'} anonKey=${anonKey ? 'set' : 'missing'}${disabled ? ' (DISABLED via env)' : ''}`,
    );
  }
  ...
  async findRoomByCode(roomCode: string): Promise<RoomRecord | null> {
    ...
    const { data, error } = await this.admin
      .from('live_tables')
      .select(
        'id, room_code, room_name, status, max_players, small_blind, big_blind, is_public, updated_at, created_by',
      )
  ...
  async listLobbyRooms(limit = 30): Promise<LobbyRoomSummary[]> {
    ...
    const { data: rows, error } = await this.admin
      .from('live_tables')
      .select(
        'id, room_code, room_name, status, max_players, small_blind, big_blind, is_public, updated_at',
      )
      .eq('status', 'OPEN')
      .eq('is_public', true)
  ...
  async closeStaleRooms(activeTableIds: Set<string>): Promise<number> {
    ...
    const staleIds = rows.map((r) => String(r.id)).filter((id) => !activeTableIds.has(id));
  ...
  async recordHandHistory(payload: PersistHandHistoryPayload): Promise<{
    inserted: boolean;
    handNo: number;
  ...
  async listHistoryHands(
    userId: string,
    roomSessionId: string,
    params?: { limit?: number; beforeEndedAt?: string },
  ): Promise<{ hands: HistoryHandSummary[]; hasMore: boolean; nextCursor?: string }> {
  ...
  async listHandsByRoom(roomId: string, limit = 100): Promise<HistoryHandSummary[]> {
    ...
    const { data, error } = await this.admin
      .from('hand_histories')
      .select(
        'id, room_id, room_session_id, hand_id, hand_no, ended_at, blinds_json, players_summary_json, summary_json',
      )
      .eq('room_id', roomId)
  ...
  async getHistoryHandDetail(
    userId: string,
    handHistoryId: string,
  ): Promise<HistoryHandDetail | null> {
...
function toHistoryPlayers(value: unknown): HistoryHandPlayerSummary[] {
  ...
  return value
    ...
    .filter(
      (row): row is HistoryHandPlayerSummary =>
        row !== null && row.seat > 0 && row.userId.length > 0,
    );
}

function toHistoryWinners(
  value: unknown,
...
function toHistorySummary(value: unknown): HistoryHandSummaryCore {
  const src = typeof value === 'object' && value !== null ? (value as Record<string, unknown>) : {};
  const flagsSource =
    typeof src.flags === 'object' && src.flags !== null
      ? (src.flags as Record<string, unknown>)
      : {};
  const myNetSource =
    typeof src.my_net_by_user === 'object' && src.my_net_by_user !== null
  ...
  const netByPositionSource =
    typeof src.net_by_position === 'object' && src.net_by_position !== null
  ...
  const bucketSource =
    typeof src.starting_hand_buckets_by_user === 'object' &&
    src.starting_hand_buckets_by_user !== null
...
function toHistoryDetail(value: unknown): HistoryHandDetailCore {
  const src = typeof value === 'object' && value !== null ? (value as Record<string, unknown>) : {};
  const board = Array.isArray(src.board)
    ? src.board.filter((card): card is string => typeof card === 'string')
    : [];
  const runoutBoards = Array.isArray(src.runoutBoards)
    ? src.runoutBoards.map((run) =>
        Array.isArray(run) ? run.filter((card): card is string => typeof card === 'string') : [],
      )
    : [];
  ...
  const potLayers = Array.isArray(src.potLayers)
    ? src.potLayers
        ...
        .filter(
          (layer): layer is { label: string; amount: number; eligibleSeats: number[] } =>
            layer !== null,
        )
    : [];

  const contributionsSource =
    typeof src.contributionsBySeat === 'object' && src.contributionsBySeat !== null
    ...
    ? src.actionTimeline
        ...
        .filter(
          (
            ...
            amount: number;
            at: number;
          } => action !== null && action.seat > 0,
        )
    : [];

  const revealedSource =
    typeof src.revealedHoles === 'object' && src.revealedHoles !== null
  ...
  const privateSource =
    typeof src.privateHoleCardsByUser === 'object' && src.privateHoleCardsByUser !== null
