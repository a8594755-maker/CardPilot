...
describe("Club auto-deal + deferred-leave contracts", () => {
  ...
  it("auto-schedules dealing when players sit", () => {
    assert.match(source, /const seatPlayerDirect = async[\s\S]*scheduleAutoDealIfNeeded\(params\.tableId\)/);
    assert.match(source, /socket\.on\("sit_down"[\s\S]*await seatPlayerDirect\(/);
  ...
  it("uses per-table minPlayersToStart in deal validation", () => {
    ...
    assert.match(source, /Auto-start skipped: need at least \$\{minPlayersToStart\} eligible players/);
  });
