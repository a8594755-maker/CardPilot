...
describe('Server round-flow regressions', () => {
  ...
  it('has runout/showdown fail-safe finalization paths to prevent stalled hands', () => {
    assert.match(
      source,
      /async function handleSequentialRunout\(tableId: string, table: GameTable\): Promise<void> \{[\s\S]*try \{/,
    );
    assert.match(source, /event: 'runout\.sequence\.failed'/);
    assert.match(
      source,
      /if \(state\.showdownPhase === 'decision'\) \{[\s\S]*settleShowdownDecision\(tableId, table, state\.handId\);/,
    );
    assert.match(source, /finalizeHandEnd\(tableId, state\);/);
    assert.match(
      source,
      /if \(state\.showdownPhase !== 'decision'\) \{[\s\S]*finalizeHandEnd\(tableId, state\);/,
    );
  });
