...
describe("Club wallet ledger", () => {
  it("balance equals SUM(ledger amounts) for a user", async () => {
    ...
    await repo.appendWalletTx({ clubId, userId, type: "deposit", amount: 1_000, createdBy: "admin-1" });
    ...
    await repo.appendWalletTx({ clubId, userId, type: "adjustment", amount: -50, createdBy: "admin-1" });
  ...
  it("deposit -> buy-in -> cash-out keeps balance correct", async () => {
    ...
    const d1 = await repo.appendWalletTx({ clubId, userId, type: "deposit", amount: 2_000, createdBy: "admin-1" });
    assert.ok(d1);
