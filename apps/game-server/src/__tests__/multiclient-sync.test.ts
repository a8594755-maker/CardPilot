...
function waitForSocketEvent<T>(
  socket: {
    on: (event: string, cb: (payload: T) => void) => void;
    off: (event: string, cb: (payload: T) => void) => void;
  },
  event: string,
...
async function connectClient(
  ...
  await waitForSocketEvent<{ socketId: string; userId: string }>(
    socket as unknown as {
      on: (event: string, cb: (payload: { socketId: string; userId: string }) => void) => void;
      off: (event: string, cb: (payload: { socketId: string; userId: string }) => void) => void;
    },
    'connected',
    () => true,
    8_000,
  ...
  it('keeps two clients in deterministic sync and hydrates reconnect with latest authoritative snapshot', async () => {
    ...
    try {
      ...
      const roomCreated = await waitForSocketEvent<{ tableId: string; roomCode: string }>(
        host as unknown as {
          on: (event: string, cb: (payload: { tableId: string; roomCode: string }) => void) => void;
          off: (
            event: string,
            cb: (payload: { tableId: string; roomCode: string }) => void,
          ) => void;
        },
        'room_created',
        () => true,
      ...
      await waitForSocketEvent<{ tableId: string }>(
        guest as unknown as {
          on: (event: string, cb: (payload: { tableId: string }) => void) => void;
          off: (event: string, cb: (payload: { tableId: string }) => void) => void;
        },
        'room_joined',
      ...
      assertMonotonicVersions(
        hostSnapshots.filter((snapshot) => snapshot.tableId === tableId),
        'host',
      );
      assertMonotonicVersions(
        guestSnapshots.filter((snapshot) => snapshot.tableId === tableId),
        'guest',
      );
      ...
      assert.ok(
        beforeActionSnapshot.actorSeat === 1 || beforeActionSnapshot.actorSeat === 2,
        'Actor seat should be one of the seated players',
      );
      ...
      const versionBeforeReconnect =
        latestSnapshotForTable(hostSnapshots, tableId)?.stateVersion ?? -1;
      ...
      guestRejoin.on('table_snapshot', (snapshot: TableState) =>
        guestRejoinSnapshots.push(snapshot),
      );

      guestRejoin.emit('join_room_code', { roomCode });
      await waitForSocketEvent<{ tableId: string }>(
        guestRejoin as unknown as {
          on: (event: string, cb: (payload: { tableId: string }) => void) => void;
          off: (event: string, cb: (payload: { tableId: string }) => void) => void;
        },
        'room_joined',
      ...
      const hydrated = await waitForSocketEvent<TableState>(
        guestRejoin as unknown as {
          on: (event: string, cb: (payload: TableState) => void) => void;
          off: (event: string, cb: (payload: TableState) => void) => void;
        },
        'table_snapshot',
      ...
      assert.equal(
        hydrated.handId,
        hostCurrentSnapshot.handId,
        'Rejoined client should hydrate current handId',
      );
      assert.equal(
        hydrated.street,
        hostCurrentSnapshot.street,
        'Rejoined client should hydrate current street',
      );
      assert.equal(
        hydrated.pot,
        hostCurrentSnapshot.pot,
        'Rejoined client should hydrate current pot',
      );

      assertMonotonicVersions(
        guestRejoinSnapshots.filter((snapshot) => snapshot.tableId === tableId),
        'guest-rejoin',
      );
    } finally {
      host?.disconnect();
