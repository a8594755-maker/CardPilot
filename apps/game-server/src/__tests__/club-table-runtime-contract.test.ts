...
describe('Club runtime contracts', () => {
  ...
  it('suspends idle club table runtime instead of keeping it open forever', () => {
    ...
    assert.match(
      source,
      /if \(isPersistentClubTable\(tableId\)\) \{[\s\S]*handleClubTableIdleSuspend\(tableId\)/,
    );
    // handleRoomAutoClose still guards against accidental full-close of club tables
    assert.match(
      source,
      /function handleRoomAutoClose\(tableId: string\): void \{[\s\S]*if \(isPersistentClubTable\(tableId\)\) \{[\s\S]*return;/,
    );
  });
  ...
  it('allows active club member to reopen closed club room by code', () => {
    assert.match(
      source,
      /if \(room\.status === 'CLOSED'\) \{[\s\S]*if \(clubInfo\) \{[\s\S]*clubManager\.isActiveMember\(clubInfo\.clubId, identity\.userId\)/,
    );
  });
  ...
  it('bypasses seat approval queue for club tables', () => {
    assert.match(
      source,
      /socket\.on\(\s*'seat_request'[\s\S]*if \(clubInfo\) \{[\s\S]*await seatPlayerDirect\(/,
    );
  });
  ...
  it('auto-approves rebuys for club tables', () => {
    assert.match(
      source,
      /const autoApprove =\s*!!clubInfo \|\|[\s\S]*roomManager\.isHostOrCoHost\(payload\.tableId, identity\.userId\)/,
    );
  });
  ...
  it('supports club_table_update with runtime safeguards', () => {
    ...
    assert.match(source, /roomManager\.updateSettings\(\s*serverTableId/);
  });
});
