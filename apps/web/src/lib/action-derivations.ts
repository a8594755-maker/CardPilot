...
export function deriveActionBar(gameState: TableState | null, playerId: string | null): DerivedActionBar {
  if (!gameState || !playerId) return { visibleActions: [], primaryAction: null };

  const player = gameState.players.find((p) => p.userId === playerId) ?? null;
  const actionable = Boolean(gameState.handId && isActionableStreet(gameState.street) && gameState.showdownPhase !== "decision");
  const isMyTurn = actionable && player && gameState.actorSeat != null && player.seat === gameState.actorSeat;
...
export function derivePreActionUI(gameState: TableState | null, playerId: string | null): DerivedPreActionUI {
  if (!gameState || !playerId) return { enabled: false, options: [] };

  const player = gameState.players.find((p) => p.userId === playerId) ?? null;
  const enabled = Boolean(gameState.handId && isActionableStreet(gameState.street) && gameState.showdownPhase !== "decision" && player);
...
export function shouldConfirmUnnecessaryFold(legal: LegalActions | null | undefined, suppressedThisSession: boolean): boolean {
