...
export function InfoCell({
  ...
  return (
    <div>
      <span className="cp-street-label text-[10px] text-slate-500 uppercase tracking-wider font-medium">
        {label}
      </span>
      <div
        className={`cp-action-label text-sm font-semibold cp-num ${highlight ? 'text-amber-400 uppercase' : cyan ? 'text-cyan-400' : 'text-white'}`}
      >
        {value}
      </div>
    </div>
...
export const SeatChip = memo(function SeatChip({
  player,
  seatNum,
  isActor,
  isMe,
  ...
  const _equityText = isAllInLocked
    ? equity
      ? `${Math.round(equity.equityRate * 100)}%`
      : '—'
    : 'Unlocked';
  ...
  const seatLabel = (
    <div
      className={`cp-seat-label relative z-10 ${heroReveal ? 'w-44 md:w-48 p-3' : 'w-52 md:w-60 p-5'} min-h-[4.5rem] rounded-xl text-center transition-all ${timerBorderClass} ${isWinner ? 'cp-seat-win' : ''} ${isWinnerPulse ? 'cp-seat-win-pop' : ''} ${
        isActor
          ? 'bg-amber-500/20 border-2 border-amber-400 shadow-[0_0_16px_rgba(245,158,11,0.3)]'
          : isMe
            ? 'bg-cyan-500/10 border-2 border-cyan-400/50'
    ...
    >
      ...
      {posLabel && (
        <div className="absolute -top-2 -left-1 z-20">
          <span className={`text-xs px-2.5 py-1 rounded-full font-bold uppercase ${posColor}`}>
            {posLabel}
          </span>
        </div>
      )}
      ...
      {(isOwner || isCoHost) && (
        <div className="absolute -top-2 -right-1 z-20">
          <span
            className={`text-[11px] px-1.5 py-0.5 rounded font-bold ${isOwner ? 'bg-amber-500 text-white' : 'bg-blue-500 text-white'}`}
          >
            {isOwner ? '👑' : '⭐'}
          </span>
        </div>
      ...
      <div className="flex items-center justify-center gap-2">
        <div className="cp-seat-stack text-2xl font-extrabold text-amber-400 cp-num">
          {fmt(player.stack)}
        </div>
      ...
      {player.status === 'sitting_out' && (
        <div className="cp-seat-status text-sm text-orange-400 font-bold uppercase">Sit Out</div>
      )}
      {player.folded && player.status !== 'sitting_out' && (
        <div className="cp-seat-status text-sm text-red-400 font-bold">FOLDED</div>
      )}
      {player.allIn && !equity && (
        <div className="cp-seat-status text-sm text-orange-400 font-bold">ALL-IN</div>
      )}
      {handHint && !player.folded && !revealedCards && (
        <div
          className="text-xs leading-tight text-cyan-200/90 max-w-[140px] truncate mx-auto"
          title={handHint}
        >
          {handHint}
        </div>
      ...
      {timer && (
        <div className="cp-timer-wrap">
          ...
          />
          <span
            className={`cp-seat-timer text-sm font-bold tabular-nums min-w-[3.5em] inline-block ${timerColor} ${timerUrgent ? 'animate-pulse' : ''}`}
          >
            {timer.usingTimeBank
              ? `⏱ ${String(Math.ceil(timer.timeBankRemaining)).padStart(2, '\u2007')}s`
              : `${String(Math.ceil(timer.remaining)).padStart(2, '\u2007')}s`}
          </span>
        </div>
      )}
      {pendingLeave && <div className="text-xs text-slate-300 italic">Leaving</div>}
    </div>
  );
  ...
  return (
    <div className="relative flex flex-col items-center gap-0.5">
      ...
      {netDelta !== undefined && netDelta !== 0 && (
        <div
          className={`cp-delta-tag ${netDelta > 0 ? 'cp-delta-tag--positive' : 'cp-delta-tag--negative'}`}
        >
          {netDelta > 0 ? '+' : ''}
          {fmtDelta(netDelta)}
        </div>
      ...
      {heroReveal ? (
        <div className="flex flex-col items-center gap-0.5">
          <div className="flex items-center gap-2">
            <button
              onClick={onClickRevealed}
              className="flex flex-col items-center gap-1 shrink-0"
              title="Tap to zoom revealed hand"
            >
              <PokerCard
                card={revealedCards![0]}
                variant="mini"
                className={isWinner ? 'cp-card-winner-highlight' : 'border-emerald-500/40'}
              />
              <PokerCard
          ...
          <div
            className={`text-sm font-semibold max-w-[200px] truncate ${isWinner ? 'text-amber-400' : 'text-emerald-300'}`}
          >
            {revealedHandName ?? 'Revealed'}
          </div>
        </div>
      ) : (
        <>
      ...
      {player.streetCommitted > 0 && !player.folded && (
        <div
          className={`cp-bet-pill bg-black/75 px-3 py-1 rounded-full text-base font-bold shadow-sm border ${actionBadgeClass}`}
        >
          <span className="cp-action-label uppercase text-sm tracking-wider mr-1">
            {actionLabel}
          </span>
