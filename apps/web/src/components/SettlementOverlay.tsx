...
export const SettlementOverlay = memo(function SettlementOverlay({
  ...
  const overlay = (
    <div
      ...
      style={{
        paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + var(--app-footer-h, 0px) + 12px)',
        paddingTop: 'calc(var(--topbar-h, 0px) + 12px)',
      }}
      onClick={onDismiss}
    >
      {/* Backdrop */}
      ...
      <div
        ...
        style={{
          maxHeight:
            'calc(100vh - var(--topbar-h, 0px) - var(--app-footer-h, 0px) - 24px - env(safe-area-inset-bottom, 0px))',
        }}
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden px-5 min-h-0">
          ...
          <div className="text-center mb-2">
            {autoStartScheduled && countdownSeconds > 0 ? (
              <span className="text-[11px] text-slate-400">
                Next hand in{' '}
                <span className="text-amber-400 font-bold text-sm">{countdownSeconds}</span>…
              </span>
            ) : showWaitingFallback ? (
              <span className="text-[11px] text-amber-300 animate-pulse">Waiting for server…</span>
            ) : autoStartBlockReason ? (
              <span className="text-[11px] text-amber-300">{autoStartBlockReason}</span>
            ) : (
              <span className="text-[10px] text-slate-500">Next hand starting soon…</span>
            )}
          </div>
          ...
          {/* Hand Summary Drawer */}
          {showDrawer && <HandSummaryDrawer settlement={settlement} playerName={playerName} />}
        </div>
        {/* end scrollable body */}
      </div>
      {/* end modal */}
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2 rounded-xl bg-amber-500/8 border border-amber-500/15 animate-[fadeSlideUp_0.35s_ease-out]">
      ...
      <div className="flex flex-col min-w-0">
        ...
        {handName && <span className="text-slate-400 text-[10px]">{handName}</span>}
      </div>
      {revealedCards && (
        <div
      ...
      <div className="ml-auto text-right shrink-0">
        <div className="text-amber-400 font-extrabold text-base">+{amount.toLocaleString()}</div>
        <div
  ...
  return (
    <div className="mt-4 space-y-3 border-t border-white/10 pt-3">
        ...
        <table className="w-full text-[10px]">
          <thead>
            <tr className="text-slate-500 border-b border-white/5">
              ...
              <th className="text-left py-1 font-medium pl-2">Eligible Seats</th>
              {settlement.runCount === 2 && (
                <>
      ...
      <div className="text-center text-[9px] text-slate-600 font-mono">
        totalPot={settlement.totalPot} · totalPaid={settlement.totalPaid} · rake={settlement.rake} ·
        Δ=
