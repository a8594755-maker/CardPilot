...
export function RoomSettingsPanel({
  ...
  const [tab, setTab] = useState<'game' | 'rules' | 'special' | 'players' | 'moderation' | 'bots'>(
    tabMap[initialTab ?? ''] ?? 'game',
  );
  ...
  const [blindLevels, setBlindLevels] = useState(
    s.blindStructure ?? [
      { smallBlind: s.smallBlind, bigBlind: s.bigBlind, ante: s.ante, durationMinutes: 20 },
    ],
  );
  ...
  const SectionTitle = ({ children }: { children: React.ReactNode }) => (
    <div className="text-[11px] uppercase tracking-wider text-slate-500 font-semibold mt-2 mb-1">
      {children}
    </div>
  );
  ...
  const YesNo = ({
    ...
    <div className="flex items-center justify-between gap-3 min-h-[40px]">
      <div className="flex items-center gap-1.5 min-w-0">
        ...
        {hint && (
          <span className="text-[9px] text-slate-600 cursor-help" title={hint}>
            ?
          </span>
        )}
      </div>
      ...
      <div className="cp-segmented w-full">
        {options.map((opt) => (
          <button
            ...
            className="cp-segmented-item flex-1"
    ...
    <div className="cp-panel p-6 space-y-5">
      ...
      <div className="cp-segmented w-full overflow-x-auto">
        {[
          ...
          <button
            ...
            className="cp-segmented-item flex-1 whitespace-nowrap"
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            ...
            {blindLevels.map((lvl, i) => (
              <div
                key={i}
                className="grid grid-cols-[2rem_1fr_1fr_1fr_1fr_1.5rem] gap-1 items-center"
              >
          ...
          <div className="grid grid-cols-2 gap-3">
            <SettingRow label="Min">
              <input
                type="number"
                value={s.buyInMin}
                onChange={(e) => updateField('buyInMin', Number(e.target.value))}
            ...
            <SettingRow label="Max">
              <input
                type="number"
                value={s.buyInMax}
                onChange={(e) => updateField('buyInMax', Number(e.target.value))}
                className="input-field text-xs !py-1.5 w-full"
              />
            </SettingRow>
          </div>
      ...
      {tab === 'rules' && (
        <div className="space-y-3">
          ...
          <TriToggle
            ...
            onChange={(v) => {
              updateField('runItTwiceMode', v);
              updateField('runItTwice', v !== 'off');
            }}
          />
          <YesNo
          ...
          />
          <YesNo
            label="Allow UTG Straddle 2BB?"
            value={s.straddleAllowed}
            onChange={(v) => updateField('straddleAllowed', v)}
          />
          <YesNo
            label="Rebuy allowed?"
            value={s.rebuyAllowed}
            onChange={(v) => updateField('rebuyAllowed', v)}
          />
          ...
          <div className="grid grid-cols-2 gap-3">
            <SettingRow label="Decision Time (sec)">
              <input
                type="number"
                value={s.actionTimerSeconds}
                onChange={(e) => updateField('actionTimerSeconds', Number(e.target.value))}
                className="input-field text-xs !py-1.5 w-full"
                min={5}
                max={120}
              />
            ...
            <SettingRow label="Time Bank (sec)">
              <input
                type="number"
                value={s.timeBankSeconds}
                onChange={(e) => updateField('timeBankSeconds', Number(e.target.value))}
                className="input-field text-xs !py-1.5 w-full"
                min={0}
                max={300}
              />
          ...
          <SettingRow label="Hands to fill Time Bank">
            <input
              type="number"
              value={s.timeBankHandsToFill}
              onChange={(e) => updateField('timeBankHandsToFill', Number(e.target.value))}
          ...
          <div className="grid grid-cols-2 gap-3">
            <SettingRow label="Extension/Use (sec)">
              <input
                type="number"
                value={s.thinkExtensionSecondsPerUse}
                onChange={(e) => updateField('thinkExtensionSecondsPerUse', Number(e.target.value))}
                className="input-field text-xs !py-1.5 w-full"
                min={1}
                max={60}
              />
            ...
            <SettingRow label="Extension Quota (/hr)">
              <input
                type="number"
                value={s.thinkExtensionQuotaPerHour}
                onChange={(e) => updateField('thinkExtensionQuotaPerHour', Number(e.target.value))}
                className="input-field text-xs !py-1.5 w-full"
                min={0}
                max={20}
              />
      ...
      {tab === 'special' && (
        <div className="space-y-3">
          ...
          <YesNo
            label="Bomb Pot enabled?"
            value={s.bombPotEnabled}
            onChange={(v) => updateField('bombPotEnabled', v)}
            ...
            <>
              ...
              {(s.bombPotTriggerMode ?? 'frequency') === 'frequency' && (
                <SettingRow label="Every N hands">
                  <input
                    type="number"
                    value={s.bombPotFrequency}
                    onChange={(e) => updateField('bombPotFrequency', Number(e.target.value))}
                    className="input-field text-xs !py-1.5 w-20"
                    min={1}
                    max={100}
                  />
              ...
              {(s.bombPotTriggerMode ?? 'frequency') === 'probability' && (
                <SettingRow label="Probability (%)">
                  <input
                    type="number"
                    value={s.bombPotProbability ?? 0}
                    onChange={(e) => updateField('bombPotProbability', Number(e.target.value))}
                    className="input-field text-xs !py-1.5 w-20"
                    min={0}
                    max={100}
                  />
              ...
              <SettingRow label={s.bombPotAnteMode === 'fixed' ? 'Ante (chips)' : 'Ante (× BB)'}>
                <input
                  type="number"
                  value={s.bombPotAnteValue ?? 1}
                  onChange={(e) => updateField('bombPotAnteValue', Number(e.target.value))}
                  className="input-field text-xs !py-1.5 w-20"
                  min={1}
                  max={100}
                />
          ...
          <SettingRow label="Bounty amount (0 = off)">
            <input
              type="number"
              value={s.sevenTwoBounty}
              onChange={(e) => updateField('sevenTwoBounty', Number(e.target.value))}
              className="input-field text-xs !py-1.5 w-24"
              min={0}
            />
          </SettingRow>
          {s.sevenTwoBounty > 0 && (
            <p className="text-[10px] text-slate-500">
              Each player pays {s.sevenTwoBounty} to the winner holding 7-2
            </p>
          )}
        </div>
      )}
      ...
      {tab === 'players' && (
        <div className="space-y-2">
          <p className="text-[10px] text-slate-500">
            Owner:{' '}
            <span className="text-amber-400 font-medium">{roomState.ownership.ownerName}</span>
          </p>
          {players.length === 0 ? (
                ...
                >
                  ...
                  {!isMe && !isPlayerOwner && (
                    <div className="flex gap-1 shrink-0">
                      {isHost && (
                        <button
                          onClick={() => onSetCoHost(p.userId, !isPlayerCoHost)}
                      ...
                      <button
                        onClick={() => onKick(p.userId, kickReason, false)}
        ...
        <div className="space-y-3">
          ...
          <SettingRow label="Kick Reason">
            <input
              ...
              className="input-field text-xs !py-1.5 w-full"
              placeholder="Optional reason for kicks..."
            />
          </SettingRow>
          <SettingRow label="Max Consecutive Timeouts">
            <input
              type="number"
              value={s.maxConsecutiveTimeouts}
              onChange={(e) => updateField('maxConsecutiveTimeouts', Number(e.target.value))}
              className="input-field text-xs !py-1.5 w-20"
              min={1}
              max={10}
            />
          ...
          {roomState.banList.length > 0 && (
            <div>
              <span className="text-[10px] text-slate-500 uppercase font-medium">
                Banned Users ({roomState.banList.length})
              </span>
              <div className="mt-1 flex flex-wrap gap-1">
                {roomState.banList.map((uid) => (
                  <span
                    key={uid}
                    className="text-[10px] bg-red-500/10 text-red-400 px-2 py-0.5 rounded"
                  >
                    {uid.slice(0, 8)}...
                  </span>
                ))}
      ...
      {tab === 'bots' && (
        <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {isHost && seatPlayer && onBotAddChips && (
                      <button
                        onClick={() => {
                          const amount = prompt(
                            `Add chips to ${seatPlayer.name} (Seat ${seat}):`,
                            String(s.bigBlind * 50),
                          );
          ...
          <div className="text-[10px] text-slate-500 mt-2 space-y-1">
            <p>
              <span className="text-indigo-400 font-medium">V0</span> = Heuristic (rule-based, no ML
              model)
            </p>
            <p>
              <span className="text-indigo-400 font-medium">V1</span> = Legacy trained MLP
            </p>
            <p>
              <span className="text-indigo-400 font-medium">V2</span> = model-v2-latest (currently
              V2.1)
            </p>
            <p>
              <span className="text-indigo-400 font-medium">V2.1</span> = 300k curated checkpoint
            </p>
            <p>
              <span className="text-indigo-400 font-medium">V2.2</span> = full-data optimized
              checkpoint
            </p>
            <p>
              <span className="text-indigo-400 font-medium">V3</span> = CFR solver trained (177M
              samples)
            </p>
            <p>
              <span className="text-indigo-400 font-medium">V3.2</span> = V3 + Preflop CFR (199K
              samples)
            </p>
            <p>
              <span className="text-indigo-400 font-medium">V4</span> = Exact CFR flop solver (all
              1,911 flops, 8.5M samples)
            </p>
          </div>
        </div>
      )}
