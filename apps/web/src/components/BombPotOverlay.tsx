...
export const BombPotOverlay = memo(function BombPotOverlay({
  ...
  const overlay = (
    <div
      className={`cp-bomb-overlay cp-bomb-overlay--${phase}`}
      role="status"
      aria-live="polite"
    >
      ...
      <div className="cp-bomb-overlay__content">
        <div className="cp-bomb-overlay__title">
          <span role="img" aria-label="bomb">💣</span> BOMB POT
        </div>
        {anteAmount > 0 && (
          <div className="cp-bomb-overlay__subtitle">
            Ante: {anteAmount.toLocaleString()}
          </div>
        )}
      </div>
    </div>
