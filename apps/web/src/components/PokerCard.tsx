...
export const PokerCard = memo(function PokerCard({
  ...
  return (
    ...
    >
      <div
        className={`absolute inset-[1px] rounded-[inherit] bg-gradient-to-b ${backTint} opacity-[0.06]`}
      />
      {showCornerPips && (
        <>
          <div
            className={`absolute top-[8%] left-[10%] flex flex-col items-center leading-none ${cfg.cornerGap}`}
          >
          ...
          <div
            className={`absolute bottom-[8%] right-[10%] rotate-180 flex flex-col items-center leading-none ${cfg.cornerGap}`}
          >
            <span className={`${suitColor} ${cfg.rank} font-black tracking-tight`}>{rankStr}</span>
            <span className={`${suitColor} ${cfg.cornerSuit} leading-none`}>{suitStr}</span>
          </div>
      ...
      {showCenterPip && (
        <span
          className={`absolute inset-0 flex items-center justify-center ${suitColor} ${cfg.centerSuit} opacity-80`}
        >
          {suitStr}
        </span>
      )}
