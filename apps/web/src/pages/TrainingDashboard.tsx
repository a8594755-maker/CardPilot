        ...
        <div className="max-w-4xl mx-auto">
          <div className="glass-card p-8 text-center">
            ...
            <p className="text-slate-400 text-sm max-w-md mx-auto">
              Play hands in Coach mode to see your GTO audit results here. Every decision is
              analyzed against optimal strategy after each hand.
            </p>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Live audit of your decisions against GTO strategy
            </p>
          </div>
          {sessionLeak && (
  ...
  return (
    <div className="glass-card p-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatBlock label="Leaked BB" value={formatBb(leak.totalLeakedBb)} color={leakedColor} />
        <StatBlock label="BB/100" value={formatBb(leak.leakedBbPer100)} color={per100Color} />
        <StatBlock label="Hands Audited" value={`${leak.handsAudited}`} color="text-slate-300" />
        <StatBlock
          label="Decisions"
...
function HandAuditCard({
  ...
  const color =
    leaked < -0.5 ? 'text-red-400' : leaked > 0.5 ? 'text-emerald-400' : 'text-slate-400';
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <span className={`text-sm font-mono font-medium ${color}`}>{formatBb(leaked)} bb</span>
      </div>
...
function HandAuditDetail({ summary }: { summary: HandAuditSummary }) {
  return (
    <div className="glass-card p-4 space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-white">Hand #{summary.handId.slice(-6)}</h3>
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-slate-500 uppercase">{summary.spotType}</span>
          <span
            className={`text-sm font-mono font-medium ${
              summary.totalLeakedBb < -0.5
                ? 'text-red-400'
                : summary.totalLeakedBb > 0.5
                  ? 'text-emerald-400'
                  : 'text-slate-300'
  ...
  return (
    <div className={`p-3 rounded-lg border ${bgColor}`}>
      <div className="flex items-center justify-between">
        ...
        <span
          className={`text-xs font-mono ${audit.evDiffBb < -0.1 ? 'text-red-400' : 'text-slate-400'}`}
        >
    ...
    <div className="glass-card p-4">
      ...
      <div className="space-y-2">
        {leaks.map((leak) => (
          <div
            key={leak.rank}
            className="flex items-start gap-3 p-2 rounded-lg bg-red-500/5 border border-red-500/10"
          >
