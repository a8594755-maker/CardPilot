...
export interface ClubSocketActions {
  ...
  updateClub: (payload: { name?: string; description?: string; requireApprovalToJoin?: boolean; badgeColor?: string }) => void;
...
export function useClubSocket(
  ...
  const actions: ClubSocketActions = {
    ...
    createTable: useCallback((name: string, templateRulesetId?: string) => {
      socket?.emit("club_table_create", { clubId, name, templateRulesetId: templateRulesetId || undefined });
      showToast("Creating table...");
    }, [socket, clubId, showToast]),

    renameTable: useCallback((tableId: string, name: string) => {
      socket?.emit("club_table_update", { clubId, tableId, name });
      showToast("Updating table...");
    }, [socket, clubId, showToast]),

    closeTable: useCallback((tableId: string) => {
    ...
    approveJoin: useCallback((targetUserId: string) => {
      socket?.emit("club_join_approve", { clubId, userId: targetUserId, approve: true });
      showToast("Approving...");
    }, [socket, clubId, showToast]),

    rejectJoin: useCallback((targetUserId: string) => {
      socket?.emit("club_join_reject", { clubId, userId: targetUserId, approve: false });
      showToast("Rejecting...");
    }, [socket, clubId, showToast]),

    changeRole: useCallback((targetUserId: string, newRole: ClubRole) => {
      socket?.emit("club_member_update_role", { clubId, userId: targetUserId, newRole });
      showToast("Updating role...");
    }, [socket, clubId, showToast]),

    kickMember: useCallback((targetUserId: string) => {
      socket?.emit("club_member_kick", { clubId, userId: targetUserId });
      showToast("Removing member...");
    }, [socket, clubId, showToast]),

    banMember: useCallback((targetUserId: string) => {
      socket?.emit("club_member_ban", { clubId, userId: targetUserId, reason: "Banned by admin" });
      showToast("Banning member...");
    }, [socket, clubId, showToast]),

    grantCredits: useCallback((targetUserId: string, amount: number) => {
      socket?.emit("club_wallet_admin_deposit", { clubId, userId: targetUserId, amount, note: "Admin credit grant" });
      showToast("Granting credits...");
    }, [socket, clubId, showToast]),

    adjustCredits: useCallback((targetUserId: string, amount: number, note?: string) => {
      socket?.emit("club_wallet_admin_adjust", { clubId, userId: targetUserId, amount, note: note ?? "Admin credit adjustment" });
    ...
    bulkApprove: useCallback((userIds: string[]) => {
    ...
    bulkGrantCredits: useCallback((userIds: string[], amount: number) => {
      for (const uid of userIds) {
        socket?.emit("club_wallet_admin_deposit", { clubId, userId: uid, amount, note: "Bulk admin credit grant" });
      }
      showToast(`Granting credits to ${userIds.length} member(s)...`);
    }, [socket, clubId, showToast]),

    bulkRoleChange: useCallback((userIds: string[], newRole: ClubRole) => {
      for (const uid of userIds) {
    ...
    bulkKick: useCallback((userIds: string[]) => {
      for (const uid of userIds) {
