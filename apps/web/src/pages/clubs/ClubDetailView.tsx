...
import type { ClubTab } from "./hooks/useClubTab";
...
export function ClubDetailView({
  ...
  const { actions, state } = useClubSocket(
    socket, club.id, userId, onJoinTable, showToast,
    myMembership?.balance ?? 0,
  );
  const { actions: chatActions, state: chatState } = useChat(socket, club.id, userId);
  const { actions: analyticsActions, state: analyticsState } = useAnalytics(socket, club.id, userId);
  ...
  const openCloseTable = useCallback((tableId: string, tableName: string) => {
    setModal({
      type: "confirm",
      title: "Close Table",
      message: `Close table "${tableName}"? This cannot be undone.`,
      confirmLabel: "Close Table",
      onConfirm: () => { actions.closeTable(tableId); closeModal(); },
    });
  }, [actions, closeModal]);

  const openKickMember = useCallback((targetUserId: string, displayName: string) => {
    setModal({
      type: "confirm",
      title: "Remove Member",
      message: `Remove ${displayName} from the club?`,
      confirmLabel: "Remove",
      onConfirm: () => { actions.kickMember(targetUserId); closeModal(); },
    });
  }, [actions, closeModal]);

  const openBanMember = useCallback((targetUserId: string, displayName: string) => {
    setModal({
      type: "confirm",
      title: "Ban Member",
      message: `Ban ${displayName} from the club? They will not be able to rejoin.`,
      confirmLabel: "Ban",
      onConfirm: () => { actions.banMember(targetUserId); closeModal(); },
    });
  }, [actions, closeModal]);

  const openGrantCredits = useCallback((preUserId?: string, preDisplayName?: string) => {
    setModal({ type: "grant", preselectedUserId: preUserId, preselectedDisplayName: preDisplayName });
  }, []);
        ...
        <div className="rounded-2xl border border-white/5 bg-white/[0.02] p-4 shadow-inner shadow-black/20 sm:p-5">
          {isMobile && (
            <div className="mb-4 flex items-center justify-between">
              <button onClick={onBack} className="inline-flex items-center gap-1 text-xs font-medium text-slate-400 transition-colors hover:text-white sm:text-sm">
                <span aria-hidden="true">←</span><span>Back</span>
              </button>
              <button onClick={actions.refresh} className="inline-flex items-center gap-1 rounded-lg border border-white/10 bg-white/5 px-3 py-1 text-[11px] font-semibold text-slate-300 transition-colors hover:bg-white/10 sm:text-xs">
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div className="flex-1 space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs text-slate-400 sm:gap-3">
                  <span>Code: <code className="font-mono text-amber-400">{club.code}</code></span>
          ...
          {isMobile && (
            <div className="mt-4 grid grid-cols-2 gap-3 text-[11px] text-slate-300">
              <button className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-left font-semibold text-amber-200" onClick={() => copyToClipboard(club.code, "club code")}>
                Share Code
              ...
              <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                ...
                <div className="text-sm font-semibold text-white">{onlineCount} online / {detail.detail.memberCount}</div>
              </div>
            </div>
          )}
        </div>
                ...
                <div key={m.userId} className="flex flex-col gap-3 rounded-lg bg-black/20 p-3 text-sm sm:flex-row sm:items-center">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-[11px] text-slate-500">Last seen {new Date(m.lastSeenAt).toLocaleDateString()}</span>
                    </div>
                  </div>
                  <div className="flex w-full gap-2 sm:w-auto">
                    <button onClick={() => actions.approveJoin(m.userId)} className="flex-1 rounded border border-emerald-500/40 bg-emerald-500/10 px-3 py-1 text-center text-[11px] font-semibold text-emerald-300 hover:bg-emerald-500/20">
                      ✓ Approve
                    </button>
                    <button onClick={() => actions.rejectJoin(m.userId)} className="flex-1 rounded border border-red-500/40 bg-red-500/10 px-3 py-1 text-center text-[11px] font-semibold text-red-300 hover:bg-red-500/20">
                      ✗ Reject
                    </button>
                  </div>
                </div>
