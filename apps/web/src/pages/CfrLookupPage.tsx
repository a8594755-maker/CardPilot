              ...
              >
                <div className="flex items-center justify-between">
                  <span className="font-medium">
                    {cfg.positions} {cfg.potType} {cfg.stack}
                  </span>
                  <span
                    className={`text-xs ${cfg.available ? 'text-emerald-400' : 'text-slate-600'}`}
                  >
                    {cfg.solvedFlops}/{cfg.totalFlops}
                  </span>
                </div>
                <div className="text-xs text-slate-500 mt-0.5">
                  {cfg.sizes} size{cfg.sizes > 1 ? 's' : ''}/street
                </div>
                {cfg.available && (
      ...
      {flops.map((flop) => (
        ...
        >
          {flop.flopCards.map((c, i) => (
            <span key={i} className={cardColorClass(c)}>
              {cardLabel(c)}
            </span>
          ))}
        </button>
  ...
  if (strategies.length === 0) {
    return (
      <div className="text-slate-500 text-sm py-4 text-center">
        No strategies found for this position
      </div>
    );
  }
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center gap-2 text-xs text-slate-400 mb-2">
        ...
        <span>
          History: <code className="bg-white/10 px-1 rounded">{history || '(root)'}</code>
        </span>
        <span className="text-slate-600">|</span>
      ...
      {selectedConfig && (
        <section>
          <h2 className="text-sm font-semibold text-slate-300 mb-2">
            Select Flop
            {flops.length > 0 && (
              <span className="text-slate-500 font-normal ml-2">({flops.length} solved)</span>
            )}
          </h2>
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <div className="text-lg font-mono">
              {selectedFlop.flopCards.map((c, i) => (
                <span key={i} className={`${cardColorClass(c)} mr-1`}>
                  {cardLabel(c)}
                </span>
              ))}
            </div>
